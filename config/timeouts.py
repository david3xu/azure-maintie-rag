...
class TimeoutConfig(BaseModel):
    ...
    # Agent operations
    agent_tool_execution: float = Field(default=2.0, description="Agent tool execution timeout")
    domain_analysis: float = Field(default=1.5, description="Domain analysis timeout")
    pattern_extraction: float = Field(default=2.0, description="Pattern extraction timeout")
    
    # Infrastructure operations  
    azure_openai_request: float = Field(default=10.0, description="Azure OpenAI API timeout")
    openai_timeout: float = Field(default=10.0, description="OpenAI API timeout (alias)")
    azure_search_request: float = Field(default=5.0, description="Azure Search API timeout")
    search_timeout: int = Field(default=120, description="Search operation timeout in seconds")
    ...
    storage_timeout: int = Field(default=60, description="Storage operation timeout in seconds")
    
    # Circuit breaker settings
    max_retries: int = Field(default=3, description="Maximum retry attempts")
    circuit_breaker_threshold: int = Field(default=5, description="Failures before circuit opens")
    circuit_breaker_reset_timeout: float = Field(default=60.0, description="Circuit breaker reset time")
...
class TimeoutEnforcer:
    ...
    async def enforce_timeout(self, operation_name: str, coro, timeout: Optional[float] = None):
        ...
        try:
            ...
            raise RuntimeError(f"Operation '{operation_name}' exceeded {timeout}s timeout - SLA violation")
    ...
    def _is_circuit_open(self, operation_name: str) -> bool:
        ...
        circuit = self._circuit_breakers.get(operation_name, {
            'failures': 0, 
            'last_failure_time': 0,
            'state': 'closed'
        })
        ...
        if circuit['state'] == 'open':
            ...
            import time
            if time.time() - circuit['last_failure_time'] > self.config.circuit_breaker_reset_timeout:

...
class UniversalExtractionConfig:
    ...
    characteristics: ContentCharacteristics = field(default_factory=ContentCharacteristics)
    
    def adapt_to_content(self, characteristics: ContentCharacteristics) -> 'UniversalExtractionConfig':
        ...
        adapted.max_relationships_per_chunk = int(8 + (density_factor * 12))  # 8-20 range
...
class UniversalSearchConfig:
    ...
    def adapt_to_query_and_content(self, query_characteristics: Dict[str, float], 
                                 content_characteristics: ContentCharacteristics) -> 'UniversalSearchConfig':
        ...
        adapted.graph_traversal_depth = min(1 + int(query_complexity * 3), 4)  # 1-4 range
...
class UniversalConfigManager:
    ...
    def analyze_content_characteristics(self, content_samples: List[str], 
                                      content_signature: str = None) -> ContentCharacteristics:
        """Analyze content to discover characteristics - no domain assumptions"""
        ...
        relationship_indicators = all_text.lower().count('and') + all_text.lower().count('with') + all_text.lower().count('of')
        relationship_complexity = min(relationship_indicators / max(len(words), 1) * 50, 1.0)
        
        # Analyze structural complexity
        paragraphs = all_text.split('\n\n')
        structural_indicators = len([p for p in paragraphs if len(p.strip()) > 50])
        structural_complexity = min(structural_indicators / max(len(paragraphs), 1), 1.0)
        
        # Calculate terminology uniqueness
        terminology_uniqueness = vocabulary_complexity  # Similar measure
        
        # Estimate cross-reference ratio
        reference_patterns = ['see', 'refer', 'section', 'chapter', 'figure', 'table']
        reference_count = sum(all_text.lower().count(pattern) for pattern in reference_patterns)
        ...
        logger.info(f"Analyzed content characteristics: vocab={vocabulary_complexity:.2f}, density={concept_density:.2f}")
        
        return characteristics
    
    def get_extraction_config(self, content_samples: List[str] = None,
                            content_signature: str = None) -> UniversalExtractionConfig:
        """Get extraction configuration adapted to discovered content characteristics"""
        
        if content_samples:
            characteristics = self.analyze_content_characteristics(content_samples, content_signature)
    ...
    def get_search_config(self, query: str = None, 
                         content_samples: List[str] = None,
                         content_signature: str = None) -> UniversalSearchConfig:
        ...
        if query:
            ...
            query_characteristics = {
                ...
                'length_factor': len(query_words) / 5  # Factor for length-based adaptation
            }
        
        # Get content characteristics
        if content_samples:
            content_characteristics = self.analyze_content_characteristics(content_samples, content_signature)
        else:
            content_characteristics = ContentCharacteristics()
        
        base_config = UniversalSearchConfig()
        return base_config.adapt_to_query_and_content(query_characteristics, content_characteristics)
...
def get_extraction_config_universal(content_samples: List[str] = None,
                                  content_signature: str = None) -> UniversalExtractionConfig:
    """Get extraction configuration adapted to discovered content characteristics"""
    config_manager = get_universal_config_manager()
    return config_manager.get_extraction_config(content_samples, content_signature)

def get_search_config_universal(query: str = None,
                              content_samples: List[str] = None, 
                              content_signature: str = None) -> UniversalSearchConfig:
    """Get search configuration adapted to query and content characteristics"""
    config_manager = get_universal_config_manager()
    return config_manager.get_search_config(query, content_samples, content_signature)
...
def analyze_content_characteristics(content_samples: List[str],
                                  content_signature: str = None) -> ContentCharacteristics:
    """Analyze content to discover characteristics"""
    config_manager = get_universal_config_manager()
    return config_manager.analyze_content_characteristics(content_samples, content_signature)
...
__all__ = [
    "SystemConfig", "ModelConfig", "UniversalExtractionConfig", "UniversalSearchConfig", 
    "UniversalQueryConfig", "ContentCharacteristics",
    "get_system_config", "get_model_config", "get_extraction_config_universal", 
    "get_search_config_universal", "get_query_config", "get_azure_config",
    "analyze_content_characteristics", "validate_configuration",
    "get_model_config_bootstrap", "initialize_configuration", "universal_config_manager"
]
...
if TYPE_CHECKING:
    from config.centralized_config import SystemConfiguration, ExtractionConfiguration, SearchConfiguration
...
class UnifiedAgentConfiguration(PydanticAIContextualModel):
    ...
    domain_name: str = Field(description="Domain name for domain-specific configuration")
    configuration_data: Dict[str, Any] = Field(description="Resolved configuration parameters")
    resolved_at: datetime = Field(description="When this configuration was resolved")
    resolver_context: Dict[str, Any] = Field(default_factory=dict, description="Context used for resolution")
    
    # Performance and quality tracking
    resolution_time_ms: Optional[float] = Field(default=None, description="Time taken to resolve configuration")
    configuration_source: str = Field(default="unified_resolver", description="Source of configuration")
...
class ConfigurationResolver:
    ...
    async def resolve_extraction_config(domain_name: str = None) -> Dict[str, Any]:
        ...
        config = await resolver.resolve_agent_configuration("knowledge_extraction", domain_name)
    ...
    async def resolve_search_config(domain_name: str = None) -> Dict[str, Any]:
        """Legacy compatibility - use unified resolver"""
        resolver = get_unified_configuration_resolver()
        config = await resolver.resolve_agent_configuration("universal_search", domain_name)
        return config.configuration_data
...
class BaseResponse(BaseModel):
    ...
    request_id: Optional[str] = Field(default=None, description="Matching request identifier")
...
class AzureServiceConfiguration(PydanticAIContextualModel):
    ...
    service_type: str = Field(description="Type of Azure service (openai, search, cosmos, storage, ml)")
    ...
    credential_scope: Optional[str] = Field(default=None, description="Azure credential scope")
    
    # Service-specific parameters
    api_version: Optional[str] = Field(default=None, description="Service API version")
    deployment_name: Optional[str] = Field(default=None, description="Model deployment name (for OpenAI)")
    index_name: Optional[str] = Field(default=None, description="Search index name (for Cognitive Search)")
    database_name: Optional[str] = Field(default=None, description="Database name (for Cosmos DB)")
    container_name: Optional[str] = Field(default=None, description="Container name")
    
    # Performance and reliability
    timeout_seconds: int = Field(default=60, ge=1, le=300, description="Request timeout in seconds")
...
class AzureServiceMetrics(BaseModel):
    ...
    service_type: str = Field(description="Service type (openai, search, cosmos, storage, ml)")
    ...
    p95_response_time_ms: Optional[float] = Field(default=None, ge=0.0, description="95th percentile response time")
    p99_response_time_ms: Optional[float] = Field(default=None, ge=0.0, description="99th percentile response time")
    error_rate: float = Field(ge=0.0, le=1.0, description="Error rate percentage")
    throughput_requests_per_second: float = Field(ge=0.0, description="Requests per second")
    availability_percentage: float = Field(ge=0.0, le=100.0, description="Service availability")
    
    # Cost and resource metrics
    cost_per_request: Optional[float] = Field(default=None, ge=0.0, description="Cost per request")
    token_usage: Optional[int] = Field(default=None, ge=0, description="Token usage (for LLM services)")
    quota_usage_percentage: Optional[float] = Field(default=None, ge=0.0, le=100.0, description="Quota usage")
    
    # Authentication and connectivity
    auth_success_rate: float = Field(default=1.0, ge=0.0, le=1.0, description="Authentication success rate")
    connection_pool_size: Optional[int] = Field(default=None, ge=0, description="Connection pool size")
    active_connections: Optional[int] = Field(default=None, ge=0, description="Active connections")
    
    # Temporal tracking
    last_updated: datetime = Field(default_factory=datetime.now, description="Last metrics update")
    measurement_window_minutes: int = Field(default=5, ge=1, le=60, description="Measurement window")
    ...
    def is_performance_degraded(
        ...
        return (
            self.response_time_ms > response_time_threshold_ms or 
...
class AzureSearchIndexSchema(BaseModel):
    ...
    scoring_profiles: List[Dict[str, Any]] = Field(default_factory=list, description="Scoring profiles")
    analyzers: List[Dict[str, Any]] = Field(default_factory=list, description="Custom analyzers")
    vector_search_config: Dict[str, Any] = Field(description="Vector search configuration")
...
class DomainStatistics(BaseModel):
    ...
    complexity_score: float = Field(ge=0.0, le=1.0, description="Domain complexity score")
    technical_density: float = Field(ge=0.0, le=1.0, description="Technical content density")
...
class DomainAnalysisContract(BaseModel):
    ...
    performance_guarantees: Dict[str, Any] = Field(description="Performance SLA guarantees")
    dependencies: List[str] = Field(description="Required dependencies")
    configuration_requirements: Dict[str, Any] = Field(description="Configuration requirements")
    supported_domains: List[str] = Field(description="Supported domain types")
    statistical_patterns: List[StatisticalPattern] = Field(description="Identified statistical patterns")
    domain_statistics: DomainStatistics = Field(description="Domain analysis statistics")
...
class KnowledgeExtractionContract(BaseModel):
    ...
    supported_relationship_types: List[str] = Field(description="Supported relationship types")
    quality_thresholds: Dict[str, float] = Field(description="Quality threshold requirements")
    performance_metrics: Dict[str, float] = Field(description="Expected performance metrics")
    validation_requirements: Dict[str, Any] = Field(description="Validation requirements")
...
class UniversalSearchContract(BaseModel):
    ...
    performance_guarantees: Dict[str, float] = Field(description="Performance SLA guarantees")
    ...
    caching_strategy: Dict[str, Any] = Field(description="Caching strategy specification")
    dependencies: List[str] = Field(description="Required dependencies")
...
class DomainConfig(BaseModel):
    ...
    cache_ttl_seconds: int = Field(default=PerformanceAdaptiveConstants.DEFAULT_CACHE_TTL, ge=0)
...
class ExtractionConfiguration(BaseModel):
    ...
    entity_confidence_threshold: float = Field(ge=0.0, le=1.0, description="Learned entity confidence threshold")
    relationship_confidence_threshold: float = Field(ge=0.0, le=1.0, description="Learned relationship confidence threshold")
    ...
    target_response_time_seconds: float = Field(ge=0.1, description="Learned response SLA from complexity")
    ...
    cache_ttl_seconds: int = Field(default=PerformanceAdaptiveConstants.DEFAULT_CACHE_TTL, ge=0)
    parallel_processing_threshold: int = Field(ge=1, description="Parallel processing threshold")
    max_concurrent_chunks: int = Field(default=PerformanceAdaptiveConstants.MAX_CONCURRENT_CHUNKS, ge=1)
    generation_confidence: float = Field(ge=0.0, le=1.0, description="Configuration generation confidence")
...
class VectorSearchConfig(BaseModel):
    ...
    top_k: int = Field(default=UniversalSearchConstants.DEFAULT_VECTOR_TOP_K, ge=1, le=50)
    similarity_threshold: float = Field(default=UniversalSearchConstants.VECTOR_SIMILARITY_THRESHOLD, ge=0.0, le=1.0)
    include_metadata: bool = Field(default=True, description="Include search metadata")
    embedding_model: str = Field(default=InfrastructureConstants.DEFAULT_EMBEDDING_MODEL, description="Embedding model name")


class GraphSearchConfig(BaseModel):
    """Graph search configuration"""
    max_depth: int = Field(default=UniversalSearchConstants.DEFAULT_MAX_DEPTH, ge=1, le=5)
    max_entities: int = Field(default=UniversalSearchConstants.DEFAULT_MAX_ENTITIES, ge=1, le=100)
    relationship_threshold: float = Field(default=UniversalSearchConstants.DEFAULT_RELATIONSHIP_THRESHOLD, ge=0.0, le=1.0)
    relationship_types: List[str] = Field(default_factory=list, description="Supported relationship types")


class GNNSearchConfig(BaseModel):
    """Graph Neural Network search configuration"""
    max_predictions: int = Field(default=UniversalSearchConstants.DEFAULT_MAX_PREDICTIONS, ge=1, le=50)
    pattern_threshold: float = Field(default=UniversalSearchConstants.DEFAULT_PATTERN_THRESHOLD, ge=0.0, le=1.0)
    node_embeddings: int = Field(default=UniversalSearchConstants.DEFAULT_GNN_NODE_EMBEDDINGS, ge=32, le=512)
    min_training_examples: int = Field(default=UniversalSearchConstants.DEFAULT_MIN_TRAINING_EXAMPLES, ge=10)
...
class QueryRequest(BaseRequest):
    ...
    domain: Optional[str] = Field(default=None, description="Target domain (auto-detect if None)")
    max_results: int = Field(default=10, ge=1, le=50, description="Maximum results to return")
    search_types: List[SearchType] = Field(default=[SearchType.ALL], description="Search types to execute")
    confidence_threshold: float = Field(default=StatisticalConstants.MIN_DOMAIN_CONFIDENCE, ge=0.0, le=1.0)
    include_metadata: bool = Field(default=True, description="Include search metadata")
...
class VectorSearchRequest(BaseRequest):
    ...
    config: VectorSearchConfig = Field(default_factory=VectorSearchConfig, description="Vector search configuration")
...
class GraphSearchRequest(BaseRequest):
    ...
    query: str = Field(min_length=1, max_length=500, description="Search query")
    config: GraphSearchConfig = Field(default_factory=GraphSearchConfig, description="Graph search configuration")
    domain: Optional[str] = Field(default=None, description="Target domain")
...
class TriModalSearchRequest(BaseRequest):
    ...
    query: str = Field(min_length=1, max_length=500, description="Search query")
    search_types: List[SearchType] = Field(default=[SearchType.ALL], description="Search modalities")
...
class DomainDetectionRequest(BaseRequest):
    ...
    confidence_threshold: float = Field(default=StatisticalConstants.DOMAIN_CLASSIFICATION_THRESHOLD, ge=0.0, le=1.0)
    include_probabilities: bool = Field(default=True, description="Include probability scores")
    max_domains: int = Field(default=3, ge=1, le=10, description="Maximum domains to return")


class PatternLearningRequest(BaseRequest):
    """Pattern learning request"""
    data: List[Dict[str, Any]] = Field(min_items=1, max_items=1000, description="Training data")
    learning_type: Literal["incremental", "batch", "reinforcement"] = Field(default="incremental")
    confidence_threshold: float = Field(default=StatisticalConstants.MIN_DOMAIN_CONFIDENCE, ge=0.0, le=1.0)
    max_patterns: int = Field(default=50, ge=1, le=200, description="Maximum patterns to extract")
...
class SearchResponse(BaseResponse):
    ...
    synthesis_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Result synthesis score")
...
class DomainDetectionResult(BaseResponse):
    ...
    alternative_domains: List[Dict[str, float]] = Field(default_factory=list, description="Alternative domains with scores")
...
class PerformanceFeedbackPoint(BaseModel):
    ...
    operation_type: str = Field(description="Type of operation (extraction, search, analysis)")
    timestamp: datetime = Field(default_factory=datetime.now, description="When this performance was recorded")
    
    # Configuration used
    configuration_used: Dict[str, Any] = Field(description="Configuration parameters that were used")
    configuration_source: str = Field(description="Source of the configuration (dynamic, static, fallback)")
    
    # Performance metrics
    execution_time_seconds: float = Field(ge=0.0, description="Total execution time")
    success: bool = Field(description="Whether the operation succeeded")
    quality_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Quality assessment score")
    
    # Input characteristics
    input_size: Optional[int] = Field(default=None, ge=0, description="Size of input data")
    input_complexity: Optional[str] = Field(default=None, description="Assessed complexity of input")
    
    # Output characteristics
    output_size: Optional[int] = Field(default=None, ge=0, description="Size of output data")
    output_quality_metrics: Dict[str, float] = Field(default_factory=dict, description="Detailed quality metrics")
    
    # Resource utilization
    memory_usage_mb: Optional[float] = Field(default=None, ge=0.0, description="Memory usage during operation")
    cpu_usage_percent: Optional[float] = Field(default=None, ge=0.0, le=100.0, description="CPU usage during operation")
    
    # Error information (if applicable)
    error_message: Optional[str] = Field(default=None, description="Error message if operation failed")
    error_category: Optional[ErrorCategory] = Field(default=None, description="Error category classification")
    
    # Context for learning
    user_satisfaction: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="User satisfaction score")
    downstream_impact: Optional[Dict[str, float]] = Field(default=None, description="Impact on downstream operations")
...
class WorkflowResultContract(PydanticAIContextualModel):
    ...
    workflow_type: str = Field(description="Type of workflow (config_extraction, search, analysis)")
    ...
    configurations_used: Dict[str, Dict[str, Any]] = Field(description="Configurations used by each agent")
    domain_context: Optional[str] = Field(default=None, description="Domain context for this workflow")
    ...
    lessons_learned: List[str] = Field(default_factory=list, description="Lessons learned from this execution")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_summary": {
                ...
                "average_quality_score": sum(self.quality_scores.values()) / len(self.quality_scores) if self.quality_scores else 0.0,
                "success_rate": 1.0 if self.execution_state == WorkflowState.COMPLETED else 0.0,
    ...
    def is_successful(self) -> bool:
        ...
        return self.execution_state == WorkflowState.COMPLETED and len(self.error_log) == 0
...
class StatisticalAnalysis(BaseModel):
    ...
    token_frequencies: Dict[str, int] = Field(description="Token frequency distribution")
    n_gram_patterns: Dict[str, int] = Field(description="N-gram pattern frequencies")
    vocabulary_size: int = Field(ge=0, description="Unique vocabulary size")
    document_structures: Dict[str, int] = Field(default_factory=dict, description="Document structure patterns")
    average_document_length: float = Field(ge=0.0, description="Average document length")
    document_count: int = Field(ge=0, description="Number of documents analyzed")
    length_distribution: Dict[str, int] = Field(default_factory=dict, description="Document length distribution")
    technical_term_density: float = Field(ge=0.0, le=1.0, description="Technical terminology density")
    domain_specificity_score: float = Field(ge=0.0, le=1.0, description="Domain specificity indicator")
    complexity_score: float = Field(ge=0.0, le=1.0, description="Content complexity score")
    analysis_confidence: float = Field(default=StatisticalConstants.TECHNICAL_CONTENT_SIMILARITY_THRESHOLD, ge=0.0, le=1.0)
...
class ExtractedKnowledge(BaseModel):
    ...
    confidence_scores: Dict[str, float] = Field(description="Extraction confidence scores")
...
class ExtractionResults(BaseModel):
    ...
    quality_assessment: Dict[str, float] = Field(description="Quality assessment scores")
    performance_metrics: Dict[str, float] = Field(description="Performance metrics")
...
class UnifiedExtractionResult(BaseModel):
    ...
    extraction_method: str = Field(default="unified", description="Extraction method used")
    confidence_distribution: Dict[str, float] = Field(default_factory=dict, description="Confidence score distribution")
...
class ValidationResult(BaseModel):
    ...
    missing_keys: List[str] = Field(default_factory=list, description="Missing required keys")
    invalid_values: List[str] = Field(default_factory=list, description="Invalid value descriptions")
    source_validation: List[Dict[str, Any]] = Field(default_factory=list, description="Source-specific validation results")
    warnings: List[str] = Field(default_factory=list, description="Validation warnings")
    confidence: float = Field(default=1.0, ge=0.0, le=1.0, description="Validation confidence")
    validation_time: float = Field(default=0.0, ge=0.0, description="Validation processing time")
...
class ErrorHandlingContract(BaseModel):
    ...
    supported_error_types: List[ErrorCategory] = Field(description="Supported error categories")
    error_recovery_strategies: Dict[str, str] = Field(description="Error recovery strategies")
    escalation_rules: Dict[str, str] = Field(description="Error escalation rules")
    monitoring_requirements: Dict[str, Any] = Field(description="Error monitoring requirements")
...
class DomainIntelligenceDeps(BaseModel):
    ...
    azure_services: Optional[Any] = Field(default=None, description="Azure services container")
    cache_manager: Optional[Any] = Field(default=None, description="Cache manager instance")
    hybrid_analyzer: Optional[Any] = Field(default=None, description="Hybrid domain analyzer")
    pattern_engine: Optional[Any] = Field(default=None, description="Pattern extraction engine")
    config_generator: Optional[Any] = Field(default=None, description="Configuration generator")
...
class KnowledgeExtractionDeps(BaseModel):
    ...
    azure_services: Optional[Any] = Field(default=None, description="Azure services container")
    cache_manager: Optional[Any] = Field(default=None, description="Cache manager instance")
    extraction_processor: Optional[Any] = Field(default=None, description="Unified extraction processor")
    validation_processor: Optional[Any] = Field(default=None, description="Validation processor")
...
class UniversalSearchDeps(BaseModel):
    ...
    azure_services: Optional[Any] = Field(default=None, description="Azure services container")
    cache_manager: Optional[Any] = Field(default=None, description="Cache manager instance")
    search_orchestrator: Optional[Any] = Field(default=None, description="Search orchestrator")
    domain_detector: Optional[Any] = Field(default=None, description="Domain detection service")
...
class MemoryStatus:
    ...
    def utilization_percent(self) -> float:
        ...
        if self.memory_limit_mb == 0:
            return 0.0
        from agents.core.math_expressions import EXPR
        return EXPR.calculate_utilization_percentage(self.estimated_usage_mb, self.memory_limit_mb)
...
class ExtractionContext(PydanticAIContextualModel):
    ...
    processing_hints: Dict[str, Any] = Field(default_factory=dict, description="Processing hints and parameters")
    
    # Quality indicators
    text_quality_score: float = Field(default=0.8, ge=0.0, le=1.0, description="Quality score of input text")
...
class TextStatistics(BaseModel):
    ...
    avg_words_per_sentence: float = Field(ge=0.0, description="Average words per sentence")
    avg_chars_per_word: float = Field(ge=0.0, description="Average characters per word")
    lexical_diversity: float = Field(ge=0.0, le=1.0, description="Lexical diversity score")
    ...
    def calculate_readability(self) -> float:
        ...
        if self.sentence_count == 0 or self.word_count == 0:
            return 0.0
        return min(100.0, max(0.0, 206.835 - (1.015 * self.avg_words_per_sentence) - (84.6 * (self.avg_chars_per_word / 4.7))))
...
class ConfidenceScore(BaseModel):
    ...
    weight: float = Field(ge=0.0, le=1.0, description="Weight of this score in aggregation")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
...
class RelationshipConfidenceFactors(BaseModel):
    ...
    entity_confidence_product: float = Field(ge=0.0, le=1.0, description="Product of related entity confidences")
    relationship_type_clarity: float = Field(ge=0.0, le=1.0, description="Relationship type clarity")
    contextual_support: float = Field(ge=0.0, le=1.0, description="Contextual evidence support")
    semantic_coherence: float = Field(ge=0.0, le=1.0, description="Semantic coherence score")
    distance_penalty: float = Field(ge=0.0, le=1.0, description="Distance-based penalty factor")
    domain_pattern_match: float = Field(ge=0.0, le=1.0, description="Domain pattern matching score")
...
class TextCleaningOptions(BaseModel):
    ...
    preserve_sentence_structure: bool = Field(default=True, description="Preserve sentence boundaries")
...
class CleanedContent(BaseModel):
    ...
    cleaning_options: TextCleaningOptions = Field(description="Cleaning options applied")
    ...
    # Quality metrics
    cleaning_quality_score: float = Field(ge=0.0, le=1.0, description="Quality of cleaning operation")
    content_preservation_score: float = Field(ge=0.0, le=1.0, description="How well original content was preserved")
    
    cleaning_warnings: List[str] = Field(default_factory=list, description="Warnings from cleaning process")


class ContentChunker(BaseModel):
    """Configuration for content chunking operations"""
    
    chunk_size: int = Field(ge=50, le=5000, description="Target chunk size in characters")
    chunk_overlap: int = Field(ge=0, le=1000, description="Overlap between chunks in characters")
    respect_sentence_boundaries: bool = Field(default=True, description="Respect sentence boundaries")
    respect_paragraph_boundaries: bool = Field(default=True, description="Respect paragraph boundaries")
...
class ContentChunk(BaseModel):
    ...
    # Overlap information
    overlap_with_previous: int = Field(ge=0, description="Characters overlapping with previous chunk")
    overlap_with_next: int = Field(ge=0, description="Characters overlapping with next chunk")
    
    # Quality indicators
    boundary_quality: float = Field(ge=0.0, le=1.0, description="Quality of chunk boundaries")
    content_completeness: float = Field(ge=0.0, le=1.0, description="Completeness of content in chunk")
...
class KnowledgeExtractionResult(PydanticAIContextualModel):
    ...
    entities: List[Dict[str, Any]] = Field(default_factory=list, description="Extracted and validated entities")
    relationships: List[Dict[str, Any]] = Field(default_factory=list, description="Extracted and validated relationships")
    
    # Extraction statistics
    entity_count: int = Field(ge=0, description="Total number of entities extracted")
    relationship_count: int = Field(ge=0, description="Total number of relationships extracted")
    unique_entity_types: int = Field(ge=0, description="Number of unique entity types")
    unique_relationship_types: int = Field(ge=0, description="Number of unique relationship types")
    
    # Quality metrics
    avg_entity_confidence: float = Field(ge=0.0, le=1.0, description="Average confidence across entities")
    avg_relationship_confidence: float = Field(ge=0.0, le=1.0, description="Average confidence across relationships")
    extraction_quality_score: float = Field(ge=0.0, le=1.0, description="Overall extraction quality")
    
    # Graph metrics
    entity_pairs_connected: int = Field(ge=0, description="Number of entity pairs connected by relationships")
    graph_density: float = Field(ge=0.0, le=1.0, description="Density of the relationship graph")
    connected_components: int = Field(ge=0, description="Number of disconnected graph components")
    ...
    strategies_used: List[str] = Field(default_factory=list, description="List of extraction strategies applied")
    
    # Validation results
    validation_passed: bool = Field(default=True, description="Whether PydanticAI validation passed")
    ...
    def validate_relationship_count(cls, v, values):
        if 'relationships' in values and v != len(values['relationships']):
            raise ValueError('relationship_count must match length of relationships list')
        return v
...
class KnowledgeValidationResult(BaseModel):
    ...
    relationship_count: int = Field(ge=0, description="Number of relationships validated")
...
class ValidatedEntity(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0, le=1.0, 
        description="Confidence score - agent determines threshold"
    )
    entity_type: str = Field(
        min_length=1, 
        description="Entity classification - learned from domain"
    )
...
class ValidatedRelationship(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0, le=1.0,
        description="Relationship confidence - agent learned threshold"
    )
...
class ContentAnalysisOutput(BaseModel):
    ...
    complexity_score: float = Field(
        ge=0.0, le=1.0, 
        description="Content complexity - agent-driven assessment"
    )
    ...
    technical_density: float = Field(
        ge=0.0, le=1.0,
        description="Technical term density - domain-specific threshold"
    )
...
class ConsolidatedAzureConfiguration(PydanticAIContextualModel):
    ...
    openai_api_version: str = Field(default_factory=lambda: InfrastructureConstants.OPENAI_API_VERSION)
    ...
    openai_timeout: int = Field(default_factory=lambda: PerformanceAdaptiveConstants.DEFAULT_TIMEOUT)
    search_timeout: int = Field(default_factory=lambda: PerformanceAdaptiveConstants.DEFAULT_TIMEOUT)
    cosmos_timeout: int = Field(default_factory=lambda: PerformanceAdaptiveConstants.DEFAULT_TIMEOUT)
    
    # Request limits (from constants, not hardcoded)
    max_tokens: int = Field(default_factory=lambda: InfrastructureConstants.MAX_TOKENS_GPT4)
    max_search_results: int = Field(default_factory=lambda: InfrastructureConstants.MAX_SEARCH_RESULTS)
    max_retries: int = Field(default_factory=lambda: PerformanceAdaptiveConstants.MAX_RETRIES)
    ...
    @classmethod
    def from_centralized_config(cls, domain_name: str = None) -> "ConsolidatedAzureConfiguration":
...
class ConsolidatedExtractionConfiguration(PydanticAIContextualModel):
    ...
    max_entities_per_chunk: int = Field(default_factory=lambda: KnowledgeExtractionConstants.MAX_ENTITIES_PER_CHUNK)
    min_relationship_strength: float = Field(default_factory=lambda: KnowledgeExtractionConstants.MIN_RELATIONSHIP_STRENGTH)
    
    # Domain context (from Domain Intelligence Agent)
    domain_name: str = Field(description="Source domain name")
    technical_vocabulary: List[str] = Field(default_factory=list, description="Domain-specific vocabulary")
    expected_entity_types: List[str] = Field(default_factory=list, description="Expected entity types for domain")
    ...
    @classmethod
    def from_centralized_config(cls, domain_name: str) -> "ConsolidatedExtractionConfiguration":
...
class ConsolidatedSearchConfiguration(PydanticAIContextualModel):
    ...
    # Domain context
    domain_name: str = Field(description="Source domain name")
    learned_at: datetime = Field(default_factory=datetime.now, description="Configuration generation timestamp")
    ...
    @classmethod
    def from_centralized_config(cls, domain_name: str) -> "ConsolidatedSearchConfiguration":
...
def validate_relationship_extraction(relationships: List[Dict[str, Any]]) -> List[ValidatedRelationship]:
...
class KnowledgeExtractionResult(PydanticAIContextualModel):
    ...
    # Core extraction results
    entities: List[ValidatedEntity] = Field(default_factory=list, description="Extracted and validated entities")
    relationships: List[ValidatedRelationship] = Field(default_factory=list, description="Extracted and validated relationships")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "processing_metadata": {
                "processing_time_ms": self.processing_time_ms,
                "text_length": self.text_length,
                "processing_efficiency": self.text_length / max(1, self.processing_time_ms) * 1000  # chars per second
            }
        }
...
class DomainAnalysisResult(PydanticAIContextualModel):
    ...
    text_statistics: TextStatistics = Field(..., description="Statistical text analysis")
    cleaning_result: CleanedContent = Field(..., description="Text preprocessing results")
    
    # Domain-specific intelligence
    domain_patterns: Dict[str, List[str]] = Field(default_factory=dict, description="Detected domain patterns")
    technical_vocabulary: List[str] = Field(default_factory=list, description="Identified technical terms")
    concept_hierarchy: Dict[str, float] = Field(default_factory=dict, description="Concept importance hierarchy")
    
    # Quality and confidence metrics
    analysis_confidence: ConfidenceScore = Field(..., description="Overall analysis confidence")
    domain_fit_score: float = Field(ge=0.0, le=1.0, description="Fit to detected domain")
    processing_quality: str = Field(..., description="Processing quality tier")
    
    # Advanced analytics
    tfidf_features: Dict[str, float] = Field(default_factory=dict, description="TF-IDF feature importance")
    semantic_clusters: Dict[str, Any] = Field(default_factory=dict, description="Semantic clustering results")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            "domain_analysis": {
                ...
                "domain_intelligence": {
                    "pattern_count": sum(len(patterns) for patterns in self.domain_patterns.values()),
            ...
            "processing_metadata": {
                ...
                "cleaning_applied": len(self.cleaning_result.preprocessing_steps) if hasattr(self.cleaning_result, 'preprocessing_steps') else 0
            }
        }
...
class DomainIntelligenceConfig(PydanticAIContextualModel):
    ...
    # Analysis depth
    enable_advanced_analytics: bool = Field(default=True, description="Enable TF-IDF and clustering")
    enable_pattern_detection: bool = Field(default=True, description="Enable domain pattern detection")
    enable_quality_assessment: bool = Field(default=True, description="Enable quality assessment")
    
    # Pattern detection parameters
    min_pattern_frequency: int = Field(default=2, ge=1, description="Minimum frequency for pattern detection")
    max_patterns_per_type: int = Field(default=20, ge=1, description="Maximum patterns per type")
    
    # TF-IDF parameters
    tfidf_max_features: int = Field(default=1000, ge=100, description="Maximum TF-IDF features")
    tfidf_min_df: int = Field(default=2, ge=1, description="Minimum document frequency")
    tfidf_max_df: float = Field(default=0.8, gt=0.0, lt=1.0, description="Maximum document frequency ratio")
    
    # Clustering parameters  
    n_clusters: int = Field(default=5, ge=2, le=20, description="Number of semantic clusters")
    cluster_random_state: int = Field(default=42, description="Random state for clustering")
    
    # Confidence calculation
    confidence_method: ConfidenceMethod = Field(default=ConfidenceMethod.ADAPTIVE, description="Confidence calculation method")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_settings": {
                "method": self.confidence_method.value if hasattr(self.confidence_method, 'value') else str(self.confidence_method),
...
class DynamicExtractionConfig(PydanticAIContextualModel):
    ...
    entity_confidence_threshold: float = Field(..., description="Dynamic entity confidence threshold")
    relationship_confidence_threshold: float = Field(..., description="Dynamic relationship confidence threshold")
    ...
    max_entities_per_chunk: int = Field(..., ge=5, le=500, description="Dynamic max entities per chunk")
    min_relationship_strength: float = Field(..., ge=0.0, le=1.0, description="Dynamic minimum relationship strength")
    quality_validation_threshold: float = Field(..., ge=0.0, le=1.0, description="Dynamic quality validation threshold")
    ...
    corpus_stats: Dict[str, Any] = Field(default_factory=dict, description="Corpus statistics used for learning")
...
class DynamicSearchConfig(PydanticAIContextualModel):
    ...
    vector_similarity_threshold: float = Field(..., ge=0.0, le=1.0, description="Dynamic vector similarity threshold")
    vector_top_k: int = Field(..., ge=1, le=100, description="Dynamic vector top-k results")
    graph_hop_count: int = Field(..., ge=1, le=5, description="Dynamic graph hop count")
    graph_min_relationship_strength: float = Field(..., ge=0.0, le=1.0, description="Dynamic graph minimum relationship strength")
    gnn_prediction_confidence: float = Field(..., ge=0.0, le=1.0, description="Dynamic GNN prediction confidence")
    gnn_node_embeddings: int = Field(..., ge=64, le=1024, description="Dynamic GNN node embeddings dimension")
    tri_modal_weights: Dict[str, float] = Field(..., description="Dynamic tri-modal weights (vector, graph, gnn)")
    result_synthesis_threshold: float = Field(..., ge=0.0, le=1.0, description="Dynamic result synthesis threshold")
    domain_name: str = Field(..., description="Domain this configuration applies to")
    learned_at: datetime = Field(..., description="When this configuration was learned")
    query_complexity_weights: Dict[str, Any] = Field(default_factory=dict, description="Query complexity weighting factors")
...
class ConsolidatedAzureServices(PydanticAIContextualModel):
    ...
    credential_initialized: bool = Field(default=False, description="Whether DefaultAzureCredential is initialized")

    # Azure AI Foundry for LLM services
    ai_foundry_provider_initialized: bool = Field(default=False, description="Whether Azure AI Foundry provider is initialized")

    # Non-LLM Azure services status
    search_client_initialized: bool = Field(default=False, description="Whether Azure Search client is initialized")
    cosmos_client_initialized: bool = Field(default=False, description="Whether Cosmos client is initialized")
    storage_client_initialized: bool = Field(default=False, description="Whether Storage client is initialized")
    ml_client_initialized: bool = Field(default=False, description="Whether ML client is initialized")

    # Search orchestration services
    tri_modal_orchestrator_initialized: bool = Field(default=False, description="Whether tri-modal orchestrator is initialized")

    # Service status tracking
    initialized_services: Dict[str, bool] = Field(default_factory=dict, description="Service initialization status")
    initialization_errors: Dict[str, str] = Field(default_factory=dict, description="Service initialization errors")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            "azure_services": {
                ...
                "status_summary": {
                    "initialized_count": sum(1 for status in self.initialized_services.values() if status),
                    "total_services": len(self.initialized_services),
                    "error_count": len(self.initialization_errors),
                    "initialization_success_rate": (
                        sum(1 for status in self.initialized_services.values() if status) / 
...
class StateTransferPacket(PydanticAIContextualModel):
    ...
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Transfer timestamp")
    expiry: Optional[datetime] = Field(default=None, description="Transfer expiry time")
    dependencies: List[str] = Field(default_factory=list, description="Transfer dependencies")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "transfer_timing": {
                ...
                "time_to_expiry": (self.expiry - datetime.now(timezone.utc)).total_seconds() if self.expiry else None
            },
            "data_integrity": {
...
class GraphConnectionInfo(PydanticAIContextualModel):
    ...
    last_transfer: Optional[datetime] = Field(default=None, description="Last successful transfer timestamp")
    total_transfers: int = Field(default=0, ge=0, description="Total number of transfers")
    failed_transfers: int = Field(default=0, ge=0, description="Number of failed transfers")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timing_info": {
                "last_transfer": self.last_transfer.isoformat() if self.last_transfer else None,
                "time_since_last_transfer": (
                    (datetime.now(timezone.utc) - self.last_transfer).total_seconds() 
                    if self.last_transfer else None
                )
            }
        }
...
class InfrastructureConfig(PydanticAIContextualModel):
    ...
    confidence: float = Field(..., ge=0.0, le=1.0, description="Configuration confidence")
    primary_concepts: List[str] = Field(default_factory=list, description="Primary domain concepts")
...
class MLModelConfig(PydanticAIContextualModel):
    ...
    domain: str = Field(..., description="Domain name")
    node_feature_dim: int = Field(..., ge=1, le=1024, description="Node feature dimension")
    ...
    entity_types: List[str] = Field(default_factory=list, description="Supported entity types")
    relationship_types: List[str] = Field(default_factory=list, description="Supported relationship types")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Model configuration confidence")
...
class CompleteDomainConfig(PydanticAIContextualModel):
    ...
    domain: str = Field(..., description="Domain name")
    infrastructure: InfrastructureConfig = Field(..., description="Infrastructure configuration")
    ...
    generation_confidence: float = Field(..., ge=0.0, le=1.0, description="Overall generation confidence")
    ...
    def run_context_data(self) -> Dict[str, Any]:
        ...
        return {
            "complete_domain_config": {
                ...
                "infrastructure": self.infrastructure.run_context_data["infrastructure_config"],
                "ml_model": self.ml_model.run_context_data["ml_model_config"],
                "patterns": {
                    "pattern_count": len(self.patterns.patterns) if hasattr(self.patterns, 'patterns') else 0,
                    "statistical_count": len(self.patterns.statistical) if hasattr(self.patterns, 'statistical') else 0,
                    "semantic_count": len(self.patterns.semantic) if hasattr(self.patterns, 'semantic') else 0
                }
            }
        }
...
__all__ = [
    # Enums
    "HealthStatus", "ProcessingStatus", "WorkflowState", "NodeState",
    "ErrorSeverity", "ErrorCategory", "SearchType", "MessageType", "ConfidenceMethod", "StateTransferType",
    # "ModelCapability", "QueryComplexity",  # Removed - using static model selection
    
    # Base Models
    "BaseRequest", "BaseResponse", "BaseAnalysisResult",
    
    # Unified Configuration Architecture
    "UnifiedAgentConfiguration",  # UnifiedConfigurationResolver deleted
    
    # Enhanced Azure Service Models
    "AzureServiceConfiguration", "AzureServiceMetrics",  # AzureServiceHealthCheck deleted in Phase 10
    "AzureMLModelMetadata", "AzureSearchIndexSchema", "AzureCosmosGraphSchema",
    
    # Performance Feedback Integration Models
    "PerformanceFeedbackPoint",  # PerformanceFeedbackCollector deleted
    
    # Agent Contract Models
    "StatisticalPattern", "DomainStatistics", "DomainAnalysisContract",
    "KnowledgeExtractionContract", "UniversalSearchContract",
    
    # Configuration Models
    "SynthesisWeights", "DomainConfig", "ExtractionConfiguration",
    "VectorSearchConfig", "GraphSearchConfig", "GNNSearchConfig",
    
    # Request Models
    "QueryRequest", "VectorSearchRequest", "GraphSearchRequest", 
    "TriModalSearchRequest", "DomainDetectionRequest", "PatternLearningRequest",
    
    # Response Models
    "SearchResult", "SearchResponse", "DomainDetectionResult", "AnalysisResult",
    
    # Workflow Models
    "WorkflowExecutionState", "NodeExecutionResult", "WorkflowResultContract",
    
    # Analysis Models
    "StatisticalAnalysis",
    "ExtractedKnowledge", "ExtractionResults", "UnifiedExtractionResult", "TriModalSearchResult",
    
    # Validation Models
    "ValidationResult", "QualityMetrics", "ErrorContext", "ErrorHandlingContract",
    
    # Dependency Models (SharedDeps deleted in Phase 4)
    "DomainIntelligenceDeps", "KnowledgeExtractionDeps", "UniversalSearchDeps",
    ...
    # Specialized Models
    "CacheEntry", "CacheMetrics", "ServiceHealth",
    ...
    "ConsolidatedAzureConfiguration", "ConsolidatedExtractionConfiguration", "ConsolidatedSearchConfiguration",
    
    # Tier 3: Enhanced Agent Contracts (DELETED - redundant with basic contracts)
    
    # Foundation Models
    "ConfigurationResolver", "PydanticAIContextualModel",
    
    # Factory Functions (deleted in Phase 4 - were unused)
    # "create_base_request", "create_validation_result", "create_error_context",
    
    # Additional Core Module Models
    "ErrorContext", "ErrorMetrics", "CacheEntry", "CachePerformanceMetrics", 
    ...
    "CacheKeyMapping", "WorkflowStateBridge", "ServiceContainerConfig",

    # Domain Intelligence Analyzer Models
    "UnifiedAnalysis", "BackgroundProcessingConfig", "BackgroundProcessingResult",  # ContentQuality deleted in Phase 9
    "LLMExtraction", "ProcessingStats", "DomainSignature", "DomainConfig", "LearnedPattern", "ExtractedPatterns",  # PatternStatistics deleted
    "DomainGenerationConfig", "DomainPatternResult", "DomainConfigResult",

    # Dynamic Manager Models (RuntimeConfigurationData deleted in Phase 4, ModelSelectionCriteria deleted in Phase 5)

    # Search Orchestrator Models
    "SearchCoordinationResult", "TriModalResult", "TriModalSearchResult",
    
    # PydanticAI Output Validator Models
    "ExtractionQualityOutput", "ValidatedEntity", "ValidatedRelationship", 
    "ContentAnalysisOutput", "DomainConfigurationOutput",
    
    # Knowledge Extraction Models
    "ValidationResult", "KnowledgeExtractionResult",
    
    # Domain Intelligence Additional Models
    "DomainAnalysisResult", "DomainIntelligenceConfig",
    
    # Core Scattered Models
    "DynamicExtractionConfig", "DynamicSearchConfig", "ConsolidatedAzureServices",
    
    # Workflow Models
    "StateTransferPacket", "GraphConnectionInfo",
    
    # Additional Domain Intelligence Models
    "InfrastructureConfig", "MLModelConfig", "CompleteDomainConfig",
    
    # Universal Search Models
    "ModalityResult",
    
    # PydanticAI Output Validator Functions
    "validate_extraction_quality", "validate_entity_extraction", "validate_relationship_extraction",
    "validate_content_analysis", "validate_domain_configuration",
]
...
class PreComputedPromptLibrary:
    ...
    async def get_cached_prompts(
        self, content: str, force_refresh: bool = False
...
async def get_or_generate_auto_prompts(
    content: str, force_refresh: bool = False, verbose: bool = False
) -> CachedPromptEntry:
    """
    ...
    if not force_refresh:
        ...
        if cached_entry:
            if verbose:
                print(
                    f"✅ Using cached prompts for content hash: {cached_entry.content_hash}"
                )
    ...
    if verbose:
        print(
            f"   ✅ Generated {len(extraction_prompts)} prompts and {len(template_variables)} template variables"
        )

...
from .config_enforcement import AntiHardcodingEnforcer, ConfigurationEnforcementError
...
class IntelligentConfigProvider:
    ...
    def get_search_config(self, domain: str) -> Dict[str, Any]:
        ...
        for key, value in config.items():
            if key.endswith('_source'):
                ...
                # Actual config values - validate through enforcer
    ...
    def _force_config_extraction(self, domain: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print("⚠️  Config-Extraction workflow not available. Creating development configuration...")
    ...
    def get_provider_status(self) -> Dict[str, Any]:
        ...
        enforcement_report = self.enforcer.get_violation_report()
        ...
        return {
            ...
            "total_violations_detected": enforcement_report["total_violations"]
        }
        ...
        try:
            ...
            for key in required_keys:
                if key not in config:
                    ...
                    if source_key not in config:
                        validation_result["source_validation"].append(f"Missing source for {key}")
                    elif "hardcoded" in config[source_key]:
                        validation_result["source_validation"].append(f"Hardcoded source for {key}")
                        validation_result["valid"] = False
            ...
            if 'similarity_threshold' in config:
                ...
                if not isinstance(threshold, (int, float)) or not 0.0 <= threshold <= 1.0:
                    validation_result["invalid_values"].append("similarity_threshold must be between 0.0 and 1.0")
                    validation_result["valid"] = False

...
class Agent1EssentialProcessingConfig(BaseModel):
    ...
    expected_extraction_quality: float = Field(
        default=0.75, ge=0.0, le=1.0, description="Used by validation - has default"
    )
    processing_complexity: str = Field(
        default="medium", description="Used by resource allocation - has default"
    )
...
class Agent1EssentialOutputSchema(BaseModel):
    ...
    content_type_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="USED BY: Validation and debug output"
    )
...
class Agent1UsageMapping:
    ...
    WORKING_INTEGRATION = {
        ...
        "sentence_complexity": [
            "Templates {{ concept_density }} via centralized mapping - WORKING"
        ],
        "content_patterns": [
            "Templates {{ discovered_content_patterns }} via centralized mapping - WORKING"
        ],
        "key_content_terms": [
            "Templates {{ discovered_entity_types }} via centralized mapping - WORKING"
        ],
    }
...
class Agent1TemplateMapping:
    ...
    def extract_template_variables(agent1_output: Agent1EssentialOutputSchema) -> dict:
        ...
        return {
            ...
            "concept_density": agent1_output.characteristics.sentence_complexity

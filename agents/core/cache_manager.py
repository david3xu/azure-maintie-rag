...
class CacheEntry:
    ...
    access_count: int
    ttl: int
    cache_type: str = "general"  # general, domain_signature, domain_config, query_mapping
...
class CachePerformanceMetrics:
    ...
    def record_request(self, is_hit: bool, lookup_time: float, lookup_type: str = "general"):
        ...
        if self.total_requests == 1:
            ...
            self.average_lookup_time = (self.average_lookup_time * 0.9) + (lookup_time * 0.1)
...
class QueryPatternIndex:
    ...
    def add_domain_patterns(self, domain: str, entity_patterns: List[Dict], action_patterns: List[Dict]):
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._lock:
            return {
                ...
                "total_pattern_mappings": sum(len(domains) for domains in self.phrase_to_domains.values())
            }
...
class UnifiedCacheManager:
    ...
    def __init__(self, max_size: int = 2000, default_ttl: int = 3600, cache_dir: Optional[Path] = None):
        """
        ...
        self.cache_dir = cache_dir or Path(__file__).parent.parent.parent / "cache" / "unified"
        ...
        self._query_cache: Dict[str, Tuple[str, float, float]] = {}  # hash -> (domain, confidence, timestamp)
        ...
        logger.info(f"Unified cache manager initialized: max_size={max_size}, ttl={default_ttl}s")
    ...
    async def _evict_lru_entries(self, target_count: int = None):
        ...
        evicted = 0
        while len(self._cache) > (self.max_size - target_count) and self._access_order and evicted < target_count:
    ...
    async def set(self, operation: str, params: Dict[str, Any], data: Any, ttl: Optional[int] = None, cache_type: str = "general") -> bool:
    ...
    async def set_domain_signature(self, domain: str, signature: Any, ttl: Optional[int] = None) -> bool:
        """Cache domain signature with pattern indexing"""
        success = await self.set("domain_signature", {"domain": domain}, signature, ttl, "domain_signature")
        
        if success and hasattr(signature, 'entity_patterns') and hasattr(signature, 'action_patterns'):
            # Add to pattern index for fast query matching
            self.pattern_index.add_domain_patterns(
                domain, 
                signature.entity_patterns, 
                signature.action_patterns
            )
    ...
    async def cached_operation(self, operation: str, params: Dict[str, Any], executor_func, ttl: Optional[int] = None):
    ...
    async def clear_expired(self):
        ...
        async with self._lock:
            ...
            expired_queries = [
                query_hash for query_hash, (_, _, timestamp) in self._query_cache.items()
            ...
            if expired_keys or expired_queries:
                logger.info(f"Cleared {len(expired_keys)} expired cache entries, {len(expired_queries)} expired queries")
        ...
        with self._sync_lock:
            return {
                "cache_stats": {
                    ...
                    "active_entries": len([e for e in self._cache.values() if not e.is_expired()]),
        ...
        try:
            for cache_file in self.cache_dir.glob("*.json"):
                ...
                if time.time() - cache_data["created_at"] < cache_data["ttl"]:
                    entry = CacheEntry(
                        data=self._deserialize_data(cache_data["data"], cache_data.get("cache_type", "general")),
                    ...
                    if (entry.cache_type == "domain_signature" and 
                        hasattr(entry.data, 'entity_patterns') and 
                        hasattr(entry.data, 'action_patterns')):
    ...
    def set_domain_config(self, domain: str, config: Any, ttl: Optional[int] = None) -> bool:
    ...
    def set_extraction_config(self, domain: str, config: Any, ttl: Optional[int] = None) -> bool:
    ...
    def set_domain_signature(self, domain: str, signature: Any, ttl: Optional[int] = None) -> bool:
    ...
    def set_query_domain_mapping(self, query_hash: str, domain: str, score: float, ttl: Optional[int] = None) -> bool:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_types": list(set(entry.cache_type for entry in self._cache.values()))
        }
...
async def cached_operation(operation: str, params: Dict[str, Any], executor_func, ttl: Optional[int] = None):

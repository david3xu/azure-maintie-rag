    ...
    try:
        ...
        if azure_services is None:
            ...
            try:
                ...
                azure_services = loop.run_until_complete(
        ...
        if not azure_endpoint:
            logger.warning(
                "AZURE_OPENAI_ENDPOINT not configured - PydanticAI will not be available"
            )
            return None
        ...
        logger.info(
            f"✅ PydanticAI Azure provider created with ConsolidatedAzureServices for: {azure_endpoint}"
        )
...
async def create_azure_pydantic_provider_async(
    ...
    try:
        ...
        logger.info(
            f"✅ PydanticAI Azure provider created (async) with ConsolidatedAzureServices for: {azure_endpoint}"
        )
        return azure_provider
...
def create_pydantic_agent(
    ...
    if not azure_provider:
        logger.error(
            "Cannot create PydanticAI agent - Azure provider initialization failed"
        )
        return None
    ...
    try:
        ...
        logger.info(
            f"✅ PydanticAI agent '{agent_name}' created successfully with model: {model_name} and ConsolidatedAzureServices"
        )
        return agent
...
async def create_pydantic_agent_async(
    ...
    try:
        ...
        logger.info(
            f"✅ PydanticAI agent '{agent_name}' created successfully (async) with model: {model_name} and ConsolidatedAzureServices"
        )
        return agent
    ...
    try:
        ...
        agent = create_pydantic_agent(
            model_name=os.getenv(
                "OPENAI_MODEL_DEPLOYMENT", model_config.deployment_name
            ),
        ...
        logger.info(
            "✅ PydanticAI Azure provider connection test successful with ConsolidatedAzureServices"
        )
        return True
...
async def test_azure_provider_connection_async(
    ...
    try:
        ...
        logger.info(
            "✅ PydanticAI Azure provider connection test successful (async) with ConsolidatedAzureServices"
        )
        return True
...
if __name__ == "__main__":
    ...
    async def test_configuration():
        ...
        print(
            "Testing Enhanced PydanticAI Azure Provider Configuration with ConsolidatedAzureServices..."
        )
        ...
        try:
            ...
            if provider:
                print(
                    "✅ Azure provider created successfully with ConsolidatedAzureServices"
                )
            else:
            ...
            if agent:
                print(
                    "✅ PydanticAI agent created successfully with ConsolidatedAzureServices"
                )
            else:

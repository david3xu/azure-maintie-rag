...
class UnifiedMemoryManager:
    ...
    def __init__(self, memory_limit_mb: float = 200.0, cleanup_threshold: float = 0.8):
        """
        ...
        self.cleanup_target = cleanup_threshold * 0.75  # Clean down to 75% of threshold
        ...
        logger.info(f"Unified memory manager initialized: limit={memory_limit_mb}MB, threshold={cleanup_threshold}")
    ...
    def _estimate_item_size(self, item: Any) -> int:
        ...
        try:
            if isinstance(item, str):
                ...
                return sample_size * max(1, len(item) // 10)
    ...
    def _should_cleanup(self) -> bool:
        ...
        return self.status.utilization_percent >= (self.cleanup_threshold * 100)
    ...
    async def _perform_cleanup(self) -> int:
        ...
        while (sum(self._item_sizes.values()) > target_usage and 
               self._access_order and 
               items_removed < safety_limit):
    ...
    def _update_status(self):
        ...
        self.status.estimated_usage_mb = sum(self._item_sizes.values()) / (1024 * 1024)
    ...
    async def store_item(self, key: str, item: Any, category: str = "default") -> bool:
        ...
        async with self._lock:
            try:
                ...
                current_usage = sum(self._item_sizes.values())
                projected_usage_mb = (current_usage + item_size) / (1024 * 1024)
    ...
    async def clear_category(self, category: str) -> int:
        ...
        async with self._lock:
            keys_to_remove = [
                key for key, cat in self._item_categories.items() 
                if cat == category
            ]
            ...
            logger.info(f"Cleared {len(keys_to_remove)} items from category '{category}'")
    ...
    async def perform_maintenance(self):
        ...
        async with self._lock:
            ...
            if initial_items != final_items:
                logger.info(f"Maintenance complete: {initial_items - final_items} items cleaned")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        hit_rate = (self.metrics['cache_hits'] / max(1, total_requests)) * 100
        ...
        return {
            ...
            "cleanup_configuration": {
                "cleanup_threshold_percent": self.cleanup_threshold * 100,
                "cleanup_target_percent": self.cleanup_target * 100
            }
        }
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_healthy": self.status.utilization_percent < 85,
            "utilization_percent": self.status.utilization_percent,
            "total_items": self.status.total_items,
            "within_limits": self.status.estimated_usage_mb < self.memory_limit_mb,
            "last_cleanup": self.status.last_cleanup,
            "performance_acceptable": self.metrics['avg_retrieval_time'] < 0.01  # <10ms acceptable
        }
    ...
    def _get_category_breakdown(self) -> Dict[str, Dict[str, Any]]:
        ...
        for category, stats in category_stats.items():
            if stats["item_count"] > 0:
                stats["avg_size_bytes"] = stats["total_size_bytes"] / stats["item_count"]
        ...
        for key in self._access_order[:limit]:  # First items are LRU
            if key in self._items:
                lru_info.append({
                    ...
                    "age_seconds": time.time() - self._access_times.get(key, time.time())
                })

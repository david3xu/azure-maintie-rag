...
class SemanticChunker:
        ...
        while start_pos < len(text):
            ...
            if end_pos < len(text):
                end_pos = self._adjust_for_boundaries(
                    text, start_pos, end_pos, semantic_boundaries,
                    preserve_sentences, preserve_code_blocks
                )
    ...
    def _adjust_for_boundaries(
        ...
        if preserve_sentences:
            ...
            search_range = min(200, end_pos - start_pos // 4)  # 25% of chunk size or 200 chars
    ...
    def get_chunk_summary(self, chunked_content: ChunkedContent) -> Dict[str, Any]:
        ...
        return {
            ...
            "size_range": {
                "min_size": min(len(chunk) for chunk in chunked_content.chunks) if chunked_content.chunks else 0,
                "max_size": max(len(chunk) for chunk in chunked_content.chunks) if chunked_content.chunks else 0,
            },
            "content_analysis": {
            ...
            "overlap_analysis": {
                ...
                "average_overlap": sum(m.overlap_with_next for m in metadata) / len(metadata) if metadata else 0,
                "chunks_with_overlap": sum(1 for m in metadata if m.overlap_with_next > 0),
            }
        }

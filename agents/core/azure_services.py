...
class ConsolidatedAzureServices:
    ...
    async def initialize_all_services(self) -> Dict[str, bool]:
        ...
        # Process results
        service_names = ["ai_foundry", "search", "cosmos", "storage", "ml", "tri_modal_orchestrator"]
        for i, result in enumerate(results):
            service_name = service_names[i]
            if isinstance(result, Exception):
                self.initialized_services[service_name] = False
                self.initialization_errors[service_name] = str(result)
                logger.warning(f"⚠️ {service_name} service initialization failed: {result}")
            else:
        ...
        logger.info(f"🎯 Azure services initialization complete: {successful_services}/{total_services} services ready")
    ...
    async def _initialize_ai_foundry_provider(self) -> bool:
        ...
        try:
            ...
            if not azure_settings.azure_openai_endpoint:
                logger.info("📝 Azure OpenAI endpoint not configured, skipping AI Foundry provider")
                return False
            
            if not azure_settings.openai_api_version:
                logger.warning("⚠️ Azure OpenAI API version not configured, using default")
            ...
            logger.info(f"🤖 Azure AI Foundry provider created with managed identity for: {azure_settings.azure_openai_endpoint}")
            return True
    ...
    async def _initialize_search_client(self) -> bool:
        ...
        try:
            if not UnifiedSearchClient:
                logger.info("📝 UnifiedSearchClient not available, skipping search service")
                return False
            
            if not azure_settings.azure_search_endpoint:
                logger.info("📝 Azure Search endpoint not configured, skipping search service")
                return False
            ...
            logger.info(f"🔍 Azure Search client initialized for endpoint: {azure_settings.azure_search_endpoint}")
            return True
    ...
    async def _initialize_cosmos_client(self) -> bool:
        ...
        try:
            if not AzureCosmosGremlinClient:
                logger.info("📝 CosmosGremlinClient not available, skipping Cosmos service")
                return False
            
            if not azure_settings.azure_cosmos_endpoint:
                logger.info("📝 Azure Cosmos endpoint not configured, skipping Cosmos service")
                return False
            ...
            logger.info(f"🌐 Azure Cosmos client initialized for endpoint: {azure_settings.azure_cosmos_endpoint}")
            return True
    ...
    async def _initialize_storage_client(self) -> bool:
        ...
        try:
            if not AzureStorageClient:
                logger.info("📝 AzureStorageClient not available, skipping storage service")
                return False
            
            if not azure_settings.azure_storage_account:
                logger.info("📝 Azure Storage account not configured, skipping storage service")
                return False
            ...
            logger.info(f"💾 Azure Storage client initialized for account: {azure_settings.azure_storage_account}")
            return True
    ...
    async def _initialize_ml_client(self) -> bool:
        ...
        try:
            ...
            logger.info(f"🧠 Azure ML client initialized for workspace: {azure_settings.azure_ml_workspace_name}")
            return True
    ...
    async def _initialize_tri_modal_orchestrator(self) -> bool:
        ...
        try:
            ...
            logger.info("📝 Tri-modal orchestrator not available, search operations may be limited")
            return False
        except Exception as e:
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_health": "healthy" if sum(self.initialized_services.values()) > 0 else "degraded"
        }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for service_name, is_initialized in self.initialized_services.items():
            ...
            try:
                if service_name == "search" and self.search_client:
                    ...
                    health_results[service_name] = "healthy"  # Provider doesn't need connection test
                elif service_name == "tri_modal_orchestrator" and self.tri_modal_orchestrator:
                    # Test orchestrator health
                    orchestrator_health = await self.tri_modal_orchestrator.health_check()
        ...
        logger.info(f"🎯 Health check complete: {healthy_services}/{total_checked} services healthy")

...
class DomainIntelligenceToolset(FunctionToolset):
    ...
    def __init__(self):
        ...
        self.add_function(self.create_learned_extraction_config, name='create_learned_extraction_config')

    async def discover_available_domains(
        self, ctx: RunContext[AzureServicesDeps], data_dir: str = "/workspace/azure-maintie-rag/data/raw"
    ) -> Dict[str, Any]:
    ...
    async def analyze_corpus_statistics(
        ...
        for i in range(len(top_tokens) - 1):
            ...
            n_gram_patterns[bigram] = (
                token_frequencies.get(top_tokens[i], 0) + 
        ...
        return StatisticalAnalysis(
            ...
            token_frequencies=dict(sorted_tokens[:ContentAnalysisConstants.MAX_TF_IDF_FEATURES]),
    ...
    async def generate_semantic_patterns(
        ...
        try:
            ...
            for line in lines:
                ...
                candidates = set(capitalized_words + acronyms + quoted_terms + code_patterns)
                ...
                if any(pattern in line for pattern in ['->', 'provides', 'supports', 'implements']):
    ...
    async def create_learned_extraction_config(
        ...
        config = ExtractionConfiguration(
            ...
            relationship_confidence_threshold=entity_threshold - 0.05,  # Slightly lower for relationships
            ...
            parallel_processing_threshold=chunk_size * StatisticalConstants.THRESHOLD_ADJUSTMENT_FACTOR,
    ...
    def _learn_entity_threshold(
        ...
        if vocabulary_diversity > StatisticalConstants.TECHNICAL_CONTENT_SIMILARITY_THRESHOLD:
            base_threshold = StatisticalConstants.TECHNICAL_CONTENT_SIMILARITY_THRESHOLD
        elif vocabulary_diversity > StatisticalConstants.CONSISTENT_DOMAIN_RELATIONSHIP_THRESHOLD:
        ...
        entity_diversity = len(entity_types) / CacheConstants.PERCENTAGE_MULTIPLIER if entity_types else 0
        adjusted_threshold = min(StatisticalConstants.HIGH_CONFIDENCE_LEVEL, base_threshold + entity_diversity)
    ...
    def _learn_optimal_chunk_size(self, stats: StatisticalAnalysis) -> int:
        ...
        if avg_doc_length > ContentAnalysisConstants.OPTIMAL_CHUNK_SIZE_MAX:  # Long documents
            return min(ContentAnalysisConstants.OPTIMAL_CHUNK_SIZE_MAX, int(avg_doc_length * StatisticalConstants.SHORT_DOCUMENT_CONFIDENCE_FACTOR))
        elif avg_doc_length > 1000:  # Medium documents (1000 char threshold)
            return min(ContentAnalysisConstants.CHUNK_SIZE_MAX_FALLBACK + ContentAnalysisConstants.CHUNK_SIZE_MIN_FALLBACK, int(avg_doc_length * StatisticalConstants.TECHNICAL_CONTENT_SIMILARITY_THRESHOLD))
        else:  # Short documents
            return min(ContentAnalysisConstants.CHUNK_SIZE_MAX_FALLBACK, max(ContentAnalysisConstants.CHUNK_SIZE_MIN_FALLBACK, int(avg_doc_length * StatisticalConstants.RICH_VOCABULARY_FACTOR)))
    ...
    def _estimate_response_sla(self, stats: StatisticalAnalysis) -> float:
        ...
        if complexity_score > StatisticalConstants.TECHNICAL_CONTENT_SIMILARITY_THRESHOLD:
            return ContentAnalysisConstants.HIGH_COMPLEXITY_RESPONSE_TIME  # High complexity
        elif complexity_score > ContentAnalysisConstants.MEDIUM_COMPLEXITY_THRESHOLD:
            return ContentAnalysisConstants.MEDIUM_COMPLEXITY_RESPONSE_TIME  # Medium complexity
        else:
            return ContentAnalysisConstants.LOW_COMPLEXITY_RESPONSE_TIME  # Low complexity

    async def _save_config_to_file(self, config: ExtractionConfiguration, corpus_path: str) -> Path:

...
class UnifiedDomainAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.entity_patterns = {
            ...
            'model_names': re.compile(r'\b(?:model|algorithm|system|framework)\s+\w+\b', re.IGNORECASE),
            'process_steps': re.compile(r'\b(?:step|phase|stage|procedure)\s+\d+\b', re.IGNORECASE),
            'measurements': re.compile(r'\b\d+(?:\.\d+)?\s*(?:mm|cm|m|kg|g|%|degrees?)\b', re.IGNORECASE),
            'identifiers': re.compile(r'\b[A-Z]\d+(?:-[A-Z]\d+)*\b')
        }
        
        self.action_patterns = {
            'instructions': re.compile(r'\b(?:install|configure|setup|initialize|create|build|deploy)\b', re.IGNORECASE),
            'operations': re.compile(r'\b(?:run|execute|perform|conduct|analyze|process)\b', re.IGNORECASE),
            'maintenance': re.compile(r'\b(?:maintain|repair|replace|check|inspect|clean)\b', re.IGNORECASE),
            'troubleshooting': re.compile(r'\b(?:troubleshoot|debug|fix|resolve|diagnose)\b', re.IGNORECASE)
        }
        ...
        self.performance_stats = {
            ...
            'strategy_performance': {
                ...
                'statistical_only': {'count': 0, 'avg_time': 0.0, 'avg_confidence': 0.0},
                'llm_enhanced': {'count': 0, 'avg_time': 0.0, 'avg_confidence': 0.0},
                'hybrid_full': {'count': 0, 'avg_time': 0.0, 'avg_confidence': 0.0}
            }
        }
        
        logger.info("Unified domain analyzer initialized with multi-strategy analysis capabilities")

    def _initialize_azure_client(self) -> Optional[AsyncAzureOpenAI]:
        """Initialize Azure OpenAI client for LLM analysis"""
        if not AZURE_OPENAI_AVAILABLE:
            logger.warning("Azure OpenAI not available, using content + statistical analysis only")
            return None
        ...
        try:
            ...
            if endpoint and api_key:
                ...
                logger.warning("Azure OpenAI credentials not found, using statistical analysis")
                return None
    ...
    async def analyze_unified(self, file_path: Path, force_strategy: Optional[str] = None) -> UnifiedAnalysis:
        """
        ...
        self._update_performance_stats(strategy, analysis.processing_time, analysis.analysis_confidence)
    ...
    async def _execute_strategy_analysis(self, text: str, strategy: str, file_path: Path) -> UnifiedAnalysis:
        ...
        return UnifiedAnalysis(
            ...
            # Statistical analysis results
            term_frequency_distribution=statistical_results.get('term_frequency_distribution', {}),
            ...
            statistical_signatures=statistical_results.get('statistical_signatures', {}),
            ...
            content_structure_analysis=llm_results.get('content_structure_analysis', {}),
    ...
    def _analyze_statistical(self, text: str) -> Dict[str, Any]:
        ...
        statistical_signatures = self._generate_statistical_signatures(term_freq, entropy_score, complexity_metrics)
        
        # Confidence intervals
        confidence_intervals = self._calculate_confidence_intervals(term_freq, complexity_metrics)
    ...
    async def classify_unified(
        ...
        statistical_classification = self._classify_statistical_based(analysis, target_domain)
        llm_classification = self._classify_llm_based(analysis, target_domain) if analysis.domain_concepts else {}
        
        # Determine best domain through weighted combination
        domain_scores = self._combine_classification_scores(
            content_classification, statistical_classification, llm_classification, analysis.processing_strategy
        )
        ...
        # Find best domain
        best_domain = max(domain_scores.keys(), key=lambda k: domain_scores[k]) if domain_scores else "general"
        ...
        hybrid_confidence = self._calculate_hybrid_classification_confidence(
            content_confidence, statistical_confidence, llm_confidence, analysis.processing_strategy
        )
        ...
        # Alternative domains
        alternative_domains = [(domain, score) for domain, score in domain_scores.items() if domain != best_domain]
        ...
        optimization_parameters = self._calculate_optimization_parameters(analysis, best_domain)
    ...
    async def generate_configuration_recommendations(
        ...
        extraction_strategy = self._recommend_extraction_strategy(analysis, classification)
        
        # Calculate optimal chunk parameters
        optimal_chunk_size = self._calculate_optimal_chunk_size(analysis, classification)
        chunk_overlap_ratio = self._calculate_optimal_overlap_ratio(analysis, classification)
        
        # Entity and relationship focus
        entity_types_focus = self._determine_entity_focus(analysis, classification)
        relationship_patterns = self._determine_relationship_patterns(analysis, classification)
        
        # Technical vocabulary (combining all sources)
        technical_vocabulary = list(set(
            analysis.technical_vocabulary + 
            analysis.entity_candidates + 
            list(analysis.concept_frequency.keys())[:10]
        ))[:25]  # Top 25 unique terms
        
        # Confidence thresholds
        confidence_thresholds = self._calculate_confidence_thresholds(analysis, classification)
        
        # Performance parameters
        performance_parameters = self._optimize_performance_parameters(analysis, classification)
        ...
        return ConfigurationRecommendations(
            ...
            processing_complexity=self._assess_processing_complexity(analysis, classification)
        )
    ...
    def _calculate_complexity_score(self, text: str, concept_frequency: Dict[str, int]) -> float:
        ...
        technical_terms = len([word for word in words if len(word) > 6 and word.isupper()])
    ...
    def _calculate_technical_density(self, text: str, entity_candidates: List[str]) -> float:
...
"""
    ...
    def _calculate_unified_confidence(
        ...
        if statistical_results:
            entropy_factor = min(1.0, statistical_results.get('entropy_score', 0.0) / 5.0)
            complexity_factor = min(1.0, sum(statistical_results.get('complexity_metrics', {}).values()) / 3.0)
        ...
        if llm_results:
            ...
            llm_confidence = llm_conf_mapping.get(llm_results.get('confidence_assessment', 'medium'), 0.6)
        ...
        if strategy == "content_only":
            ...
            return content_confidence * 0.2 + stat_confidence * 0.3 + llm_confidence * 0.5
        else:
            return content_confidence

    def _classify_content_based(self, analysis: UnifiedAnalysis, target_domain: Optional[str]) -> Dict[str, float]:
    ...
    def _classify_statistical_based(self, analysis: UnifiedAnalysis, target_domain: Optional[str]) -> Dict[str, float]:
        ...
        if analysis.entropy_score > 0:
            ...
            if 2.0 < analysis.entropy_score < 4.0 and analysis.vocabulary_richness > 0.6:
                scores['academic'] += (analysis.entropy_score / 10) * analysis.vocabulary_richness
    ...
    def _classify_llm_based(self, analysis: UnifiedAnalysis, target_domain: Optional[str]) -> Dict[str, float]:
        """Perform LLM-based classification"""
        scores = defaultdict(float)
        
        if analysis.domain_concepts:
            # Use LLM domain classification if available
            llm_domain = analysis.content_structure_analysis.get('domain_classification', 'general')
            confidence_mapping = {"high": 0.9, "medium": 0.7, "low": 0.5}
            confidence = confidence_mapping.get(
                analysis.content_structure_analysis.get('confidence_assessment', 'medium'), 0.6
            )
    ...
    def _combine_classification_scores(
        ...
        all_domains = set(content_scores.keys()) | set(statistical_scores.keys()) | set(llm_scores.keys())
        combined_scores = {}
        ...
        for domain in all_domains:
            ...
            if strategy == "content_only":
                ...
                combined_scores[domain] = content_score * 0.2 + stat_score * 0.3 + llm_score * 0.5
            else:
                combined_scores[domain] = content_score
    ...
    def _recommend_extraction_strategy(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> str:
    ...
    def _calculate_optimal_chunk_size(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> int:
    ...
    def _calculate_optimal_overlap_ratio(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> float:
    ...
    def _determine_entity_focus(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> List[str]:
    ...
    def _determine_relationship_patterns(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> List[str]:
    ...
    def _calculate_confidence_thresholds(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> Dict[str, float]:
    ...
    def _optimize_performance_parameters(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> Dict[str, Any]:
    ...
    def _assess_processing_complexity(self, analysis: UnifiedAnalysis, classification: UnifiedClassification) -> str:
        """Assess processing complexity level"""
        if analysis.complexity_score > 0.7 or classification.domain == "technical":
            return "high"
        elif analysis.complexity_score > 0.4 or len(analysis.semantic_relationships) > 3:
            return "medium"
        else:
            return "low"
    ...
    def _get_content_indicators(self, analysis: UnifiedAnalysis, domain: str) -> List[str]:
        ...
        # Relevant entities
        relevant_entities = [entity for entity in analysis.entity_candidates if self._is_relevant_to_domain(entity, domain)][:3]
        indicators.extend(relevant_entities)
        
        return indicators

    def _generate_statistical_evidence(self, analysis: UnifiedAnalysis, domain: str) -> List[str]:
        ...
        if analysis.complexity_score > 0.5:
            evidence.append(f"Above-average linguistic complexity ({analysis.complexity_score:.2f})")
    ...
    def _generate_llm_reasoning(self, analysis: UnifiedAnalysis, domain: str) -> str:
        ...
        confidence = analysis.content_structure_analysis.get('confidence_assessment', 'medium')
        complexity = analysis.content_structure_analysis.get('processing_complexity', 'medium')
        
        return (f"LLM analysis identified {len(analysis.domain_concepts)} domain concepts "
                f"with {confidence} confidence. Processing complexity assessed as {complexity}. "
                f"Key concepts: {', '.join(analysis.domain_concepts[:3])}")

    def _calculate_mathematical_foundation(self, analysis: UnifiedAnalysis) -> Dict[str, float]:
    ...
    def _calculate_optimization_parameters(self, analysis: UnifiedAnalysis, domain: str) -> Dict[str, float]:
        """Calculate optimization parameters"""
        return {
            "chunk_size_ratio": self._calculate_optimal_chunk_size(analysis, None) / 1000.0,
    ...
    def _is_relevant_to_domain(self, term: str, domain: str) -> bool:
        ...
        if 'technical' in domain:
            ...
            return any(word in term_lower for word in ['step', 'process', 'procedure', 'action'])
        elif 'academic' in domain:
            return any(word in term_lower for word in ['research', 'study', 'analysis', 'theory'])
        else:
    ...
    def _is_meaningful_concept(self, concept: str) -> bool:
        ...
        words = concept.split()
        meaningful_words = [word for word in words if word not in self._get_stop_words()]
        return len(meaningful_words) > 0 and len(concept) > 5

    def _update_performance_stats(self, strategy: str, processing_time: float, confidence: float):
        ...
        if strategy in self.performance_stats['strategy_performance']:
            ...
            count = stats['count']
            stats['avg_time'] = ((current_avg_time * (count - 1)) + processing_time) / count
            
            # Update average confidence
            current_avg_conf = stats['avg_confidence']
            stats['avg_confidence'] = ((current_avg_conf * (count - 1)) + confidence) / count
    ...
    def _create_empty_analysis(self, file_path: str, processing_time: float) -> UnifiedAnalysis:
        """Create empty analysis for error cases"""
        return UnifiedAnalysis(
            word_count=0, unique_words=0, avg_sentence_length=0.0, vocabulary_richness=0.0,
            concept_frequency={}, entity_candidates=[], action_patterns=[],
            complexity_score=0.0, technical_density=0.0,
            term_frequency_distribution={}, entropy_score=0.0, complexity_metrics={},
            clustering_results={}, statistical_signatures={}, confidence_intervals={},
            domain_concepts=[], key_entities=[], semantic_relationships=[],
            technical_vocabulary=[], content_structure_analysis={},
            processing_strategy="content_only", analysis_confidence=0.1,
            source_file=file_path, processing_time=processing_time
        )

    def get_performance_stats(self) -> Dict[str, Any]:
        """Get comprehensive performance statistics"""
        cache_hit_rate = self.performance_stats['cache_hits'] / max(self.performance_stats['total_analyses'], 1)

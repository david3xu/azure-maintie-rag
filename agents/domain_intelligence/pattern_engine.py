...
class LearnedPattern:
    ...
    def calculate_relevance_score(self, domain: str = None) -> float:
        """Calculate relevance score for pattern"""
        base_score = self.confidence * (1 + self.frequency / 100)
        
        if domain and domain in self.domains:
            base_score *= 1.5  # Boost for domain match
        
        # Age penalty (patterns lose relevance over time without updates)
        age_days = (time.time() - self.last_updated) / (24 * 3600)
        age_factor = max(0.5, 1.0 - (age_days / 30))  # 30-day half-life
...
class ExtractedPatterns:
    ...
    def get_top_patterns(self, pattern_type: str, limit: int = 10) -> List[LearnedPattern]:
        """Get top patterns of specified type by relevance"""
        pattern_list = getattr(self, f"{pattern_type}_patterns", [])
        return sorted(pattern_list, key=lambda p: p.calculate_relevance_score(), reverse=True)[:limit]
...
class PatternEngine:
    ...
    def __init__(self, config: Dict[str, Any] = None, cache_dir: Optional[Path] = None):
        ...
        self.cache_dir = cache_dir or Path(__file__).parent.parent.parent / "cache" / "patterns"
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        
        # Pattern storage and indexing
        self.learned_patterns: Dict[str, LearnedPattern] = {}
        self.pattern_index: Dict[str, Set[str]] = defaultdict(set)  # word -> pattern_ids
        self.domain_patterns: Dict[str, Set[str]] = defaultdict(set)  # domain -> pattern_ids
        ...
        self.max_patterns_per_type = self.config.get('max_patterns_per_type', 100)
        ...
        logger.info(f"Pattern engine initialized: {len(self.learned_patterns)} patterns loaded")
    
    def extract_domain_patterns(self, domain: str, content_analysis: Any, confidence: float) -> ExtractedPatterns:
        """
        ...
        try:
            ...
            entity_patterns = self._extract_entity_patterns(text_content, domain, confidence)
            action_patterns = self._extract_action_patterns(text_content, domain, confidence)
            relationship_patterns = self._extract_relationship_patterns(text_content, domain, confidence)
            temporal_patterns = self._extract_temporal_patterns(text_content, domain, confidence)
            ...
            all_new_patterns = entity_patterns + action_patterns + relationship_patterns + temporal_patterns
            ...
            self.stats['avg_extraction_time'] = (
                (self.stats['avg_extraction_time'] * (self.stats['total_extractions'] - 1) + processing_time) /
    ...
    def _extract_entity_patterns(self, text: str, domain: str, confidence: float) -> List[LearnedPattern]:
            ...
            try:
                ...
                for match in matches:
                    ...
                    if len(match.strip()) > 2:  # Filter out very short matches
                        ...
                        entities.append(LearnedPattern(
                            pattern_id=pattern_id,
                            pattern_text=match.strip(),
                            pattern_type='entity',
                            confidence=confidence * 0.8,  # Slightly lower confidence for extracted patterns
        ...
        return self._filter_patterns_by_statistics(entities)[:self.max_patterns_per_type]
    
    def _extract_action_patterns(self, text: str, domain: str, confidence: float) -> List[LearnedPattern]:
        ...
        return self._filter_patterns_by_statistics(actions)[:self.max_patterns_per_type]
    
    def _extract_relationship_patterns(self, text: str, domain: str, confidence: float) -> List[LearnedPattern]:
        ...
        for extractor_name, extractor in self.relationship_extractors.items():
            try:
                ...
                for match in matches:
                    ...
                    if len(match.strip()) > 5:  # Relationships need to be longer
                        pattern_id = self._generate_pattern_id(match, 'relationship')
                        
                        relationships.append(LearnedPattern(
                            pattern_id=pattern_id,
                            pattern_text=match.strip(),
                            pattern_type='relationship',
                            confidence=confidence * 0.7,  # Lower confidence for relationships
                            frequency=text.lower().count(match.lower()),
                            domains=[domain],
                            learned_from=[f"extraction_{extractor_name}"],
                            first_seen=current_time,
        ...
        return self._filter_patterns_by_statistics(relationships)[:self.max_patterns_per_type]
    
    def _extract_temporal_patterns(self, text: str, domain: str, confidence: float) -> List[LearnedPattern]:
        ...
        temporal_extractors = {
            'time_expressions': re.compile(r'\b(?:after|before|during|while|when|then|next|first|finally)\s+\w+(?:\s+\w+)*\b', re.IGNORECASE),
            'sequences': re.compile(r'\b(?:step|phase|stage)\s+\d+[:\s]+[^.!?]*', re.IGNORECASE),
            'durations': re.compile(r'\b\d+\s*(?:minutes?|hours?|days?|weeks?|months?|years?)\b', re.IGNORECASE)
        }
            ...
            try:
                ...
                for match in matches:
                    if len(match.strip()) > 5:
                        ...
                        temporal_patterns.append(LearnedPattern(
                            pattern_id=pattern_id,
                            pattern_text=match.strip(),
        ...
        return self._filter_patterns_by_statistics(temporal_patterns)[:self.max_patterns_per_type]
    
    def _create_semantic_clusters(self, patterns: List[LearnedPattern], domain: str) -> List[Dict[str, Any]]:
        ...
        for pattern in patterns:
            ...
            for other_pattern in patterns:
                if (other_pattern.pattern_id != pattern.pattern_id and 
                    other_pattern.pattern_id not in processed_patterns):
                    
                    other_words = set(other_pattern.pattern_text.lower().split())
                    overlap = len(pattern_words & other_words) / len(pattern_words | other_words)
                    
                    if overlap > 0.3:  # 30% word overlap threshold
                        cluster['patterns'].append(other_pattern.pattern_text)
                        cluster['confidence'] = max(cluster['confidence'], other_pattern.confidence)
    ...
    def _filter_patterns_by_statistics(self, patterns: List[LearnedPattern]) -> List[LearnedPattern]:
        ...
        filtered_patterns = [
            p for p in patterns 
            if p.frequency >= self.min_pattern_frequency and p.confidence >= self.min_confidence_threshold
        ]
    ...
    def _initialize_entity_extractors(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'model_names': re.compile(r'\b(?:model|algorithm|system|framework|version)\s+([A-Za-z0-9\-\.]+)\b', re.IGNORECASE),
            'identifiers': re.compile(r'\b[A-Z]\d+(?:-[A-Z0-9]+)*\b'),
            'measurements': re.compile(r'\b\d+(?:\.\d+)?\s*(mm|cm|m|kg|g|%|degrees?|rpm|hz|mhz|ghz)\b', re.IGNORECASE),
            'codes': re.compile(r'\b[A-Z]{2,}\d{2,}\b'),
            'proper_nouns': re.compile(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b')
        }
    
    def _initialize_action_extractors(self) -> Dict[str, re.Pattern]:
        """Initialize action extraction patterns"""
        return {
            'instructions': re.compile(r'\b(install|configure|setup|initialize|create|build|deploy|start|stop|restart)\b', re.IGNORECASE),
            'operations': re.compile(r'\b(run|execute|perform|conduct|analyze|process|calculate|measure)\b', re.IGNORECASE),
            'maintenance': re.compile(r'\b(maintain|repair|replace|check|inspect|clean|service|troubleshoot)\b', re.IGNORECASE),
            'monitoring': re.compile(r'\b(monitor|observe|track|log|record|measure|assess)\b', re.IGNORECASE),
            'management': re.compile(r'\b(manage|control|supervise|coordinate|organize|plan)\b', re.IGNORECASE)
        }
    
    def _initialize_relationship_extractors(self) -> Dict[str, re.Pattern]:
        """Initialize relationship extraction patterns"""
        return {
            'causation': re.compile(r'\b(\w+(?:\s+\w+)*)\s+(?:causes?|results?\s+in|leads?\s+to)\s+(\w+(?:\s+\w+)*)\b', re.IGNORECASE),
            'dependency': re.compile(r'\b(\w+(?:\s+\w+)*)\s+(?:depends?\s+on|requires?|needs?)\s+(\w+(?:\s+\w+)*)\b', re.IGNORECASE),
            'composition': re.compile(r'\b(\w+(?:\s+\w+)*)\s+(?:contains?|includes?|comprises?)\s+(\w+(?:\s+\w+)*)\b', re.IGNORECASE),
            'association': re.compile(r'\b(\w+(?:\s+\w+)*)\s+(?:is\s+(?:associated|connected|linked)\s+with|relates?\s+to)\s+(\w+(?:\s+\w+)*)\b', re.IGNORECASE)
        }
    
    def find_matching_patterns(self, query: str, domain: Optional[str] = None, limit: int = 20) -> List[LearnedPattern]:
            ...
            if word in self.pattern_index:
                for pattern_id in self.pattern_index[word]:
                    ...
                    if pattern:
                        ...
                        pattern_words = set(pattern.pattern_text.lower().split())
                        word_overlap = len(query_words & pattern_words) / len(query_words | pattern_words)
                        ...
                        pattern_scores[pattern_id] = max(pattern_scores[pattern_id], score)
    ...
    def get_domain_patterns(self, domain: str, pattern_type: Optional[str] = None, limit: int = 50) -> List[LearnedPattern]:
        ...
        # Sort by relevance
        domain_pattern_list.sort(key=lambda p: p.calculate_relevance_score(domain), reverse=True)
    ...
    def _load_learned_patterns(self):
        ...
        try:
            ...
            if patterns_file.exists():
                ...
                logger.info(f"Loaded {len(self.learned_patterns)} learned patterns from cache")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'index_stats': {
                ...
                'avg_patterns_per_word': statistics.mean([len(pattern_ids) for pattern_ids in self.pattern_index.values()]) if self.pattern_index else 0
            },
            'configuration': {

                    ...
                    if len(match.strip()) > 2:  # Filter out very short matches
                        ...
                        entities.append(
                            LearnedPattern(
                                ...
                                pattern_type="entity",
                                confidence=confidence
                                * 0.8,  # Slightly lower confidence for extracted patterns
                    ...
                    if len(match.strip()) > 5:  # Relationships need to be longer
                        ...
                        relationships.append(
                            LearnedPattern(
                                ...
                                confidence=confidence
                                * 0.7,  # Lower confidence for relationships
                                frequency=text.lower().count(match.lower()),
                                domains=[domain],
                                learned_from=[f"extraction_{extractor_name}"],
                                first_seen=current_time,
    ...
    def _extract_temporal_patterns(
        ...
        temporal_extractors = {
            "time_expressions": re.compile(
                r"\b(?:after|before|during|while|when|then|next|first|finally)\s+\w+(?:\s+\w+)*\b",
                re.IGNORECASE,
            ),
            "sequences": re.compile(
                r"\b(?:step|phase|stage)\s+\d+[:\s]+[^.!?]*", re.IGNORECASE
            ),
    ...
    def _initialize_entity_extractors(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "model_names": re.compile(
                r"\b(?:model|algorithm|system|framework|version)\s+([A-Za-z0-9\-\.]+)\b",
                re.IGNORECASE,
            ),
            "identifiers": re.compile(r"\b[A-Z]\d+(?:-[A-Z0-9]+)*\b"),
            "measurements": re.compile(
                r"\b\d+(?:\.\d+)?\s*(mm|cm|m|kg|g|%|degrees?|rpm|hz|mhz|ghz)\b",
                re.IGNORECASE,
            ),
    ...
    def _initialize_action_extractors(self) -> Dict[str, re.Pattern]:
        ...
        return {
            "instructions": re.compile(
                r"\b(install|configure|setup|initialize|create|build|deploy|start|stop|restart)\b",
                re.IGNORECASE,
            ),
            "operations": re.compile(
                r"\b(run|execute|perform|conduct|analyze|process|calculate|measure)\b",
                re.IGNORECASE,
            ),
            "maintenance": re.compile(
                r"\b(maintain|repair|replace|check|inspect|clean|service|troubleshoot)\b",
                re.IGNORECASE,
            ),
            "monitoring": re.compile(
                r"\b(monitor|observe|track|log|record|measure|assess)\b", re.IGNORECASE
            ),
            "management": re.compile(
                r"\b(manage|control|supervise|coordinate|organize|plan)\b",
                re.IGNORECASE,
            ),
        }
    ...
    def _initialize_relationship_extractors(self) -> Dict[str, re.Pattern]:
        ...
        return {
            "causation": re.compile(
                r"\b(\w+(?:\s+\w+)*)\s+(?:causes?|results?\s+in|leads?\s+to)\s+(\w+(?:\s+\w+)*)\b",
                re.IGNORECASE,
            ),
            "dependency": re.compile(
                r"\b(\w+(?:\s+\w+)*)\s+(?:depends?\s+on|requires?|needs?)\s+(\w+(?:\s+\w+)*)\b",
                re.IGNORECASE,
            ),
            "composition": re.compile(
                r"\b(\w+(?:\s+\w+)*)\s+(?:contains?|includes?|comprises?)\s+(\w+(?:\s+\w+)*)\b",
                re.IGNORECASE,
            ),
            "association": re.compile(
                r"\b(\w+(?:\s+\w+)*)\s+(?:is\s+(?:associated|connected|linked)\s+with|relates?\s+to)\s+(\w+(?:\s+\w+)*)\b",
                re.IGNORECASE,
            ),
        }

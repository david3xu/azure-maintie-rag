...
from .domain_analyzer import ContentAnalysis
from .domain_analyzer import DomainAnalyzer as ContentAnalyzer
from .domain_analyzer import DomainAnalyzer as DomainClassifier
from .domain_analyzer import DomainClassification
...
class DomainBackgroundProcessor:
    ...
    async def _discover_domains(self) -> List[str]:
        ...
        for subdir in self.data_dir.iterdir():
            if subdir.is_dir() and not subdir.name.startswith("."):
                ...
                has_files = any(subdir.glob("**/*.md") or subdir.glob("**/*.txt"))
                if has_files:
    ...
    async def _process_file_for_domain(
        ...
        try:
            ...
            classification = self.domain_classifier.classify_content_domain(
                content_analysis, domain
            )

            # 3. Extract statistical patterns
            patterns = self.pattern_extractor.extract_domain_patterns(
                classification.domain, content_analysis, classification.confidence
            )
    ...
    async def _create_consolidated_domain_signature(
        ...
        primary_concepts = [
            p.pattern_text for p in merged_entities if p.confidence > 0.8
        ][
            :5
        ]  # Top 5 concepts

...
async def discover_domain_tool(
    ...
    try:
        ...
        logger.info(f"PydanticAI Domain Intelligence discovered domain: {domain} (confidence: {result.confidence:.2f})")
        return domain
...
async def analyze_domain_patterns_tool(
    ...
    try:
        ...
        pattern_analysis = {
            ...
            "competitive_advantage_score": min(1.0, result.confidence * 1.2),  # Boost for competitive scoring
...
async def validate_domain_confidence_tool(
    ...
    try:
        ...
        validation_results["competitive_advantage_score"] = min(1.0, confidence * (1.0 + (0.1 if len(text) > 200 else 0)))
        validation_results["zero_config_advantage"] = confidence > 0.5  # Success without configuration

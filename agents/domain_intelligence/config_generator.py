...
class ConfigGenerator:
    ...
    def generate_ml_config(
        ...
        entity_types = [
            p.pattern_text for p in patterns.entity_patterns if p.confidence > 0.6
        ][:20]
        relationship_types = [
            p.pattern_text for p in patterns.relationship_patterns if p.confidence > 0.5
        ]
        ...
        if not relationship_types:
            ...
            relationship_types = (
                inferred_relationships if inferred_relationships else ["connects"]
            )
    ...
    def _assess_domain_complexity(self, patterns: ExtractedPatterns) -> str:
        ...
        if patterns.extraction_confidence > 0.8:
            complexity_score += 1

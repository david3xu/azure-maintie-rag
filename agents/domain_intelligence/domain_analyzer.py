...
class DomainAnalyzer:
    ...
    def analyze_raw_content(self, content_source: Path) -> ContentAnalysis:
        ...
        try:
            ...
            domain_indicators = self._calculate_domain_indicators(text, concept_frequency)
    ...
    def classify_content_domain(self, analysis: ContentAnalysis, user_domain: Optional[str] = None) -> DomainClassification:
        """
        ...
        try:
            ...
            secondary_indicators = self._get_secondary_indicators(analysis, primary_domain)
            
            # Alternative domains
            alternative_domains = [(domain, score) for domain, score in sorted_domains[1:6]]  # Top 5 alternatives
            ...
            statistical_features = {
                ...
                'entity_density': len(analysis.entity_candidates) / max(1, analysis.word_count),
                'action_density': len(analysis.action_patterns) / max(1, analysis.word_count)
            }
    ...
    def _extract_entity_candidates(self, text: str) -> List[str]:
        ...
        return [entity for entity in unique_entities if len(entity.strip()) > 2][:20]  # Top 20
    ...
    def _calculate_domain_indicators(self, text: str, concept_frequency: Dict[str, int]) -> Dict[str, float]:
        """
        ...
        top_concepts = sorted(concept_frequency.items(), key=lambda x: x[1], reverse=True)[:50]
        ...
        if top_concepts:
            ...
            domain_patterns = self._extract_domain_patterns_from_data(top_concepts, text_lower)
        ...
        if not indicators:
            indicators = self._calculate_statistical_domain_indicators(text_lower, concept_frequency)
        
        return indicators
    
    def _extract_domain_patterns_from_data(self, top_concepts: List[tuple], text: str) -> Dict[str, List[str]]:
        ...
        concept_words = [concept[0].lower() for concept in top_concepts if concept[1] > 1]
    ...
    def _calculate_statistical_domain_indicators(self, text: str, concept_frequency: Dict[str, int]) -> Dict[str, float]:
        ...
        if concept_frequency:
            ...
            indicators['vocabulary_richness'] = len(concept_frequency) / total_concepts if total_concepts > 0 else 0
    ...
    def _calculate_domain_scores(self, analysis: ContentAnalysis) -> Dict[str, float]:
        ...
        for domain, indicator_score in analysis.domain_indicators.items():
            scores[domain] += indicator_score * 10  # Weight domain-specific indicators highly
        
        # Data-driven entity scoring: learn patterns from actual content
        if analysis.entity_candidates:
            entity_scores = self._calculate_entity_based_scores(analysis.entity_candidates)
        ...
        if analysis.action_patterns:
            action_scores = self._calculate_action_based_scores(analysis.action_patterns)
    ...
    def _get_primary_indicators(self, analysis: ContentAnalysis, domain: str) -> List[str]:
        ...
        relevant_entities = [entity for entity in analysis.entity_candidates 
                           if self._is_relevant_to_domain(entity, domain)][:3]
        indicators.extend(relevant_entities)
        
        return indicators
    
    def _get_secondary_indicators(self, analysis: ContentAnalysis, domain: str) -> List[str]:
        """Get secondary indicators for domain classification"""
        indicators = []
        
        # Add relevant action patterns
        relevant_actions = [action for action in analysis.action_patterns
                          if self._is_relevant_to_domain(action, domain)][:3]
        indicators.extend(relevant_actions)
        
        # Add statistical features
        if analysis.technical_density > 0.1:
            indicators.append(f"high_technical_density_{analysis.technical_density:.2f}")
    ...
    def _has_process_characteristics(self, entity: str) -> bool:
        ...
        return any(re.search(pattern, entity, re.IGNORECASE) for pattern in process_patterns)
    ...
    def _has_academic_characteristics(self, entity: str) -> bool:
        ...
        return any(re.search(pattern, entity, re.IGNORECASE) for pattern in academic_patterns)
    ...
    def _is_relevant_to_domain(self, term: str, domain: str) -> bool:
        ...
        if 'technical' in domain or 'technology' in domain:
            ...
            return self._has_process_characteristics(term_lower) or 'maintain' in term_lower
    ...
    def _generate_classification_reasoning(self, domain: str, score: float, indicators: List[str], analysis: ContentAnalysis) -> str:
        """Generate human-readable classification reasoning"""
        return (f"Classified as '{domain}' domain based on {len(indicators)} key indicators "
    ...
    def _create_fallback_classification(self, analysis: ContentAnalysis, domain: str) -> DomainClassification:

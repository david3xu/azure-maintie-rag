...
class StatisticalDomainAnalyzer:
    ...
    def __init__(self):
        ...
        logger.info(
            "Statistical domain analyzer initialized with mathematical foundations"
        )
    ...
    def classify_domain_statistically(
        self, analysis: StatisticalAnalysis, target_domain: Optional[str] = None
        ...
        if target_domain and target_domain in hypothesis_scores:
            ...
            best_domain = max(
                hypothesis_scores.keys(), key=lambda k: hypothesis_scores[k]
            )
        ...
        alternative_hypotheses = [
            (domain, score)
            for domain, score in hypothesis_scores.items()
        ...
        significance_level = self._calculate_significance_level(
            confidence, len(hypothesis_scores)
        )
    ...
    def _generate_domain_hypotheses(
        ...
        hypotheses["technical"] = {
            "score": technical_score,
            "features": ["lexical_diversity", "entropy", "word_length"],
        }

        # Process domain hypothesis
        process_score = (
            analysis.complexity_metrics.get("sentence_complexity", 0.0) * 0.5
            + analysis.statistical_signatures.get("frequency_distribution_skew", 0.0)
            * 0.3
        ...
        hypotheses["academic"] = {
            ...
            "features": ["entropy", "lexical_diversity", "sentence_length"],
        }
    ...
    def _calculate_statistical_confidence(
        self, analysis: StatisticalAnalysis, features: Dict[str, float]
    ) -> float:
        ...
        sample_size_factor = min(
            1.0, analysis.word_count / 1000.0
        )  # More words = higher confidence
        entropy_factor = min(
            1.0, analysis.entropy_score / 5.0
        )  # Higher entropy = more reliable
    ...
    def _generate_statistical_evidence(
        self, analysis: StatisticalAnalysis, domain: str
    ) -> List[str]:
        ...
        # Complexity evidence
        avg_complexity = sum(analysis.complexity_metrics.values()) / len(
            analysis.complexity_metrics
        )
        if avg_complexity > 0.5:
            evidence.append(
                f"Above-average linguistic complexity ({avg_complexity:.2f})"
            )
    ...
    def _calculate_cluster_membership(
        self, analysis: StatisticalAnalysis
    ) -> Dict[str, float]:
        ...
        medium_importance = sum(
            1 for score in tfidf_scores.values() if 0.05 <= score <= 0.1
        )
    ...
    def _analyze_entropy_by_domain(
        self, analysis: StatisticalAnalysis
    ) -> Dict[str, float]:
    ...
    def _calculate_significance_level(
        self, confidence: float, num_hypotheses: int
    ) -> float:

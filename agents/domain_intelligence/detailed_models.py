...
class StatisticalAnalysis(BaseModel):
    ...
    document_structures: Dict[str, int] = Field(description="Document structure patterns")
    average_document_length: float = Field(description="Average document length")
    length_distribution: Dict[str, int] = Field(description="Document length distribution")
...
class SemanticPatterns(BaseModel):
    ...
    concept_relationships: List[Dict[str, str]] = Field(description="Concept relationships")
    
    # Entity patterns
    entity_types: List[str] = Field(description="Identified entity types")
    entity_examples: Dict[str, List[str]] = Field(description="Examples for each entity type")
    entity_confidence: Dict[str, float] = Field(description="Confidence per entity type")
    
    # Relationship patterns
    relationship_types: List[str] = Field(description="Identified relationship types")
    relationship_examples: Dict[str, List[str]] = Field(description="Examples for each relationship")
    relationship_confidence: Dict[str, float] = Field(description="Confidence per relationship")
    
    # Content structure insights
    content_structure_analysis: Dict[str, Any] = Field(description="Content structure insights")
    processing_strategy_recommendation: str = Field(description="Recommended processing strategy")
...
class CombinedPatterns(BaseModel):
    ...
    # Pattern fusion
    validated_entities: List[Dict[str, Any]] = Field(description="Cross-validated entity patterns")
    validated_relationships: List[Dict[str, Any]] = Field(description="Cross-validated relationship patterns")
    confidence_scores: Dict[str, float] = Field(description="Combined confidence scores")
    
    # Processing recommendations
    optimal_extraction_strategy: str = Field(description="Optimal extraction strategy")
    recommended_thresholds: Dict[str, float] = Field(description="Recommended confidence thresholds")
    processing_parameters: Dict[str, Any] = Field(description="Optimal processing parameters")
    
    # Quality metrics
    pattern_consistency_score: float = Field(description="Pattern consistency between approaches")
    overall_confidence: float = Field(description="Overall pattern confidence")


class QualityMetrics(BaseModel):
    """Pattern and configuration quality assessment"""
    config_path: Optional[str] = Field(description="Path to configuration being validated")
    ...
    relationship_precision_estimate: float = Field(description="Estimated relationship precision")
    relationship_recall_estimate: float = Field(description="Estimated relationship recall")
    ...
    threshold_appropriateness: float = Field(description="Threshold appropriateness score")
    
    # Performance predictions
    predicted_processing_time: float = Field(description="Predicted processing time per document")

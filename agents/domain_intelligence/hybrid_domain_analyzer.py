...
class HybridDomainAnalyzer:
    ...
    def __init__(self):
        ...
        logger.info(
            "Hybrid domain analyzer initialized with LLM + Statistical foundation"
        )
    ...
    def _initialize_azure_client(self) -> Optional[AsyncAzureOpenAI]:
        ...
        if not AZURE_OPENAI_AVAILABLE:
            logger.warning(
                "Azure OpenAI not available, falling back to statistical-only analysis"
            )
            return None
        ...
        try:
            ...
            if endpoint and api_key:
                ...
                logger.warning(
                    "Azure OpenAI credentials not found, using statistical-only mode"
                )
                return None
    ...
    async def analyze_domain_hybrid(self, file_path: Path) -> HybridAnalysis:
        ...
        statistical_features = await self._analyze_statistical_features(
            text, llm_extraction
        )

        # Stage 3: Hybrid Optimization
        hybrid_confidence = self._calculate_hybrid_confidence(
            llm_extraction, statistical_features
        )
        config_recommendations = self._generate_config_recommendations(
            llm_extraction, statistical_features
        )
        optimization_parameters = self._optimize_parameters(
            llm_extraction, statistical_features
        )
...
"""
    ...
    def _fallback_semantic_extraction(self, text: str) -> LLMExtraction:
        ...
        common_concepts = [
            word for word, count in word_freq.most_common(10) if len(word) > 4
        ]
        technical_terms = [
            word
            for word in words
            if word.isupper() or "_" in word or any(char.isdigit() for char in word)
        ]
    ...
    async def _analyze_statistical_features(
        self, text: str, llm_extraction: LLMExtraction
    ) -> StatisticalFeatures:
        ...
        entity_density = (
            len(llm_extraction.key_entities) / max(word_count, 1) * 1000
        )  # per 1000 words
        relationship_density = (
            len(llm_extraction.semantic_relationships) / max(word_count, 1) * 1000
        )

        # Vocabulary complexity analysis
        vocabulary_complexity = self._analyze_vocabulary_complexity(
            text, llm_extraction
        )

        # Processing load estimate
        processing_load_estimate = self._estimate_processing_load(
            llm_extraction, word_count
        )
    ...
    def _calculate_optimal_chunk_size(
        self, word_count: int, llm_extraction: LLMExtraction
    ) -> int:
    ...
    def _analyze_vocabulary_complexity(
        ...
        tech_density = (
            len(llm_extraction.technical_vocabulary) / max(len(words), 1) * 1000
        )
    ...
    def _estimate_processing_load(
        self, llm_extraction: LLMExtraction, word_count: int
    ) -> float:
        """Estimate processing load for performance optimization"""
        base_load = word_count / 1000  # Base load per 1000 words

        # Adjust for complexity
        complexity_factor = {"high": 1.5, "medium": 1.0, "low": 0.7}.get(
            llm_extraction.processing_complexity, 1.0
        )
    ...
    def _calculate_confidence_thresholds(
        self, llm_extraction: LLMExtraction
    ) -> Dict[str, float]:
        ...
        domain_multiplier = domain_adjustments.get(
            llm_extraction.domain_classification, 1.0
        )
    ...
    def _optimize_performance_parameters(
        ...
        complexity_factor = {"high": 1.5, "medium": 1.0, "low": 0.8}.get(
            llm_extraction.processing_complexity, 1.0
        )
        ...
        quality_settings = {
            ...
            "min_relationship_count": max(
                2, len(llm_extraction.semantic_relationships) // 2
            ),
            "enable_caching": True,
            "cache_ttl": 3600,
        }
    ...
    def _calculate_hybrid_confidence(
        self, llm_extraction: LLMExtraction, statistical_features: StatisticalFeatures
    ) -> float:
        """Calculate overall confidence combining LLM and statistical analysis"""

        # LLM confidence score
        llm_confidence = {"high": 0.9, "medium": 0.7, "low": 0.5}.get(
            llm_extraction.confidence_assessment, 0.6
        )
    ...
    def _generate_config_recommendations(
        ...
        return {
            ...
            "relationship_patterns": [
                f"{r[0]} {r[1]} {r[2]}"
                for r in llm_extraction.semantic_relationships[:15]
            ],
            "technical_vocabulary": llm_extraction.technical_vocabulary,
            "processing_parameters": {
                "chunk_size": statistical_features.optimal_chunk_size,
                "chunk_overlap": int(
                    statistical_features.optimal_chunk_size
                    * statistical_features.chunk_overlap_ratio
                ),
                "parallel_processing": statistical_features.processing_load_estimate
                > 2.0,
    ...
    def _recommend_extraction_strategy(self, llm_extraction: LLMExtraction) -> str:
        ...
        return domain_strategies.get(
            llm_extraction.domain_classification, "MIXED_CONTENT"
        )
    ...
    def _optimize_parameters(
        ...
        return {
            "entity_confidence_threshold": statistical_features.confidence_thresholds[
                "entity_confidence"
            ],
            "relationship_confidence_threshold": statistical_features.confidence_thresholds[
            ...
            "processing_complexity_score": self._complexity_to_score(
                llm_extraction.processing_complexity
            ),
            "vocabulary_complexity_score": statistical_features.vocabulary_complexity,
            "load_factor": min(
                1.0, statistical_features.processing_load_estimate / 5.0
            ),
        }

    def _complexity_to_score(self, complexity: str) -> float:
        """Convert complexity assessment to numerical score"""
        return {"high": 0.8, "medium": 0.5, "low": 0.2}.get(complexity, 0.5)
...
class ConfigurationOptimizer:
    ...
    def optimize_for_domain(
        self, domain: str, analysis: HybridAnalysis
    ) -> Dict[str, Any]:

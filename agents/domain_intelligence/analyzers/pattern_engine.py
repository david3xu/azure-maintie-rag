...
class DataDrivenPatternEngine:
    ...
    def discover_patterns_from_corpus(self, documents: List[str], domain_hint: str = None) -> Dict[str, List[LearnedPattern]]:
        """
        ...
        """
        logger.info(f"🔍 Discovering patterns from {len(documents)} documents using statistical analysis")
        ...
        pattern_candidates = self._discover_patterns_by_entropy(documents, word_frequencies)
        ...
        logger.info(f"✅ Discovered {sum(len(p) for p in clustered_patterns.values())} patterns using statistical analysis")
    ...
    def _cluster_patterns_statistically(self, candidates: List[Dict[str, Any]]) -> Dict[str, List[LearnedPattern]]:
        ...
        clusters = self._statistical_kmeans_clustering(candidates, k=4)  # Data-driven cluster count
    ...
    def _statistical_kmeans_clustering(self, candidates: List[Dict[str, Any]], k: int) -> List[List[Dict[str, Any]]]:
        ...
        for i, feature in enumerate(features):
            ...
            distances = [sum((a - b) ** 2 for a, b in zip(feature, centroid)) for centroid in centroids]

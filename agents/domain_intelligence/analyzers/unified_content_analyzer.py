...
class UnifiedContentAnalyzer:
    ...
    def __init__(self, config: Optional[DomainIntelligenceConfig] = None):
        ...
        self.domain_patterns = {
            ...
            "model_names": re.compile(
                r"\b(?:gpt|bert|llama|claude|openai|azure)\w*\b", re.IGNORECASE
            ),
            "process_steps": re.compile(
                r"\b(?:step|phase|stage|process|workflow)\s+\d+\b", re.IGNORECASE
            ),
            "measurements": re.compile(
                r"\b\d+(?:\.\d+)?\s*(?:ms|sec|min|mb|gb|kb|%)\b", re.IGNORECASE
            ),
            "identifiers": re.compile(
                r"\b[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\b"
            ),
            "instructions": re.compile(
                r"\b(?:install|configure|setup|initialize|run|execute|start|stop)\b",
                re.IGNORECASE,
            ),
            "operations": re.compile(
                r"\b(?:create|update|delete|modify|process|analyze|generate)\b",
                re.IGNORECASE,
            ),
            "troubleshooting": re.compile(
                r"\b(?:debug|fix|resolve|troubleshoot|diagnose|error)\b", re.IGNORECASE
            ),
        }
        ...
        logger.info(
            "Unified content analyzer initialized with PydanticAI patterns and shared utilities"
        )
    ...
    def analyze_content(self, content_source: Union[Path, str]) -> DomainAnalysisResult:
        ...
        try:
            ...
            if isinstance(content_source, Path) or (
                isinstance(content_source, str) and Path(content_source).exists()
            ):
            ...
            cleaning_result: CleanedContent = clean_text_content(
                raw_text, cleaning_options
            )

            # Step 2: Statistical analysis using shared utilities
            text_statistics: TextStatistics = calculate_text_statistics(
                cleaning_result.cleaned_text
            )

            # Step 3: Domain complexity analysis using shared utilities
            domain_keywords = self._extract_domain_vocabulary(
                cleaning_result.cleaned_text
            )
            technical_patterns = [
                pattern.pattern for pattern in self.domain_patterns.values()
            ]

            document_complexity: DocumentComplexityProfile = (
                analyze_document_complexity(
                    cleaning_result.cleaned_text, domain_keywords, technical_patterns
                )
            )
            ...
            if self.config.enable_pattern_detection:
                domain_patterns = self._detect_domain_patterns(
                    cleaning_result.cleaned_text
                )
            ...
            if (
                ...
                try:
                    ...
                    if (
                        len(split_into_sentences(cleaning_result.cleaned_text))
                        >= self.config.n_clusters
                    ):
                        sentences = split_into_sentences(cleaning_result.cleaned_text)[
                            :50
                        ...
                        if len(sentences) >= 2:
                            ...
                            semantic_clusters = {
                                ...
                                "cluster_centers": (
                                    len(self.clusterer.cluster_centers_)
            ...
            analysis_confidence_score = calculate_adaptive_confidence(
                [
                    document_complexity.statistics.lexical_diversity,
                    cleaning_result.cleaning_quality_score,
                ],
                confidence_factors,
            )
            ...
            logger.info(
                f"Domain intelligence analysis completed in {processing_time_ms:.2f}ms"
            )
    ...
    def _detect_domain_patterns(self, text: str) -> Dict[str, List[str]]:
        ...
        for pattern_name, pattern in self.domain_patterns.items():
            ...
            if matches:
                ...
                if frequent_matches:
                    detected_patterns[pattern_name] = frequent_matches[
                        : self.config.max_patterns_per_type
                    ]
    ...
    def _extract_concept_hierarchy(
        ...
        sorted_concepts = sorted(
            concept_hierarchy.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _calculate_domain_fit(
        ...
        pattern_density = len(
            [p for patterns in domain_patterns.values() for p in patterns]
        )
    ...
    def get_analysis_statistics(self) -> Dict[str, Any]:
        ...
        success_rate = self.analysis_stats["successful_analyses"] / max(
            1, self.analysis_stats["total_analyses"]
        )

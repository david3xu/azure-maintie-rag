...
from .hybrid_domain_analyzer import HybridDomainAnalyzer, HybridAnalysis, LLMExtraction, StatisticalFeatures
from .pattern_engine import PatternEngine, ExtractedPatterns
from .config_generator import DomainConfig, ConfigGenerator
from .detailed_models import StatisticalAnalysis, SemanticPatterns, CombinedPatterns, QualityMetrics, DomainDeps
...
class ExtractionStrategy(BaseModel):
    ...
    approach: str = Field(default="statistical_learning", description="Learning approach")
    domain_adaptation: bool = Field(default=True, description="Enable domain adaptation")
    chunk_optimization: bool = Field(default=True, description="Enable chunk size optimization")
...
class ExtractionConfiguration(BaseModel):
    ...
    max_entities_per_chunk: int = Field(default=15, description="Maximum entities per chunk")
    entity_confidence_threshold: float = Field(default=0.7, description="Entity confidence threshold")
    relationship_patterns: List[str] = Field(default_factory=list, description="Relationship patterns")
    classification_rules: Dict[str, List[str]] = Field(default_factory=dict, description="Classification rules")
    response_sla_ms: int = Field(default=2500, description="Response SLA in milliseconds")
    
    # Additional configuration fields
    domain_name: Optional[str] = Field(default=None, description="Domain name")
    relationship_confidence_threshold: float = Field(default=0.6, description="Relationship confidence threshold")
...
class DomainSignature(BaseModel):
    ...
    entity_patterns: List[Dict] = Field(default_factory=list, description="Entity patterns with metadata")
    action_patterns: List[Dict] = Field(default_factory=list, description="Action patterns with metadata")
    relationship_patterns: List[Dict] = Field(default_factory=list, description="Relationship patterns with metadata")
...
if domain_agent is not None:
    @domain_agent.tool
    async def analyze_raw_content(ctx: RunContext[None], file_path: str) -> HybridAnalysis:
    ...
    @domain_agent.tool
    async def classify_domain(ctx: RunContext[None], file_path: str, user_domain: Optional[str] = None) -> LLMExtraction:
    ...
    @domain_agent.tool
    async def extract_domain_patterns(ctx: RunContext[None], file_path: str, domain: Optional[str] = None) -> ExtractedPatterns:
    ...
    @domain_agent.tool
    async def generate_extraction_config(ctx: RunContext[None], domain: str, file_path: str) -> ExtractionConfiguration:
        ...
        extraction_config = _convert_hybrid_analysis_to_extraction_config(domain, hybrid_analysis)
    ...
    @domain_agent.tool
    async def generate_domain_config(ctx: RunContext[None], domain: str, file_path: str) -> DomainConfig:
    ...
    async def analyze_query_tools(ctx: RunContext[None], query: str) -> List[str]:
        ...
        try:
            ...
            for pattern in detection_result.matched_patterns:
                if any(search_term in pattern.lower() for search_term in ['search', 'find', 'retrieve', 'query']):
                    recommended_tools.append('tri_modal_search')
                elif any(analysis_term in pattern.lower() for analysis_term in ['analyze', 'examination', 'study']):
                    recommended_tools.append('analyze_content')
                elif any(pattern_term in pattern.lower() for pattern_term in ['pattern', 'trend', 'extract', 'mining']):
                    recommended_tools.append('extract_patterns')
                elif any(domain_term in pattern.lower() for domain_term in ['domain', 'classify', 'category', 'type']):
                    recommended_tools.append('classify_domain')

            # Use domain-specific tools if domain is detected with high confidence
            if detection_result.confidence > 0.7:
                domain_specific_tools = _get_domain_specific_tools(detection_result.domain)
...
def _discover_domains_from_filesystem(raw_data_path: str = "data/raw") -> List[str]:
    ...
    @domain_agent.tool
    async def discover_available_domains(ctx: RunContext[None], data_dir: str = "/workspace/azure-maintie-rag/data/raw") -> AvailableDomainsResult:
    ...
    @domain_agent.tool
    async def detect_domain_from_query(ctx: RunContext[None], query: str) -> DomainDetectionResult:
    ...
    @domain_agent.tool
    async def process_domain_documents(ctx: RunContext[None], domain: str, data_dir: str = "/workspace/azure-maintie-rag/data/raw") -> DomainAnalysisResult:
        ...
        return DomainAnalysisResult(
            ...
            patterns_extracted=len(config.patterns.entity_patterns) + len(config.patterns.action_patterns),
    ...
    async def analyze_corpus_statistics(ctx: RunContext[DomainDeps], corpus_path: str) -> StatisticalAnalysis:
        ...
        for doc_path in corpus_path_obj.rglob("*.txt"):
            try:
                ...
                document_structures[structure_key] = document_structures.get(structure_key, 0) + 1
        ...
        average_doc_length = sum(document_lengths) / len(document_lengths) if document_lengths else 0
        technical_density = len(technical_terms) / vocabulary_size if vocabulary_size > 0 else 0
        
        # Domain specificity score (based on technical density and vocabulary diversity)
        domain_specificity = min(1.0, technical_density * 2 + (vocabulary_size / total_tokens))
        ...
        return StatisticalAnalysis(
            ...
            token_frequencies=dict(sorted(token_frequencies.items(), key=lambda x: x[1], reverse=True)[:1000]),
            n_gram_patterns=dict(sorted(n_gram_patterns.items(), key=lambda x: x[1], reverse=True)[:500]),
    ...
    async def generate_semantic_patterns(ctx: RunContext[DomainDeps], content_sample: str) -> SemanticPatterns:
        ...
        try:
            # Get LLM analysis
            hybrid_analysis = await hybrid_analyzer.analyze_content_async(Path(tmp_path))
            ...
            for entity in entity_types:
                ...
                entity_examples[entity] = [entity.lower(), entity.upper(), entity.title()]
                entity_confidence[entity] = 0.8  # Would calculate from LLM confidence
            
            # Extract relationship patterns
            relationship_types = [f"{r[1]}" for r in llm_extraction.semantic_relationships[:15]]
            ...
            return SemanticPatterns(
                content_sample=content_sample[:500] + "..." if len(content_sample) > 500 else content_sample,
                ...
                content_structure_analysis={
                    ...
                    "estimated_complexity": "high" if len(entity_types) > 15 else "medium"
                },
    ...
    async def create_fully_learned_extraction_config(
        ...
        classification_rules = await _learn_classification_rules(basic_stats.token_frequencies)
    ...
    async def validate_pattern_quality(ctx: RunContext[DomainDeps], config: ExtractionConfiguration) -> QualityMetrics:
        ...
        entity_coverage = min(1.0, len(config.expected_entity_types) / 30)  # Target: 30 entity types
        ...
        relationship_coverage = min(1.0, len(config.relationship_patterns) / 20)  # Target: 20 patterns
        relationship_precision_estimate = config.relationship_confidence_threshold
        relationship_recall_estimate = max(0.5, 1.0 - config.relationship_confidence_threshold)
        
        # Configuration completeness
        required_fields = ['domain_name', 'entity_confidence_threshold', 'expected_entity_types', 
                          'relationship_patterns', 'processing_strategy']
        completeness_score = sum(1 for field in required_fields if getattr(config, field, None)) / len(required_fields)
        ...
        threshold_score = 1.0
        if config.entity_confidence_threshold < 0.5 or config.entity_confidence_threshold > 0.9:
            threshold_score -= 0.3
        if config.relationship_confidence_threshold < 0.4 or config.relationship_confidence_threshold > 0.8:
            threshold_score -= 0.2
        
        # Performance predictions
        predicted_time = config.chunk_size / 1000 * 0.5  # Rough estimate: 0.5s per 1000 tokens
        predicted_memory = len(config.expected_entity_types) * 0.1  # Rough estimate: 0.1MB per entity type
        predicted_accuracy = (entity_precision_estimate + relationship_precision_estimate) / 2
...
# Helper Functions
def _convert_hybrid_analysis_to_extraction_config(domain: str, hybrid_analysis: HybridAnalysis) -> ExtractionConfiguration:
    """
    ...
    relationship_patterns = [f"{r[0]} {r[1]} {r[2]}" for r in llm.semantic_relationships[:30]]
    ...
    return ExtractionConfiguration(
        ...
        max_concurrent_chunks=stats.performance_parameters.get("max_concurrent_chunks", 5),
        ...
        validation_criteria={
            ...
            "max_processing_time_seconds": stats.performance_parameters.get("extraction_timeout_seconds", 30)
        },
        extraction_timeout_seconds=stats.performance_parameters.get("extraction_timeout_seconds", 30),
...
def _determine_extraction_strategy_from_llm(llm_extraction: LLMExtraction) -> ExtractionStrategyType:
    ...
    strategy = domain_strategies.get(llm_extraction.domain_classification, ExtractionStrategyType.MIXED_CONTENT)
...
def _determine_extraction_strategy(patterns: ExtractedPatterns, domain_config: DomainConfig) -> ExtractionStrategyType:
...
def _calculate_optimal_entity_threshold(patterns: ExtractedPatterns, domain_config: DomainConfig) -> float:
    ...
    if patterns.entity_patterns:
        ...
        return max(0.6, sorted_conf[threshold_index] if threshold_index < len(sorted_conf) else 0.7)
    return 0.7  # Safe default


def _calculate_optimal_relationship_threshold(patterns: ExtractedPatterns, domain_config: DomainConfig) -> float:
...
def _calculate_optimal_chunk_params(patterns: ExtractedPatterns, domain_config: DomainConfig) -> Tuple[int, int]:
...
def create_fully_learned_extraction_config(*args, **kwargs):
    ...
    if hasattr(agent, '_function_tools') and 'create_fully_learned_extraction_config' in agent._function_tools:

...
from ..interfaces.agent_contracts import (
    AzureServiceMetrics, CacheContract, ErrorHandlingContract, 
    MonitoringContract, StatisticalPattern
)
...
class StatisticalAnalysisCapability(Protocol):
    ...
    async def calculate_confidence_interval(self, data: List[float], confidence_level: float = 0.95) -> Dict[str, float]:
        """Calculate confidence interval for data"""
        ...
    
    async def perform_significance_test(self, sample1: List[float], sample2: List[float]) -> Dict[str, float]:
        """Perform statistical significance test"""
        ...
    
    async def learn_patterns_from_data(self, data: List[str], pattern_type: str) -> List[StatisticalPattern]:
        """Learn statistical patterns from data"""
        ...
    
    async def validate_pattern_significance(self, patterns: List[StatisticalPattern]) -> List[StatisticalPattern]:
...
class AzureServiceOrchestrationCapability(Protocol):
    ...
    async def optimize_azure_costs(self, service_usage: Dict[str, Any]) -> Dict[str, Any]:
...
class ErrorRecoveryCapability(Protocol):
    ...
    async def handle_azure_service_error(self, error: Exception, service_name: str, operation: str) -> Optional[Any]:
        """Handle Azure service errors with recovery strategies"""
        ...
    
    async def handle_agent_error(self, error: Exception, agent_name: str, operation: str) -> Optional[Any]:
        """Handle agent-specific errors with fallback strategies"""
        ...
    
    async def get_recovery_strategy(self, error_type: str, context: Dict[str, Any]) -> Optional[str]:
...
class PerformanceMonitoringCapability(Protocol):
    ...
    async def track_operation_performance(self, operation: str, component: str, metrics: Dict[str, float]) -> None:
        """Track operation performance metrics"""
        ...
    
    async def track_azure_service_usage(self, service_name: str, metrics: AzureServiceMetrics) -> None:
        """Track Azure service usage and costs"""
        ...
    
    async def get_performance_insights(self, component: str, time_window_hours: int = 24) -> Dict[str, Any]:
...
class SharedCacheCapability:
    ...
    async def get(self, key: str, namespace: str = "default") -> Optional[Any]:
        ...
        try:
            ...
            if redis_value is not None:
                ...
                # Warm local cache
                self._store_local(namespaced_key, redis_value, ttl=300)  # 5 min local TTL
                return redis_value
        
        except Exception as e:
            await self.context.error_handler.handle_error(
                error=e,
                operation="azure_redis_get",
    ...
    async def invalidate(self, pattern: str, namespace: str = "default") -> int:
        ...
        try:
            ...
            local_keys_to_remove = [k for k in self.local_cache.keys() if k.startswith(namespaced_pattern.replace("*", ""))]
    ...
    async def get_stats(self, namespace: str = "default") -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_tier": "high" if hit_rate > 80 else "medium" if hit_rate > 60 else "low"
        }
...
class SharedStatisticalAnalysisCapability:
    ...
    async def perform_significance_test(self, sample1: List[float], sample2: List[float]) -> Dict[str, float]:
        ...
        try:
            ...
            u_statistic, u_p_value = stats.mannwhitneyu(sample1, sample2, alternative='two-sided')
        ...
        try:
            ...
            for pattern_data in ml_result.get("patterns", []):
                pattern = StatisticalPattern(
                    ...
                    confidence_interval=pattern_data.get("confidence_interval", {"lower": 0.0, "upper": 1.0}),
    ...
    async def validate_pattern_significance(self, patterns: List[StatisticalPattern]) -> List[StatisticalPattern]:
        ...
        for pattern in patterns:
            ...
            if (pattern.chi_square_p_value < 0.05 and  # Statistically significant
                pattern.frequency >= 3 and  # Minimum frequency
                pattern.confidence >= 0.6):  # Minimum confidence
...
class SharedAzureServiceOrchestrationCapability:
    ...
    async def execute_azure_ml_job(self, job_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._track_service_metrics("azure_ml", execution_time, True, job_config)
            ...
            execution_time = time.time() - start_time
            await self._track_service_metrics("azure_ml", execution_time, False, job_config)
    ...
    async def query_azure_search(self, query_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Track performance metrics
            execution_time = time.time() - start_time
            await self._track_service_metrics("azure_search", execution_time, True, optimized_config)
            ...
            execution_time = time.time() - start_time
            await self._track_service_metrics("azure_search", execution_time, False, query_config)
    ...
    async def execute_cosmos_query(self, query: str, database: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Track performance metrics
            execution_time = time.time() - start_time
            await self._track_service_metrics("azure_cosmos", execution_time, True, {"query": query})
            ...
            execution_time = time.time() - start_time
            await self._track_service_metrics("azure_cosmos", execution_time, False, {"query": query})
    ...
    async def _track_service_metrics(self, service: str, execution_time: float, success: bool, config: Dict[str, Any]):
    ...
    async def _optimize_search_query(self, query_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _estimate_service_cost(self, service: str, config: Dict[str, Any]) -> float:
...
class CapabilityManager:
    ...
    async def get_statistical_analysis_capability(self) -> StatisticalAnalysisCapability:
        ...
        if key not in self._capabilities:
            context = CapabilityContext(
                ...
                performance_tracker=await self._get_performance_tracker(),
                error_handler=await self._get_error_handler(),
                cache_manager=await self.get_cache_capability(CacheContract(cache_key_namespace="stats")),
    ...
    async def get_azure_orchestration_capability(self) -> AzureServiceOrchestrationCapability:
        ...
        if key not in self._capabilities:
            context = CapabilityContext(
                ...
                performance_tracker=await self._get_performance_tracker(),
                error_handler=await self._get_error_handler(),
                cache_manager=await self.get_cache_capability(CacheContract(cache_key_namespace="azure")),
    ...
    def decorator(func):
        async def wrapper(self, *args, **kwargs):
            ...
            cache_contract = CacheContract(
                cache_key_namespace=namespace,
                ttl_seconds=ttl
            )
            cache_capability = await self.capability_manager.get_cache_capability(cache_contract)
...
def with_statistical_analysis_capability():
    ...
    def decorator(func):
        async def wrapper(self, *args, **kwargs):
            stats_capability = await self.capability_manager.get_statistical_analysis_capability()
...
def with_azure_orchestration_capability():
    ...
    def decorator(func):
        async def wrapper(self, *args, **kwargs):
            azure_capability = await self.capability_manager.get_azure_orchestration_capability()
...
class ExampleAgentWithSharedCapabilities:
    ...
    async def analyze_domain_with_caching(self, cache: CacheCapability, domain_data: List[str]) -> Dict[str, Any]:
    ...
    async def validate_patterns_with_statistics(self, stats: StatisticalAnalysisCapability, patterns: List[Dict]) -> List[Dict]:
        ...
        validated_patterns = await stats.validate_pattern_significance(statistical_patterns)
        
        return [p.model_dump() for p in validated_patterns]
    
    @with_azure_orchestration_capability()
    async def execute_ml_analysis_with_orchestration(self, azure_orch: AzureServiceOrchestrationCapability, analysis_config: Dict) -> Dict[str, Any]:

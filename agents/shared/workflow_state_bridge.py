...
from agents.core.constants import StatisticalConstants, UniversalSearchConstants, CacheConstants, MLModelConstants
...
class WorkflowStateBridge:
    ...
    def is_config_fresh(self, domain: str, max_age_hours: int = 24) -> bool:
        ...
        age_hours = (datetime.now() - config_age).total_seconds() / CacheConstants.SECONDS_PER_HOUR
        return age_hours <= max_age_hours
    
    def cleanup_old_configs(self, max_age_hours: int = 168) -> int:  # 7 days default
        """Clean up old configuration files"""
        cleaned_count = 0
        cutoff_time = datetime.now().timestamp() - (max_age_hours * CacheConstants.SECONDS_PER_HOUR)
...
def create_sample_config(domain: str = "test_domain") -> WorkflowConfig:
    ...
    return WorkflowConfig(
        ...
        processing_patterns={
            "vector_params": {"model": AzureServiceConstants.DEFAULT_EMBEDDING_MODEL, "dimensions": MLModelConstants.EMBEDDING_DIMENSION},
            "graph_params": {"max_depth": 3, "min_score": UniversalSearchConstants.VECTOR_SIMILARITY_THRESHOLD},
            "gnn_params": {"hidden_dim": MLModelConstants.GNN_HIDDEN_DIM, "num_layers": MLModelConstants.GNN_NUM_LAYERS}
        },
        synthesis_weights={

...
class AzureServiceToolset(FunctionToolset):
        ...
        try:
            ...
            if ctx.deps:
                ...
                return {
                    ...
                    "model_name": (
                        ctx.model.name() if hasattr(ctx.model, "name") else "unknown"
                    ),
    ...
    async def validate_credentials(
        ...
        try:
            ...
            return {"credentials_valid": False, "access_level": "none", "error": str(e)}
    ...
    async def get_service_limits(
        ...
        try:
            ...
            return {"limits_healthy": False, "error": str(e)}
...
class PerformanceToolset(FunctionToolset):
    ...
    async def get_performance_metrics(
        ...
        ctx: RunContext[
            AzureServicesDeps
        ],  # CacheManagerDeps deleted - use AzureServicesDeps
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if ctx.deps:
                ...
                cache_stats = getattr(
                    ctx.deps.cache_instance, "get_stats", lambda: {}
                )()
                base_metrics.update(
                    {
                        "cache_hit_rate": cache_stats.get(
                            "hit_rate", CacheConstants.ZERO_FLOAT
                        ),
                        "cache_size_mb": cache_stats.get(
                            "total_size_mb", CacheConstants.ZERO_FLOAT
                        ),
        ...
        try:
            if ctx.deps and ctx.deps.cache_manager:
                ...
                return {
                    ...
                    "entries_removed": initial_stats.get("total_entries", 0)
                    - final_stats.get("total_entries", 0),
        ...
        try:
            ...
            return {
                "total_memory_gb": round(
                    memory.total / CacheConstants.BYTES_TO_GB_DIVISOR,
                    CacheConstants.MEMORY_PRECISION_DECIMAL,
                ),
                "available_memory_gb": round(
                    memory.available / CacheConstants.BYTES_TO_GB_DIVISOR,
                    CacheConstants.MEMORY_PRECISION_DECIMAL,
                ),
                "used_memory_gb": round(
                    memory.used / CacheConstants.BYTES_TO_GB_DIVISOR,
                    CacheConstants.MEMORY_PRECISION_DECIMAL,
                ),
            ...
            return {"memory_monitoring_error": str(e), "memory_status": "error"}

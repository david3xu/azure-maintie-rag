...
class AgentBoundaryViolation(BaseModel):
    ...
    conflicting_responsibility: str = Field(
        ..., description="Responsibility that conflicts with another agent"
    )
    conflicting_agent: Optional[str] = Field(
        default=None, description="Agent with conflicting responsibility"
    )
...
class AzureServiceIntegrationGap(BaseModel):
    ...
    mock_usage_detected: bool = Field(
        default=False, description="Whether mock usage was detected"
    )
    severity: str = Field(..., description="Severity level")
...
class ToolDelegationViolation(BaseModel):
    ...
    self_contained_logic: str = Field(
        ..., description="Self-contained logic that should be delegated"
    )
...
class PerformanceValidationResult(BaseModel):
    ...
    compliance_status: str = Field(
        ..., description="Compliance status: compliant, warning, violation"
    )
    competitive_advantage_preserved: bool = Field(
        ..., description="Whether competitive advantage is preserved"
    )
...
class ArchitectureComplianceReport(BaseModel):
    ...
    overall_compliance_status: str = Field(
        ..., description="Overall compliance: compliant, warning, violation"
    )
    compliance_score: float = Field(
        ..., ge=0.0, le=100.0, description="Overall compliance score percentage"
    )

    # Detailed violation findings
    hardcoded_value_violations: List[HardcodedValueDetection] = Field(
        default_factory=list
    )
    agent_boundary_violations: List[AgentBoundaryViolation] = Field(
        default_factory=list
    )
    azure_integration_gaps: List[AzureServiceIntegrationGap] = Field(
        default_factory=list
    )
    tool_delegation_violations: List[ToolDelegationViolation] = Field(
        default_factory=list
    )
    performance_validation_results: List[PerformanceValidationResult] = Field(
        default_factory=list
    )
    ...
    critical_violations: int = Field(
        ..., ge=0, description="Critical violations requiring immediate attention"
    )
    high_severity_violations: int = Field(
        ..., ge=0, description="High severity violations"
    )

    # Competitive advantages validation
    competitive_advantages_preserved: Dict[str, bool] = Field(
        ..., description="Status of competitive advantages"
    )

    # Recommendations summary
    priority_recommendations: List[str] = Field(
        ..., description="Priority recommendations for compliance"
    )
    ...
    def is_compliant(self) -> bool:
        ...
        return (
            ...
            and self.critical_violations == 0
            and len(self.hardcoded_value_violations) == 0
            and len(self.agent_boundary_violations) == 0
            and all(self.competitive_advantages_preserved.values())
        )
...
class HardcodedValueAnalyzer:
    ...
    def __init__(self):
        ...
        self.allowed_constants = {
            ...
            "3",
            "4",
            "5",  # Basic numeric constants
            "true",
            "false",
            "null",
            "none",  # Basic boolean/null constants
            "utf-8",
            "ascii",  # Standard encodings
            "GET",
            "POST",
            "PUT",
            "DELETE",  # HTTP methods
    ...
    async def analyze_file(self, file_path: Path) -> List[HardcodedValueDetection]:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                line_violations = self._analyze_line_for_hardcoded_values(
    ...
    def _analyze_ast_for_hardcoded_values(
        self, tree: ast.AST
    ) -> List[HardcodedValueDetection]:
    ...
    def _analyze_line_for_hardcoded_values(
    ...
    def _is_suspicious_hardcoded_number(self, value: str) -> bool:
        ...
        try:
            ...
            return (
                (0.0 < num_value < 1.0 and num_value not in [0.5])
                or (num_value > 100 and num_value % 100 == 0)  # Likely thresholds
                or (  # Round numbers over 100
                    num_value in [1024, 2048, 4096]
...
class AgentBoundaryAnalyzer:
        ...
        self.agent_responsibilities = {
            "DomainIntelligenceAgent": {
                ...
                "forbidden": [
                    "document_extraction",
                    "search_operations",
                    "direct_content_processing",
                ],
            },
            "KnowledgeExtractionAgent": {
                "allowed": [
                    "tool_orchestration",
                    "extraction_coordination",
                    "result_aggregation",
                    "quality_monitoring",
                ],
                "forbidden": [
    ...
    async def analyze_agent_boundaries(
        self, agent_files: Dict[str, Path]
    ) -> List[AgentBoundaryViolation]:
    ...
    async def _analyze_single_agent(
        self, agent_name: str, file_path: Path
    ) -> List[AgentBoundaryViolation]:
        ...
        try:
            ...
            if agent_name in self.agent_responsibilities:
                forbidden_responsibilities = self.agent_responsibilities[agent_name][
                    "forbidden"
                ]
    ...
    def _detect_responsibility_overlaps(
        self, agent_files: Dict[str, Path]
    ) -> List[AgentBoundaryViolation]:
...
class AzureServiceIntegrationAnalyzer:
    ...
    def __init__(self):
        self.required_azure_services = {
            "azure_openai": {
                "patterns": ["openai", "gpt", "embedding"],
                "mock_indicators": ["dummy", "fake", "mock"],
            },
            "azure_search": {
                "patterns": ["search", "index", "query"],
                "mock_indicators": ["fake_search", "mock_search"],
            },
            "azure_cosmos": {
                "patterns": ["cosmos", "graph", "gremlin"],
                "mock_indicators": ["fake_cosmos", "mock_graph"],
            },
            "azure_ml": {
                "patterns": ["ml", "model", "training"],
                "mock_indicators": ["dummy_model", "fake_ml"],
            },
            "azure_storage": {
                "patterns": ["storage", "blob", "container"],
                "mock_indicators": ["fake_storage", "mock_blob"],
            },
        }

    async def analyze_azure_integration(
        self, codebase_path: Path
    ) -> List[AzureServiceIntegrationGap]:
    ...
    async def _analyze_file_for_azure_usage(
        self, file_path: Path
    ) -> List[AzureServiceIntegrationGap]:
        ...
        try:
            ...
            for service_name, service_config in self.required_azure_services.items():
                ...
                uses_patterns = any(
                    pattern in content.lower() for pattern in service_config["patterns"]
                )
                ...
                if uses_patterns:
                    ...
                    has_mock_usage = any(
                        ...
                        for mock in service_config["mock_indicators"]
                    )

                    # Check for proper Azure client usage
                    has_azure_client = (
                        f"azure_{service_name.split('_')[1]}" in content.lower()
                    )

                    if has_mock_usage:
                        gaps.append(
                            AzureServiceIntegrationGap(
...
class ToolDelegationAnalyzer:
    ...
    async def analyze_tool_delegation(
        self, agent_files: Dict[str, Path]
    ) -> List[ToolDelegationViolation]:
        """Analyze agent files for tool delegation violations"""
        violations = []

        for agent_name, file_path in agent_files.items():
            agent_violations = await self._analyze_agent_for_self_contained_logic(
    ...
    async def _analyze_agent_for_self_contained_logic(
        self, agent_name: str, file_path: Path
    ) -> List[ToolDelegationViolation]:
...
class MultiAgentArchitectureComplianceValidator:
    ...
    async def validate_architecture(self) -> ArchitectureComplianceReport:
        ...
        critical_violations = sum(
            1 for v in hardcoded_violations if v.severity == "critical"
        )
        critical_violations += sum(
            1 for v in boundary_violations if v.severity == "critical"
        )
        critical_violations += sum(1 for v in azure_gaps if v.severity == "critical")

        high_severity_violations = sum(
            1 for v in hardcoded_violations if v.severity == "high"
        )
        high_severity_violations += sum(
            1 for v in boundary_violations if v.severity == "high"
        )
        high_severity_violations += sum(1 for v in azure_gaps if v.severity == "high")
        high_severity_violations += sum(
            1 for v in tool_violations if v.severity == "high"
        )
        ...
        report = ArchitectureComplianceReport(
            ...
            overall_compliance_status=self._determine_overall_status(
    ...
    async def _validate_agent_boundaries(
        self, agent_files: Dict[str, Path]
    ) -> List[AgentBoundaryViolation]:
    ...
    async def _validate_tool_delegation(
        self, agent_files: Dict[str, Path]
    ) -> List[ToolDelegationViolation]:
    ...
    async def _validate_performance_requirements(
        self,
    ) -> List[PerformanceValidationResult]:
    ...
    def _calculate_compliance_score(
        ...
        penalty = min(
            100.0, weighted_violations * 5
        )  # Each weighted violation costs 5 points

        return max(0.0, 100.0 - penalty)

    def _determine_overall_status(
        self, compliance_score: float, critical_violations: int
    ) -> str:
    ...
    def _generate_priority_recommendations(
        self, hardcoded_violations, boundary_violations, azure_gaps, tool_violations
    ) -> List[str]:
        ...
        critical_hardcoded = [
            v for v in hardcoded_violations if v.severity == "critical"
        ]
        if critical_hardcoded:
            recommendations.append(
                f"CRITICAL: Replace {len(critical_hardcoded)} hardcoded values with Azure service integration"
            )

        # Agent boundary violations
        if boundary_violations:
            recommendations.append(
                f"HIGH: Fix {len(boundary_violations)} agent boundary violations to establish clear responsibilities"
            )
        ...
        if critical_azure:
            recommendations.append(
                f"CRITICAL: Replace {len(critical_azure)} mock Azure services with real integration"
            )

        # Tool delegation violations
        if tool_violations:
            recommendations.append(
                f"MEDIUM: Implement tool delegation for {len(tool_violations)} self-contained logic blocks"
            )
...
async def run_architecture_compliance_validation(
    codebase_path: str,

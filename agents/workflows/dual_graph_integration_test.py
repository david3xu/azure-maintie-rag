...
from agents.workflows.implementation_guide import (
    migration_manager,
...
class DualGraphIntegrationTests:
    ...
    async def test_configuration_bridge_integration(self):
        ...
        try:
            ...
            assert (
                extraction_config.get("hardcoded_values") == False
    ...
    async def test_enhanced_state_bridge_communication(self):
        ...
        retrieved_state = await enhanced_state_bridge.get_state(
            target_workflow="search", transfer_type=StateTransferType.CONFIG_GENERATION
        )
    ...
    async def test_config_extraction_workflow_integration(self):
        ...
        result = await dual_graph_orchestrator.execute_config_extraction_pipeline(
            corpus_path=test_corpus_path, domain_name="test_programming"
        )
        ...
        if result["state"] == "completed":
            ...
            ), f"Generated config invalid: {config_validation.missing_keys}"

        else:
            logger.warning(
                f"⚠️  Config-Extraction workflow did not complete: {result.get('error')}"
            )
    ...
    async def test_search_workflow_with_dynamic_config(self):
        ...
        result = await dual_graph_orchestrator.execute_search_pipeline(
            query="What is asyncio in Python?", domain="test_programming", max_results=5
        )
    ...
    async def test_zero_hardcoded_values_compliance(self):
        ...
        try:
            config = await config_bridge.get_workflow_config(
                "extraction", "test_domain"
            )
    ...
    async def test_error_handling_and_recovery(self):
        ...
        result = await dual_graph_orchestrator.execute_config_extraction_pipeline(
            corpus_path="/nonexistent/path", domain_name="test_error_handling"
        )
        ...
        search_result = await dual_graph_orchestrator.execute_search_pipeline(
            query="", domain="nonexistent_domain"  # Empty query
        )
    ...
    async def test_performance_benchmarks(self):
        ...
        result = await dual_graph_orchestrator.execute_search_pipeline(
            query="Test performance query", domain="test_performance"
        )
...
async def run_comprehensive_integration_tests():
    ...
    if passed_tests == total_tests:
        ...
        logger.warning(
            f"⚠️  {total_tests - passed_tests} tests failed - review required"
        )
...
async def validate_dual_graph_architecture():
    ...
    try:
        ...
        pipeline_status = await dual_graph_orchestrator.get_pipeline_status()
        validation_results["components_operational"]["dual_graph_orchestrator"] = all(
        ...
        validation_results["components_operational"]["state_bridge"] = (
            bridge_status["overall_success_rate"]
            >= WorkflowConstants.MINIMUM_SUCCESS_RATE
        )
        ...
        integration_ok = (
            sum(1 for r in integration_results.values() if r["status"] == "passed")

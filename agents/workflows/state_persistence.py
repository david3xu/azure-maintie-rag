...
class WorkflowStateManager:
    ...
    async def load_workflow_state(self, workflow_id: str) -> Optional[WorkflowStateRecord]:
        """
    ...
    async def _load_workflow_record(self, workflow_id: str) -> Optional[WorkflowStateRecord]:
    ...
    async def get_workflow_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for state in WorkflowState:
                stats["by_state"][state.value] = len([w for w in workflows if w.state == state])
            
            # Count by type
            workflow_types = set(w.workflow_type for w in workflows)
            for wf_type in workflow_types:
                stats["by_type"][wf_type] = len([w for w in workflows if w.workflow_type == wf_type])

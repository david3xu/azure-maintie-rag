...
class SearchWorkflow:
        ...
        return {
            ...
            "performance_grade": (
                ...
                if total_time < 1.0
                    ...
                    "good"
                    if total_time < 2.0
                    else "acceptable" if total_time < 3.0 else "needs_optimization"
                )
            ),
            "domain_detection_time": self.nodes.get(
                "domain_detection", WorkflowNode("", "", lambda: None)
            ).execution_time,
            "search_execution_time": self.nodes.get(
                "tri_modal_search", WorkflowNode("", "", lambda: None)
            ).execution_time,
            "synthesis_time": self.nodes.get(
                "result_synthesis", WorkflowNode("", "", lambda: None)
            ).execution_time,
    ...
    async def _execute_domain_detection(
    ...
    async def _execute_search_strategy_selection(
        ...
        return {
            ...
            "search_weights": {
                "vector": WorkflowConstants.DEFAULT_VECTOR_SEARCH_WEIGHT,
                "graph": WorkflowConstants.DEFAULT_GRAPH_SEARCH_WEIGHT,
                "gnn": WorkflowConstants.DEFAULT_GNN_SEARCH_WEIGHT,
            },
    ...
    async def _execute_tri_modal_search(
        ...
        result = await agent.run(
            ...
            message_history=[{"role": "user", "content": f"Search for: {query}"}],
        )

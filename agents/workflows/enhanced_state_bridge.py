...
class EnhancedStateBridge:
    ...
    async def transfer_state(
        ...
        transfer_id = f"{source_workflow}_to_{target_workflow}_{int(datetime.now().timestamp())}"
        ...
        try:
            ...
            if not await self._validate_dependencies(packet.dependencies):
                raise Exception(f"Transfer dependencies not satisfied: {packet.dependencies}")
            ...
            await self._notify_listeners("state_transferred", {
                "transfer_id": transfer_id,
            ...
            logger.info(f"✅ State transferred: {source_workflow} → {target_workflow} ({transfer_type.value})")
            return transfer_id
            
        except Exception as e:
            logger.error(f"❌ State transfer failed: {source_workflow} → {target_workflow}: {e}")
    ...
    async def get_state(
        ...
        for packet in self.transfer_queue.values():
            ...
            if not packet.validate_integrity():
                logger.warning(f"⚠️  State packet integrity check failed: {packet.transfer_id}")
                continue
        ...
        logger.debug(f"📋 Retrieved state for {target_workflow}: {latest_transfer.transfer_id}")
    ...
    async def wait_for_dependency(
        ...
        logger.info(f"⏳ Waiting for dependency: {target_workflow} needs {dependency_type.value}")
        
        start_time = datetime.now()
        timeout = datetime.now().replace(
            second=datetime.now().second + timeout_seconds
        )
        ...
        while datetime.now() < timeout:
            ...
            if dependency_state:
                logger.info(f"✅ Dependency satisfied: {target_workflow} got {dependency_type.value}")
                return dependency_state
            
            # Wait before checking again
            await asyncio.sleep(1.0)
        
        logger.error(f"⏰ Dependency timeout: {target_workflow} waiting for {dependency_type.value}")
        return None
    ...
    async def cleanup_expired_transfers(self):
        ...
        expired_transfers = [
            transfer_id for transfer_id, packet in self.transfer_queue.items()
            if packet.is_expired()
        ]
        ...
        if expired_transfers:
            logger.info(f"🗑️  Cleaned up {len(expired_transfers)} expired state transfers")
        ...
        return {
            ...
            "graph_connections": {
                key: {
                    ...
                    "last_transfer": conn.last_transfer.isoformat() if conn.last_transfer else None
                }
                for key, conn in self.connection_info.items()
            },
            "dependency_chains": self.dependency_graph,
            "event_listeners": {
                event: len(listeners) for event, listeners in self.event_listeners.items()
            }
        }

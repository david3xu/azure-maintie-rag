...
class DualGraphOrchestrator:
    ...
    async def execute_config_extraction_pipeline(
        ...
        logger.info(f"📂 Analyzing corpus at: {corpus_path}")
        ...
        try:
            ...
            if result["state"] == WorkflowState.COMPLETED.value:
                ...
                logger.info(f"✅ Config-Extraction workflow completed")
                logger.info(f"   🧠 Domain configurations generated for: {domain_name}")
                logger.info(f"   ⏱️  Total execution time: {result['total_time_seconds']:.2f}s")
            else:
                logger.error(f"❌ Config-Extraction workflow failed: {result.get('error', 'Unknown error')}")
                
            return result
            
        except Exception as e:
            logger.error(f"❌ Config-Extraction pipeline failed: {e}")
    ...
    async def execute_search_pipeline(
        ...
        logger.info(f"🔍 Starting Search workflow for query: {query[:50]}...")
        if domain:
        ...
        try:
            ...
            if result["state"] == WorkflowState.COMPLETED.value:
                logger.info(f"✅ Search workflow completed")
                logger.info(f"   📊 Results found: {len(result.get('search_results', {}).get('results', []))}")
                logger.info(f"   ⏱️  Total execution time: {result['total_time_seconds']:.2f}s")
            else:
                logger.error(f"❌ Search workflow failed: {result.get('error', 'Unknown error')}")
    ...
    async def execute_full_pipeline(
        ...
        """
        Execute both graphs in sequence: Config-Extraction → Search
        ...
        logger.info(f"🚀 Starting full dual-graph pipeline")
        ...
        try:
            ...
            if config_result["state"] != WorkflowState.COMPLETED.value:
                raise Exception(f"Config-Extraction workflow failed: {config_result.get('error')}")
            
            # Step 2: Execute Search workflow if query provided
            if query:
                search_result = await self.execute_search_pipeline(
                    query, domain_name
                )
            ...
            logger.info(f"✅ Full dual-graph pipeline completed")
            logger.info(f"   ⏱️  Total pipeline time: {results['total_pipeline_time']:.2f}s")
    ...
    async def _ensure_domain_config_availability(self, domain: str):
        ...
        try:
            ...
            logger.info(f"📋 Loaded existing configuration for domain: {domain}")
            return True
            
        except Exception as e:
            logger.warning(f"⚠️  No configuration found for domain '{domain}': {e}")
            logger.info(f"🔄 Auto-triggering Config-Extraction workflow")
            ...
            if Path(corpus_path).exists():
                ...
                raise Exception(f"Cannot generate config for domain '{domain}' - corpus not found")
    ...
    async def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        return {
            "config_extraction_graph": {
                "status": "operational",
                "last_execution": self.inter_graph_state.get("config_extraction", {}).get("timestamp"),
            },
            "search_graph": {
                "status": "operational", 
                "last_execution": self.inter_graph_state.get("search", {}).get("timestamp"),
            },
...
__all__ = [
    "DualGraphOrchestrator",
    "dual_graph_orchestrator"
]
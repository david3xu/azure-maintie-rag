...
class SearchRequest(BaseModel):
    ...
    strategy: Optional[SearchStrategy] = Field(None, description="Optional search strategy override")
...
class SearchResults(BaseModel):
    ...
    # Unified results
    synthesized_results: List[Dict[str, Any]] = Field(..., description="Synthesized unified results")
    synthesis_confidence: float = Field(..., description="Synthesis confidence score")
    result_rankings: Dict[str, float] = Field(..., description="Result relevance rankings")
...
class SearchOrchestrator:
    ...
    async def execute_search(
        ...
        try:
            ...
            synthesized_results, synthesis_confidence, rankings = await self._synthesize_results(
    ...
    def _calculate_performance_metrics(
        ...
        parallel_efficiency = total_modality_time / total_time if total_time > 0 else 0.0
        
        # Estimate cache hit rate (simplified)
        fast_results = sum(1 for r in modality_results.values() if r.execution_time < 0.1)
        cache_hit_rate = fast_results / len(modality_results) if modality_results else 0.0
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._search_stats,
            "success_rate": (
                self._search_stats["successful_searches"] / self._search_stats["total_searches"]
                if self._search_stats["total_searches"] > 0 else 0.0
            )
        }

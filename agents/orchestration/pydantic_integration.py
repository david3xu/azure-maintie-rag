...
from ..pydantic_ai_integration import (  # azure_rag_agent,  # Commented out - not available yet
...
class PydanticAIWorkflowIntegration:
    ...
    async def delegate_to_agent(
        self, request: AgentDelegationRequest
        ...
        try:
            ...
            if request.delegation_strategy == AgentDelegationStrategy.SINGLE_AGENT:
                ...
                == AgentDelegationStrategy.MULTI_AGENT_PARALLEL
            ):
                result = await self._multi_agent_parallel_delegation(request)
            elif (
                request.delegation_strategy
                == AgentDelegationStrategy.MULTI_AGENT_SEQUENTIAL
            ):
                result = await self._multi_agent_sequential_delegation(request)
            elif (
                request.delegation_strategy
                == AgentDelegationStrategy.ADAPTIVE_DELEGATION
            ):
                result = await self._adaptive_delegation(request)
            else:
                raise ValueError(
                    f"Unknown delegation strategy: {request.delegation_strategy}"
                )
            ...
            if result.success:
                ...
                current_compliance = self.delegation_metrics["sla_compliance_rate"] * (
                    total_delegations - 1
                )
                self.delegation_metrics["sla_compliance_rate"] = (
                    current_compliance + (1 if sla_met else 0)
                ) / total_delegations
            else:
            ...
            total_delegations = self.delegation_metrics["total_delegations"]
            current_avg = self.delegation_metrics["average_execution_time"] * (
                total_delegations - 1
            )
            self.delegation_metrics["average_execution_time"] = (
                current_avg + execution_time
            ) / total_delegations
    ...
    async def _single_agent_delegation(
        ...
        try:
            # Create or get agent
            agent_config = (
                request.agent_configs[0]
                if request.agent_configs
                else PydanticAgentConfig("primary-agent")
            )
        ...
        try:
            if not request.agent_configs:
                ...
                request.agent_configs = [
                    PydanticAgentConfig(
                        "search-agent",
                        system_prompt="You specialize in search and retrieval.",
                    ),
                    PydanticAgentConfig(
                        "analysis-agent",
                        system_prompt="You specialize in analysis and synthesis.",
                    ),
                    PydanticAgentConfig(
                        "domain-agent",
                        system_prompt="You specialize in domain understanding.",
                    ),
                ]
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    failed_count += 1
                    logger.error(
                        f"Agent {request.agent_configs[i].agent_name} failed: {result}"
                    )
                else:
        ...
        try:
            if not request.agent_configs:
                ...
                request.agent_configs = [
                    PydanticAgentConfig(
                        "domain-agent",
                        system_prompt="First, identify the domain and context.",
                    ),
                    PydanticAgentConfig(
                        "search-agent",
                        system_prompt="Then, perform comprehensive search.",
                    ),
                    PydanticAgentConfig(
                        "synthesis-agent",
                        system_prompt="Finally, synthesize and format results.",
                    ),
                ]
    ...
    async def integrate_with_workflow(
        self, workflow_request: WorkflowRequest, use_pydantic_agents: bool = True
    ) -> WorkflowResults:
        """
        ...
        try:
            if not use_pydantic_agents:
                ...
                return await self.workflow_orchestrator.process_workflow(
                    workflow_request
                )

            # Enhanced workflow with PydanticAI integration
            logger.info(
                f"Processing workflow with PydanticAI integration: {workflow_request.query[:50]}..."
            )
            ...
            if delegation_result.success and delegation_result.results:
                ...
                workflow_results = WorkflowResults(
                    ...
                    performance_met=delegation_result.competitive_advantages_achieved.get(
                ...
                logger.info(
                    f"Workflow with PydanticAI completed successfully: {delegation_result.execution_time:.2f}s"
                )
                return workflow_results
            else:
                # Handle delegation failure
                logger.error(
                    f"PydanticAI delegation failed: {delegation_result.error_message}"
                )

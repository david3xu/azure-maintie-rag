...
class ConfigExtractionOrchestrator:
    ...
    async def process_domain_documents(
        self, domain_path: Path, force_regenerate_config: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if not extraction_config:
            raise ValueError(
                f"Failed to generate extraction configuration for domain: {domain_name}"
            )

        logger.info(
            f"✅ Stage 1 Complete: ExtractionConfiguration generated for {domain_name}"
        )
        logger.info(
            f"   Entity threshold: {extraction_config.entity_confidence_threshold}"
        )
        logger.info(f"   Processing strategy: {extraction_config.processing_strategy}")
        logger.info(
            f"   Expected entity types: {len(extraction_config.expected_entity_types)}"
        )
        ...
        logger.info(
            f"✅ Stage 2 Complete: Knowledge extraction completed for {domain_name}"
        )
        logger.info(f"   Documents processed: {extraction_results.documents_processed}")
        logger.info(
            f"   Entities extracted: {extraction_results.total_entities_extracted}"
        )
        logger.info(
            f"   Relationships extracted: {extraction_results.total_relationships_extracted}"
        )
    ...
    async def _generate_extraction_configuration(
        self, domain_name: str, domain_path: Path, force_regenerate: bool = False
        ...
        if not force_regenerate:
            ...
            if cached_config:
                logger.info(
                    f"📋 Using cached extraction configuration for {domain_name}"
                )
        ...
        try:
            ...
            if extraction_config:
                logger.info(
                    f"✅ Domain Intelligence Agent generated configuration for {domain_name}"
                )
    ...
    async def _extract_knowledge_with_config(
        self, domain_path: Path, config: ExtractionConfiguration
    ) -> ExtractionResults:
        """
        ...
        logger.info(
            f"✅ Knowledge extraction complete: {successful_extractions}/{len(documents)} successful"
        )
    ...
    def _create_extraction_results(
        ...
        total_entities = sum(
            len(ext.entities) for ext in extractions if hasattr(ext, "entities")
        )
        total_relationships = sum(
            len(ext.relationships)
            for ext in extractions
            if hasattr(ext, "relationships")
        )
        ...
        return ExtractionResults(
            ...
            # Validation
            extraction_passed_validation=extraction_accuracy
...
async def process_domain_with_config_extraction(
    domain_path: Path, force_regenerate_config: bool = False
) -> Dict[str, Any]:
    """
    ...
    return await orchestrator.process_domain_documents(
        domain_path, force_regenerate_config
    )


# Export key components
__all__ = ["ConfigExtractionOrchestrator", "process_domain_with_config_extraction"]

...
class UnifiedResults(BaseModel):
    ...
    orchestration_strategy: OrchestrationStrategy = Field(..., description="Strategy used")
    detected_domain: str = Field(..., description="Detected or specified domain")
    
    # Workflow results
    domain_analysis: Optional[Dict[str, Any]] = Field(None, description="Domain analysis results")
    extraction_config: Optional[Dict[str, Any]] = Field(None, description="Extraction configuration")
    extraction_results: Optional[Dict[str, Any]] = Field(None, description="Knowledge extraction results")
    
    # Search results
    vector_results: Optional[ModalityResult] = Field(None, description="Vector search results")
    graph_results: Optional[ModalityResult] = Field(None, description="Graph search results")
    gnn_results: Optional[ModalityResult] = Field(None, description="GNN search results")
    synthesized_results: List[Dict[str, Any]] = Field(default_factory=list, description="Synthesized results")
    synthesis_confidence: float = Field(0.0, description="Synthesis confidence score")
    
    # Unified output
    final_response: Dict[str, Any] = Field(..., description="Final synthesized response")
    ...
    stage_execution_times: Dict[str, float] = Field(default_factory=dict, description="Time per stage")
    ...
    cache_utilization: Dict[str, Any] = Field(default_factory=dict, description="Cache statistics")
...
class UnifiedOrchestrator:
    ...
    def __init__(self):
        ...
        self._performance_stats = {
            ...
            "stage_performance": {
                stage.value: {"count": 0, "avg_time": 0.0}
        ...
        logger.info("Unified orchestrator initialized with comprehensive workflow and search capabilities")
    ...
    async def execute_unified(
        ...
        try:
            ...
            if request.orchestration_strategy == OrchestrationStrategy.WORKFLOW_ONLY:
                results = await self._execute_workflow_only(request, azure_services, progress)
            elif request.orchestration_strategy == OrchestrationStrategy.SEARCH_ONLY:
                results = await self._execute_search_only(request, azure_services, progress)
            elif request.orchestration_strategy == OrchestrationStrategy.UNIFIED_WORKFLOW:
                results = await self._execute_unified_workflow(request, azure_services, progress)
            elif request.orchestration_strategy == OrchestrationStrategy.LIGHTWEIGHT:
                results = await self._execute_lightweight(request, azure_services, progress)
            elif request.orchestration_strategy == OrchestrationStrategy.COMPREHENSIVE:
                results = await self._execute_comprehensive(request, azure_services, progress)
            else:
    ...
    async def _execute_workflow_only(
        ...
        stage_start = time.time()
        extraction_config = await self._execute_config_generation(request, domain_analysis, azure_services)
        ...
        stage_start = time.time()
        extraction_results = await self._execute_knowledge_extraction(request, extraction_config, azure_services)
        ...
        stage_start = time.time()
        search_results = await self._execute_basic_search(request, domain_analysis, azure_services)
        ...
        return UnifiedResults(
            ...
            performance_metrics=self._calculate_performance_metrics(stage_times, azure_services),
    ...
    async def _execute_search_only(
        ...
        stage_start = time.time()
        domain = request.domain or await self._quick_domain_detection(request.query, azure_services)
        stage_times["domain_analysis"] = time.time() - stage_start
        ...
        stage_start = time.time()
        modality_results = await self._execute_parallel_search(request, search_strategy, azure_services)
        stage_times["search_execution"] = time.time() - stage_start
        ...
        stage_start = time.time()
        synthesized_results, synthesis_confidence, rankings = await self._synthesize_search_results(
            modality_results, search_strategy
        )
        stage_times["result_synthesis"] = time.time() - stage_start
        ...
        return UnifiedResults(
            ...
            total_execution_time=0.0,  # Will be set by caller
            stage_execution_times=stage_times,
            performance_metrics=self._calculate_search_performance_metrics(modality_results),
            processing_status=ProcessingStatus.COMPLETED,
            errors_encountered=[],
            cache_utilization=self._get_cache_utilization_stats()
        )
    ...
    async def _execute_unified_workflow(
        ...
        workflow_results = await self._execute_workflow_only(request, azure_services, progress)
        ...
        stage_start = time.time()
        synthesized_results, synthesis_confidence, rankings = await self._synthesize_unified_results(
        ...
        final_response = {
            ...
            "extraction_summary": self._summarize_extraction_results(workflow_results.extraction_results),
            "search_results": synthesized_results,
            "confidence_breakdown": {
                "domain_confidence": workflow_results.domain_analysis.get("confidence", 0.0),
                "extraction_confidence": workflow_results.extraction_results.get("extraction_accuracy", 0.0) if workflow_results.extraction_results else 0.0,
                "search_confidence": synthesis_confidence,
                "overall_confidence": (workflow_results.domain_analysis.get("confidence", 0.0) + synthesis_confidence) / 2
            },
            "performance_metrics": {
        ...
        return UnifiedResults(
            ...
            synthesis_confidence=synthesis_confidence,
            final_response=final_response,
            overall_confidence=final_response["confidence_breakdown"]["overall_confidence"],
            total_execution_time=0.0,  # Will be set by caller
    ...
    async def _execute_lightweight(
        ...
        stage_start = time.time()
        search_results = await self._execute_fast_search(request, domain, azure_services)
    ...
    async def _execute_comprehensive(
        ...
        unified_results = await self._execute_unified_workflow(request, azure_services, progress)
    ...
    async def _quick_domain_detection(
        ...
        technical_keywords = ['system', 'configuration', 'installation', 'setup', 'technical']
    ...
    async def _determine_search_strategy(
        ...
        has_relationships = any(word in request.query.lower() for word in ["and", "with", "between", "related"])
        ...
        return SearchStrategy(
            ...
            secondary_modalities=[m for m in ["vector", "graph", "gnn"] if m != primary_modality],
    ...
    async def _determine_enhanced_search_strategy(
        ...
        base_strategy = await self._determine_search_strategy(request, domain_analysis.get("detected_domain", "general"))
    ...
    async def _execute_parallel_search(
        ...
        for modality in request.search_modalities:
            if modality == "vector":
                search_tasks["vector"] = self._execute_vector_search(request, strategy, azure_services)
            elif modality == "graph":
                search_tasks["graph"] = self._execute_graph_search(request, strategy, azure_services)
            elif modality == "gnn":
                search_tasks["gnn"] = self._execute_gnn_search(request, strategy, azure_services)
        ...
        try:
            ...
            for modality, result in zip(search_tasks.keys(), results):
                if isinstance(result, Exception):
                    ...
                    modality_results[modality] = self._create_empty_modality_result(modality)
                else:
    ...
    async def _synthesize_search_results(
        ...
        for result_id, source_list in result_sources.items():
            if len(source_list) == 1:
                ...
                score = self._calculate_result_score(result, strategy) * 1.2  # Boost multi-source
            
            synthesized_results.append(result)
            rankings[result_id] = score
        
        # Sort by score
        synthesized_results.sort(key=lambda r: rankings.get(r.get("id", ""), 0), reverse=True)
        synthesized_results = synthesized_results[:strategy.max_results_per_modality * 2]
        
        # Calculate synthesis confidence
        synthesis_confidence = self._calculate_synthesis_confidence(modality_results, rankings)
    ...
    async def _synthesize_unified_results(
        ...
        search_results, search_confidence, rankings = await self._synthesize_search_results(
            modality_results, strategy
        )
        ...
        for result in search_results:
            enhanced_result = {
                **result,
                "workflow_context": {
                    "domain": workflow_results.detected_domain,
                    "extraction_available": workflow_results.extraction_results is not None,
        ...
        domain_confidence = workflow_results.domain_analysis.get("confidence", 0.5) if workflow_results.domain_analysis else 0.5
    ...
    async def _execute_result_synthesis(
        ...
        try:
            ...
            overall_confidence = (domain_analysis.get("confidence", 0.0) + confidence) / 2
    ...
    async def _execute_comprehensive_analysis(
        ...
        return {
            ...
            "confidence_analysis": self._analyze_confidence_distribution(unified_results),
            "performance_analysis": self._analyze_performance_characteristics(unified_results),
            "recommendation_analysis": self._generate_improvement_recommendations(unified_results)
        }

    # Utility and helper methods
    
    def _calculate_modality_confidence(self, results: List[Dict[str, Any]], modality: str) -> float:
    ...
    def _calculate_result_score(self, result: Dict[str, Any], strategy: SearchStrategy) -> float:
    ...
    def _merge_multi_source_result(self, source_list: List[Dict[str, Any]], strategy: SearchStrategy) -> Dict[str, Any]:
        ...
        avg_confidence = sum(r.get("modality_confidence", 0) for r in source_list) / len(source_list)
    ...
    def _calculate_synthesis_confidence(
        ...
        ranking_consistency = 1.0 - (max(ranking_scores) - min(ranking_scores)) if ranking_scores else 0.0
    ...
    def _calculate_search_performance_metrics(self, modality_results: Dict[str, ModalityResult]) -> Dict[str, Any]:
        """Calculate search-specific performance metrics"""
        
        total_modality_time = sum(r.execution_time for r in modality_results.values())
        max_time = max(r.execution_time for r in modality_results.values()) if modality_results else 0.0
        ...
        return {
            "search_performance": {
                ...
                "modalities_successful": len([r for r in modality_results.values() if r.result_count > 0])
            }
        }
    ...
    def _summarize_extraction_results(self, extraction_results: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "relationships_extracted": extraction_results.get("total_relationships_extracted", 0),
            "extraction_accuracy": extraction_results.get("extraction_accuracy", 0.0),
            "validation_passed": extraction_results.get("extraction_passed_validation", False)
        }
    ...
    def _analyze_confidence_distribution(self, results: UnifiedResults) -> Dict[str, float]:
        """Analyze confidence distribution in results"""
        confidences = [r.get("confidence", 0.0) for r in results.synthesized_results if "confidence" in r]
    ...
    def _analyze_performance_characteristics(self, results: UnifiedResults) -> Dict[str, Any]:
        """Analyze performance characteristics"""
        return {
            "total_time": results.total_execution_time,
            "stage_breakdown": results.stage_execution_times,
            "efficiency_score": 1.0 - (results.total_execution_time / 120.0),  # Against 2-minute target
            "sla_compliance": results.total_execution_time < 3.0
        }

    def _generate_improvement_recommendations(self, results: UnifiedResults) -> List[str]:
        """Generate recommendations for improvement"""
        recommendations = []
        
        if results.total_execution_time > 3.0:
            recommendations.append("Consider using LIGHTWEIGHT strategy for faster responses")
        
        if results.overall_confidence < 0.6:
            recommendations.append("Consider using COMPREHENSIVE strategy for higher quality")
        
        if len(results.synthesized_results) < 5:
            recommendations.append("Consider increasing max_results for more comprehensive results")
    ...
    def _calculate_quality_metrics(self, results: UnifiedResults) -> Dict[str, Any]:
        ...
        return {
            "completeness": len(results.synthesized_results) / max(1, 10),  # Against target of 10 results
            "confidence": results.overall_confidence,
            "diversity": len(self._analyze_source_diversity(results)),
            "performance": 1.0 - (results.total_execution_time / 120.0)  # Against 2-minute target
        }
    ...
    def _update_performance_stats(
        ...
        strategy_stats["avg_time"] = ((current_avg * (count - 1) + execution_time) / count)
    ...
    def _update_modality_stats(self, modality: str, execution_time: float, success: bool):
        ...
        if modality in self._performance_stats["search_modality_performance"]:
            ...
            stats["avg_time"] = ((current_avg * (executions - 1) + execution_time) / executions)
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._performance_stats,
            "success_rate": (
                self._performance_stats["successful_requests"] / self._performance_stats["total_requests"]
                if self._performance_stats["total_requests"] > 0 else 0.0
            ),

...
async def extract_entities_tool(
    ...
    try:
        ...
        if hasattr(ctx, 'deps') and hasattr(ctx.deps, 'app_settings'):
            confidence_threshold = ctx.deps.app_settings.tri_modal_search.competitive_advantage.confidence_threshold
        ...
        logger.info(f"PydanticAI Knowledge Extraction executing entity extraction: {len(text)} chars, strategy={extraction_strategy}")
...
async def extract_relationships_tool(
    ...
    try:
        ...
        logger.info(f"PydanticAI Knowledge Extraction executing relationship extraction: {len(text)} chars, strategy={extraction_strategy}")
        ...
        logger.error(f"PydanticAI Knowledge Extraction relationship extraction failed: {e}")
...
async def build_knowledge_graph_tool(
    ...
    try:
        ...
        graph_structure["metrics"] = {
            ...
            "average_node_confidence": sum(node["confidence"] for node in graph_structure["nodes"]) / max(1, len(graph_structure["nodes"])),
            "average_edge_confidence": sum(edge["confidence"] for edge in graph_structure["edges"]) / max(1, len(graph_structure["edges"])),
            "connectivity_ratio": len(graph_structure["edges"]) / max(1, len(graph_structure["nodes"])),
        ...
        logger.error(f"PydanticAI Knowledge Extraction knowledge graph construction failed: {e}")
        return {
            "nodes": [],
            "edges": [],
            "metadata": {
...
async def validate_extraction_quality_tool(
    ...
    try:
        ...
        entity_density = len(entities) / max(1, text_length / 100)  # Entities per 100 characters
        relationship_coverage = len(relationships) / max(1, len(entities))  # Relationships per entity
        ...
        avg_entity_confidence = sum(entity_confidences) / max(1, len(entity_confidences))
        avg_relationship_confidence = sum(relationship_confidences) / max(1, len(relationship_confidences))
        ...
        if entity_density < 0.5:
            quality_results["quality_indicators"].append("Low entity extraction density")
            quality_results["recommendations"].append("Consider providing more detailed text")
            score *= 0.8
        
        if relationship_coverage < 0.3:
            quality_results["quality_indicators"].append("Low relationship coverage")
            quality_results["recommendations"].append("Text may benefit from more explicit relationship descriptions")
            score *= 0.7
        
        if avg_entity_confidence < 0.7:
            quality_results["quality_indicators"].append("Low entity confidence")
            quality_results["recommendations"].append("Review text clarity for better entity extraction")
            score *= 0.8
        
        if avg_relationship_confidence < 0.7:
            quality_results["quality_indicators"].append("Low relationship confidence")
            quality_results["recommendations"].append("Consider more explicit relationship language")
            score *= 0.8

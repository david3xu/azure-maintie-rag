...
from services.interfaces.extraction_interface import (
    ExtractionConfiguration, ExtractionResults, ExtractionStrategy,
...
from .extraction_tools import (
    ...
    KnowledgeGraphBuilder, VectorEmbeddingGenerator
)
...
class KnowledgeExtractionAgent:
    ...
    async def get_agent(self) -> Agent:
        ...
        try:
            ...
            azure_endpoint = os.getenv('AZURE_OPENAI_ENDPOINT', 'https://maintie-rag-staging-oeeopj3ksgnlo.openai.azure.com/')
            ...
            if azure_endpoint and api_key:
                ...
                raise RuntimeError("Azure OpenAI configuration required - no mock services allowed")
                
        except ImportError as e:
            print(f"⚠️ Azure OpenAI import failed: {e}")
            raise RuntimeError(f"Knowledge Extraction Agent requires Azure OpenAI: {str(e)}") from e
        except Exception as e:
            print(f"❌ Failed to initialize Azure OpenAI agent: {e}")
            raise RuntimeError(f"Knowledge Extraction Agent initialization failed: {str(e)}") from e
    ...
    async def extract_knowledge_from_document(
        ...
        try:
            ...
            if config.enable_caching:
                cached_result = await self._get_cached_extraction(document_content, config)
                if cached_result:
            ...
            validated_result = await self._validate_extraction(extraction_result, config)
            ...
            await self.error_handler.handle_error(
                ...
                parameters={"document_id": document_id, "processing_time": processing_time}
            )
            raise ExtractionError(f"Knowledge extraction failed for {document_id}: {str(e)}") from e
        ...
        try:
            ...
            async def process_document(content: str, doc_id: str) -> ExtractedKnowledge:
                async with semaphore:
                    return await self.extract_knowledge_from_document(content, config, doc_id)
            
            # Create tasks for parallel processing
            tasks = [
                process_document(content, doc_id) 
            ...
            quality_metrics = await self._calculate_quality_metrics(successful_extractions, config)
            ...
            results = ExtractionResults(
                ...
                unique_entity_types_found=len(set(e.get('type', 'unknown') for extraction in successful_extractions for e in extraction.entities)),
                unique_relationship_types_found=len(set(r.get('type', 'unknown') for extraction in successful_extractions for r in extraction.relationships)),
                extraction_passed_validation=len(failed_extractions) == 0,
                validation_error_count=len(failed_extractions),
                validation_warnings=[w for extraction in successful_extractions for w in extraction.validation_warnings]
            )
            
            return results
            
        except asyncio.TimeoutError:
            raise ExtractionError(f"Extraction timeout after {config.extraction_timeout_seconds} seconds")
        except Exception as e:
            raise ExtractionError(f"Batch extraction failed: {str(e)}") from e

    async def _perform_extraction(self, content: str, config: ExtractionConfiguration) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            relationships = await self.relationship_extractor.extract_relationships(content, entities, config)
            
            # Step 3: Build knowledge graph using KnowledgeGraphBuilder tool
            knowledge_graph = await self.knowledge_graph_builder.build_knowledge_graph(entities, relationships, config)
            
            # Step 4: Generate vector embeddings using VectorEmbeddingGenerator tool
            embeddings = await self.vector_embedding_generator.generate_embeddings(entities, relationships, config)
            ...
            extraction_confidence = self._calculate_extraction_confidence(entities, relationships, config)
            
            return {
                "entities": [self._format_entity_for_response(entity) for entity in entities],
                "relationships": [self._format_relationship_for_response(rel) for rel in relationships],
    ...
    def _extract_key_concepts(self, entities: List[Dict[str, Any]], config: ExtractionConfiguration) -> List[str]:
    ...
    def _extract_technical_terms(self, entities: List[Dict[str, Any]], config: ExtractionConfiguration) -> List[str]:
        """Extract technical terms from entities"""
        technical_terms = []
        
        # Extract terms that are marked as technical or have technical characteristics
        for entity in entities:
            if entity.get('type') in ['technical_term', 'api_interface', 'system_component', 'code_element']:
                technical_terms.append(entity['name'])
            elif entity.get('confidence', 0.0) >= config.entity_confidence_threshold and len(entity['name']) > 3:
                # Include high-confidence entities that might be technical
                technical_terms.append(entity['name'])
        
        return list(set(technical_terms))[:30]
    
    def _calculate_extraction_confidence(self, entities: List[Dict[str, Any]], relationships: List[Dict[str, Any]], config: ExtractionConfiguration) -> float:
        ...
        overall_confidence = avg_entity_confidence * 0.7 + avg_relationship_confidence * 0.3
    ...
    def _format_relationship_for_response(self, relationship: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _minimal_extraction_fallback(self, content: str, config: ExtractionConfiguration) -> Dict[str, Any]:
    ...
    async def _validate_extraction(self, extraction: Dict[str, Any], config: ExtractionConfiguration) -> Dict[str, Any]:
        ...
        if extraction.get('confidence', 0.0) < config.minimum_quality_score:
            validation_warnings.append(f"Extraction confidence {extraction.get('confidence', 0.0)} below minimum {config.minimum_quality_score}")
        ...
        if 'min_entities_per_document' in criteria:
            ...
            if len(extraction.get('entities', [])) < min_entities:
                validation_warnings.append(f"Only {len(extraction.get('entities', []))} entities found, minimum {min_entities}")
        ...
        if 'min_relationships_per_document' in criteria:
            ...
            if len(extraction.get('relationships', [])) < min_relationships:
                validation_warnings.append(f"Only {len(extraction.get('relationships', []))} relationships found, minimum {min_relationships}")
    ...
    async def _calculate_quality_metrics(self, extractions: List[ExtractedKnowledge], config: ExtractionConfiguration) -> Dict[str, float]:
        ...
        validation_pass_rate = sum(1 for e in extractions if e.passed_validation) / len(extractions)
    ...
    async def _get_cached_extraction(self, content: str, config: ExtractionConfiguration) -> Optional[ExtractedKnowledge]:
    ...
    async def _cache_extraction(self, content: str, config: ExtractionConfiguration, knowledge: ExtractedKnowledge):
    ...
    try:
        ...
        test_config = ExtractionConfiguration(
            ...
            technical_vocabulary=[
                "hydraulic system", "pressure sensors", "fluid levels", 
                "hydraulic pump", "reservoir", "filter assembly",
                "pressure gauges", "maintenance procedures", "flight hours"
            ],
        ...
        print(f"✅ Extraction confidence: {extracted_knowledge.extraction_confidence:.3f}")
        ...
        if extracted_knowledge.entities:
            ...
            for i, entity in enumerate(extracted_knowledge.entities[:5]):
                print(f"  {i+1}. {entity.get('name', 'Unknown')} (type: {entity.get('type', 'unknown')}, confidence: {entity.get('confidence', 0.0):.2f})")
        ...
        if extracted_knowledge.relationships:
            ...
            for i, rel in enumerate(extracted_knowledge.relationships[:3]):
                print(f"  {i+1}. {rel.get('source', 'Unknown')} → {rel.get('relation', 'related')} → {rel.get('target', 'Unknown')}")
        
        print("\n🎯 Tool 2: Advanced Quality Assessment")
        
        # Calculate comprehensive quality metrics
        quality_metrics = {
            "entity_density": len(extracted_knowledge.entities) / max(1, len(test_document) / 100),
            "relationship_coverage": len(extracted_knowledge.relationships) / max(1, len(extracted_knowledge.entities)),
            "confidence_distribution": {
                "high_confidence_entities": len([e for e in extracted_knowledge.entities if e.get('confidence', 0) > 0.8]),
                "medium_confidence_entities": len([e for e in extracted_knowledge.entities if 0.6 <= e.get('confidence', 0) <= 0.8]),
                "low_confidence_entities": len([e for e in extracted_knowledge.entities if e.get('confidence', 0) < 0.6])
            },
            "technical_coverage": len([e for e in extracted_knowledge.entities if e.get('name', '') in test_config.technical_vocabulary]),
            "domain_alignment": len(extracted_knowledge.key_concepts) / max(1, len(test_config.key_concepts))
        }
        
        print(f"✅ Entity density: {quality_metrics['entity_density']:.2f} entities per 100 chars")
        print(f"✅ Relationship coverage: {quality_metrics['relationship_coverage']:.2f} rels per entity")
        print(f"✅ High confidence entities: {quality_metrics['confidence_distribution']['high_confidence_entities']}")
        print(f"✅ Technical vocabulary coverage: {quality_metrics['technical_coverage']} terms")
        ...
        print(f"✅ Azure OpenAI agent: {'Connected' if agent_available else 'Not available'}")
        ...
        print("\n📋 Summary:")
        print(f"  ✅ Multi-strategy entity extraction: {extracted_knowledge.entity_count} entities")
        print(f"  ✅ Advanced relationship extraction: {extracted_knowledge.relationship_count} relationships")
        print(f"  ✅ Knowledge graph construction: {len(knowledge_graph.get('nodes', []))} nodes")
        print(f"  ✅ Azure OpenAI integration: {'Connected' if agent_available else 'Fallback mode'}")
        print(f"  ✅ Performance compliance: {extraction_time:.3f}s processing time")
        print(f"  ✅ Quality validation: {extracted_knowledge.extraction_confidence:.3f} confidence")

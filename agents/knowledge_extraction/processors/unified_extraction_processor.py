...
class UnifiedExtractionResult(BaseModel):
    ...
    entity_confidence_distribution: Dict[str, int] = Field(..., description="Entity confidence distribution")
    ...
    average_entity_confidence: float = Field(..., description="Average entity confidence")
    
    # Relationship results
    relationships: List[Dict[str, Any]] = Field(..., description="Extracted relationships")
    relationship_confidence_distribution: Dict[str, int] = Field(..., description="Relationship confidence distribution")
    ...
    average_relationship_confidence: float = Field(..., description="Average relationship confidence")
...
class UnifiedExtractionProcessor:
    ...
    def _init_components(self):
        ...
        self._performance_stats = {
            ...
            "method_performance": {
                "pattern_based": {"count": 0, "avg_time": 0.0, "avg_entities": 0.0, "avg_relationships": 0.0},
                "nlp_based": {"count": 0, "avg_time": 0.0, "avg_entities": 0.0, "avg_relationships": 0.0},
    ...
    async def extract_knowledge_complete(
        ...
        try:
            ...
            entities = await self._extract_entities_unified(content, config, extraction_method)
            
            # Phase 2: Relationship extraction (consolidated from RelationshipProcessor)
            relationships = await self._extract_relationships_unified(content, entities, config, extraction_method)
            
            # Phase 3: Cross-validation and enhancement
            validated_result = await self._validate_and_enhance(entities, relationships, config, content)
            
            processing_time = time.time() - start_time
            
            # Create unified result
            result = self._create_unified_result(
                entities, relationships, validated_result, 
                extraction_method, processing_time, content
            )
            
            # Update performance statistics
            self._update_performance_stats(
                extraction_method, processing_time, 
                len(entities), len(relationships), True
            )
            ...
            self._update_performance_stats(extraction_method, processing_time, 0, 0, False)
    ...
    async def _extract_entities_hybrid(
        ...
        enhanced_entities = self._enhance_multi_method_confidence(deduplicated_entities, "entity")
    ...
    async def _extract_relationships_unified(
        ...
        if extraction_method == "pattern_based":
            return await self._extract_relationships_pattern_based(content, entities, config)
        elif extraction_method == "nlp_based":
        ...
        for entity1_match in entities:
            for entity2_match in entities:
                ...
                for pattern_template in syntactic_patterns:
                    ...
                    for match in matches:
                        relation_type = self._determine_relation_type(match, pattern_template)
                    ...
                    for relation_type, indicators in semantic_indicators.items():
                        for indicator in indicators:
                            if indicator in sentence.lower():
                                confidence = self._calculate_relationship_confidence(
                                    None, sentence, entity1, entity2, "semantic", indicator
                                )
    ...
    def _calculate_entity_confidence(
        ...
        if method == "pattern_based":
            return self._calculate_pattern_entity_confidence(match, content, entity_type)
        else:  # nlp_based
    ...
    def _calculate_pattern_entity_confidence(
        ...
        position_factor = 0.8 if match.start() < len(content) / 4 else 0.6  # Early vs late position
    ...
    def _calculate_relationship_confidence(
        ...
        if method == "pattern_based":
            return self._calculate_syntactic_confidence(match, content, entity1, entity2)
        else:  # semantic
            return self._calculate_semantic_confidence(content, entity1, entity2, indicator)
    ...
    def _calculate_syntactic_confidence(
        ...
        distance_factor = max(
            ...
            self.relationship_config.max_distance_factor - (distance / self.relationship_config.distance_divisor)
        )
    ...
    def _calculate_semantic_confidence(
        ...
        length_factor = max(
            ...
            self.relationship_config.max_distance_factor - (
                len(sentence.split()) / self.relationship_config.max_sentence_length_divisor
            )
        )
        ...
        prominence_factor = min(
            self.relationship_config.max_distance_factor,
            (entity1_count + entity2_count) / self.relationship_config.max_prominence_divisor
        )
    ...
    def _create_unified_result(
        ...
        return UnifiedExtractionResult(
            ...
            entity_confidence_distribution=self._calculate_entity_confidence_distribution(entities),
            entity_type_counts=self._calculate_entity_type_counts(entities),
            total_entities=len(entities),
            high_confidence_entities=len([
                e for e in entities if e.confidence > self.entity_config.high_confidence_threshold
            ]),
            average_entity_confidence=sum(e.confidence for e in entities) / len(entities) if entities else 0.0,
            
            # Relationship results
            relationships=[self._relationship_to_dict(r) for r in relationships],
            relationship_confidence_distribution=self._calculate_relationship_confidence_distribution(relationships),
            relation_type_counts=self._calculate_relation_type_counts(relationships),
            total_relationships=len(relationships),
            high_confidence_relationships=len([
                r for r in relationships if r.confidence > self.relationship_config.high_confidence_threshold
            ]),
            average_relationship_confidence=sum(r.confidence for r in relationships) / len(relationships) if relationships else 0.0,
            ...
            # Graph metrics
            unique_entity_pairs=len(set((r.source_entity, r.target_entity) for r in relationships)),
    ...
    def _calculate_entity_confidence_distribution(self, entities: List[EntityMatch]) -> Dict[str, int]:
    ...
    def _calculate_relationship_confidence_distribution(self, relationships: List[RelationshipMatch]) -> Dict[str, int]:
    ...
    def _calculate_entity_type_counts(self, entities: List[EntityMatch]) -> Dict[str, int]:
    ...
    def _calculate_relation_type_counts(self, relationships: List[RelationshipMatch]) -> Dict[str, int]:
    ...
    def _calculate_coverage_percentage(self, entities: List[EntityMatch], content: str) -> float:

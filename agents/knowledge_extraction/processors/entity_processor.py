...
class EntityProcessor:
    ...
    async def extract_entities(
        ...
        try:
            ...
            # Validate results
            validation_result = self._validate_extraction_results(filtered_entities, config)
            ...
            result = EntityExtractionResult(
                ...
                confidence_distribution=self._calculate_confidence_distribution(filtered_entities),
                entity_type_counts=self._calculate_type_counts(filtered_entities),
                processing_time=processing_time,
                total_entities=len(filtered_entities),
                high_confidence_entities=len([e for e in filtered_entities if e.confidence > 0.8]),
                average_confidence=sum(e.confidence for e in filtered_entities) / len(filtered_entities) if filtered_entities else 0.0,
                coverage_percentage=self._calculate_coverage_percentage(filtered_entities, content),
                validation_passed=validation_result["passed"],
                validation_warnings=validation_result["warnings"]
            )
            
            # Update performance statistics
            self._update_performance_stats(extraction_method, processing_time, len(filtered_entities), True)
    ...
    async def _extract_entities_pattern_based(
        ...
        patterns = self._get_entity_patterns(config.expected_entity_types, config.technical_vocabulary)
        ...
        for entity_type, pattern_list in patterns.items():
            for pattern in pattern_list:
                ...
                for match in matches:
                    ...
                    confidence = self._calculate_pattern_confidence(match, content, entity_type)
                    ...
                    if confidence >= config.entity_confidence_threshold:
                        entities.append(EntityMatch(
                            ...
                            metadata={"pattern": pattern.pattern, "match_groups": match.groups()}
                        ))
    ...
    def _get_entity_patterns(
        ...
        pattern_templates = {
            ...
            "technical_term": [
                rf'\b{re.escape(term)}\b' for term in technical_vocabulary[:50]  # Limit for performance
            ],
            "api_interface": [
    ...
    def _calculate_pattern_confidence(
        ...
        position_factor = 0.8 if match.start() < len(content) / 4 else 0.6  # Early matches get boost
    ...
    def _enhance_multi_method_confidence(self, entities: List[EntityMatch]) -> List[EntityMatch]:
        ...
        for text_key, entity_list in text_method_map.items():
            if len(entity_list) > 1:
                ...
                best_entity.metadata["methods_used"] = [e.extraction_method for e in entity_list]
    ...
    def _validate_extraction_results(
        ...
        if len(entities) < min_entities:
            warnings.append(f"Only {len(entities)} entities found, minimum {min_entities} expected")
            passed = False
        ...
        if entities:
            ...
            if avg_confidence < config.entity_confidence_threshold:
                warnings.append(f"Average confidence {avg_confidence:.2f} below threshold {config.entity_confidence_threshold}")
        ...
        if len(missing_types) > len(expected_types) / 2:
            warnings.append(f"Many expected entity types not found: {list(missing_types)[:5]}")
    ...
    def _calculate_confidence_distribution(self, entities: List[EntityMatch]) -> Dict[str, int]:
    ...
    def _update_performance_stats(
        ...
        if method in self._performance_stats["method_performance"]:
            ...
            method_stats["avg_entities"] = ((current_avg_entities * (count - 1) + entity_count) / count)

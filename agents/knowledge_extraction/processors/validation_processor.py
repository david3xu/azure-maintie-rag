...
# Interface contracts
from config.extraction_interface import ConfigurationFeedback, ExtractionConfiguration
...
class ValidationReport(BaseModel):
    ...
    relationship_quality_score: float = Field(
        ..., description="Relationship extraction quality"
    )
    ...
    # Recommendations
    configuration_feedback: Dict[str, Any] = Field(
        ..., description="Configuration optimization feedback"
    )
...
class ValidationProcessor:
    ...
    async def validate_extraction_results(
        ...
        try:
            ...
            # Validate entities
            entity_issues, entity_score = await self._validate_entities(
                entities, config
            )
            ...
            (
                relationship_issues,
                relationship_score,
            ) = await self._validate_relationships(relationships, entities, config)
            ...
            self._update_validation_stats(
                processing_time, overall_score, validation_passed, issues
            )
    ...
    async def _validate_entities(
        ...
        if entities:
            ...
            duplicates = [
                text for text, count in Counter(entity_texts).items() if count > 1
            ]
                ...
                issues.append(
                    ValidationIssue(
                        ...
                        suggestions=[
                            ...
                            "Improve relationship classification",
                        ],
                    )
                )

        # Calculate relationship quality score
        relationship_score = self._calculate_relationship_quality_score(
            relationships, config, issues
        )
    ...
    async def _validate_knowledge_graph(
        ...
        if not entities or not relationships:
            issues.append(
                ValidationIssue(
                    ...
                    confidence_impact=0.2,
                    suggestions=[
                        "Ensure both entity and relationship extraction are working"
                    ],
                )
            )
            return issues, 0.3
        ...
        connectivity_ratio = (
            len(connected_entities) / len(entity_names) if entity_names else 0
        )
        ...
        graph_score = self._calculate_graph_quality_score(
    ...
    async def _validate_consistency(
        ...
        if entities:
            ...
            if entity_confidences and relationship_confidences:
                ...
                relationship_avg = sum(relationship_confidences) / len(
                    relationship_confidences
                )
    ...
    async def _validate_performance(
        self, performance_metrics: Dict[str, Any], config: ExtractionConfiguration
    ) -> Tuple[List[ValidationIssue], float]:
        ...
        performance_score = self._calculate_performance_score(
            performance_metrics, config, issues
        )
    ...
    def _calculate_entity_quality_score(
        ...
        found_types = set(e.get("type", "") for e in entities)
        coverage_factor = (
            len(found_types & expected_types) / len(expected_types)
            if expected_types
            else 1.0
        )
        ...
        entity_issues = [
            i
            for i in issues
            if i.category == "entity" and i.severity in ["error", "warning"]
        ]
    ...
    def _calculate_relationship_quality_score(
        ...
        rel_issues = [
            i
            for i in issues
            if i.category == "relationship" and i.severity in ["error", "warning"]
        ]
    ...
    def _calculate_graph_quality_score(
        ...
        graph_issues = [
            i
            for i in issues
            if i.category == "graph" and i.severity in ["error", "warning"]
        ]
    ...
    def _calculate_consistency_score(
        ...
        error_penalty = (
            len([i for i in consistency_issues if i.severity == "error"]) * 0.3
        )
        warning_penalty = (
            len([i for i in consistency_issues if i.severity == "warning"]) * 0.1
        )
    ...
    def _calculate_performance_score(
        ...
        perf_issues = [
            i
            for i in issues
            if i.category == "performance" and i.severity in ["error", "warning"]
        ]
    ...
    def _generate_configuration_feedback(
        ...
        if relationships:
            ...
            if avg_confidence < config.relationship_confidence_threshold + 0.1:
                feedback["relationship_threshold_adjustment"] = {
                    ...
                    "suggested": max(
                        0.1, config.relationship_confidence_threshold - 0.1
                    ),
        ...
        for issue in issues:
            if "missing" in issue.message.lower() and issue.category == "entity":
                ...
                "missing" in issue.message.lower() and issue.category == "relationship"
            ):
                feedback["pattern_additions"].append(
                    {
    ...
    def _generate_improvement_suggestions(
        self, issues: List[ValidationIssue], config: ExtractionConfiguration
    ) -> List[str]:
    ...
    def _calculate_confidence_metrics(
        ...
        if entities:
            ...
            metrics["entity_avg_confidence"] = sum(entity_confidences) / len(
                entity_confidences
            )
        ...
        if relationships:
            ...
            metrics["relationship_avg_confidence"] = sum(rel_confidences) / len(
                rel_confidences
            )
    ...
    def _calculate_coverage_metrics(
        ...
        if expected_types:
            metrics["entity_type_coverage"] = len(found_types & expected_types) / len(
                expected_types
            )
        else:
        ...
        if hasattr(config, "relationship_patterns") and config.relationship_patterns:
            pattern_relations = [
                p.split()[1]
                for p in config.relationship_patterns
                if len(p.split()) >= 3
            ]
            found_relations = set(r.get("relation", "") for r in relationships)

            if pattern_relations:
                metrics["relationship_pattern_coverage"] = len(
                    set(pattern_relations) & found_relations
                ) / len(pattern_relations)
            else:
    ...
    def _calculate_performance_metrics(
        ...
        return {
            ...
            "relationships_per_second": performance_metrics.get(
                "relationships_per_second", 0.0
            ),
    ...
    def _update_validation_stats(
        ...
        for issue in issues:
            if issue.category in self._validation_stats["issue_frequency"]:
                self._validation_stats["issue_frequency"][issue.category][
                    issue.severity
                ] += 1
...
__all__ = ["ValidationProcessor", "ValidationIssue", "ValidationReport"]

...
class RelationshipMatch:
    ...
    relation_direction: str = "bidirectional"  # "source_to_target", "target_to_source", "bidirectional"
...
class RelationshipExtractionResult(BaseModel):
    ...
    high_confidence_relationships: int = Field(..., description="High confidence relationships")
...
class RelationshipProcessor:
    ...
    def __init__(self):
        ...
        self._performance_stats = {
            ...
            "method_performance": {
                ...
                "pattern_based": {"count": 0, "avg_time": 0.0, "avg_relationships": 0.0},
    ...
    async def extract_relationships(
        ...
        try:
            ...
            if extraction_method == "syntactic":
                relationships = await self._extract_relationships_syntactic(content, entities, config)
            elif extraction_method == "semantic":
                relationships = await self._extract_relationships_semantic(content, entities, config)
            elif extraction_method == "pattern_based":
                relationships = await self._extract_relationships_pattern_based(content, entities, config)
            else:  # hybrid
                relationships = await self._extract_relationships_hybrid(content, entities, config)
            ...
            # Validate results
            validation_result = self._validate_relationship_results(filtered_relationships, config)
            
            # Calculate graph metrics
            graph_metrics = self._calculate_graph_metrics(filtered_relationships, entities)
            
            processing_time = time.time() - start_time
            
            # Create result
            result = RelationshipExtractionResult(
                relationships=[self._relationship_to_dict(r) for r in filtered_relationships],
                extraction_method=extraction_method,
                confidence_distribution=self._calculate_confidence_distribution(filtered_relationships),
                relation_type_counts=self._calculate_type_counts(filtered_relationships),
                processing_time=processing_time,
                total_relationships=len(filtered_relationships),
                high_confidence_relationships=len([r for r in filtered_relationships if r.confidence > 0.8]),
                unique_entity_pairs=len(set((r.source_entity, r.target_entity) for r in filtered_relationships)),
                average_confidence=sum(r.confidence for r in filtered_relationships) / len(filtered_relationships) if filtered_relationships else 0.0,
            ...
            self._update_performance_stats(extraction_method, processing_time, len(filtered_relationships), True)
            ...
            for entity2 in entity_texts:
                ...
                for pattern_template in syntactic_patterns:
                    ...
                    for match in matches:
                        relation_type = self._determine_syntactic_relation_type(match, pattern_template)
                        confidence = self._calculate_syntactic_confidence(match, content, entity1, entity2)
                        ...
                        if confidence >= config.relationship_confidence_threshold:
                            relationships.append(RelationshipMatch(
                                ...
                                context=self._extract_relationship_context(content, match.start(), match.end()),
                                metadata={"pattern": pattern, "match_text": match.group()}
                            ))
    ...
    async def _extract_relationships_semantic(
        ...
        for entity1 in entity_texts:
            for entity2 in entity_texts:
                ...
                sentences = self._find_sentences_with_entities(content, entity1, entity2)
                            ...
                            if indicator in sentence.lower():
                                ...
                                if confidence >= config.relationship_confidence_threshold:
                                    ...
                                    relationships.append(RelationshipMatch(
                                        ...
                                        context=sentence,
                                        metadata={"indicator": indicator, "sentence": sentence}
                                    ))
            ...
            if len(pattern_parts) >= 3:
                ...
                for match in matches:
                    ...
                    if len(groups) >= 2:
                        ...
                        if any(e1.lower() in entity1.lower() for e1 in entity_texts) and \
                           any(e2.lower() in entity2.lower() for e2 in entity_texts):
                            
                            confidence = self._calculate_pattern_confidence(match, content, pattern_str)
                            ...
                            if confidence >= config.relationship_confidence_threshold:
                                relationships.append(RelationshipMatch(
                                    ...
                                    end_position=match.end(),
                                    context=self._extract_relationship_context(content, match.start(), match.end()),
                                    metadata={"pattern": pattern_str, "match_text": match.group()}
                                ))
    ...
    async def _extract_relationships_hybrid(
        ...
        syntactic_relationships = await self._extract_relationships_syntactic(content, entities, config)
        semantic_relationships = await self._extract_relationships_semantic(content, entities, config)
        pattern_relationships = await self._extract_relationships_pattern_based(content, entities, config)
        
        # Combine all relationships
        all_relationships = syntactic_relationships + semantic_relationships + pattern_relationships
        
        # Deduplicate and enhance confidence
        deduplicated_relationships = self._deduplicate_relationships(all_relationships)
        enhanced_relationships = self._enhance_multi_method_confidence(deduplicated_relationships)
        
        return enhanced_relationships
    
    def _find_entity_positions(self, content: str, entity_texts: List[str]) -> Dict[str, List[Tuple[int, int]]]:
    ...
    def _determine_syntactic_relation_type(self, match: re.Match, pattern_template: str) -> str:
    ...
    def _calculate_syntactic_confidence(
        ...
        distance = abs(entity2_pos - entity1_pos) if entity1_pos != -1 and entity2_pos != -1 else len(match_text)
        distance_factor = max(0.3, 1.0 - (distance / 100))
        
        # Context quality factor
        context = self._extract_relationship_context(content, match.start(), match.end(), window=100)
        context_factor = 0.7
        
        # Check for relationship-supporting words in context
        support_words = ["relationship", "connection", "interaction", "dependency", "association"]
        if any(word in context.lower() for word in support_words):
            context_factor = 0.9
        
        confidence = base_confidence * 0.4 + distance_factor * 0.3 + context_factor * 0.3
        
        return min(1.0, confidence)
    
    def _find_sentences_with_entities(self, content: str, entity1: str, entity2: str) -> List[str]:
        """Find sentences containing both entities"""
    ...
    def _calculate_semantic_confidence(
        ...
        confidence = base_confidence * 0.5 + length_factor * 0.3 + prominence_factor * 0.2
    ...
    def _create_relationship_regex(self, pattern_str: str) -> re.Pattern:
        ...
        if len(parts) >= 3:
            ...
            pattern = r'(\w+(?:\s+\w+)*)\s+' + re.escape(parts[1]) + r'\s+(\w+(?:\s+\w+)*)'
        else:
            # Fallback pattern
            pattern = r'(\w+)\s+(\w+)\s+(\w+)'
    ...
    def _calculate_pattern_confidence(
        ...
        confidence = base_confidence * 0.6 + specificity_factor * 0.2 + context_factor * 0.2
    ...
    def _deduplicate_relationships(self, relationships: List[RelationshipMatch]) -> List[RelationshipMatch]:
        ...
        for relationship in relationships:
            ...
            if key not in relationship_map or relationship.confidence > relationship_map[key].confidence:
                relationship_map[key] = relationship
        
        return list(relationship_map.values())
    
    def _enhance_multi_method_confidence(self, relationships: List[RelationshipMatch]) -> List[RelationshipMatch]:
        ...
        for triple, relationship_list in triple_method_map.items():
            if len(relationship_list) > 1:
                ...
                best_relationship.confidence = min(1.0, best_relationship.confidence * 1.3)
                best_relationship.extraction_method = "hybrid_multi_method"
                best_relationship.metadata["multi_method_count"] = len(relationship_list)
                best_relationship.metadata["methods_used"] = [r.extraction_method for r in relationship_list]
    ...
    def _validate_relationship_results(
        ...
        if len(relationships) < min_relationships:
            warnings.append(f"Only {len(relationships)} relationships found, minimum {min_relationships} expected")
            passed = False
        ...
        if relationships:
            ...
            if avg_confidence < config.relationship_confidence_threshold:
                warnings.append(f"Average confidence {avg_confidence:.2f} below threshold {config.relationship_confidence_threshold}")
    ...
    def _calculate_graph_metrics(
        ...
        for relationship in relationships:
            ...
            adjacency[target].add(source)  # Treat as undirected for component calculation
    ...
    def _calculate_confidence_distribution(self, relationships: List[RelationshipMatch]) -> Dict[str, int]:
    ...
    def _calculate_type_counts(self, relationships: List[RelationshipMatch]) -> Dict[str, int]:
        """Calculate count by relation type"""
        
        type_counts = {}
        for relationship in relationships:
            type_counts[relationship.relation_type] = type_counts.get(relationship.relation_type, 0) + 1
    ...
    def _update_performance_stats(
        ...
        if method in self._performance_stats["method_performance"]:
            ...
            method_stats["avg_time"] = ((current_avg_time * (count - 1) + processing_time) / count)
            
            # Update average relationship count
            current_avg_relationships = method_stats["avg_relationships"]
            method_stats["avg_relationships"] = ((current_avg_relationships * (count - 1) + relationship_count) / count)
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._performance_stats,
            "success_rate": (
                self._performance_stats["successful_extractions"] / self._performance_stats["total_extractions"]
                if self._performance_stats["total_extractions"] > 0 else 0.0
            )
        }

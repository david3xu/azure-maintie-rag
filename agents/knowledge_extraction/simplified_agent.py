...
def create_extraction_agent() -> Agent[ExtractionDeps, ExtractionResult]:
    """Create knowledge extraction agent with PydanticAI best practices"""
    global _agent
    if _agent is None:
        _agent = _create_extraction_agent()
    return _agent

# Simple factory function  
def get_extraction_agent() -> Agent[ExtractionDeps, ExtractionResult]:
    """Get knowledge extraction agent (no global state)"""
...
async def extract_knowledge(
    ...
    """Simple knowledge extraction function"""
    ...
    result = await agent.run(
        f"Extract knowledge from this text: {text}",
        deps=deps
    )
    
    processing_time = time.time() - start_time
    
    # The agent will return ExtractionResult directly
    return result.data

...
class RelationshipExtractionRequest(BaseModel):
    ...
    content: str = Field(..., min_length=1, description="Document content to process")
    entities: List[Dict[str, Any]] = Field(
        ..., description="Previously extracted entities"
    )
    config: Dict[str, Any] = Field(
        ..., description="Extraction configuration parameters"
    )
...
class KnowledgeGraphResponse(BaseModel):
    ...
    knowledge_graph: Dict[str, Any] = Field(
        ..., description="Built knowledge graph structure"
    )
...
def _dict_to_extraction_config(config_dict: Dict[str, Any]) -> ExtractionConfiguration:
    ...
    return ExtractionConfiguration(
        ...
        entity_extraction_focus=config_dict.get(
            "entity_extraction_focus", "mixed_content"
        ),
        ...
        relationship_confidence_threshold=config_dict.get(
        ...
        target_response_time_seconds=config_dict.get(
...
class EntityExtractor:
    ...
    async def _extract_entities_by_type(
        self, content: str, entity_type: str, config: ExtractionConfiguration
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_pattern_confidence(
        self, match: re.Match, content: str, config: ExtractionConfiguration
    ) -> float:
...
class RelationshipExtractor:
    ...
    async def extract_relationships(
        ...
        try:
            ...
            proximity_relationships = await self._extract_proximity_relationships(
                content, entities, config
            )
            relationships.extend(proximity_relationships)

            # Extract using linguistic patterns
            linguistic_relationships = await self._extract_linguistic_relationships(
                content, entities, config
            )
            ...
            filtered_relationships = [
                ...
                if rel.get("confidence", 0.0)
                >= config.relationship_confidence_threshold
            ]

            limited_relationships = filtered_relationships[
    ...
    async def _extract_pattern_relationships(
        ...
        for pattern in config.relationship_patterns:
            if " -> " in pattern:
                ...
                parts = pattern.split(" -> ")
                if len(parts) >= 3:
                    source_pattern, relation, target_pattern = (
                        parts[0],
                        parts[1],
                        parts[2],
                    )
    ...
    async def _check_pattern_match(
        self, content: str, source: str, relation: str, target: str
        ...
        if relation_lower in between_text:
            ...
            return {"confidence": confidence, "context": context.strip()}
    ...
    async def _extract_proximity_relationships(
        ...
        positioned_entities = [entity for entity in entities if "position" in entity]
...
class VectorEmbeddingGenerator:
    ...
    async def generate_embeddings(
        ...
        try:
            ...
            embeddings = {
                "entity_embeddings": await self._generate_entity_embeddings(
                    entities, config
                ),
                "relationship_embeddings": await self._generate_relationship_embeddings(
            ...
            raise ExtractionError(
                f"Vector embedding generation failed: {str(e)}"
            ) from e
...
async def test_extraction_tools():
    ...
    try:
        ...
        print(
            f"✅ Tool co-location: {entity_result.metadata.get('tool_colocation_complete', False)}"
        )
    except Exception as e:
    ...
    if "entity_result" in locals() and entity_result.entities:
        ...
        try:
            relationship_result = await execute_relationship_extraction(
                None, relationship_request
            )
            print(
                f"✅ Relationship extraction: {len(relationship_result.relationships)} relationships extracted"
            )
            print(
                f"✅ Extraction methods: {relationship_result.metadata.get('extraction_methods', [])}"
            )
        except Exception as e:
        ...
        if "relationship_result" in locals():
            ...
            try:
                ...
                print(
                    f"✅ Knowledge graph: {stats.get('node_count', 0)} nodes, {stats.get('edge_count', 0)} edges"
                )

    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sla_violations": sum(
                ...
                if t > self.performance_targets["total_response_time"]
            ),
...
class SearchRequest(BaseModel):
    ...
    domain: Optional[str] = Field(
        default=None, description="Domain context for optimization"
    )
...
class IntelligenceRequest(BaseModel):
    ...
    domain: Optional[str] = Field(
        default=None, description="Domain context for analysis"
    )
...
class ConsolidatedToolManager:
    ...
    async def _execute_tri_modal_search(
        self, query: str, domain: str = None, **kwargs
    ) -> Dict[str, Any]:
        """Execute tri-modal search using the search tools module"""
        try:
            from .pydantic_tools import TriModalSearchRequest, execute_tri_modal_search

            # Create request
            request = TriModalSearchRequest(
                query=query, domain=domain, max_results=kwargs.get("max_results", 10)
            )
    ...
    async def _execute_vector_search(self, query: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            from .pydantic_tools import VectorSearchRequest, execute_vector_search

            request = VectorSearchRequest(
                query=query, max_results=kwargs.get("max_results", 10)
            )
    ...
    async def _execute_graph_search(self, query: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            from .pydantic_tools import GraphSearchRequest, execute_graph_search

            request = GraphSearchRequest(
                query=query, max_depth=kwargs.get("max_depth", 3)
            )
        ...
        try:
            ...
            if config.get("cache_results", True):
                cached_result = await self.cache_manager.get(
                    "tool_chain", {"query": query, "domain": domain, "config": config}
                )
                if cached_result:
            ...
            if config.get("parallel_execution", True):
                result = await self._execute_parallel_chain(
                    query, domain, tool_executor, config
                )
            else:
                result = await self._execute_sequential_chain(
                    query, domain, tool_executor, config
                )
    ...
    async def _execute_parallel_chain(
        self, query: str, domain: str, tool_executor: Callable, config: Dict[str, Any]
    ) -> ToolExecutionResult:
        ...
        search_tasks.append(
            self._execute_with_retry(
                lambda: tool_executor(
                    "vector_search", {"query": query, "domain": domain}
                ),
        ...
        search_tasks.append(
            self._execute_with_retry(
                lambda: tool_executor(
                    "graph_search", {"query": query, "domain": domain}
                ),
                config.get("max_retries", 2),
            )
        )
            ...
            for i, result in enumerate(search_results):
                if isinstance(result, Exception):
                    processed_results["errors"].append(
                        f"Search {i+1} failed: {str(result)}"
                    )
                else:
            ...
            processed_results.update(
                {
                    ...
                    "modalities_used": len(
                        [r for r in search_results if not isinstance(r, Exception)]
                    ),
                }
            )
        ...
        for operation, result_key in search_operations:
            try:
                result = await self._execute_with_retry(
                    lambda op=operation: tool_executor(
                        op, {"query": query, "domain": domain}
                    ),
                    config.get("max_retries", 2),
                )
                results[result_key] = result
            except Exception as e:
                results["errors"].append({"operation": operation, "error": str(e)})

        # Calculate success metrics
        successful_operations = sum(
            1
            for key in ["vector_results", "graph_results", "gnn_results"]
            if results[key]
        )
    ...
    async def _execute_with_retry(
        self, operation: Callable, max_retries: int = 2
    ) -> Any:
    ...
    async def _get_performance_metrics(self, **kwargs) -> Dict[str, Any]:
        ...
        return {
            "average_execution_time": self.execution_stats.get(
                "avg_execution_time", 0.0
            ),
            "success_rate": (
...
async def execute_tri_modal_search(
    ctx: RunContext[ConsolidatedAzureServices], request: SearchRequest
) -> ToolResponse:
    """
    ...
    try:
        ...
        result = await tool_manager._execute_tri_modal_search(
            query=request.query, domain=request.domain, max_results=request.max_results
        )
        ...
        ) and result.get("confidence", 0.0) >= request.performance_requirements.get(
            "min_confidence", 0.7
        )
...
async def execute_domain_intelligence(
    ctx: RunContext[ConsolidatedAzureServices], request: IntelligenceRequest
) -> ToolResponse:
    """
...
async def execute_search_chain(
    query: str, domain: str, tool_executor: Callable
) -> ToolExecutionResult:
...
async def test_consolidated_tools():
    ...
    search_request = SearchRequest(
        query="test consolidated search functionality", domain="technical"
    )

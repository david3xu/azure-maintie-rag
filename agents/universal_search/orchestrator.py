...
class TriModalOrchestrator:
    ...
    async def search(
        ...
        try:
            ...
            if "vector" in search_types:
                search_tasks.append(
                    self._execute_with_timeout(
                        self.vector_engine.execute_search(query, context),
                        "vector"
                    )
                )
            ...
            if "graph" in search_types:
                search_tasks.append(
                    self._execute_with_timeout(
                        self.graph_engine.execute_search(query, context),
                        "graph"
                    )
                )
            ...
            if "gnn" in search_types:
                search_tasks.append(
                    self._execute_with_timeout(
                        self.gnn_engine.execute_search(query, context),
                        "gnn"
                    )
                )
            ...
            for i, result in enumerate(modality_results):
                modality_name = modality_names[i] if i < len(modality_names) else f"unknown_{i}"
                
                if isinstance(result, Exception):
                    execution_errors.append(f"{modality_name} search failed: {str(result)}")
                    logger.warning(f"{modality_name} search failed: {result}")
                else:
                    processed_results[modality_name] = result
                    self._update_modality_stats(modality_name, result.execution_time, True)
            ...
            execution_time = time.time() - start_time
            logger.error(f"Tri-modal search failed: {e}", extra={'correlation_id': correlation_id})
    ...
    async def _execute_with_timeout(self, search_task, modality_name: str):
        ...
        try:
            ...
            raise TimeoutError(f"{modality_name} search exceeded {self.timeout}s timeout")
    ...
    def _synthesize_results(
        ...
        modality_breakdown['synthesis'] = {
            ...
            'tri_modal_bonus': tri_modal_bonus if num_successful_modalities > 0 else 0.0,
            'base_confidence': base_confidence if num_successful_modalities > 0 else 0.0,
    ...
    def _update_execution_stats(self, execution_time: float, success: bool):
        ...
        self.execution_stats['average_response_time'] = ((avg * (total - 1)) + execution_time) / total
        ...
        for name, engine in [
            ...
            try:
                ...
                await asyncio.wait_for(
                    engine.execute_search(test_query, test_context),
                    timeout=1.0
                )

...
class GraphSearchConfig:
    ...
    def __init__(self, domain: str = "general"):
        ...
        self.min_relationship_strength = (
            UniversalSearchConstants.DEFAULT_RELATIONSHIP_THRESHOLD
        )
...
class GNNSearchConfig:
    ...
    def __init__(self, domain: str = "general"):
        # Load from real domain analysis and centralized configuration
        self.default_node_embeddings = (
            UniversalSearchConstants.DEFAULT_GNN_NODE_EMBEDDINGS
        )
        self.min_prediction_confidence = (
            UniversalSearchConstants.DEFAULT_PATTERN_THRESHOLD
        )
...
class ConsolidatedSearchOrchestrator:
    ...
    async def execute_tri_modal_search(
        ...
        try:
            ...
            if "vector" in search_types:
                search_tasks.append(
                    self._execute_vector_search(query, domain, max_results)
                )
                modalities_to_execute.append("vector")

            if "graph" in search_types:
                search_tasks.append(
                    self._execute_graph_search(query, domain, max_results)
                )
                modalities_to_execute.append("graph")

            if "gnn" in search_types:
                search_tasks.append(
                    self._execute_gnn_search(query, domain, max_results)
                )
                ...
                results.append(
                    SearchResult(
                        ...
                        metadata={
                            ...
                            "domain": domain,
                            "embedding_model": result.metadata.get(
    ...
    def _synthesize_results(
        ...
        domain_high_confidence_threshold = (
            UniversalSearchConstants.HIGH_CONFIDENCE_THRESHOLD
        )

        high_confidence_results = len(
            [r for r in all_results if r.confidence >= domain_high_confidence_threshold]
        )
        ...
        cross_modal_agreement = EXPR.calculate_cross_modal_agreement(
            active_modalities, average_confidence
        )
        ...
        synthesis_score = EXPR.calculate_synthesis_score(
            ...
            domain_agreement_weight,
            domain_quality_weight,
        )

...
class SearchDocument(BaseModel):
    ...
    source_type: SearchResultType = Field(..., description="Type of search that found this document")
...
class TriModalSearchResult(BaseModel):
    ...
    search_types_used: List[SearchResultType] = Field(..., description="Search types that were executed")
    ...
    confidence_distribution: Dict[ConfidenceLevel, int] = Field(
        ...
        description="Distribution of results by confidence level"
    )
    ...
    def _calculate_confidence_distribution(self) -> Dict[ConfidenceLevel, int]:
        ...
        distribution = {ConfidenceLevel.HIGH: 0, ConfidenceLevel.MEDIUM: 0, ConfidenceLevel.LOW: 0}
...
class DomainDetectionResult(BaseModel):
    ...
    domain_probabilities: Dict[str, float] = Field(
        default_factory=dict,
        description="Probability distribution across all domains"
    )
    
    # Analysis metadata
    features_used: List[str] = Field(
        default_factory=list,
        description="Features used for domain detection"
    )
...
class PatternLearningResult(BaseModel):
    ...
    patterns_learned: int = Field(..., ge=0, description="Number of new patterns learned")
    patterns_updated: int = Field(..., ge=0, description="Number of existing patterns updated")
    confidence_scores: List[float] = Field(default_factory=list, description="Confidence scores for learned patterns")
    
    # Pattern details
    pattern_summary: Dict[str, Any] = Field(
        default_factory=dict,
        description="Summary of learned patterns"
    )
    ...
    error_message: Optional[str] = Field(None, description="Error message if learning failed")
...
class AgentHealthStatus(BaseModel):
    ...
    azure_services_connected: bool = Field(..., description="Azure services connectivity")
    
    # Performance metrics
    average_response_time: float = Field(..., ge=0.0, description="Average response time in seconds")
    success_rate: float = Field(..., ge=0.0, le=1.0, description="Success rate over recent queries")
    ...
    def overall_health_score(self) -> float:
        ...
        if not self.service_health:
            ...
            healthy_services = sum(1 for healthy in self.service_health.values() if healthy)
...
class UniversalEntity(BaseModel):
    ...
    confidence: float = Field(1.0, ge=0.0, le=1.0, description="Entity confidence score")
...
class UniversalRelation(BaseModel):
    ...
    relation_type: str = Field(..., description="Dynamic relation type (domain-specific)")
    confidence: float = Field(1.0, ge=0.0, le=1.0, description="Relation confidence score")
    context: Optional[str] = Field(None, description="Contextual information")

...
class VectorSearchRequest(BaseModel):
    ...
    domain: Optional[str] = Field(default=None, description="Domain name from data/raw/ subdirectories")
...
class GraphSearchRequest(BaseModel):
    ...
    relationship_types: List[str] = Field(
        default_factory=lambda: ["connects"],  # Minimal fallback - should be learned from domain patterns
        max_items=20
    )
...
class TriModalSearchRequest(BaseModel):
    ...
    parallel_execution: bool = Field(
        default=True,
        description="Execute searches in parallel for sub-3s performance"
    )

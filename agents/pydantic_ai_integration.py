...
from .models.responses import TriModalSearchResult, AgentResponse
...
class PydanticAIQueryResponse(BaseModel):
    ...
    competitive_advantages_used: Dict[str, bool] = Field(
        ...
        description="Competitive advantages utilized"
    )
...
async def search_with_tri_modal(
    ...
    return await search_with_tri_modal_tool(ctx, query, domain, max_results, ctx.deps.azure_services)
...
async def extract_entities(
    ...
    result = await extract_entities_tool(ctx, text, domain, extraction_strategy, ctx.deps.azure_services)
...
async def extract_relationships(
    ...
    """
    # Delegate to the specialized tool in knowledge_extraction
    result = await extract_relationships_tool(ctx, text, entities, domain, "semantic", ctx.deps.azure_services)
...
async def process_query(
    ...
    try:
        ...
        performance_metrics = {
            ...
            "modalities_used": search_result.modality_contributions.keys() if search_result.modality_contributions else [],
...
async def process_intelligent_query(
    ...
    dependencies = PydanticAIDependencies(
        azure_services=azure_services,
        app_settings=app_settings or Any()
    )

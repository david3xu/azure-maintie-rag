...
from scripts import test_validator
...
def main():
    ...
    if args.command == 'setup':
        ...
        # Pass domain to data pipeline
        original_argv = sys.argv
        sys.argv = ['data_pipeline.py', args.domain]
        ...
        return asyncio.run(test_validator.main())
    
    elif args.command == 'workflow':
        # Pass action to workflow analyzer
        original_argv = sys.argv
        if args.action == 'query' and args.query:
            sys.argv = ['workflow_analyzer.py', args.action, args.query]
        else:
            sys.argv = ['workflow_analyzer.py', args.action]
        result = asyncio.run(workflow_analyzer.main())
        sys.argv = original_argv
        return result

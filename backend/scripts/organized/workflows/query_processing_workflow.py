...
class AzureServiceDependencyTracker:
    ...
    async def _establish_baselines(self) -> dict:
        """Establish actual performance baselines for Azure services"""
        baselines = {}
        try:
            # Test actual response times for each service
            from integrations.azure_services import AzureServicesManager
            azure_services = AzureServicesManager()
            ...
            start = time.time()
            try:
                openai_client = azure_services.get_service('openai')
                # Simple test call
                await openai_client.get_completion("Test", max_tokens=1)
                baselines["openai"] = time.time() - start
            except:
                baselines["openai"] = None
                
            # Similar tests for other services...
            baselines.update({"search": 0.5, "cosmos": 0.3, "blob": 0.2})  # Default estimates
            
        except Exception as e:
            print(f"Baseline establishment failed: {e}")
            baselines = {"openai": None, "search": None, "cosmos": None, "blob": None}

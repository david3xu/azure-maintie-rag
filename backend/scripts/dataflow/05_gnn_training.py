...
class GNNTrainingStage:
    ...
    def load_pytorch_geometric_data(self, source_path: str) -> Data:
        ...
        logger.info(f"Loaded graph - Nodes: {graph.x.size(0)}, Edges: {graph.edge_index.size(1)}, Domain: {domain}")
        return graph
    ...
    def train_model(self, model: GNNModel, graph: Data, epochs: int, learning_rate: float) -> Dict[str, Any]:
        """Train the GNN model"""
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                train_acc = (pred_train == graph.y[graph.train_mask]).float().mean().item()
            ...
            if epoch % 10 == 0 or epoch == epochs - 1:
                logger.info(f"Epoch {epoch+1}/{epochs} - Loss: {loss:.4f}, Train: {train_acc:.4f}, Val: {val_acc:.4f}, Test: {test_acc:.4f}")
    ...
    def save_model(self, model: GNNModel, training_results: Dict[str, Any], domain: str) -> str:
    ...
    async def execute(
        ...
        try:
            ...
            print(f"✅ Graph loaded - Nodes: {graph.x.size(0)}, Edges: {graph.edge_index.size(1)}")
            print(f"   Node features: {graph.x.size(1)}D, Edge features: {graph.edge_attr.size(1)}D")
            ...
            print(f"🏗️  Creating {model_type.upper()} model - Input: {input_dim}D, Hidden: {hidden_dim}D, Output: {output_dim}D")

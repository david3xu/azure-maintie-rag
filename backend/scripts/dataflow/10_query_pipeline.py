    ...
    async def execute_query_pipeline(
        ...
        try:
            ...
            if streaming_callback:
                await streaming_callback("query_analysis", "started", {"query": user_query})
            ...
            pipeline_results["pipeline_metrics"]["query_analysis_time"] = round(stage06_duration, 2)
            ...
            print(f"✅ Stage 06 Complete: {analysis_data.get('query_type', 'unknown')} query analyzed")
            
            if streaming_callback:
                await streaming_callback("query_analysis", "completed", {
            ...
            print(f"\n🔄 Stage 07: Unified Search")
            if streaming_callback:
                await streaming_callback("unified_search", "started", {"search_strategy": stage06_result.get("search_strategy", {})})
            ...
            pipeline_results["pipeline_metrics"]["search_time"] = round(stage07_duration, 2)
            pipeline_results["pipeline_metrics"]["total_search_results"] = stage07_result.get("total_results", 0)
            ...
            print(f"✅ Stage 07 Complete: {stage07_result.get('total_results', 0)} unified results")
            
            if streaming_callback:
                await streaming_callback("unified_search", "completed", {
            ...
            print(f"\n🔄 Stage 08: Context Retrieval")
            if streaming_callback:
                await streaming_callback("context_retrieval", "started", {"max_context_length": max_context_length})
            ...
            pipeline_results["pipeline_metrics"]["context_preparation_time"] = round(stage08_duration, 2)
            ...
            print(f"✅ Stage 08 Complete: {total_context_items} context items, {total_citations} citations")
            
            if streaming_callback:
                await streaming_callback("context_retrieval", "completed", {
            ...
            print(f"\n🔄 Stage 09: Response Generation")
            if streaming_callback:
                await streaming_callback("response_generation", "started", {"response_style": response_style})
            ...
            pipeline_results["pipeline_metrics"]["response_generation_time"] = round(stage09_duration, 2)
            ...
            pipeline_results["pipeline_metrics"]["response_length"] = len(final_answer_text)
            
            response_length = stage09_result.get("response_length", 0)
            print(f"✅ Stage 09 Complete: {response_length} char response")
            
            if streaming_callback:
                await streaming_callback("response_generation", "completed", {
            ...
            print(f"   🎯 Search results: {pipeline_results['pipeline_metrics']['total_search_results']}")
            print(f"   📚 Context length: {pipeline_results['pipeline_metrics']['context_length']:,} chars")
            print(f"   💬 Response length: {pipeline_results['pipeline_metrics']['response_length']:,} chars")
            print(f"   📎 Citations: {pipeline_results['pipeline_metrics']['citations_count']}")
            print(f"   ⏱️  Total duration: {pipeline_results['pipeline_metrics']['total_duration']}s")
            
            if streaming_callback:
                await streaming_callback("pipeline", "completed", pipeline_results["pipeline_metrics"])
...
async def main():
    ...
    if results.get("success"):
        ...
        for citation in final_answer.get("citations", []):
            ...
            print(f"{citation_id} {citation.get('source_type', 'Unknown')}: {citation.get('content_preview', '')}")
        print(f"{'='*75}")

        ...
        try:
            ...
            for step_name, step_result in self.results.items():
                if isinstance(step_result, dict):
                    ...
                    print(f"⚠️  Warning: {step_name} result is not a dictionary: {type(step_result)}")
                    successful_steps += 1

            print(f"📊 Workflow Completed:")
            print(f"   ✅ Successful Steps: {successful_steps}/{len(self.results)}")
            print(f"   ⏱️  Total Processing Time: {total_time:.2f}s")
            print(f"   🎯 Success Rate: {(successful_steps/len(self.results)*100):.1f}%")
            ...
            print(f"📊 Results collected: {len(self.results)} steps")

...
def build_knowledge_graph(
    ...
    try:
        ...
        knowledge_summary = {
            ...
            "relation_types": list(set(r["relation_type"] for r in processed_relations)),
            "processing_timestamp": datetime.now().isoformat(),
            "extraction_source": "azure_prompt_flow_universal"
        }
        
        logger.info(f"Built universal knowledge graph: {len(processed_entities)} entities, {len(processed_relations)} relations")
...
def main(entities: str, relations: str, confidence_threshold: float = 0.7, max_entities: int = 50) -> Dict[str, Any]:
    """Main function called by Azure Prompt Flow"""
    return build_knowledge_graph(entities, relations, confidence_threshold, max_entities)

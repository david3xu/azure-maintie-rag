...
# Import our PydanticAI agent
try:
    from agents.universal_agent import agent, health_check as agent_health_check
    from agents.azure_integration import AzureServiceContainer, create_azure_service_container
    AGENT_AVAILABLE = True
except ImportError:
    AGENT_AVAILABLE = False
    agent = None
    agent_health_check = None
...
class ConsolidatedAgentService(ServicesToAgentsInterface):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info("Consolidated Agent Service initialized with PydanticAI + coordination patterns")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not health_status:
                self.logger.error(f"Agent health check failed: {health_status}")
                return False
    ...
    async def process_request(self, request: AgentServiceRequest) -> AgentServiceResponse:
        ...
        try:
            ...
            result = await self._execute_pydantic_agent(request.query, correlation_id)
            ...
            if hasattr(result, 'all_messages'):
                tool_calls = [msg for msg in result.all_messages() if hasattr(msg, 'parts')]
            ...
            response = AgentServiceResponse(
                ...
                correlation_id=correlation_id,
                metadata={
                    "performance_met": execution_time <= request.performance_requirements.get("max_response_time", 3.0),
    ...
    async def request_intelligent_analysis(self, request: AgentRequest) -> OperationResult:
        ...
        context = AgentCoordinationContext(
            ...
            timeout_seconds=request.performance_requirements.get("max_response_time", 30)
        )
        
        self.active_requests[request_id] = context
        
        try:
            logger.info("Coordinating agent intelligence request", extra={
                'request_id': request_id,
            ...
            logger.info("Agent intelligence request completed", extra={
                'request_id': request_id,
                'execution_time': execution_time,
            ...
            execution_time = time.time() - start_time
            logger.error(f"Agent coordination error: {e}", extra={
                'request_id': request_id,
                'correlation_id': context.correlation_id,
                'execution_time': execution_time
            })
    ...
    async def coordinate_reasoning_workflow(self, request: AgentRequest) -> OperationResult:
        ...
        context = AgentCoordinationContext(
            ...
            correlation_id=request.correlation_id or request_id,
            start_time=start_time,
            timeout_seconds=request.performance_requirements.get("max_response_time", 60)
        )
        
        try:
            logger.info("Coordinating reasoning workflow", extra={
                'request_id': request_id,
                'reasoning_constraints': getattr(request, 'reasoning_constraints', {})
            })
            ...
            final_result = await self._synthesize_reasoning_results(reasoning_steps, context)
    ...
    async def request_domain_adaptation(
        ...
        try:
            logger.info("Coordinating domain adaptation", extra={
                'request_id': request_id,
                'strategy': strategy,
            ...
            adaptation_result = await self.request_intelligent_analysis(adaptation_request)
            
            if adaptation_result.status == OperationStatus.SUCCESS:
                logger.info("Domain adaptation completed successfully", extra={
                    'request_id': request_id,
    ...
    async def _execute_pydantic_agent(self, query: str, correlation_id: str) -> Any:
        """Execute PydanticAI agent (legacy method)"""
        await self._ensure_azure_container()
        
        if not AGENT_AVAILABLE or agent is None:
            raise RuntimeError("PydanticAI agent not available - system misconfigured")
        
        # Execute the actual PydanticAI agent
        result = await agent.run(query, deps=self.azure_service_container)
    ...
    async def _execute_agent_intelligence(
        ...
        try:
            ...
            raise Exception(f"Agent execution timed out after {context.timeout_seconds} seconds")
        except Exception as e:
            if context.retry_count < context.max_retries:
                context.retry_count += 1
                logger.warning(f"Agent execution failed, retrying ({context.retry_count}/{context.max_retries}): {e}")
                ...
                raise Exception(f"Agent execution failed after {context.max_retries} retries: {str(e)}")

    async def _call_pydantic_agent(self, agent_context: Dict[str, Any]) -> str:
        """Call PydanticAI agent with proper dependency injection"""
        
        # Ensure Azure container is available
        await self._ensure_azure_container()
        
        if not AGENT_AVAILABLE or agent is None:
            raise RuntimeError("PydanticAI agent not available - system misconfigured")
        
        # Execute the actual PydanticAI agent with context
        query = agent_context.get("query", "")
        domain = agent_context.get("domain")
        operation_type = agent_context.get("operation_type", "general")
        
        # Enhance query with context and operation type for better agent understanding
        enhanced_query = query
        if domain:
            enhanced_query = f"[Domain: {domain}] {query}"
        if operation_type != "general" and operation_type != "unknown":
            enhanced_query = f"[Operation: {operation_type}] {enhanced_query}"
        
        # Execute the PydanticAI agent
        result = await agent.run(enhanced_query, deps=self.azure_service_container)
        return result.data if hasattr(result, 'data') else str(result)
    ...
    async def _synthesize_reasoning_results(
        ...
        successful_steps = [s for s in reasoning_steps if s.get("status") == "completed"]
    ...
    def _update_legacy_metrics(self, success: bool, execution_time: float, tools_used: List[str]):
        ...
        for tool in tools_used:
            self.metrics["tools_used_count"][tool] = self.metrics["tools_used_count"].get(tool, 0) + 1
    ...
    async def get_service_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check agent health
            agent_health = await agent_health_check()
    ...
    async def get_agent_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update metrics
            self.metrics["agent_health_status"] = "healthy" if agent_health else "unhealthy"
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "healthy" if agent_status.get("agent_available") else "degraded",
                "initialized": self._initialized,
...
async def test_agent_service():
    ...
    print(f"✅ Legacy request processed: {legacy_response.execution_time:.2f}s, {len(legacy_response.tools_used)} tools used")
    ...
    coordination_result = await service.request_intelligent_analysis(coordination_request)
    print(f"✅ Coordination request processed: {coordination_result.status.value}, {coordination_result.execution_time:.2f}s")
    
    # Get metrics
    metrics = service.get_service_metrics()
    print(f"✅ Service metrics: {metrics['requests_processed']} legacy + {metrics['requests_total']} coordination requests")
    print(f"   Success rates: Legacy {metrics['success_rate']:.2f}, Coordination {metrics['coordination_success_rate']:.2f}")

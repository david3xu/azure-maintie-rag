...
class AzureServiceCostTracker:
    ...
    def estimate_gnn_training_cost(self, domain: str, data_size_gb: float, training_hours: float) -> Dict[str, float]:
        """Estimate cost for GNN training workflow"""
        costs = {}
        
        # Azure ML training cost
        costs["azure_ml_training"] = training_hours * self.cost_per_service["azure_ml"]["per_training_hour"]
        
        # Cosmos DB operations (graph queries)
        estimated_graph_operations = int(data_size_gb * 1000)  # Rough estimate
        costs["cosmos_db_operations"] = self._calculate_service_cost("cosmos_db", {
        ...
        costs["blob_storage"] = self._calculate_service_cost("blob_storage", {
            "gb_month": data_size_gb,
            "operation": 100  # Read/write operations
        })
        
        # Total cost
        costs["total_estimated"] = sum(costs.values())
        
        logger.info(f"Estimated GNN training cost for {domain}: ${costs['total_estimated']:.4f}")
        return costs
    ...
    def estimate_operation_cost(self, service: str, operation_type: str, usage_count: int) -> float:

...
class BaseAzureClient(ABC):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not self.endpoint:
            raise ValueError(f"{self.__class__.__name__} requires endpoint configuration")
        
        # Enforce Azure-only deployment pattern
        self.use_managed_identity = getattr(azure_settings, 'use_managed_identity', True)
        if not self.use_managed_identity:
            raise ValueError(f"{self.__class__.__name__} requires managed identity authentication")
            
        logger.info(f"{self.__class__.__name__} initialized with managed identity")
    ...
    async def _execute_with_retry(self, func: Callable, operation_name: str, *args, **kwargs) -> Any:
        """
        ...
        for attempt in range(1, self.retry_config.max_attempts + 1):
            try:
                ...
                if not self._is_retryable_error(e) or attempt == self.retry_config.max_attempts:
                ...
                logger.warning(f"{self.__class__.__name__}.{operation_name} attempt {attempt} failed: {e}. "
    ...
    def _record_success(self, operation: str, duration_ms: float):
        ...
        logger.debug(f"{self.__class__.__name__}.{operation} completed in {duration_ms:.2f}ms")
    ...
    def _record_error(self, operation: str, error: Exception, duration_ms: float):
        ...
        logger.error(f"{self.__class__.__name__}.{operation} failed after {duration_ms:.2f}ms: {error}")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        """
        success_rate = (self.metrics.success_count / self.metrics.operation_count 
                       if self.metrics.operation_count > 0 else 0.0)
        
        avg_duration = (self.metrics.total_duration_ms / self.metrics.operation_count
                       if self.metrics.operation_count > 0 else 0.0)
    ...
    def create_error_response(self, operation: str, error: Union[str, Exception]) -> Dict[str, Any]:
        """

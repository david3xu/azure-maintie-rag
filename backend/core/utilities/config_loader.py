...
class ConfigLoader:
    ...
    def get_azd_environment_config(self) -> Dict[str, Any]:
        """Get azd-managed environment configuration (no .env needed)."""
        env_config = {}

        # Load from azd environment variables only
        azd_vars = [
            'AZURE_OPENAI_ENDPOINT', 'AZURE_SEARCH_ENDPOINT', 'AZURE_COSMOS_ENDPOINT',
            'AZURE_STORAGE_ACCOUNT', 'AZURE_SUBSCRIPTION_ID', 'AZURE_RESOURCE_GROUP',
            'AZURE_ML_WORKSPACE_NAME', 'AZURE_TENANT_ID', 'AZURE_CLIENT_ID',
            'OPENAI_MODEL_DEPLOYMENT', 'EMBEDDING_MODEL_DEPLOYMENT',
            'AZURE_SEARCH_INDEX', 'AZURE_STORAGE_CONTAINER',
            'COSMOS_DATABASE_NAME', 'COSMOS_GRAPH_NAME'
        ]

...
class UnifiedSearchClient(BaseAzureClient):
    ...
    async def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = self._index_client.get_index_statistics(azure_settings.azure_search_index)
    ...
    async def create_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            if result.get('success'):
                ...
                raise RuntimeError(f"Embedding generation failed: {result.get('error')}")
            
        except Exception as e:
            logger.error(f"Failed to create embeddings via Azure OpenAI: {e}")
            raise RuntimeError(f"Embedding creation failed: {e}")
    
    async def vector_search(self, query_vector: List[float], top: int = 10, filters: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Vector search not available, falling back to semantic search")
    ...
    async def create_index(self, index_name: str, domain: str = "general", schema: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            error_message = str(e)
            if "ResourceNameAlreadyInUse" in error_message or "already exists" in error_message:
    ...
    async def get_or_create_vector_index(self, index_name: str, domain: str = "general", vector_dimension: int = 1536) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return self.create_success_response('get_or_create_vector_index', {
                    'index_name': index_name,
            ...
            from azure.search.documents.indexes.models import (
                ...
                HnswAlgorithmConfiguration, SemanticConfiguration, SemanticPrioritizedFields,
            ...
            fields = [
                ...
                SearchableField(name="title", type="Edm.String", searchable=True, filterable=True),
            ...
            error_message = str(e)
            if "ResourceNameAlreadyInUse" in error_message or "already exists" in error_message:
                logger.info(f"✅ Vector index already exists (caught exception): {index_name}")
                return self.create_success_response('get_or_create_vector_index', {
                    'index_name': index_name,
                    'message': 'Vector index already exists',
                    'existed': True

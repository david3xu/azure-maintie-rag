...
class ConfigurationValidator:
    ...
    def _validate_retrieval_settings(self) -> None:
        ...
        if settings.faiss_index_type not in valid_index_types:
            self.warnings.append(f"FAISS_INDEX_TYPE '{settings.faiss_index_type}' is not standard")
    ...
    def _validate_generation_settings(self) -> None:
        ...
        if not (-2.0 <= settings.llm_frequency_penalty <= 2.0):
            self.errors.append("LLM_FREQUENCY_PENALTY must be between -2.0 and 2.0")

        if not (-2.0 <= settings.llm_presence_penalty <= 2.0):

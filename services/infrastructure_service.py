...
class AsyncInfrastructureService:
        ...
        try:
            ...
            for result in initialization_results:
                if isinstance(result, Exception):
                    logger.error(
                        f"Service initialization failed with exception: {result}"
                    )
                    self.initialization_results.append(
            ...
            successful_services = sum(
                1 for r in self.initialization_results if r.success
            )
            ...
            logger.info(
                f"✅ Azure services initialization completed: "
    ...
    async def _initialize_openai_service(self) -> ServiceInitializationResult:
        ...
        try:
            ...
            logger.info(
                f"✅ Azure OpenAI client initialized in {initialization_time:.2f}s - endpoint: {endpoint}"
            )
            ...
            initialization_time = time.time() - start_time
            logger.error(
                f"❌ Azure OpenAI initialization failed in {initialization_time:.2f}s: {e}"
            )
    ...
    async def _initialize_search_service(self) -> ServiceInitializationResult:
        ...
        try:
            ...
            logger.info(
                f"✅ Azure Cognitive Search initialized in {initialization_time:.2f}s - endpoint: {endpoint}"
            )
            ...
            initialization_time = time.time() - start_time
            logger.error(
                f"❌ Azure Search initialization failed in {initialization_time:.2f}s: {e}"
            )
    ...
    async def _initialize_storage_service(self) -> ServiceInitializationResult:
        ...
        try:
            ...
            logger.info(
                f"✅ Azure Blob Storage initialized in {initialization_time:.2f}s - account: {account}"
            )
            ...
            initialization_time = time.time() - start_time
            logger.error(
                f"❌ Azure Storage initialization failed in {initialization_time:.2f}s: {e}"
            )
    ...
    async def _initialize_cosmos_service(self) -> ServiceInitializationResult:
        ...
        try:
            ...
            logger.info(
                f"✅ Azure Cosmos DB initialized in {initialization_time:.2f}s - endpoint: {endpoint}, database: {database}"
            )
            ...
            initialization_time = time.time() - start_time
            logger.error(
                f"❌ Azure Cosmos DB initialization failed in {initialization_time:.2f}s: {e}"
            )
    ...
    async def _initialize_ml_service(self) -> ServiceInitializationResult:
        ...
        try:
            ...
            logger.info(
                f"✅ Azure ML Workspace initialized in {initialization_time:.2f}s - workspace: {workspace}"
            )
            ...
            initialization_time = time.time() - start_time
            logger.error(
                f"❌ Azure ML initialization failed in {initialization_time:.2f}s: {e}"
            )
    ...
    async def _initialize_app_insights_service(self) -> ServiceInitializationResult:
        ...
        try:
            ...
            initialization_time = time.time() - start_time
            connection_string = (
                self.azure_settings.applicationinsights_connection_string
            )

            if connection_string:
                logger.info(
                    f"✅ Azure Application Insights initialized in {initialization_time:.2f}s - azd managed"
                )
            else:
                logger.info(
                    f"✅ Azure Application Insights initialized in {initialization_time:.2f}s - legacy config"
                )
            ...
            initialization_time = time.time() - start_time
            logger.error(
                f"❌ Application Insights initialization failed in {initialization_time:.2f}s: {e}"
            )
    ...
    async def _initialize_vector_service(self) -> ServiceInitializationResult:
        ...
        try:
            ...
            initialization_time = time.time() - start_time
            logger.error(
                f"❌ Vector service initialization failed in {initialization_time:.2f}s: {e}"
            )
    ...
    def _get_initialization_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "successful_services": [
                {"name": r.service_name, "initialization_time": r.initialization_time}
    ...
    async def health_check_async(self) -> Dict[str, Any]:
        ...
        for service_name, service_client in services_to_check:
            if service_client:
                ...
                if hasattr(service_client, "health_check_async"):
                    ...
                    health_tasks.append(
                        self._simple_health_check(service_name, service_client)
                    )
        ...
        for i, result in enumerate(health_results):
            service_name = (
                service_names[i] if i < len(service_names) else f"unknown_{i}"
            )
        ...
        overall_status = (
            "healthy" if healthy_count == len(services_health) else "degraded"
        )
    ...
    async def _simple_health_check(
        self, service_name: str, service_client: Any
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if service_client is None:
                return {"status": "unhealthy", "reason": "Service client is None"}

            return {"status": "healthy", "service_type": type(service_client).__name__}
        ...
        for service_name, service_client in services_to_shutdown:
            if service_client and hasattr(service_client, "shutdown_async"):
                shutdown_tasks.append(
                    self._shutdown_service_safely(service_name, service_client)
                )
    ...
    async def _shutdown_service_safely(
        self, service_name: str, service_client: Any
    ) -> None:

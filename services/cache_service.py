...
class ConsolidatedCacheService:
    ...
    def __init__(
        self, use_redis: bool = False, config: Optional[CacheConfiguration] = None
    ):
        """
        ...
        logger.info(
            f"Consolidated Cache Service initialized (Redis: {self.use_redis}, Multi-level: {self.config.multi_level_enabled})"
        )
    ...
    async def _set_in_redis(self, cache_key: str, data: Any, ttl_seconds: int):
        ...
        try:
            await self.redis_client.setex(
                cache_key, ttl_seconds, json.dumps(data, default=str)
            )
        except Exception as e:
    ...
    async def get_or_compute(
        self, cache_key: str, compute_fn: Callable, ttl_seconds: int = 300
    ) -> Any:
    ...
    async def cache_query_result(
        self, query: str, domain: str, result: Any, ttl_seconds: int = 300
    ):
    ...
    async def get_cached_query_result(self, query: str, domain: str) -> Optional[Any]:
    ...
    async def set_cached_query_result(
        self, cache_key: str, result: Any, ttl_seconds: int = 300
    ):
    ...
    async def get_cached_search_result(
        self, search_type: str, query: str, domain: str
    ) -> Optional[Any]:
        ...
        try:
            ...
            if cache_level in ["L1", "multi"]:
                ...
                await self._set_in_memory(
                    cache_key, data, min(ttl_seconds, 3600)  # L1 max 1 hour
                )
                success = True
                ...
                if self.memory_manager:
                    try:
                        await self.memory_manager.store_pattern(
                            cache_key, data, cache_type="result"
                        )
                        success = True
                    except Exception as e:
    ...
    async def clear_cache(self):
        ...
        try:
            ...
            logger.info(
                f"All caches cleared (Memory: {cleared_memory}, Redis: {cleared_redis > 0})"
            )
    ...
    async def _cleanup_memory_cache(self):
        ...
        now = datetime.now()
        expired_keys = [
            key for key, entry in self.memory_cache.items() if now >= entry["expires"]
        ]
            ...
            if self.config.multi_level_enabled:
                ...
                if self.memory_manager:
                    try:
                        ...
                        health_status["memory_manager"] = memory_health.get(
                            "status", "unknown"
                        )
                    except Exception as e:

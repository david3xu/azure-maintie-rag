...
class ExtractionConfiguration(BaseModel):
    ...
    expected_entity_types: List[str] = Field(
        default_factory=list, description="Entity types discovered from domain patterns"
    )
    ...
    max_entities_per_chunk: int = Field(
        default=50, ge=1, le=200, description="Maximum entities per processing chunk"
    )
    ...
    parallel_processing: bool = Field(
        default=True, description="Enable parallel processing for performance"
    )
    ...
    extraction_timeout_seconds: int = Field(
        default=30, ge=5, le=300, description="Timeout for extraction operations"
    )

    # Performance optimization settings
    enable_caching: bool = Field(
        default=True, description="Enable extraction result caching"
    )
...
class ExtractionResults(BaseModel):
    ...
    extraction_accuracy: float = Field(
        ..., ge=0.0, le=1.0, description="Overall extraction accuracy score"
    )
    entity_precision: float = Field(
        ..., ge=0.0, le=1.0, description="Entity extraction precision"
    )
    entity_recall: float = Field(
        ..., ge=0.0, le=1.0, description="Entity extraction recall"
    )
    relationship_precision: float = Field(
        ..., ge=0.0, le=1.0, description="Relationship extraction precision"
    )
    relationship_recall: float = Field(
        ..., ge=0.0, le=1.0, description="Relationship extraction recall"
    )
    ...
    new_entity_types_discovered: List[str] = Field(
        default_factory=list, description="New entity types found during processing"
    )
    unexpected_relationship_patterns: List[str] = Field(
        default_factory=list, description="Unexpected relationship patterns discovered"
    )
    ...
    quality_issues_detected: List[str] = Field(
        default_factory=list, description="Quality issues that could improve config"
    )
...
class ConfigurationFeedback(BaseModel):
    ...
    recommended_parameter_changes: Dict[str, Any] = Field(
        default_factory=dict, description="Recommended changes to processing parameters"
    )
    recommended_vocabulary_additions: List[str] = Field(
        default_factory=list, description="Terms to add to domain vocabulary"
    )
    ...
    optimization_opportunities: List[str] = Field(
        default_factory=list, description="Opportunities for further optimization"
    )

    # Quality improvements
    quality_improvement_suggestions: List[str] = Field(
        default_factory=list, description="Suggestions for improving extraction quality"
    )

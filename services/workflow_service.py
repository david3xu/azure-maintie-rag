...
class SimpleWorkflowService:
    ...
    def start_workflow(self, workflow_id: str, workflow_type: str, steps: List[str]) -> Dict[str, Any]:
    ...
    def update_workflow_progress(self, workflow_id: str, step_name: str, step_result: Any = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate progress
            total_steps = len(workflow["completed_steps"]) + len(workflow["remaining_steps"])
            if total_steps > 0:
                workflow["progress"] = len(workflow["completed_steps"]) / total_steps * 100
            ...
            logger.info(f"Workflow updated: {workflow_id} - {step_name} completed ({workflow['progress']:.1f}%)")
            return {
                "success": True,
                "workflow_id": workflow_id,
    ...
    def fail_workflow(self, workflow_id: str, error: str, step_name: str = None) -> Dict[str, Any]:
    ...
    def cleanup_completed_workflows(self, older_than_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            for workflow_id, workflow in self.workflows.items():
                if (workflow["status"] in ["completed", "failed"] and 
                    workflow["end_time"] and 
                    workflow["end_time"] < cutoff_time):
...
def start_workflow(workflow_id: str, workflow_type: str, steps: List[str]) -> Dict[str, Any]:
    """Backward compatibility function"""
    return _workflow_service.start_workflow(workflow_id, workflow_type, steps)

def update_workflow_progress(workflow_id: str, step_name: str, step_result: Any = None) -> Dict[str, Any]:
    """Backward compatibility function"""
    return _workflow_service.update_workflow_progress(workflow_id, step_name, step_result)

def fail_workflow(workflow_id: str, error: str, step_name: str = None) -> Dict[str, Any]:
    """Backward compatibility function"""

    ...
    async def get_agent_health(self, agent_name: str = None) -> Dict[str, Any]:
        ...
        try:
            if agent_name:
                ...
                agent = self.agents[agent_name]
                healthy = hasattr(agent, 'process_query') or hasattr(agent, 'process_request')
                ...
                return {
                    "overall_status": "healthy" if all(h["healthy"] for h in health_status.values()) else "partial",
...
async def process_agent_request(agent_name: str, request: Dict[str, Any]) -> Dict[str, Any]:

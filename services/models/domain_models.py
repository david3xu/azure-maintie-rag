...
class DataDrivenExtraction(BaseModel):
    ...
    extraction_method: str = Field(default="universal_agent", description="Extraction method used")
    
    # Extracted content
    entities: List[str] = Field(default_factory=list, description="Extracted entities")
    relationships: List[Dict[str, str]] = Field(default_factory=list, description="Extracted relationships")
    ...
    technical_terms: List[str] = Field(default_factory=list, description="Technical terminology")
    
    # Quality metrics
    extraction_quality: DataExtractionQuality = Field(default=DataExtractionQuality.FAIR)
    quality_score: float = Field(default=0.5, ge=0.0, le=1.0, description="Quality score 0-1")
    fragmented_entities: int = Field(default=0, description="Number of fragmented entities")
    
    def model_post_init(self, __context) -> None:
        """Calculate quality metrics after model initialization"""
        
        # Calculate quality score based on extraction completeness
        total_items = len(self.entities) + len(self.relationships) + len(self.key_concepts)
        ...
        relationship_density = min(len(self.relationships) / max(len(self.entities), 1), 1.0)
...
class DomainConfiguration(BaseModel):
    ...
    # Generation metadata
    generated_from_data: bool = Field(default=True, description="Generated from raw data")
    ...
    primary_entities: List[str] = Field(default_factory=list, description="Primary domain entities")
    entity_types: Dict[str, List[str]] = Field(default_factory=dict, description="Dynamically classified entities")
    ...
    relationship_types: List[str] = Field(default_factory=list, description="Relationship type vocabulary")
    
    # Search and query configuration
    key_concepts: List[str] = Field(default_factory=list, description="Key concepts for querying")
    technical_vocabulary: List[str] = Field(default_factory=list, description="Technical term vocabulary")
    query_expansion_terms: List[str] = Field(default_factory=list, description="Query expansion vocabulary")
    
    # Processing configuration
    recommended_chunk_size: int = Field(default=1000, description="Recommended text chunk size")
    recommended_overlap: int = Field(default=200, description="Recommended chunk overlap")
    extraction_confidence_threshold: float = Field(default=0.7, description="Extraction confidence threshold")
    
    # Performance configuration
    expected_response_time: float = Field(default=3.0, description="Expected response time in seconds")
    ...
    def model_post_init(self, __context) -> None:
        ...
        if self.entity_count < 3:  # Reduced from 5 to be more lenient
            print(f"⚠️ Warning: Low entity count for domain config: {self.entity_count}")
        ...
        if self.data_quality == DataExtractionQuality.POOR and self.expected_response_time < 5.0:
            object.__setattr__(self, 'expected_response_time', 5.0)  # Adjust for poor data quality
    ...
    def prompt_flow_readiness(self) -> Dict[str, Any]:
        ...
        return {
            'ready': (
                self.data_quality in [DataExtractionQuality.GOOD, DataExtractionQuality.EXCELLENT] and
                len(self.key_concepts) >= 5 and
                len(self.technical_vocabulary) >= 10 and
                self.entity_count >= 10
            ),
    ...
    def _calculate_confidence_score(self) -> float:
        ...
        vocab_score = min(len(self.technical_vocabulary) / 15.0, 1.0)  # Target 15+ terms
...
class UnifiedDataDrivenConfig(BaseModel):
    ...
    processed_files: List[str] = Field(default_factory=list, description="Processed file list")
    
    # Data-driven extractions
    extractions: List[DataDrivenExtraction] = Field(default_factory=list, description="Raw data extractions")
    
    # Generated domain configurations
    domain_configs: Dict[str, DomainConfiguration] = Field(default_factory=dict, description="Generated domain configs")
    
    # Azure service configuration (data-driven)
    azure_services: Dict[str, AzureServiceConfig] = Field(default_factory=dict, description="Azure service configs")
    
    # Tri-modal search configuration (data-driven)
    tri_modal_search: TriModalSearchConfig = Field(default_factory=TriModalSearchConfig, description="Search configuration")
    
    # Competitive advantage configuration
    competitive_advantage: CompetitiveAdvantageConfig = Field(default_factory=CompetitiveAdvantageConfig)
    ...
    def model_post_init(self, __context) -> None:
        ...
        if len(poor_quality_extractions) == len(self.extractions):
            raise ValueError("All extractions are poor quality - cannot generate reliable config")
    ...
    def overall_readiness(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'production_ready': (
                avg_quality >= 0.7 and
                competitive_score >= 0.8 and
                len(self.domain_configs) >= 1
            ),
    ...
    def add_extraction(self, extraction: DataDrivenExtraction):
        ...
        if not extraction.is_suitable_for_config:
            raise ValueError(f"Extraction quality too low for config generation: {extraction.quality_score}")
    ...
    def add_domain_config(self, domain_config: DomainConfiguration):
        ...
        if domain_config.domain_name in self.domain_configs:
            raise ValueError(f"Domain configuration already exists: {domain_config.domain_name}")
    ...
    def save_to_file(self, output_path: Union[str, Path]) -> Path:
        ...
        config_dict = self.model_dump(mode='json', exclude={
...
class DataDrivenConfigGenerator:
    ...
    def __init__(self, raw_data_path: str = "data/raw", config_output_path: str = "config"):
    ...
    def _determine_entity_type_from_content(self, entity: str) -> str:
        ...
        elif any(term in entity for term in ["function", "method", "class", "variable"]):
            return "code_element"
    ...
    def generate_domain_config(self, extraction: DataDrivenExtraction) -> DomainConfiguration:
        ...
        return DomainConfiguration(
            ...
            relationship_types=list(set(r.get('relation', '') for r in extraction.relationships)),
    ...
    def generate_unified_config(self, extractions: List[DataDrivenExtraction]) -> UnifiedDataDrivenConfig:
    ...
    for item in raw_path.iterdir():
        if item.is_dir() and not item.name.startswith('.'):
            ...
            if data_files:
                ...
                print(f"✅ Discovered domain: '{domain_name}' from directory '{item.name}' ({len(data_files)} files)")
...
def get_domain_files(domain_name: str, raw_data_path: str = "data/raw") -> List[Path]:
    ...
    dir_variations = [
        ...
        domain_name.replace('_', '-').title(),   # programming_language -> Programming-Language

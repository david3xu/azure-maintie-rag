...
#### **🌍 Agent 1: Domain Intelligence Agent** (129 lines)

- **File**: `agents/domain_intelligence/agent.py`
- **Purpose**: Universal content characteristic discovery without domain assumptions
- **Key Innovation**: Uses Azure OpenAI to analyze vocabulary complexity, concept density, and structural patterns, then generates domain signatures dynamically
- **Output**: `UniversalDomainAnalysis` with adaptive processing configuration
- **Tools**: `analyze_content_characteristics()`, `predict_entity_types()`, `generate_processing_configuration()`

#### **📚 Agent 2: Knowledge Extraction Agent** (852 lines)

- **File**: `agents/knowledge_extraction/agent.py`
- **Purpose**: Multi-method entity/relationship extraction using Agent 1's discoveries
- **Key Innovation**: Cached prompt library eliminates double LLM calling, Agent 1 → Agent 2 communication
- **Methods**: LLM-based, pattern-based, and hybrid approaches
- **Storage**: Real Azure Cosmos DB Gremlin integration
- **Tools**: `extract_entities_and_relationships()`, `generate_extraction_prompts()`, graph storage

#### **🔍 Agent 3: Universal Search Agent** (701 lines)

- **File**: `agents/universal_search/agent.py`
- **Purpose**: **Mandatory tri-modal search** orchestration (Vector + Graph + GNN)
- **Key Innovation**: No fallback logic - all three modalities required or system fails fast
- **Optimization**: Skips real-time Agent 1 analysis for queries (production speed)
- **Tools**: `orchestrate_universal_search()`, `search_vector_index()`, `search_knowledge_graph()`

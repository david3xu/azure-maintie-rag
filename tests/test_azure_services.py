    ...
    async def test_azure_openai_connection(self):
        ...
        if test_use_managed_identity:
            ...
            try:
                ...
                if not api_key:
                    pytest.fail(
                        f"Neither Azure CLI nor API key authentication available. CLI error: {cli_error}"
                    )
    ...
    async def test_azure_cosmos_connection(self):
        ...
        try:
            cosmos_client = CosmosClient(
                url=os.getenv("AZURE_COSMOS_ENDPOINT"), credential=credential
            )
            ...
            try:
                ...
                print(
                    f"✅ Azure Cosmos DB: Connection successful, found {len(databases)} databases"
                )
                ...
                if "403" in str(db_error) or "Forbidden" in str(db_error):
                    pytest.skip(
                        f"Azure Cosmos DB: Access denied (check permissions) - {db_error}"
                    )
                elif "404" in str(db_error) or "NotFound" in str(db_error):
                    pytest.skip(
                        f"Azure Cosmos DB: Service not found (check deployment) - {db_error}"
                    )
                else:
...
class TestEnvironmentConfiguration:
    ...
    def test_required_environment_variables(self):
        ...
        assert (
            not missing_vars
        ), f"Missing required environment variables for {auth_method}: {missing_vars}"
        print(
            f"✅ Environment Configuration: All required variables set for {auth_method}"
        )

...
from agents.domain_intelligence.analyzers.unified_content_analyzer import (
...
async def test_domain_intelligence():
    ...
    try:
        ...
        print(
            f"📏 Average document length: {domain_profile.avg_document_length:.0f} words"
        )
        print(
            f"🎯 Learned entity threshold: {domain_profile.entity_confidence_threshold:.3f}"
        )
        print(f"📦 Optimal chunk size: {domain_profile.optimal_chunk_size}")
        print(
            f"🔗 Relationship threshold: {domain_profile.relationship_confidence_threshold:.3f}"
        )
        ...
        extraction_config = await config_manager.get_extraction_config(
            "programming_language"
        )

        print(f"✅ Generated extraction config:")
        print(
            f"   🎯 Entity threshold: {extraction_config.entity_confidence_threshold:.3f}"
        )
        print(
            f"   🔗 Relationship threshold: {extraction_config.relationship_confidence_threshold:.3f}"
        )
        print(f"   📦 Chunk size: {extraction_config.chunk_size}")
        print(
            f"   📊 Max entities per chunk: {extraction_config.max_entities_per_chunk}"
        )
        print(
            f"   ⚖️ Quality threshold: {extraction_config.quality_validation_threshold:.3f}"
        )
        ...
        if (
            extraction_config.entity_confidence_threshold != fallback_entity
            or extraction_config.chunk_size != fallback_chunk
        ):
            print("✅ SUCCESS: Learned parameters differ from fallback constants!")
            print(
                f"   📊 Fallback entity threshold: {fallback_entity:.3f} → Learned: {extraction_config.entity_confidence_threshold:.3f}"
            )
            print(
                f"   📦 Fallback chunk size: {fallback_chunk} → Learned: {extraction_config.chunk_size}"
            )
        else:
            print(
                "⚠️  WARNING: Parameters match fallback constants (may indicate fallback was used)"
            )
        ...
        if stats:
            print(
                f"   📚 Technical vocabulary: {stats.get('technical_vocabulary_size', 0)} terms"
            )
            print(f"   💡 Key concepts: {stats.get('key_concepts_found', 0)} concepts")
            print(f"   🔍 Entity density: {stats.get('entity_density', 0):.4f}")
            print(
                f"   📊 Analysis confidence: {stats.get('analysis_confidence', 0):.2f}"
            )

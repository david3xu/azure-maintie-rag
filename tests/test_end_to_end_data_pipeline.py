...
class TestEndToEndDataPipelineValidation:
    ...
    async def test_complete_pipeline_with_full_dataset(
        ...
        if data_report["suitable_files"] < 10:
            pytest.skip(
                f"Insufficient suitable files for full pipeline testing: {data_report['suitable_files']}"
            )

        # Get representative sample for comprehensive testing
        pipeline_files = enhanced_test_data_manager.get_diverse_test_set(
            count=min(25, data_report["suitable_files"])
        )
        ...
        for i, file_path in enumerate(pipeline_files):
            ...
            try:
                ...
                pipeline_results["stage_performance"]["domain_intelligence"][
                    "successes"
                ] += 1
                pipeline_results["stage_performance"]["domain_intelligence"][
                    "times"
                ...
                pipeline_results["quality_metrics"]["domain_characteristics"].append(
                    {
                        ...
                        "concept_density": domain_characteristics.concept_density,
                        "patterns_count": len(
                            domain_characteristics.structural_patterns
                        ),
                    }
                )
                ...
                print(
                    f"            Vocabulary Complexity: {domain_characteristics.vocabulary_complexity:.3f}"
                )
                print(
                    f"            Concept Density: {domain_characteristics.concept_density:.3f}"
                )
                ...
                pipeline_results["stage_performance"]["knowledge_extraction"][
                    "successes"
                ] += 1
                pipeline_results["stage_performance"]["knowledge_extraction"][
                    "times"
                ...
                print(
                    f"            Confidence: {extraction_result.extraction_confidence:.3f}"
                )
                ...
                if extraction_result.entities:
                    ...
                    pipeline_results["stage_performance"]["universal_search"][
                        "successes"
                    ] += 1
                    pipeline_results["stage_performance"]["universal_search"][
                        "times"
                    ...
                    print(
                        f"            Confidence: {search_result.search_confidence:.3f}"
                    )
                ...
                print(
                    f"      ✅ Complete Pipeline: {file_result['total_processing_time']:.2f}s"
                )
        ...
        successful_files = [
            f for f in pipeline_results["files_processed"] if f["success"]
        ]
        ...
        for stage_name, stage_data in pipeline_results["stage_performance"].items():
            ...
            stage_stats[stage_name] = {
                "success_rate": (
                    stage_data["successes"] / total_attempts
                    if total_attempts > 0
                    else 0
                ),
                "avg_processing_time": (
                    statistics.mean(stage_data["times"]) if stage_data["times"] else 0
                ),
        ...
        if pipeline_results["quality_metrics"]["domain_characteristics"]:
            domain_complexities = [
                d["vocabulary_complexity"]
                for d in pipeline_results["quality_metrics"]["domain_characteristics"]
            ]
            ...
            entities_counts = [
                e["entities_count"]
                for e in pipeline_results["quality_metrics"]["extracted_entities"]
            ]
            ...
            search_counts = [
                s["results_count"]
                for s in pipeline_results["quality_metrics"]["search_results"]
            ]
        ...
        assert (
            stage_stats["domain_intelligence"]["success_rate"] >= 0.85
        ), f"Domain Intelligence success rate too low: {stage_stats['domain_intelligence']['success_rate']:.2%}"
        assert (
            stage_stats["knowledge_extraction"]["success_rate"] >= 0.75
        ), f"Knowledge Extraction success rate too low: {stage_stats['knowledge_extraction']['success_rate']:.2%}"

        # Performance assertions
        if stage_stats["domain_intelligence"]["avg_processing_time"] > 0:
            assert (
                stage_stats["domain_intelligence"]["avg_processing_time"] <= 12.0
            ), f"Domain Intelligence too slow: {stage_stats['domain_intelligence']['avg_processing_time']:.2f}s"
    ...
    async def test_data_quality_and_diversity_validation(
        ...
        for content_type, count in quality_report["content_type_distribution"].items():
            percentage = (
                (count / quality_report["suitable_files"]) * 100
                if quality_report["suitable_files"] > 0
                else 0
            )
            print(
                f"     {content_type.replace('_', ' ').title()}: {count} files ({percentage:.1f}%)"
            )
        ...
        for file_path in diverse_sample:
            ...
            diversity_metrics["complexity_distribution"].append(
        ...
        size_stats = {
            ...
            "std": (
                statistics.stdev(diversity_metrics["size_distribution"])
                if len(diversity_metrics["size_distribution"]) > 1
                else 0
            ),
            "range": max(diversity_metrics["size_distribution"])
            - min(diversity_metrics["size_distribution"]),
        }

        complexity_stats = {
            "mean": statistics.mean(diversity_metrics["complexity_distribution"]),
            "std": (
                statistics.stdev(diversity_metrics["complexity_distribution"])
                if len(diversity_metrics["complexity_distribution"]) > 1
                else 0
            ),
        }
        ...
        for feature, count in diversity_metrics["language_features"].items():
            ...
            print(
                f"       {feature.replace('_', ' ').title()}: {count}/{len(diverse_sample)} ({percentage:.1f}%)"
            )

        # Data quality and diversity assertions
        assert (
            quality_report["suitability_ratio"] >= 0.6
        ), f"Dataset suitability too low: {quality_report['suitability_ratio']:.2%}"
        assert (
            quality_report["suitable_files"] >= 15
        ), f"Insufficient suitable files: {quality_report['suitable_files']}"
        assert (
            len(diversity_metrics["content_types"]) >= 3
        ), f"Insufficient content type diversity: {len(diversity_metrics['content_types'])}"
        assert (
            size_stats["std"] > 500
        ), f"Insufficient size diversity: {size_stats['std']:.0f}"
        assert (
            complexity_stats["mean"] > 0.3
        ), f"Content complexity too low: {complexity_stats['mean']:.3f}"

        # Language feature coverage
        api_coverage = diversity_metrics["language_features"]["has_api_content"] / len(
            diverse_sample
        )
        code_coverage = diversity_metrics["language_features"]["has_code_blocks"] / len(
            diverse_sample
        )
        assert (
            api_coverage >= 0.4
        ), f"Insufficient API content coverage: {api_coverage:.2%}"
        assert (
            code_coverage >= 0.5
        ), f"Insufficient code content coverage: {code_coverage:.2%}"
    ...
    async def test_pipeline_scalability_with_large_dataset(
        ...
        for scenario in scalability_scenarios:
            ...
            batch_files = enhanced_test_data_manager.get_diverse_test_set(
                count=batch_size
            )

            if len(batch_files) < batch_size:
                print(
                    f"   ⚠️  Only {len(batch_files)} files available, proceeding with available files"
                )
            ...
            for i, file_path in enumerate(batch_files):
                ...
                try:
                    ...
                    batch_results["total_relationships"] += len(
                        extraction_result.relationships
                    )

                    print(
                        f"     📄 File {i+1}/{len(batch_files)}: {file_processing_time:.2f}s"
                    )

                except Exception as e:
                    batch_results["errors"].append(
                        {"file": file_path.name, "error": str(e)[:100]}
                    )
                    print(
                        f"     📄 File {i+1}/{len(batch_files)}: ERROR - {str(e)[:50]}"
                    )
            ...
            if successful_files > 0:
                batch_metrics = {
                    ...
                    "avg_file_processing_time": statistics.mean(
                        batch_results["processing_times"]
                    ),
                    "total_entities_extracted": batch_results["total_entities"],
                    "total_relationships_extracted": batch_results[
                        "total_relationships"
                    ],
                    "files_per_minute": (
                        (successful_files / batch_total_time) * 60
                        if batch_total_time > 0
                        else 0
                    ),
                    "avg_vocabulary_complexity": statistics.mean(
                        [
                            r["vocabulary_complexity"]
                            for r in batch_results["domain_analysis_results"]
                        ]
                    ),
                    "avg_entities_per_file": (
                        batch_results["total_entities"] / successful_files
                        if successful_files > 0
                        else 0
                    ),
                }
                ...
                print(
                    f"     Avg File Time: {batch_metrics['avg_file_processing_time']:.2f}s"
                )
                print(f"     Files/Minute: {batch_metrics['files_per_minute']:.1f}")
                print(
                    f"     Total Entities: {batch_metrics['total_entities_extracted']}"
                )
                print(
                    f"     Avg Entities/File: {batch_metrics['avg_entities_per_file']:.1f}"
                )
        ...
        if len(scalability_results) >= 2:
            ...
            for result in scalability_results:
                print(
                    f"     {result['scenario']}: {result['files_per_minute']:.1f} files/minute"
                )

            print(f"   🎯 Quality Scaling:")
            for result in scalability_results:
                print(
                    f"     {result['scenario']}: {result['success_rate']:.2%} success, {result['avg_entities_per_file']:.1f} entities/file"
                )
            ...
            ), f"Success rate degrades too much at scale: {min_success_rate:.2%}"
            assert (
                min_throughput >= 2.0
            ), f"Throughput too low at scale: {min_throughput:.1f} files/minute"
            ...
            if len(scalability_results) >= 2:
                ...
                throughput_degradation = (
                    small_batch_result["files_per_minute"]
                    / large_batch_result["files_per_minute"]
                    if large_batch_result["files_per_minute"] > 0
                    else 1
                )
                quality_degradation = (
                    small_batch_result["success_rate"]
                    / large_batch_result["success_rate"]
                    if large_batch_result["success_rate"] > 0
                    else 1
                )
                ...
                assert (
                    throughput_degradation <= 2.0
                ), f"Throughput degrades too much at scale: {throughput_degradation:.2f}x"
                assert (
                    quality_degradation <= 1.2
                ), f"Quality degrades too much at scale: {quality_degradation:.2f}x"
    ...
    async def test_pipeline_accuracy_with_ground_truth(
        ...
        accuracy_test_files = enhanced_test_data_manager.get_test_files_by_criteria(
            min_size=1000, max_size=4000, content_types=["api", "azure"], limit=10
        )
        ...
        for file_path in accuracy_test_files:
            ...
            file_accuracy = {
                "file_name": file_name,
                "content_size": len(content),
                "ground_truth_indicators": self._extract_ground_truth_indicators(
                    content
                ),
                "results": {},
            }
            ...
            try:
                ...
                vocabulary_reasonable = (
                    0.0 <= domain_characteristics.vocabulary_complexity <= 1.0
                )
                density_reasonable = (
                    0.0 <= domain_characteristics.concept_density <= 1.0
                )
                has_patterns = len(domain_characteristics.structural_patterns) > 0

                domain_accuracy_score = (
                    sum([vocabulary_reasonable, density_reasonable, has_patterns]) / 3
                )
                ...
                accuracy_results["domain_analysis_accuracy"].append(
                    domain_accuracy_score
                )
                ...
                extraction_result = await run_knowledge_extraction(
                    content[:2000], use_domain_analysis=True
                )
                ...
                expected_entities = self._count_expected_entities(
                    content, file_accuracy["ground_truth_indicators"]
                )
                ...
                if expected_entities > 0:
                    ...
                    entity_recall = (
                        1.0 if entities_extracted == 0 else 0.5
                    )  # Partial credit
                ...
                accuracy_results["entity_extraction_accuracy"].append(
                    extraction_accuracy_score
                )

                print(f"      🔍 Extraction Accuracy: {extraction_accuracy_score:.3f}")
                print(
                    f"         Entities: {entities_extracted} (expected ~{expected_entities})"
                )
        ...
        if accuracy_results["domain_analysis_accuracy"]:
            domain_avg_accuracy = statistics.mean(
                accuracy_results["domain_analysis_accuracy"]
            )
            print(f"\n   🧠 Domain Analysis Avg Accuracy: {domain_avg_accuracy:.3f}")

        if accuracy_results["entity_extraction_accuracy"]:
            extraction_avg_accuracy = statistics.mean(
                accuracy_results["entity_extraction_accuracy"]
            )
            print(
                f"   🔍 Entity Extraction Avg Accuracy: {extraction_avg_accuracy:.3f}"
            )

        if accuracy_results["search_relevance_scores"]:
            search_avg_relevance = statistics.mean(
                accuracy_results["search_relevance_scores"]
            )
            print(f"   🔎 Search Avg Relevance: {search_avg_relevance:.3f}")

        print(f"\n✅ Pipeline Accuracy Summary:")
        files_with_results = len(
            [f for f in accuracy_results["files_tested"] if "error" not in f]
        )
        print(
            f"   Files Successfully Tested: {files_with_results}/{len(accuracy_test_files)}"
        )
        ...
        if accuracy_results["entity_extraction_accuracy"]:
            overall_accuracy_scores.extend(
                accuracy_results["entity_extraction_accuracy"]
            )
        if accuracy_results["search_relevance_scores"]:
        ...
        if overall_accuracy_scores:
            ...
            ), f"Overall pipeline accuracy too low: {overall_accuracy:.3f}"
            assert (
                domain_avg_accuracy >= 0.8
            ), f"Domain analysis accuracy too low: {domain_avg_accuracy:.3f}"
            assert (
                extraction_avg_accuracy >= 0.6
            ), f"Entity extraction accuracy too low: {extraction_avg_accuracy:.3f}"
    ...
    def _extract_ground_truth_indicators(self, content: str) -> Dict[str, Any]:
        ...
        if any(word in content_lower for word in ["tutorial", "how to", "guide"]):
            ...
            word in content_lower for word in ["concept", "overview", "understand"]
        ):
    ...
    def _evaluate_search_relevance(
        self, query: str, search_result, ground_truth: Dict[str, Any]
    ) -> float:
        ...
        results_factor = min(
            len(search_result.unified_results) / 5.0, 1.0
        )  # Normalize to max 5 results
        relevance_factors.append(results_factor)

        # Factor 3: Strategy appropriateness
        strategy_score = (
            0.8
            if search_result.search_strategy_used in ["vector_search", "hybrid_search"]
            else 0.5
        )

...
REAL_CONTENT_SAMPLES = {
    "programming": {
        ...
        """,
        "language": "en",
        "expected_entities": ["Object-oriented programming", "OOP", "objects", "Classes", "Inheritance", "Polymorphism", "Encapsulation"],
    ...
    "legal": {
        ...
        """,
        "language": "en", 
        "expected_entities": ["Contract law", "agreements", "offer", "acceptance", "consideration", "breach", "Damages"],
    ...
    "medical": {
        ...
        """,
        "language": "en",
        "expected_entities": ["Cardiovascular disease", "heart", "blood vessels", "hypertension", "diabetes", "medication"],
    ...
    "spanish_technical": {
        ...
        """,
        "language": "es",
        "expected_entities": ["inteligencia artificial", "algoritmos", "aprendizaje automático", "redes neuronales", "procesamiento de lenguaje natural"],
    ...
    "business": {
        ...
        """,
        "language": "en",
        "expected_entities": ["Business intelligence", "data mining", "Cloud computing", "Artificial intelligence"],
...
class TestUniversalContentProcessing:
    ...
    async def test_multilingual_entity_extraction(self):
        ...
        for content_name, content_data in REAL_CONTENT_SAMPLES.items():
            ...
            assert any(len(entity.split()) > 1 for entity in entity_texts)  # Multi-word entities
        ...
        print(f"   Total entities: {sum(r['entity_count'] for r in extraction_results.values())}")
    ...
    async def test_real_data_processing_pipeline(self):
        ...
        avg_time = sum(r["total_time"] for r in pipeline_results) / len(pipeline_results)
    ...
    async def test_content_adaptation_without_configuration(self):
        ...
        print(f"   Average adaptation time: {sum(r['adaptation_time'] for r in adaptation_results.values()) / len(adaptation_results):.2f}s")
    ...
    async def test_cross_domain_knowledge_integration(self):
        ...
        tech_terms = any(term in ' '.join(entity_texts) for term in ['machine learning', 'algorithms', 'software', 'ai'])
        
        # Should find medical terms  
        medical_terms = any(term in ' '.join(entity_texts) for term in ['medical', 'healthcare', 'diagnosis'])
        
        # Should find legal terms
        legal_terms = any(term in ' '.join(entity_texts) for term in ['legal', 'regulations', 'privacy'])
        ...
        relationship_texts = [f"{r.subject} {r.predicate} {r.object}" for r in extraction_result.relationships]

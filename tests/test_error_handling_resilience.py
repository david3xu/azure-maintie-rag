...
class TestErrorHandlingAndResilience:
    ...
    async def test_azure_service_failure_resilience(
        self, azure_services, comprehensive_azure_health_monitor
    ):
        ...
        for scenario in failure_scenarios:
            ...
            try:
                if scenario["name"] == "api_timeout":
                    ...
                    with patch(
                        ...
                        try:
                            result = await run_domain_analysis(
                                "Test content for timeout scenario"
                            )
                    ...
                    scenario_result["successful_rate_limited_requests"] = (
                        successful_requests
                    )
                    scenario_result["error_handled_gracefully"] = (
                        successful_requests > 0
                    )

                    print(
                        f"      📊 Rapid requests: {successful_requests}/5 successful"
                    )
                    ...
                    try:
                        ...
                        scenario_result["error_handled_gracefully"] = True
                        print(
                            f"      ✅ Auth-related error handled: {type(e).__name__}"
                        )
                    ...
                    handled_gracefully = sum(
                        1 for r in malformed_results if r["success"] or "error" in r
                    )
                    scenario_result["error_handled_gracefully"] = handled_gracefully > 0

                    print(
                        f"      📊 Malformed input tests: {handled_gracefully}/{len(malformed_inputs)} handled"
                    )
        ...
        graceful_handling_count = sum(
            1 for r in resilience_results if r["error_handled_gracefully"]
        )
        ...
        print(
            f"   Graceful Error Handling: {graceful_handling_count}/{len(resilience_results)}"
        )
        ...
        ), f"System resilience too low: {resilience_score:.2%}"
    ...
    async def test_agent_error_handling_and_recovery(
        ...
        agent_scenarios = {
            ...
            "knowledge_extraction": [
                {
                    "input": "No entities here whatsoever just plain text",
                    "description": "no extractable entities",
                },
                {
                    "input": "A B C D E F G " * 500,
                    "description": "repetitive short words",
                },
                {
                ...
                {
                    "input": "AAAAAAA" * 2000,
                    "description": "single character repetition",
                },
            ...
            "universal_search": [
                ...
                {
                    "input": "ZZZZZZZZZZZZZZZZZZ",
                    "description": "likely no results query",
                },
            ],
        }
        ...
        for agent_name, scenarios in agent_scenarios.items():
            ...
            for scenario in scenarios:
                ...
                scenario_result = {
                    ...
                    "input_type": (
                        type(test_input).__name__ if test_input is not None else "None"
                    ),
                    "input_length": (
                        len(str(test_input)) if test_input is not None else 0
                    ),
                ...
                try:
                    ...
                    if agent_name == "domain_intelligence" and result:
                        scenario_result["vocab_complexity"] = (
                            result.discovered_characteristics.vocabulary_complexity
                        )
                    ...
                    print(
                        f"      ✅ {description}: handled gracefully ({execution_time:.2f}s)"
                    )
                    ...
                    if any(
                        expected in type(e).__name__ for expected in expected_errors
                    ):
                        scenario_result["handled_gracefully"] = True
                        agent_results["successful_error_handling"] += 1
                        print(
                            f"      ✅ {description}: expected error handled ({type(e).__name__})"
                        )
                    else:
                        print(
                            f"      ⚠️  {description}: unexpected error ({type(e).__name__}: {str(e)[:50]})"
                        )
            ...
            success_rate = (
                agent_results["successful_error_handling"]
                / agent_results["total_scenarios"]
            )
            print(f"      📊 Error Handling Success Rate: {success_rate:.2%}")

        # Overall agent error handling analysis
        total_scenarios = sum(
            results["total_scenarios"] for results in agent_error_results.values()
        )
        total_successful = sum(
            results["successful_error_handling"]
            for results in agent_error_results.values()
        )
        overall_success_rate = (
            total_successful / total_scenarios if total_scenarios > 0 else 0
        )
        ...
        for agent_name, results in agent_error_results.items():
            agent_success_rate = (
                results["successful_error_handling"] / results["total_scenarios"]
            )
            print(
                f"   {agent_name.replace('_', ' ').title()}: {agent_success_rate:.2%}"
            )

        # Error handling assertions
        assert (
            overall_success_rate >= 0.7
        ), f"Agent error handling success rate too low: {overall_success_rate:.2%}"
        ...
        for agent_name, results in agent_error_results.items():
            ...
            ), f"{agent_name} error handling too low: {agent_success_rate:.2%}"
    ...
    async def test_data_quality_error_handling(
        ...
        problematic_data_scenarios = [
            ...
            {
                "name": "no_structure",
                "content": "nospacesnopunctuationnostructureatallinthisdocumentjustonebiglongstringoftext"
                * 50,
            ...
            {
                "name": "extreme_length",
                "content": "This is an extremely long sentence that goes on and on and on without any clear structure or meaningful content just to test how the system handles very long continuous text "
                * 200,
            ...
            try:
                ...
                try:
                    extraction_result = await run_knowledge_extraction(
                        scenario["content"][:2000], use_domain_analysis=False
                    )
                    ...
                    print(
                        f"         ✅ Knowledge Extraction: {len(extraction_result.entities)} entities"
                    )
                    ...
                    print(
                        f"         ❌ Knowledge Extraction failed: {type(e).__name__}"
                    )

                # Stage 3: Search (if extraction succeeded)
                if (
                    scenario_result["pipeline_stages"]
                    ...
                    try:
                        ...
                        print(
                            f"         ✅ Universal Search: {len(search_result.unified_results)} results"
                        )
                        ...
                        print(
                            f"         ❌ Universal Search failed: {type(e).__name__}"
                        )

                # Determine overall success
                successful_stages = sum(
                    1
                    for stage in scenario_result["pipeline_stages"].values()
                    if stage.get("success")
                )
                scenario_result["successful_stages"] = successful_stages
                scenario_result["overall_success"] = (
                    successful_stages >= 1
                )  # At least one stage should succeed
        ...
        successful_scenarios = sum(
            1 for r in data_quality_results if r["overall_success"]
        )
        ...
        for stage_name in [
            ...
            stage_successes = sum(
                1
                for r in data_quality_results
                if r["pipeline_stages"].get(stage_name, {}).get("success", False)
            )
            stage_attempts = sum(
                1 for r in data_quality_results if stage_name in r["pipeline_stages"]
            )
            stage_success_rates[stage_name] = (
                stage_successes / stage_attempts if stage_attempts > 0 else 0
            )
        ...
        for stage_name, success_rate_stage in stage_success_rates.items():
            print(
                f"     {stage_name.replace('_', ' ').title()}: {success_rate_stage:.2%}"
            )
        ...
        assert (
            success_rate >= 0.5
        ), f"Data quality error handling success rate too low: {success_rate:.2%}"
        assert (
            stage_success_rates["domain_analysis"] >= 0.6
        ), f"Domain analysis too fragile with poor data: {stage_success_rates['domain_analysis']:.2%}"

        return data_quality_results

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_network_timeout_scenarios(self, azure_services):
        ...
        timeout_scenarios = [
            {
                "name": "short_timeout",
                "timeout": 1.0,
                "description": "Very short timeout (1s)",
            },
            {
                "name": "medium_timeout",
                "timeout": 5.0,
                "description": "Medium timeout (5s)",
            },
            {
                "name": "long_timeout",
                "timeout": 30.0,
                "description": "Long timeout (30s)",
            },
        ]
        ...
        for scenario in timeout_scenarios:
            ...
            test_operations = [
                {
                    ...
                    "content": "Test content for timeout analysis",
                },
                {
                    "name": "knowledge_extraction",
                    "content": "Test content with entities like Azure OpenAI",
                },
            ...
            for operation in test_operations:
                ...
                try:
                    if operation["name"] == "domain_analysis":
                        ...
                        result = await asyncio.wait_for(
                            run_knowledge_extraction(
                                operation["content"], use_domain_analysis=False
                            ),
                            timeout=scenario["timeout"],
                        )
                    ...
                    operation_result["completed_within_timeout"] = (
                    ...
                    print(
                        f"      ✅ {operation['name']}: {execution_time:.2f}s (within {scenario['timeout']}s)"
                    )
                    ...
                    print(
                        f"      ⏱️  {operation['name']}: timeout after {execution_time:.2f}s"
                    )
        ...
        for result in timeout_results:
            ...
            success_rate = (
                result["successful_operations"] / total_ops if total_ops > 0 else 0
            )
            timeout_rate = (
                result["timeouts_handled"] / total_ops if total_ops > 0 else 0
            )

            print(f"   {result['scenario']} ({result['timeout_duration']}s timeout):")
            print(
                f"     Successful Operations: {result['successful_operations']}/{total_ops} ({success_rate:.2%})"
            )
            print(
                f"     Timeouts Handled: {result['timeouts_handled']}/{total_ops} ({timeout_rate:.2%})"
            )

        # Timeout handling assertions
        long_timeout_result = next(
            r for r in timeout_results if r["scenario"] == "long_timeout"
        )
        long_timeout_success_rate = long_timeout_result["successful_operations"] / len(
            long_timeout_result["operations_tested"]
        )

        assert (
            long_timeout_success_rate >= 0.8
        ), f"Too many timeouts even with long timeout: {long_timeout_success_rate:.2%}"

        # System should handle timeouts gracefully (not crash)
        total_timeout_operations = sum(
            len(r["operations_tested"]) for r in timeout_results
        )
    ...
    async def test_graceful_degradation_patterns(
        ...
        for scenario in degradation_scenarios:
            ...
            for i, file_path in enumerate(
                test_files[:3]
            ):  # Test with 3 files per scenario
                ...
                try:
                    ...
                    extraction_result = await run_knowledge_extraction(
                        limited_content, use_domain_analysis=False
                    )
                    ...
                    if extraction_result.entities:
                        ...
                        search_result = await run_universal_search(
                            search_query, max_results=scenario["max_results"]
                        )
                    ...
                    file_result["processing_successful"] = (
                        len(file_result["stages_completed"]) >= 2
                    )
                ...
                status = (
                    "✅" if file_result.get("processing_successful", False) else "❌"
                )
                print(
                    f"      📄 {file_path.name}: {status} ({len(file_result['stages_completed'])} stages)"
                )

            # Calculate degradation quality metrics
            successful_files = [
                f
                for f in scenario_result["files_processed"]
                if f.get("processing_successful", False)
            ]

            if successful_files:
                scenario_result["quality_metrics"] = {
                    "success_rate": len(successful_files)
                    / len(scenario_result["files_processed"]),
                    "avg_stages_completed": sum(
                        len(f["stages_completed"]) for f in successful_files
                    )
                    / len(successful_files),
                    "avg_processing_efficiency": sum(
                        f["processed_size"] / f["original_size"]
        ...
        overall_degradation_successful = all(
            r["degradation_successful"] for r in degradation_results
        )
        ...
        for result in degradation_results:
            ...
            print(
                f"     Degradation Handled: {'✅' if result['degradation_successful'] else '❌'}"
            )
        ...
        for result in degradation_results:
            ...
            assert (
                success_rate >= 0.5
            ), f"Degradation scenario {result['scenario']} success rate too low: {success_rate:.2%}"

        return degradation_results

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_system_recovery_after_failures(self, azure_services):
        ...
        for scenario in recovery_scenarios:
            ...
            try:
                if scenario["name"] == "service_restart_simulation":
                    ...
                    scenario_result["post_recovery_functionality"][
                        "domain_analysis"
                    ...
                    scenario_result["recovery_successful"] = (
                        same_instance and test_result is not None
                    )
                    scenario_result["post_recovery_functionality"][
                        "dependency_reuse"
                    ] = same_instance
                    ...
                    operations = [
                        ...
                        (
                            "knowledge_extraction",
                            "State test with entities Azure OpenAI",
                        ),
                    ]
                    ...
                    for op_name, content in operations:
                        try:
                            if op_name == "domain_analysis":
                                ...
                                result = await run_knowledge_extraction(
                                    content, use_domain_analysis=False
                                )
                    ...
                    successful_operations = sum(
                        1 for op in operation_results if op["success"]
                    )
                    scenario_result["recovery_successful"] = (
        ...
        avg_recovery_time = sum(r["recovery_time"] for r in recovery_results) / len(
            recovery_results
        )
        ...
        print(
            f"   Successful Recoveries: {successful_recoveries}/{len(recovery_results)}"
        )
        ...
        ), f"System recovery success rate too low: {recovery_success_rate:.2%}"
        assert (
        ...
        critical_scenarios = [
            ...
            "dependency_reinitialization",
        ]
        for scenario_name in critical_scenarios:
            scenario_result = next(
                (r for r in recovery_results if r["scenario"] == scenario_name), None
            )
            if scenario_result:
                assert scenario_result[
                    "recovery_successful"
                ], f"Critical recovery scenario failed: {scenario_name}"

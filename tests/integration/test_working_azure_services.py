...
class TestWorkingAzureServices:
    ...
    async def test_azure_services_partial_connectivity(self, azure_services):
        ...
        print(
            f"✅ Azure Services Status: {status['successful_services']}/{status['total_services']} services working"
        )
    ...
    async def test_ai_foundry_connectivity(self, azure_services):
        ...
        if (
            not hasattr(azure_services, "ai_foundry_client")
            or not azure_services.ai_foundry_client
        ):
        ...
        try:
            result = await azure_services.ai_foundry_client.get_completion(
                "Test prompt for Azure integration testing", max_tokens=10
            )
            ...
            if result.get("success"):
                ...
                print(
                    f"⚠️ AI Foundry service returned error: {result.get('error', 'Unknown error')}"
                )
    ...
    async def test_service_health_monitoring_working(self, azure_services):
        ...
        working_count = sum(
            1 for service in expected_working if services_init.get(service, False)
        )
        assert (
            working_count >= 1
        ), f"Expected at least 1 working service, got {working_count}"
...
class TestAgentInitializationWithPartialAzure:
    ...
    @pytest.mark.asyncio
    async def test_knowledge_extraction_agent_initialization(
        self, knowledge_extraction_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_agents_graceful_degradation(
        ...
        try:
            ...
            print(
                f"📊 Knowledge Extraction: {'✅ Success' if ke_result and ke_result.get('success') else '⚠️ Graceful failure'}"
            )
        except Exception as e:
        ...
        try:
            ...
            search_result = await universal_search_agent.process_query(
                {"query": test_query["query"], "limit": 3}
            )
            print(
                f"🔍 Universal Search: {'✅ Success' if search_result and search_result.get('success') else '⚠️ Graceful failure'}"
            )
        except Exception as e:

...
class TestKnowledgeExtractionAgentAzure:
    ...
    @pytest.mark.asyncio
    async def test_document_processing_real_azure(
        ...
        async with performance_monitor.measure_operation(
            "knowledge_extraction", sla_target=3.0
        ):
    ...
    @pytest.mark.asyncio
    async def test_cross_domain_processing(
        ...
        for domain in domains_to_test:
            ...
            try:
                ...
                print(
                    f"✅ {domain.title()} domain processing: "
        ...
        print(
            f"📊 Cross-domain results: {successful_domains}/{len(results)} domains successful"
        )
...
class TestUniversalSearchAgentAzure:
    ...
    @pytest.mark.asyncio
    async def test_search_functionality_real_azure(
        ...
        for query in search_queries:
            async with performance_monitor.measure_operation(
                f"search_{query.replace(' ', '_')}", sla_target=3.0
            ):
                ...
                if result.get("success"):
                    ...
                    print(
                        f"⚠️ Search '{query}' failed: {result.get('error', 'Unknown error')}"
                    )
    ...
    async def test_search_performance_sla(
        ...
        for query in test_queries:
            async with performance_monitor.measure_operation(
                f"sla_test_{hash(query) % 1000}", sla_target=3.0
            ):
                result = await universal_search_agent.process_query(
                    {"query": query, "limit": 3}
                )
    ...
    async def test_concurrent_search_operations(self, universal_search_agent):
        ...
        tasks = [
            universal_search_agent.process_query({"query": query, "limit": 2})
        ...
        try:
            ...
            ), f"All concurrent searches failed: {results}"

            print(
                f"✅ Concurrent searches: {completed_operations}/{len(results)} completed"
            )
...
class TestAgentIntegration:
    ...
    @pytest.mark.asyncio
    async def test_knowledge_to_search_workflow(
        self, knowledge_extraction_agent, universal_search_agent, sample_documents
    ):
        ...
        if not extraction_result.get("success"):
            pytest.skip(
                f"Knowledge extraction failed: {extraction_result.get('error')}"
            )
        ...
        search_query = (
            entities[0].get("text", "programming") if entities else "programming"
        )

        search_result = await universal_search_agent.process_query(
            {"query": f"Find information about {search_query}", "limit": 3}
        )
    ...
    async def test_error_propagation_across_agents(
        self, knowledge_extraction_agent, universal_search_agent
    ):

...
class TestRealDataAvailability:
    ...
    def test_azure_ai_data_directory_exists(self):
        ...
        data_dir = Path(__file__).parent.parent / "data" / "raw" / "azure-ai-services-language-service_output"
    ...
    def test_azure_ai_test_files_availability(self, test_data_directory):
        ...
        assert len(markdown_files) > 0, "No Azure AI markdown files found in test data directory"
        ...
        assert len(readable_files) >= 10, f"Expected at least 10 readable files, found {len(readable_files)}"
    ...
    def test_azure_ai_content_quality(self, test_data_quality_report):
        ...
        assert report['total_files'] >= 10, f"Expected at least 10 files, found {report['total_files']}"
        assert report['quality_ratio'] >= 0.6, f"Quality ratio too low: {report['quality_ratio']:.2f}"
        assert report['average_size_chars'] >= 1000, f"Files too small on average: {report['average_size_chars']:.0f}"
        ...
        print(f"   Files with Tutorial Content: {report['files_with_tutorial_content']}")
    ...
    def test_content_diversity_analysis(self, azure_ai_test_files):
        ...
        for file_path in azure_ai_test_files[:10]:  # Analyze first 10 files
            ...
            if any(term in content for term in ['api', 'endpoint', 'request', 'response', 'method']):
                content_types['api_documentation'] += 1
            if any(term in content for term in ['how to', 'tutorial', 'step', 'guide', 'walkthrough']):
                content_types['how_to_guides'] += 1
            if any(term in content for term in ['```', 'code', 'example', 'sample']):
                content_types['code_examples'] += 1
            if any(term in content for term in ['concept', 'overview', 'introduction', 'understand']):
                content_types['conceptual_content'] += 1
            if any(term in content for term in ['reference', 'parameter', 'property', 'field']):
        ...
        assert diverse_types >= 3, f"Content not diverse enough: {diverse_types} types found"
        assert 0.1 <= avg_vocab_complexity <= 0.9, f"Vocabulary complexity out of range: {avg_vocab_complexity:.3f}"
...
class TestDomainIntelligenceWithRealData:
    ...
    async def test_domain_analysis_on_real_files(self, azure_ai_test_files):
        ...
        assert processed_files >= 3, f"Expected to process at least 3 files, processed {processed_files}"
        ...
        print(f"   Complexity Range: {min(vocab_complexities):.3f} - {max(vocab_complexities):.3f}")
    ...
    @pytest.mark.azure
    @pytest.mark.asyncio
    async def test_domain_analysis_performance_with_real_data(self, azure_ai_test_files):
        ...
        for file_path in azure_ai_test_files[:3]:  # Test with first 3 files
            ...
            assert processing_time < 20.0, f"File {file_path.name} took too long: {processing_time:.2f}s"
        ...
        print(f"   All files < 20s target: {'✅' if max(processing_times) < 20 else '❌'}")
...
class TestKnowledgeExtractionWithRealData:
    ...
    async def test_knowledge_extraction_on_real_files(self, azure_ai_test_files):
        ...
        for file_path in azure_ai_test_files[:3]:  # Process first 3 files
            try:
                ...
                extraction_results.append({
                    ...
                    'entities': [entity.text for entity in extraction.entities[:5]],  # First 5 entities
                    'relationships': [rel.relationship_type for rel in extraction.relationships[:3]]  # First 3 relationships
                })
        ...
        assert processed_files >= 2, f"Expected to process at least 2 files, processed {processed_files}"
    ...
    async def test_knowledge_extraction_accuracy_validation(self, azure_ai_test_files):
        ...
        result = await knowledge_extraction_agent.run(
            f"Extract entities and relationships from: {content[:1000]}",
            deps=deps
        )
...
class TestUniversalSearchWithRealData:
    ...
    async def test_universal_search_on_real_queries(self, azure_ai_test_files):
        ...
        for query in search_queries[:2]:  # Test with first 2 queries
            try:
                result = await universal_search_agent.run(
                    f"Execute universal search: {query}",
                    deps=deps
                )
        ...
        assert processed_queries >= 1, f"Expected at least 1 successful search, got {processed_queries}"
...
class TestDataPipelineIntegration:
    ...
    async def test_complete_pipeline_with_real_file(self, azure_ai_test_files):
        ...
        extraction_result = await knowledge_extraction_agent.run(
            f"Extract entities and relationships: {content[:1500]}",
            deps=deps
        )
        extraction_time = time.time() - start_time
        
        assert isinstance(extraction_result.output, ExtractionResult)
        print(f"   ✅ Knowledge Extraction: {extraction_time:.2f}s ({len(extraction_result.output.entities)} entities)")
        ...
        total_time = domain_time + extraction_time + (search_time if 'search_time' in locals() else 0)
        
        print("✅ Complete Pipeline: Successful end-to-end processing")
        print(f"   Total Pipeline Time: {total_time:.2f}s")
        print(f"   File: {test_file.name}")
        print(f"   Vocab Complexity: {domain_analysis.discovered_characteristics.vocabulary_complexity:.3f}")
    ...
    async def test_pipeline_performance_benchmarks(self, azure_ai_test_files):
        ...
        for file_path in azure_ai_test_files[:2]:  # Test with first 2 files
            ...
            performance_results.append({
                ...
                'meets_sla': total_time < 30.0  # 30 second SLA for individual file processing
            })
        ...
        print(f"   SLA Compliant Files: {sla_compliant_files}/{len(performance_results)}")
        
        for result in performance_results:
            print(f"   {result['file']}: {result['total_time']:.2f}s ({'✅' if result['meets_sla'] else '❌'})")
        
        # At least 50% of files should meet SLA
        sla_ratio = sla_compliant_files / len(performance_results)
        assert sla_ratio >= 0.5, f"Too many files exceed SLA: {sla_ratio:.2f} compliance rate"
...
class TestSLACompliance:
    ...
    @pytest.mark.asyncio
    async def test_search_response_time_sla(
        self, universal_search_agent, performance_monitor
    ):
        ...
        for query in test_queries:
            async with performance_monitor.measure_operation(
                f"search_sla_{hash(query) % 1000}", sla_target=3.0
            ) as measurement:
                ...
                if duration > 3.0:
                    sla_violations.append(
                        {"query": query, "duration": duration, "result": result}
                    )

                print(
                    f"⏱️  Query '{query[:30]}...': {duration:.3f}s "
        ...
        print(
            f"📊 SLA Compliance: {compliance_rate:.1f}% ({total_queries - violations}/{total_queries})"
        )

        # Fail if more than 10% SLA violations
        assert (
            compliance_rate >= 90.0
        ), f"SLA compliance too low: {compliance_rate:.1f}% (violations: {sla_violations})"

    @pytest.mark.asyncio
    async def test_knowledge_extraction_sla(
        self, knowledge_extraction_agent, sample_documents, performance_monitor
    ):
        ...
        for domain, document in sample_documents.items():
            async with performance_monitor.measure_operation(
                f"extraction_sla_{domain}", sla_target=3.0
            ):
                start_time = time.time()
                ...
                print(
                    f"⏱️  {domain.title()} extraction: {duration:.3f}s "
                    f"({'✅ SLA OK' if duration <= 3.0 else '❌ SLA VIOLATION'})"
                )
        ...
        print(
            f"📊 Extraction SLA Compliance: {compliance_rate:.1f}% ({total_extractions - violations}/{total_extractions})"
        )

        # Knowledge extraction is more complex, allow slightly lower compliance
        assert (
            compliance_rate >= 80.0
        ), f"Extraction SLA compliance too low: {compliance_rate:.1f}% (violations: {sla_violations})"

    @pytest.mark.asyncio
    async def test_concurrent_operations_sla(
        self, universal_search_agent, performance_monitor
    ):
        ...
        concurrent_queries = [
            f"concurrent search query {i}" for i in range(5)  # 5 concurrent operations
        ]

        async def execute_search_with_timing(query: str, query_id: int):
            """Execute search with individual timing"""
            async with performance_monitor.measure_operation(
                f"concurrent_sla_{query_id}", sla_target=3.0
            ):
                start_time = time.time()
        ...
        assert (
            len(successful_operations) > 0
        ), f"All concurrent operations failed: {results}"

        # Check individual SLA compliance
        sla_compliant_ops = [
            op for op in successful_operations if op.get("sla_compliant", False)
        ]
        ...
        ), f"Concurrent SLA compliance too low: {compliance_rate:.1f}%"

    @pytest.mark.asyncio
    async def test_azure_service_response_times(
        self, azure_services, performance_monitor
    ):
        ...
        if azure_services.openai_client:
            async with performance_monitor.measure_operation(
                "azure_openai_sla", sla_target=2.0
            ):
                start_time = time.time()

                result = await azure_services.openai_client.get_completion(
                    "Test prompt for timing", max_tokens=20
                )
        ...
        if azure_services.search_client:
            async with performance_monitor.measure_operation(
                "azure_search_sla", sla_target=1.0
            ):
                start_time = time.time()
        ...
        for service, timing in service_timings.items():
            print(
                f"   {service.title()}: {timing['duration']:.3f}s "
        ...
        assert (
            len(compliant_services) > 0
        ), f"No Azure services meeting SLA: {service_timings}"
...
class TestPerformanceDegradation:
    ...
    async def test_large_document_processing(
        self, knowledge_extraction_agent, performance_monitor
    ):
        ...
        for size_multiplier in document_sizes:
            ...
            async with performance_monitor.measure_operation(
                f"large_doc_{size_multiplier}x", sla_target=sla_target
            ):
                start_time = time.time()
                ...
                print(
                    f"📄 Large document ({document_size_kb:.1f}KB): {duration:.3f}s "
    ...
    async def test_rapid_sequential_requests(
        ...
        assert (
            avg_duration <= 3.5
        ), f"Average request time too high: {avg_duration:.3f}s"
        assert (
            max_duration <= 5.0
        ), f"Maximum request time too high: {max_duration:.3f}s"

...
class TestDataDrivenDomainDetection:
    ...
    async def test_statistical_domain_analysis(self, sample_documents, azure_services):
            ...
            if result and result.get("success"):
                ...
                if ":" in response_text:
                    try:
                        ...
                        print(f"📊 {expected_domain.title()}: "
                              f"detected='{detected_domain.strip()}', "
                              f"confidence={confidence:.3f}, "
                              f"time={analysis_time:.3f}s")
                              
                    except (ValueError, IndexError) as e:
                        print(f"⚠️ Could not parse domain analysis for {expected_domain}: {response_text}")
                        domain_results[expected_domain] = {
        ...
        confident_results = [
            result for result in domain_results.values()
            if result.get("confidence", 0) > 0.5
        ]
        
        confidence_rate = len(confident_results) / len(domain_results) * 100
        print(f"🎯 Domain Detection Results: {confidence_rate:.1f}% high-confidence detections")
        
        # Validate at least some domains detected with confidence
        assert confidence_rate >= 50.0, f"Too few confident domain detections: {confidence_rate:.1f}%"
        
        return domain_results
    
    @pytest.mark.asyncio
    async def test_cross_domain_vocabulary_analysis(self, sample_documents, azure_services):
        ...
        successful_analyses = [
            analysis for analysis in vocabulary_analysis.values()
        ...
        assert success_rate >= 75.0, f"Vocabulary analysis success rate too low: {success_rate:.1f}%"
    ...
    async def test_domain_confidence_thresholds(self, sample_documents, azure_services):
        ...
        for domain, document in sample_documents.items():
            ...
            if result and result.get("success"):
                try:
                    ...
                    print(f"⚠️ Could not parse confidence for {domain}: {confidence_text}")
        ...
        if confidence_scores:
            ...
            p25 = sorted_confidences[len(sorted_confidences) // 4] if len(sorted_confidences) >= 4 else min_confidence
            p75 = sorted_confidences[3 * len(sorted_confidences) // 4] if len(sorted_confidences) >= 4 else max_confidence
            ...
            assert 0.0 <= avg_confidence <= 1.0, f"Invalid average confidence: {avg_confidence}"
            assert p25 <= p75, f"Invalid percentile ordering: {p25} > {p75}"
            assert len(confidences) >= 2, f"Insufficient confidence samples: {len(confidences)}"
...
class TestUniversalDomainProcessing:
    ...
    async def test_unknown_domain_handling(self, azure_services):
        ...
        if result and result.get("success"):
            ...
            try:
                ...
                assert len(domain_percentages) >= 2, f"Should detect multiple domains: {domain_percentages}"
                
                total_percentage = sum(domain_percentages.values())
                assert 80 <= total_percentage <= 120, f"Domain percentages should sum to ~100%: {total_percentage}"
    ...
    @pytest.mark.asyncio
    async def test_domain_agnostic_processing(self, knowledge_extraction_agent, sample_documents):
        ...
        for domain, document in sample_documents.items():
            ...
            processing_results[domain] = {
                ...
                "has_entities": bool(result.get("knowledge", {}).get("entities", [])) if result else False,
                "has_relationships": bool(result.get("knowledge", {}).get("relationships", [])) if result else False,
                "universal_processing": True
            }
            
            print(f"🌐 {domain.title()} universal processing: "
                  f"{'✅ Success' if processing_results[domain]['success'] else '❌ Failed'} "
                  f"({processing_time:.3f}s)")
        
        # Validate universal behavior
        successful_domains = [
            domain for domain, result in processing_results.items()
            if result["success"]
        ]
        ...
        assert success_rate >= 75.0, f"Universal processing success rate too low: {success_rate:.1f}%"
        
        # Check processing time consistency (no domain should be significantly slower)
        processing_times = [result["processing_time"] for result in processing_results.values()]
        ...
        assert time_consistency, f"Processing time inconsistency: max={max_time:.3f}s, avg={avg_time:.3f}s"
        
        print(f"⏱️  Processing time consistency: avg={avg_time:.3f}s, max={max_time:.3f}s")

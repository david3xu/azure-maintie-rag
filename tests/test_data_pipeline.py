...
class TestRealDataAvailability:
    ...
    def test_data_directory_exists(self):
        ...
        assert len(azure_files) >= 10, f"Expected 10+ Azure AI files for testing, found {len(azure_files)}"
    ...
    def test_data_file_content_quality(self):
        ...
        for file_path in azure_files:
            try:
                ...
                assert any(keyword in content_lower for keyword in [
                    'azure', 'microsoft', 'cognitive', 'api', 'language', 'ai'
        ...
        print(f"✅ Real Data Quality: {valid_files}/{len(azure_files)} files with substantial content")
    ...
    def test_data_content_diversity(self):
        ...
        for file_path in azure_files:
            ...
            if any(keyword in content for keyword in ['concept', 'overview', 'introduction']):
                content_types['concepts'] += 1
            if any(keyword in content for keyword in ['quickstart', 'getting started', 'quick']):
...
class TestRealAzureDataIngestion:
    ...
    async def test_azure_storage_upload_real_data(self):
        ...
        try:
            # Get real test file
            data_dir = Path("/workspace/azure-maintie-rag/data/raw")
            test_files = list(data_dir.rglob("*.md"))[:3]  # Upload first 3 files as test
            ...
            print(f"✅ Real Data Upload: Successfully uploaded {len(uploaded_files)} files to Azure Storage")
...
class TestRealAzureSearch:
    ...
    async def test_azure_search_index_creation(self):
        ...
        try:
            ...
            print(f"✅ Azure Search: Document indexed and searchable, found {len(result_list)} results")
    ...
    async def test_real_content_analysis(self):
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "Analyze this Azure AI documentation and extract key topics, entities, and concepts. Respond in JSON format."},
            ...
            print(f"✅ Real Content Analysis: Processed {len(content)} chars from {test_file.name}")
    ...
    async def test_real_embeddings_generation(self):
        ...
        try:
            ...
            for test_file in test_files:
                content = test_file.read_text(encoding='utf-8')[:2000]  # First 2000 chars
            ...
            print(f"✅ Real Embeddings: Generated embeddings for {embeddings_generated} Azure AI documents")
...
class TestRealDataPipelineIntegration:
    ...
    async def test_azure_state_check_with_real_services(self):
        ...
        ], capture_output=True, text=True, cwd="/workspace/azure-maintie-rag")
...
class TestRealDataResults:
    ...
    def test_document_expected_processing_results(self):
        ...
        results_file = Path("/workspace/azure-maintie-rag/tests/expected_real_data_results.json")

...
class TestSystemIntegration:
    ...
    async def test_multi_agent_orchestration(self):
        ...
        if extraction_result.output.entities:
            ...
            search_result = await universal_search_agent.run(
                f"Search for detailed information about: {primary_entity}", deps=deps
            )
        ...
        print(
            f"   Domain Complexity: {orchestration_results['domain_analysis']['vocab_complexity']:.3f}"
        )
        print(
            f"   Entities Extracted: {orchestration_results['extraction']['entities_count']}"
        )
        if "search" in orchestration_results:
            print(
                f"   Search Strategy: {orchestration_results['search']['strategy_used']}"
            )
    ...
    async def test_agent_communication_patterns(self):
        ...
        if extraction_result.output.entities:
            ...
            search_query = (
                f"Search for information related to: {', '.join(top_entities)}"
            )
        ...
        assert (
            len(communication_flow) >= 2
        ), "Communication flow should include at least 2 agents"
        ...
        for step in communication_flow:
            ...
            if "input_from" in step:
                print(
                    f"     Uses data from: {step['input_from'].replace('_', ' ').title()}"
                )
    ...
    async def test_error_handling_and_recovery(self):
        ...
        try:
            ...
            error_scenarios.append(
                {"scenario": "minimal_content", "success": False, "error": str(e)}
            )

        # Test with malformed input
        try:
            malformed_result = await knowledge_extraction_agent.run(
                "Extract from: " + "x" * 50000, deps=deps  # Very long input
            )
            error_scenarios.append(
                {
        ...
        try:
            ...
            error_scenarios.append(
                {"scenario": "empty_search", "success": False, "error": str(e)[:100]}
            )

        # Validate that system handles errors gracefully (should not crash completely)
        successful_scenarios = sum(
            1 for scenario in error_scenarios if scenario["success"]
        )
    ...
    @pytest.mark.layer4
    @pytest.mark.integration
    def test_system_configuration_consistency(
        self, environment_config, azure_model_deployments
    ):
        ...
        if environment_config["environment"] != "prod":
            config_issues.append(
                f"Environment mismatch: {environment_config['environment']}"
            )
...
class TestPerformanceAndSLA:
    ...
    async def test_individual_agent_performance_sla(self):
        ...
        start_time = time.time()
        extraction_result = await knowledge_extraction_agent.run(
            f"Extract entities and relationships: {performance_test_content}", deps=deps
        )
        ...
        start_time = time.time()
        search_result = await universal_search_agent.run(
            "Search for Azure Machine Learning information", deps=deps
        )
        ...
        sla_compliant_agents = sum(
            1 for result in performance_results.values() if result["meets_sla"]
        )
        ...
        for agent_name, result in performance_results.items():
            ...
            print(
                f"   {agent_name}: {result['duration']:.2f}s (target: {result['sla_target']}s) {status}"
            )
        ...
        assert (
            sla_ratio >= 0.8
        ), f"Too many agents exceed SLA: {sla_ratio:.2f} compliance rate"
    ...
    async def test_end_to_end_workflow_performance(self):
        ...
        extraction_result = await knowledge_extraction_agent.run(
            f"Extract from: {workflow_content}", deps=deps
        )
        ...
        print(
            f"   Total Workflow: {total_workflow_time:.2f}s (target: {workflow_sla_target}s)"
        )
        print(f"   SLA Compliance: {'✅ Met' if meets_workflow_sla else '❌ Exceeded'}")

        assert (
            meets_workflow_sla
        ), f"Workflow exceeded SLA: {total_workflow_time:.2f}s > {workflow_sla_target}s"
    ...
    async def test_concurrent_request_handling(self):
        ...
        assert (
            successful_tasks >= len(concurrent_tasks) * 0.8
        ), f"Too many concurrent failures: {successful_tasks}/{len(concurrent_tasks)}"
        assert (
            total_concurrent_time < 60.0
...
class TestProductionReadiness:
    ...
    async def test_system_scalability_indicators(self):
        ...
        scalability_metrics["resource_sharing"] = (
            domain_agent_deps is extraction_agent_deps
        )

        print("✅ System Scalability Indicators: Evaluated")
        print(f"   Service Init Time: {init_time:.2f}s")
        print(
            f"   Resource Sharing: {'✅ Yes' if scalability_metrics['resource_sharing'] else '❌ No'}"
        )
        print(
            f"   Stateless Agents: {'✅ Yes' if scalability_metrics['stateless_agent_design'] else '❌ No'}"
        )

        # Validate scalability requirements
        assert init_time < 30.0, f"Service initialization too slow: {init_time:.2f}s"
        assert scalability_metrics[
            "resource_sharing"
        ], "Agents should share resource instances"
    ...
    def test_monitoring_and_observability(self, environment_config):
        ...
        missing_configs = [
            key for key, configured in observability_config.items() if not configured
        ]
        ...
        print(
            f"   Application Insights: {'✅ Configured' if observability_config['application_insights_configured'] else '❌ Missing'}"
        )
        print(
            f"   Logging Level: {'✅ Set' if observability_config['logging_configured'] else '❌ Missing'}"
        )
        print(
            f"   Environment Tracking: {'✅ Yes' if observability_config['environment_tracking'] else '❌ No'}"
        )
    ...
    async def test_production_data_flow_validation(self, azure_ai_test_files):
        ...
        try:
            ...
            production_flow["domain_analysis"] = {
                ...
                "vocab_complexity": domain_analysis.discovered_characteristics.vocabulary_complexity,
                "patterns_found": len(
                    domain_analysis.discovered_characteristics.structural_patterns
                ),
            }

            # Step 2: Knowledge Extraction
            extraction_result = await knowledge_extraction_agent.run(
                f"Production extraction from: {content[:1500]}", deps=deps
            )
        ...
        print(
            f"   Domain Analysis: {production_flow['domain_analysis'].get('status', 'failed')}"
        )
        print(
            f"   Knowledge Extraction: {production_flow['knowledge_extraction'].get('status', 'failed')}"
        )
        print(
            f"   Search Integration: {production_flow['search_integration'].get('status', 'failed')}"
        )
        print(
            f"   Overall Success: {'✅ Yes' if production_flow['overall_success'] else '❌ No'}"
        )

        assert production_flow[
            "overall_success"
        ], "Production data flow validation failed"
...
class TestSystemHealthAndReliability:
    ...
    async def test_service_health_monitoring(self):
        ...
        health_report = {
            ...
            "critical_services_healthy": all(
                ...
                for service in [
                    "openai"
                ]  # Only OpenAI is critical for basic functionality
            ),
            "service_details": service_status,
        }

        health_ratio = (
            health_report["healthy_services"] / health_report["total_services"]
        )

        print("✅ Service Health Monitoring: Complete health check")
        print(
            f"   Healthy Services: {health_report['healthy_services']}/{health_report['total_services']}"
        )
        print(f"   Health Ratio: {health_ratio:.2f}")
        print(
            f"   Critical Services: {'✅ Healthy' if health_report['critical_services_healthy'] else '❌ Issues'}"
        )
    ...
    async def test_system_reliability_metrics(self):
        ...
        reliability_test_content = (
            "Azure provides reliable cloud computing services worldwide."
        )
        ...
        for i in range(iteration_count):
            try:
                ...
                reliability_results.append(
                    {"iteration": i + 1, "success": False, "error": str(e)[:50]}
                )

        # Analyze reliability metrics
        successful_iterations = sum(
            1 for result in reliability_results if result["success"]
        )
        success_rate = successful_iterations / iteration_count

        if successful_iterations > 0:
            processing_times = [
                r["processing_time"] for r in reliability_results if r["success"]
            ]
        ...
        assert (
            processing_consistency < 10.0
        ), f"Processing time too inconsistent: {processing_consistency:.2f}s"
    ...
    async def test_production_readiness_checklist(self):
        ...
        service_status = await deps.initialize_all_services()
        readiness_checklist["azure_services_connected"] = service_status.get(
            "openai", False
        )
        ...
        try:
            extraction_result = await knowledge_extraction_agent.run(
                "Test extraction: Azure Cosmos DB", deps=deps
            )
            readiness_checklist["data_processing_working"] = (
                extraction_result.output is not None
            )
        except:
        ...
        start_time = time.time()
        try:
            await run_domain_analysis(
                "Performance test content for production readiness validation"
            )
        ...
        try:
            ...
            readiness_checklist["error_handling_robust"] = (
                True  # Should handle gracefully
            )
        except:
            readiness_checklist["error_handling_robust"] = (
                True  # Expected to handle errors
            )

        # Check 6: Monitoring Configured
        import os

        readiness_checklist["monitoring_configured"] = bool(
            os.getenv("AZURE_APP_INSIGHTS_CONNECTION_STRING")
        )
        ...
        print(
            f"   Overall Readiness: {ready_count}/{total_checks} ({readiness_score:.2f})"
        )
        ...
        print(
            f"   Production Ready: {'✅ Yes' if readiness_score >= 0.8 else '❌ Needs work'}"
        )

        # Production readiness requires at least 80% of checks to pass
        assert (
            readiness_score >= 0.8
        ), f"System not production ready: {readiness_score:.2f} score"

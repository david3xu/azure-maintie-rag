...
class TestAzureServiceHealth:
    ...
    async def test_azure_service_initialization(self, azure_services):
        ...
        assert (
            health_status["successful_services"] >= 0
        ), "Negative successful services count"
        assert (
            health_status["successful_services"] <= health_status["total_services"]
        ), "More successful than total services"

        print(f"🏥 Azure Health Status:")
        print(
            f"   Overall Health: {'✅ Healthy' if health_status['overall_health'] else '❌ Degraded'}"
        )
        print(
            f"   Services: {health_status['successful_services']}/{health_status['total_services']}"
        )
    ...
    async def test_individual_service_connectivity(self, azure_services):
        ...
        if hasattr(azure_services, "search_client") and azure_services.search_client:
            try:
                start_time = time.time()
                result = await azure_services.search_client.search(
                    "connectivity test", top=1
                )
                connectivity_time = time.time() - start_time
        ...
        for service_name, test_result in service_tests.items():
            if test_result["available"]:
                ...
                print(
                    f"   {status_icon} {service_name.title()}: "
    ...
    async def test_service_health_monitoring(self, azure_services):
        ...
        ) <= 1  # Allow 1 service variation

        print(f"🔍 Health Monitoring Results:")
        for check in health_checks:
            print(
                f"   Check {check['check_number']}: "
        ...
        print(
            f"   Health consistency: {'✅ Stable' if health_consistency else '❌ Unstable'}"
        )
        print(
            f"   Service consistency: {'✅ Stable' if service_consistency else '❌ Unstable'}"
        )
    ...
    async def test_service_failover_simulation(self, azure_services):
        ...
        print(
            f"🔄 Initial Health: {initial_health['successful_services']}/{initial_health['total_services']}"
        )
...
class TestAzureServicePerformance:
    ...
    async def test_service_response_times(self, azure_services):
        ...
        if azure_services.openai_client:
            ...
            try:
                result = await azure_services.openai_client.get_completion(
                    "Response time test", max_tokens=10
                )
        ...
        for service_name, timing in response_times.items():
            ...
            print(
                f"   {status_icon} {service_name.title()}: {response_time:.3f}s "
        ...
        services_meeting_sla = [
            service
            for service, timing in response_times.items()
            if timing.get("success", False)
            and timing["response_time"] <= timing.get("target_sla", 3.0)
        ]

        if response_times:  # Only validate if we have services to test
            assert (
                len(services_meeting_sla) > 0
            ), f"No services meeting SLA: {response_times}"
    ...
    async def test_concurrent_service_load(self, azure_services):
            ...
            for i in range(3):
                concurrent_tasks.append(
                    azure_services.openai_client.get_completion(
                        f"Concurrent test {i}", max_tokens=10
                    )
                )
        ...
        assert (
            success_rate >= 70.0
        ), f"Concurrent success rate too low: {success_rate:.1f}%"

...
class TestPydanticAIAgentConfiguration:
    ...
    def test_environment_configuration_for_agents(self):
        ...
        if missing_vars:
            pytest.fail(f"PydanticAI agents require these environment variables: {missing_vars}")
    ...
    async def test_universal_deps_initialization(self):
        ...
        try:
            ...
            print(f"   OpenAI Client: {'Available' if service_status.get('openai') else 'Failed'}")
            print(f"   Cosmos Client: {'Available' if service_status.get('cosmos') else 'Failed'}")
            print(f"   Search Client: {'Available' if service_status.get('search') else 'Failed'}")
            print(f"   Storage Client: {'Available' if service_status.get('storage') else 'Failed'}")
            
            # At minimum, OpenAI should be available for agent testing
            if not service_status.get('openai'):
                pytest.fail("OpenAI service is required for agent testing but not available")
...
class TestDomainIntelligenceAgent:
    ...
    def test_domain_intelligence_agent_import_and_structure(self):
        ...
        print(f"   System Prompt Length: {len(domain_intelligence_agent.system_prompt)} chars")
    ...
    async def test_domain_intelligence_agent_run_method(self):
        ...
        try:
            ...
            if "404" in str(e):
                pytest.fail(f"Azure OpenAI 404 error - check model deployment name: {e}")
            else:
    ...
    async def test_knowledge_extraction_agent_run_method(self):
        ...
        try:
            ...
            if extraction.entities:
                print(f"   Sample Entity: {extraction.entities[0].text if extraction.entities else 'None'}")
            if extraction.relationships:
                print(f"   Sample Relationship: {extraction.relationships[0].relationship_type if extraction.relationships else 'None'}")
...
class TestUniversalSearchAgent:
    ...
    async def test_universal_search_agent_run_method(self):
        ...
        try:
            ...
            search_query = "Find information about Azure Cosmos DB performance optimization"
            
            result = await universal_search_agent.run(
                f"Execute universal search for: {search_query}",
                deps=deps
            )
...
class TestAgentOrchestration:
    ...
    async def test_multi_agent_workflow(self):
        ...
        try:
            ...
            if extraction_result.output.entities:
                first_entity = extraction_result.output.entities[0].text if extraction_result.output.entities else "Azure"
                search_result = await universal_search_agent.run(
                    f"Search for information about: {first_entity}",
                    deps=deps
                )
            ...
            print(f"   Knowledge Extraction: {len(extraction_result.output.entities)} entities")
    ...
    async def test_agent_performance_characteristics(self):
        ...
        try:
            ...
            domain_result = await run_domain_analysis("Test content for performance measurement")
            ...
            assert domain_duration < 15.0, f"Domain analysis too slow: {domain_duration:.2f}s"
            assert extraction_duration < 15.0, f"Extraction too slow: {extraction_duration:.2f}s"
            ...
            print(f"   Both agents < 15s target: {'✅' if max(domain_duration, extraction_duration) < 15 else '❌'}")
    ...
    async def test_agent_dependency_failure_handling(self):
        ...
        try:
            ...
            if service_status.get('openai'):
                result = await run_domain_analysis("Test content for dependency testing")
                assert isinstance(result, UniversalDomainAnalysis)
                print("✅ Domain Intelligence Agent: Works with minimal dependencies")
            
            print(f"✅ Dependency Handling: Tested with {len(available_services)} available services")

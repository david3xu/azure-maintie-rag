...
class TestAzureInfrastructureServices:
    ...
    async def test_azure_openai_embeddings(self):
        ...
        try:
            ...
            assert len(embedding) == 1536  # text-embedding-ada-002 produces 1536D vectors
    ...
    def test_azure_cognitive_search_connectivity(self):
        ...
        try:
            ...
            search_key = os.getenv("AZURE_SEARCH_API_KEY")  # May not be set in current config
            ...
            try:
                ...
                if "index" in str(search_e).lower():
                    print("✅ Azure Cognitive Search: Connectivity successful (index needs setup)")
                    print(f"   Endpoint: {search_endpoint}")
    ...
    def test_azure_cosmos_db_connectivity(self):
        ...
        try:
            ...
            if not cosmos_key:
                ...
                print("✅ Azure Cosmos DB: Endpoint configured (detailed testing requires key)")
            ...
            if "credentials" in str(e).lower() or "unauthorized" in str(e).lower():
                print("⚠️  Azure Cosmos DB: Endpoint reachable but credentials need configuration")
        ...
        try:
            ...
            try:
                ...
                if "authentication" in str(storage_e).lower():
                    print("⚠️  Azure Blob Storage: Endpoint reachable but authentication needs configuration")
                    print(f"   Endpoint: {storage_endpoint}")
                else:
                    raise storage_e
    ...
    def test_azure_application_insights_connectivity(self):
        ...
        try:
            insights_connection_string = os.getenv("AZURE_APP_INSIGHTS_CONNECTION_STRING")
            ...
            print(f"   Connection String configured: {len(insights_connection_string)} chars")
...
class TestAzureModelDeployments:
    ...
    async def test_model_performance_baseline(self):
        ...
        try:
            ...
            assert chat_duration < 10.0, f"Chat completion too slow: {chat_duration:.2f}s"
            assert embedding_duration < 10.0, f"Embedding generation too slow: {embedding_duration:.2f}s"
...
class TestSystemIntegrationReadiness:
    ...
    def test_all_services_minimal_requirements(self):
        ...
        critical_services = {
            "Azure OpenAI": os.getenv("AZURE_OPENAI_ENDPOINT") and os.getenv("OPENAI_API_KEY"),
            "Model Deployments": os.getenv("OPENAI_MODEL_DEPLOYMENT") and os.getenv("EMBEDDING_MODEL_DEPLOYMENT"),
        }
        ...
        failed_critical = [name for name, configured in critical_services.items() if not configured]
        if failed_critical:
            pytest.fail(f"Critical services not configured: {failed_critical}")
        
        # Report on optional services
        available_optional = sum(1 for configured in optional_services.values() if configured)
        
        print("✅ System Integration Readiness: Ready for comprehensive testing")
        print(f"   Critical Services: {len(critical_services)}/{len(critical_services)} configured")
        print(f"   Optional Services: {available_optional}/{len(optional_services)} configured")

...
class TestAuthenticationDiagnostics:
    ...
    def test_environment_variables_diagnostic(self):
        ...
        for var_name, var_value in required_vars.items():
            ...
            display_value = f"***{var_value[-10:]}" if var_value and len(var_value) > 10 else var_value
        ...
        assert not missing_critical, f"Missing critical environment variables: {missing_critical}"
    ...
    def test_azure_openai_endpoint_format(self):
        ...
        assert endpoint.startswith('https://'), f"Endpoint must start with https://: {endpoint}"
        assert 'openai.azure.com' in endpoint, f"Endpoint must contain openai.azure.com: {endpoint}"

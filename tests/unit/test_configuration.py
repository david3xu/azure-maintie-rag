...
class TestConfigurationValidation:
    ...
    def test_system_config_defaults(self):
        ...
        print(
            f"✅ System config validation passed: {config.max_workers} workers, {config.openai_timeout}s timeout"
        )
    ...
    def test_extraction_config_defaults(self):
        ...
        assert (
            ...
            <= config.entity_confidence_threshold + 0.2
        ...
        print(
            f"✅ Extraction config validation passed: {config.entity_confidence_threshold} entity confidence"
        )
    ...
    def test_search_config_validation(self):
        ...
        print(
            f"✅ Search config validation passed: {config.vector_top_k} vector results, {config.graph_max_depth} graph depth"
        )
    ...
    def test_model_config_validation(self):
        ...
        print(
            f"✅ Model config validation passed: {config.gpt4o_deployment_name}, API {config.openai_api_version}"
        )
    ...
    def test_environment_override(self):
        ...
        print(
            f"✅ Environment override validation passed: workers={config.max_workers}, timeout={config.openai_timeout}"
        )
...
class TestConfigurationEdgeCases:
    ...
    def test_config_interdependencies(self):
        ...
        total_potential_results = (
            search_config.max_results_per_modality
            * 3  # 3 modalities: vector, graph, GNN
        )
        assert (
            total_potential_results >= search_config.max_final_results
            or search_config.max_final_results <= 100
        )

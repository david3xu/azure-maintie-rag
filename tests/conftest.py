...
def comprehensive_azure_health_monitor():
    ...
    class ComprehensiveAzureHealthMonitor:
        ...
        async def perform_health_check(
            self, service_name: str, check_func, timeout: float = 30.0
        ):
        ...
        def get_health_summary(self) -> Dict[str, Any]:
            ...
            healthy_services = sum(
                1 for metrics in self.service_metrics.values() if metrics["healthy"]
            )
            ...
            critical_healthy = all(
                self.service_metrics.get(service, {}).get("healthy", False)
        ...
        def is_production_ready(self) -> bool:
            ...
            return (
                summary["critical_services_healthy"]
                and summary["health_ratio"] >= 0.6  # At least 60% of services healthy
                and summary["average_response_time"]
                < 10.0  # Average response time under 10s
            )
    ...
    class IntegrationTestDataManager:
        ...
        def analyze_file_quality(self, file_path: Path) -> Dict[str, Any]:
            ...
            analysis = {
                ...
                "has_api_content": any(
                    ...
                    for word in ["api", "endpoint", "request", "response", "curl"]
                ),
                "has_tutorial_content": any(
                    word in content.lower()
                    for word in ["how to", "tutorial", "example", "step"]
                ),
                "has_conceptual_content": any(
                    word in content.lower()
                    for word in ["concept", "overview", "introduction", "understand"]
                ),
                "has_code_examples": any(
                    word in content.lower()
                    for word in ["code", "example", "sample", "snippet"]
                ),
        ...
        def _calculate_complexity_score(self, content: str) -> float:
            ...
            factors = [
                ...
                any(
                    word in content for word in ["JSON", "REST", "API", "SDK"]
                ),  # Technical terms
            ]
        ...
        def _is_suitable_for_testing(self, content: str) -> bool:
            ...
            return (
                len(content) > 300  # Minimum content
                and len(content) < 20000  # Not too large
                and content.count("\n") > 5  # Multi-line
                and not content.count("error") > 5  # Not error-heavy
            )
        ...
        def generate_test_data_report(self) -> Dict[str, Any]:
            ...
            suitable_files = [
                a for a in analyses if a.get("is_suitable_for_testing", False)
            ]
            ...
            report = {
                ...
                "suitability_ratio": (
                    len(suitable_files) / len(analyses) if analyses else 0
                ),
                "average_size": (
                    sum(a.get("size_chars", 0) for a in suitable_files)
                    / len(suitable_files)
                    if suitable_files
                    else 0
                ),
                "content_type_distribution": {
                    "api_docs": sum(
                        1 for a in suitable_files if a.get("has_api_content", False)
                    ),
                    "tutorials": sum(
                        1
                        for a in suitable_files
                        if a.get("has_tutorial_content", False)
                    ),
                    "conceptual": sum(
                        1
                        for a in suitable_files
                        if a.get("has_conceptual_content", False)
                    ),
                    "code_examples": sum(
                        1 for a in suitable_files if a.get("has_code_examples", False)
                    ),
                },
                "quality_metrics": {
                    "average_quality_score": (
                        sum(a.get("quality_score", 0) for a in suitable_files)
                        / len(suitable_files)
                        if suitable_files
                        else 0
                    ),
                    "average_complexity_score": (
                        sum(a.get("complexity_score", 0) for a in suitable_files)
                        / len(suitable_files)
                        if suitable_files
                        else 0
                    ),
                    "files_with_code_blocks": sum(
                        1 for a in suitable_files if a.get("has_code_blocks", False)
                    ),
                    "files_with_headers": sum(
                        1 for a in suitable_files if a.get("has_headers", False)
                    ),
                },
...
def environment_config():
    ...
    return {
        ...
        "use_managed_identity": os.getenv("USE_MANAGED_IDENTITY", "false").lower()
        == "true",
    }
...
async def azure_health_check(azure_services):
    ...
    available_services = [
        service for service, status in service_status.items() if status
    ]
    ...
    if not overall_health:
        ...
        pytest.skip(
            f"Critical Azure services unavailable. Available: {len(available_services)}/{len(service_status)}"
        )
...
def test_data_quality_report(azure_ai_test_files):
    ...
    avg_size = sum(report["size_chars"] for report in quality_reports) / max(
        total_files, 1
    )
    ...
    summary = {
        ...
        "files_with_api_content": sum(
            1 for report in quality_reports if report["has_api_content"]
        ),
        "files_with_tutorial_content": sum(
            1 for report in quality_reports if report["has_tutorial_content"]
        ),
        "files_with_code_blocks": sum(
            1 for report in quality_reports if report["has_code_blocks"]
        ),
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "layer4: Layer 4 Integration & Performance Tests"
    )

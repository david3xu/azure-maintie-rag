...
class TestAzureIntegrationComprehensive:
    ...
    async def test_domain_intelligence_agent_minimal(self):
        ...
        try:
            ...
            try:
                ...
                pytest.skip("Agent execution timed out - may indicate configuration issues")
    ...
    async def test_end_to_end_minimal_workflow(self):
        ...
        if test_data_dir.exists():
            ...
            if test_files:
                ...
                content = small_file.read_text(encoding='utf-8')[:500]  # First 500 chars
    ...
    def test_environment_configuration_complete(self):
        ...
        assert not missing_essential, f"Missing essential environment variables: {missing_essential}"
    ...
    async def test_performance_benchmarking(self):
        ...
        print(f"📊 Service availability: {sum(service_status.values())}/{len(service_status)}")
    ...
    def test_integration_test_architecture(self):
        ...
        print(f"📁 Test Data Directory: {'EXISTS' if test_data_dir.exists() else 'MISSING'}")
...
class TestIntegrationHealthReport:
    ...
    async def test_generate_integration_health_report(self):
        ...
        try:
            ...
            if init_time > 5:
                health_report["recommendations"].append("Slow initialization time - consider optimizing service startup")
            
        except Exception as e:
            health_report["performance"]["error"] = str(e)
        
        # Overall health score
        service_health = sum(health_report["services"].values()) / max(len(health_report["services"]), 1) if isinstance(health_report["services"], dict) and "error" not in health_report["services"] else 0
        auth_health = 1.0 if health_report["authentication"].get("default_credential", False) else 0.0

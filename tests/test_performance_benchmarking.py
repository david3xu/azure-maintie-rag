...
class TestPerformanceBenchmarking:
    ...
    async def test_individual_agent_performance_benchmarks(
        ...
        for agent_name, config in agent_benchmarks.items():
            ...
            for i, test_file in enumerate(benchmark_files):
                ...
                if agent_name == "domain_intelligence":
                    test_input = content[
                        :2000
                    ]  # Domain analysis works well with moderate content
                elif agent_name == "knowledge_extraction":
                    test_input = content[
                        :1500
                    ]  # Knowledge extraction on focused content
                else:  # universal_search
                    # Extract a meaningful search query from content
                    lines = content.split("\n")[:5]
                    test_input = f"Information about {' '.join(lines).split()[:10]}"[
                        :100
                    ]

                async with performance_monitor.measure_operation(
                    f"{agent_name}_benchmark_{i}", sla_target=config["sla_target"]
                ) as measurement:

                    start_time = time.time()
                    try:
                        if agent_name == "universal_search":
                            result = await config["agent_func"](
                                test_input, max_results=5
                            )
                        elif agent_name == "knowledge_extraction":
                            result = await config["agent_func"](
                                test_input, use_domain_analysis=False
                            )
                        else:
                        ...
                        if agent_name == "domain_intelligence":
                            metrics = {
                                ...
                                "patterns_found": len(
                                    result.discovered_characteristics.structural_patterns
                                ),
                            }
                        elif agent_name == "knowledge_extraction":
                            metrics = {
                        ...
                        print(
                            f"     📄 {test_file.name}: {processing_time:.2f}s {'✅' if processing_time <= config['sla_target'] else '❌'}"
                        )
                        ...
                        print(
                            f"     📄 {test_file.name}: {processing_time:.2f}s ❌ (Error: {str(e)[:50]})"
                        )
        ...
        for agent_name, config in agent_benchmarks.items():
            ...
            if successful_measurements:
                processing_times = [
                    m["processing_time"] for m in successful_measurements
                ]
                sla_compliant_count = sum(
                    1 for m in successful_measurements if m["sla_compliant"]
                )
                ...
                summary = {
                    ...
                    "sla_compliance_rate": sla_compliant_count
                ...
                print(
                    f"     SLA Compliance: {summary['sla_compliance_rate']:.2%} (target: {summary['sla_target']}s)"
                )
                print(
                    f"     Avg Processing Time: {summary['avg_processing_time']:.2f}s"
                )
                print(
                    f"     Median Processing Time: {summary['median_processing_time']:.2f}s"
                )
                print(
                    f"     Time Range: {summary['min_processing_time']:.2f}s - {summary['max_processing_time']:.2f}s"
                )
        ...
        overall_sla_compliance = (
            ...
            / sum(summary["successful_tests"] for summary in benchmark_summary.values())
        ...
        overall_success_rate = (
            sum(summary["successful_tests"] for summary in benchmark_summary.values())
            / sum(summary["total_tests"] for summary in benchmark_summary.values())
            if benchmark_summary
            else 0
        )
        ...
        ), f"Agent success rate too low: {overall_success_rate:.2%}"
        assert (
            overall_sla_compliance >= 0.7
        ), f"SLA compliance too low: {overall_sla_compliance:.2%}"
        ...
        for agent_name, summary in benchmark_summary.items():
            ...
            ), f"{agent_name} success rate too low: {summary['success_rate']:.2%}"
            assert (
                summary["sla_compliance_rate"] >= 0.6
            ), f"{agent_name} SLA compliance too low: {summary['sla_compliance_rate']:.2%}"
    ...
    async def test_end_to_end_workflow_performance(
        ...
        workflow_files = enhanced_test_data_manager.get_test_files_by_criteria(
            min_size=800, max_size=4000, content_types=["azure", "api"], limit=5
        )
            ...
            async with performance_monitor.measure_operation(
                ...
                try:
                    ...
                    if workflow_result.extraction_summary:
                        measurement["extraction_results"] = (
                            workflow_result.extraction_summary
                        )
                    ...
                    status = (
                        "✅"
                        if workflow_result.success and workflow_time <= 45.0
                        else "⚠️" if workflow_result.success else "❌"
                    )
                    ...
                    for agent_name, stage_time in stage_times.items():
                        print(
                            f"     {agent_name.replace('_', ' ').title()}: {stage_time:.2f}s"
                        )
                    ...
                    workflow_measurements.append(measurement)
                    print(
                        f"     Total Time: {workflow_time:.2f}s ❌ (Error: {str(e)[:50]})"
                    )

        # Analyze end-to-end workflow performance
        successful_workflows = [
            m for m in workflow_measurements if m["workflow_success"]
        ]

        if successful_workflows:
            workflow_times = [m["total_workflow_time"] for m in successful_workflows]
            sla_compliant_workflows = [
                m for m in successful_workflows if m["sla_compliant"]
            ]
            ...
            performance_summary = {
                ...
                "sla_compliance_rate": len(sla_compliant_workflows)
                ...
                "std_workflow_time": (
                    statistics.stdev(workflow_times) if len(workflow_times) > 1 else 0
                ),
            }
            ...
            print(
                f"   SLA Compliance: {performance_summary['sla_compliance_rate']:.2%} (target: 45s)"
            )
            print(
                f"   Avg Workflow Time: {performance_summary['avg_workflow_time']:.2f}s"
            )
            print(
                f"   Median Workflow Time: {performance_summary['median_workflow_time']:.2f}s"
            )
            print(
                f"   Time Range: {performance_summary['min_workflow_time']:.2f}s - {performance_summary['max_workflow_time']:.2f}s"
            )
            ...
            for stage, times in stage_performance.items():
                if times:
                    ...
                    print(
                        f"     {stage.replace('_', ' ').title()}: {avg_time:.2f}s avg"
                    )

            # Assertions for workflow performance
            assert (
                performance_summary["success_rate"] >= 0.8
            ), f"Workflow success rate too low: {performance_summary['success_rate']:.2%}"
            assert (
                performance_summary["sla_compliance_rate"] >= 0.7
            ), f"Workflow SLA compliance too low: {performance_summary['sla_compliance_rate']:.2%}"
            assert (
                performance_summary["avg_workflow_time"] <= 50.0
            ), f"Average workflow time too high: {performance_summary['avg_workflow_time']:.2f}s"
    ...
    async def test_concurrent_user_performance_scalability(
        ...
        for scenario in concurrency_scenarios:
            ...
            async def simulate_user_load(
                user_id: int, requests: int, files: List[Path]
            ):
            ...
            user_tasks = [
                simulate_user_load(
                    user_id, scenario["requests_per_user"], concurrent_test_files
                )
                for user_id in range(scenario["users"])
            ]

            user_results_list = await asyncio.gather(
            ...
            if successful_requests:
                ...
                scenario_result = {
                    ...
                    "success_rate": (
                        len(successful_requests) / len(all_requests)
                        if all_requests
                        else 0
                    ),
                    ...
                    "requests_per_second": (
                        len(all_requests) / scenario_time if scenario_time > 0 else 0
                    ),
                    "concurrent_efficiency": (
                        scenario_time
                        / (statistics.mean(response_times) * scenario["users"])
                        if response_times
                        else 1
                    ),
                }
                ...
                print(
                    f"     Avg Response Time: {scenario_result['avg_response_time']:.2f}s"
                )
                print(
                    f"     P95 Response Time: {scenario_result['p95_response_time']:.2f}s"
                )
                print(
                    f"     Max Response Time: {scenario_result['max_response_time']:.2f}s"
                )
                print(
                    f"     Requests/Second: {scenario_result['requests_per_second']:.2f}"
                )
                print(
                    f"     Concurrent Efficiency: {scenario_result['concurrent_efficiency']:.2f}x"
                )
        ...
        if concurrency_results:
            ...
            for result in concurrency_results:
                scalability_status = (
                    ...
                    and result["avg_response_time"] <= 15.0
                    else "⚠️"
                )
                print(
                    f"   {result['scenario']}: {result['success_rate']:.2%} success, {result['avg_response_time']:.2f}s avg {scalability_status}"
                )
            ...
            overall_success_rate = statistics.mean(
                [r["success_rate"] for r in concurrency_results]
            )
            overall_avg_response_time = statistics.mean(
                [r["avg_response_time"] for r in concurrency_results]
            )

            assert (
                overall_success_rate >= 0.75
            ), f"Overall concurrent success rate too low: {overall_success_rate:.2%}"
            assert (
                overall_avg_response_time <= 20.0
            ), f"Overall concurrent response time too high: {overall_avg_response_time:.2f}s"
            assert (
                max_users_tested >= 10
            ), f"Insufficient concurrent user testing: {max_users_tested} users"
    ...
    async def test_performance_consistency_and_reliability(
        ...
        consistency_files = enhanced_test_data_manager.get_test_files_by_criteria(
            min_size=1000, max_size=3000, limit=3
        )
        ...
        for iteration in range(iteration_count):
            async with performance_monitor.measure_operation(
                f"consistency_test_{iteration}", sla_target=10.0
            ):
            ...
            print(
                f"   Iteration {iteration + 1}: {iteration_time:.2f}s {'✅' if iteration_time <= 10.0 else '❌'}"
            )
        ...
        if (
            len(successful_measurements) >= 5
        ):  # Need at least 5 successful measurements for analysis
            processing_times = [m["processing_time"] for m in successful_measurements]
            complexity_values = [
                m["vocabulary_complexity"] for m in successful_measurements
            ]
            ...
            consistency_metrics = {
                ...
                "performance_stats": {
                    ...
                    "coefficient_of_variation": (
                        ...
                        if len(processing_times) > 1
                        and statistics.mean(processing_times) > 0
                        else 0
                    ),
                },
                "result_consistency": {
                    "complexity_std": (
                        statistics.stdev(complexity_values)
            ...
            print(
                f"   Mean Processing Time: {consistency_metrics['performance_stats']['mean_time']:.2f}s"
            )
            print(
                f"   Std Deviation: {consistency_metrics['performance_stats']['std_time']:.2f}s"
            )
            print(
                f"   Coefficient of Variation: {consistency_metrics['performance_stats']['coefficient_of_variation']:.3f}"
            )
            print(
                f"   Time Range: {consistency_metrics['performance_stats']['min_time']:.2f}s - {consistency_metrics['performance_stats']['max_time']:.2f}s"
            )

            print(f"\n   📊 Result Consistency:")
            print(
                f"   Vocabulary Complexity Std: {consistency_metrics['result_consistency']['complexity_std']:.4f}"
            )
            print(
                f"   Concept Density Std: {consistency_metrics['result_consistency']['density_std']:.4f}"
            )
            print(
                f"   Complexity Range: {consistency_metrics['result_consistency']['complexity_range']:.4f}"
            )
            print(
                f"   Density Range: {consistency_metrics['result_consistency']['density_range']:.4f}"
            )

            # Assertions for consistency and reliability
            assert (
                consistency_metrics["success_rate"] >= 0.9
            ), f"Consistency success rate too low: {consistency_metrics['success_rate']:.2%}"
            assert (
                consistency_metrics["performance_stats"]["coefficient_of_variation"]
                <= 0.3
            ), f"Performance too inconsistent: CV = {consistency_metrics['performance_stats']['coefficient_of_variation']:.3f}"
            assert (
                consistency_metrics["result_consistency"]["complexity_std"] <= 0.1
            ), f"Result consistency too low: complexity std = {consistency_metrics['result_consistency']['complexity_std']:.4f}"
            assert (
                consistency_metrics["performance_stats"]["mean_time"] <= 12.0
            ), f"Mean performance outside SLA: {consistency_metrics['performance_stats']['mean_time']:.2f}s"

        else:
            pytest.fail(
                f"Insufficient successful measurements for consistency analysis: {len(successful_measurements)}/{iteration_count}"
            )
    ...
    async def test_performance_degradation_under_stress(
        ...
        for scenario in stress_scenarios:
            ...
            async def stress_operation(
                operation_id: int, files: List[Path], content_multiplier: int
            ):
                ...
                if content_multiplier > 1:
                    content = (content * content_multiplier)[
                        :5000
            ...
            if successful_operations:
                ...
                stress_metrics = {
                    ...
                    "operations_per_second": (
                        len(scenario_results) / scenario_time
                        if scenario_time > 0
                        else 0
                    ),
                }
                ...
                print(
                    f"   Avg Processing Time: {stress_metrics['avg_processing_time']:.2f}s"
                )
                print(
                    f"   Max Processing Time: {stress_metrics['max_processing_time']:.2f}s"
                )
                print(
                    f"   Operations/Second: {stress_metrics['operations_per_second']:.2f}"
                )
        ...
        if len(stress_results) >= 2:
            baseline = next(
                (r for r in stress_results if r["scenario"] == "baseline"), None
            )
            ...
            if baseline:
                ...
                print(
                    f"     Avg Processing Time: {baseline['avg_processing_time']:.2f}s"
                )
                print(
                    f"     Operations/Second: {baseline['operations_per_second']:.2f}"
                )
                ...
                for result in stress_results:
                    if result["scenario"] != "baseline":
                        ...
                        print(
                            f"       Performance Degradation: {degradation_factor:.2f}x"
                        )
                        print(f"       Throughput Impact: {throughput_impact:.2f}x")
                        print(f"       Success Rate: {result['success_rate']:.2%}")

            # Assertions for stress resistance
            high_stress_result = next(
                (r for r in stress_results if r["scenario"] == "high_stress"), None
            )

            if high_stress_result:
                assert (
                    high_stress_result["success_rate"] >= 0.6
                ), f"System fails under high stress: {high_stress_result['success_rate']:.2%}"
                assert (
                    high_stress_result["avg_processing_time"] <= 30.0
                ), f"Performance degrades too much under stress: {high_stress_result['avg_processing_time']:.2f}s"
            ...
            ), f"Minimum success rate too low across stress scenarios: {min_success_rate:.2%}"
            assert (
                max_processing_time <= 35.0
            ), f"Maximum processing time too high across stress scenarios: {max_processing_time:.2f}s"

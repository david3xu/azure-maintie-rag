...
async def test_agent1_real_output():
        ...
        if isinstance(sample_output, dict):
            ...
            if centralized_fields:
                ...
                analysis_summary["centralized_schema_coverage"] = {
                    ...
                    * 100,
                    "missing_centralized_fields": list(
                        centralized_fields - actual_fields
                    ),
                }

            print(f"✅ Schema analysis complete:")
            print(
                f"   Full schema coverage: {analysis_summary['field_coverage']['coverage_percentage']:.1f}%"
            )
            if centralized_fields:
                print(
                    f"   Centralized schema coverage: {analysis_summary['centralized_schema_coverage']['centralized_coverage_percentage']:.1f}%"
                )
    ...
    if successful_results:
        ...
        print(
            f"✅ Processed {len(successful_results)}/{len(real_files)} real Azure AI documentation files"
        )
        ...
        if centralized_fields:
            centralized_coverage = analysis_summary["centralized_schema_coverage"][
                "centralized_coverage_percentage"
            ]

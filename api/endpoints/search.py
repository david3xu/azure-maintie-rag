...
class SearchRequest(BaseModel):
    ...
    search_type: str = Field(default="tri_modal", description="Type of search: vector, graph, gnn, or tri_modal")
    max_results: int = Field(default=10, description="Maximum number of results")
    domain: Optional[str] = Field(default=None, description="Domain context (auto-detected if not provided)")
    ...
    try:
        ...
        if not request.domain:
            try:
                ...
                domain_classification = domain_analyzer.classify_content_domain(content_analysis)
                ...
                logger.info(f"Auto-detected domain: {detected_domain} (confidence: {domain_confidence:.3f})")
...
async def analyze_domain(request: DomainAnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Domain analysis completed: {domain_classification.domain} (confidence: {domain_classification.confidence:.3f})")
...
async def search_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            domain_analyzer.classify_content_domain(None)  # Should handle None gracefully
            engine_status["domain_analyzer"] = {"status": "healthy"}
        except Exception as e:
            engine_status["domain_analyzer"] = {
                "status": "error",
                "error": str(e)
            }
        
        healthy_engines = sum(1 for status in engine_status.values() if status.get("status") == "healthy")

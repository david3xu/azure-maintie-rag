...
# Project root for script execution - handle both local and container environments
def get_project_root():
    """Get project root dynamically for both local and container environments"""
    # Try container path first (Docker WORKDIR)
    container_root = Path("/app")
    if container_root.exists() and (container_root / "scripts").exists():
        return container_root

    # Fallback to local development path
    local_root = Path("/workspace/azure-maintie-rag")
    if local_root.exists():
        return local_root

    # Final fallback - relative to current file
...
async def run_dataflow_script(script_path: str, timeout: int = 300) -> Dict[str, Any]:
    ...
    env = {
        ...
        "USE_MANAGED_IDENTITY": "false",  # Use key-based auth to match prod.env config

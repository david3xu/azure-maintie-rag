...
from agents.core.agent_toolsets import generate_extraction_prompts, predict_entity_types
...
class PromptInspector:
    ...
    async def initialize(self):
        ...
        print(
            f"🔧 Initialized with services: {list(self.deps.get_available_services())}"
        )
    ...
    async def inspect_content_prompts(self, content: str, save_to_file: bool = False):
        ...
        domain_result = await domain_intelligence_agent.run(
            f"Analyze for prompt generation: {content}", deps=self.deps
        )
        ...
        print(
            f"   Content patterns: {domain_analysis.characteristics.content_patterns}"
        )
        ...
        print(
            f"   📊 Generation method: {entity_predictions.get('generation_method', 'unknown')}"
        )
        ...
        for prompt_type, prompt_text in extraction_prompts.items():
            ...
            print(
                f"   Contains predicted types: {'predicted entity types' in prompt_text.lower()}"
            )
        ...
        if save_to_file:
            ...
            timestamp = domain_analysis.domain_signature[
                :8
            ]  # Use first 8 chars of signature
...
async def main():
    ...
    if len(sys.argv) < 2:
        ...
        data_dir = (
            ...
            / "data/raw/azure-ai-services-language-service_output"
        )
    ...
    print(
        f"   Entity types generated: {len(results['entity_predictions'].get('predicted_entity_types', []))}"
    )

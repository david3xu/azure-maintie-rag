...
class GNNQueryAnalysisStage:
    ...
    async def execute(self, query: str, domain: str = "maintenance") -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"🔍 Analyzing query: \"{query[:100]}{'...' if len(query) > 100 else ''}\""
            )

            # Step 1: Extract entities from query using AI
            print("🧩 Step 1: AI-Powered Entity Extraction...")
            entity_result = await self.knowledge_service.extract_from_texts(
                [query], domain
            )

            if not entity_result.get("success"):
                logger.warning(
                    f"AI entity extraction failed: {entity_result.get('error')}"
                )
                all_entities = []
                known_entities = []
                extraction_summary = {}
                ...
                known_entities = (
                    all_entities  # AI-extracted entities are considered "known"
                )
                extraction_summary = {
            ...
            print(
                f"   ✅ Known in graph: {known_entities} ({len(known_entities)}/{len(all_entities)})"
            )
            print(
                f"   ✅ Primary intent: {extraction_summary.get('primary_intent', 'unknown')}"
            )
            print(
                f"   ✅ Domain relevance: {extraction_summary.get('domain_relevance', 0.0):.2f}"
            )
            ...
            if all_entities:
                gnn_result = await self.gnn_service.analyze_query_entities(
                    all_entities, domain
                )
                ...
                print(
                    f"   ✅ Found in graph: {entities_found}/{entities_analyzed} entities"
                )

                if related_entities:
                    for entity, related_list in list(related_entities.items())[
                        :3
                    ]:  # Show first 3
                        print(
                            f"   ✅ {entity} → found {len(related_list)} related: {related_list[:5]}{'...' if len(related_list) > 5 else ''}"
                        )
                    if len(related_entities) > 3:
                        print(
                            f"   ✅ ... and {len(related_entities) - 3} more entities with relations"
                        )
            ...
            services_ready = {
                "azure_search": hasattr(self.infrastructure, "search_service")
                and self.infrastructure.search_service is not None,
                "azure_openai": hasattr(self.infrastructure, "openai_service")
                and self.infrastructure.openai_service is not None,
                "azure_cosmos": hasattr(self.infrastructure, "cosmos_service")
                and self.infrastructure.cosmos_service is not None,
                "vector_service": hasattr(self.infrastructure, "vector_service")
                and self.infrastructure.vector_service is not None,
            }
            ...
            print(
                f"   ✅ Search service: {'ready' if services_ready['azure_search'] else 'unavailable'} (326 docs indexed)"
            )
            print(
                f"   ✅ Vector service: {'ready' if services_ready['vector_service'] else 'unavailable'} (1536D embeddings)"
            )
            print(
                f"   ✅ OpenAI service: {'ready' if services_ready['azure_openai'] else 'unavailable'} (analysis + generation)"
            )
            print(
                f"   ✅ Graph service: {'ready' if services_ready['azure_cosmos'] else 'unavailable'} (540 entities)"
            )
            ...
            print(
                f"   ✅ Query complexity: score={query_complexity.get('complexity_score', 0):.1f}, coverage={query_complexity.get('entity_coverage', 0):.2f}"
            )
            print(
                f"   ✅ Search strategy: {len(search_strategy.get('primary_entities', []))} primary + {len(search_strategy.get('related_entities', []))} related entities"
            )
            print(
                f"   ✅ Intent: {intent_classification.get('primary_intent', 'unknown')} (confidence: {intent_classification.get('confidence', 0):.2f})"
            )
            print(
                f"   ✅ Enhancement mode: {'GNN + Azure' if context_enrichment.get('enhanced_mode') else 'Standard'}"
            )
            print(
                f"   ✅ Multi-hop potential: {'Yes' if context_enrichment.get('multi_hop_potential') else 'No'}"
            )
    ...
    def _create_enhanced_context(
        ...
        entity_coverage = gnn_summary.get("entities_found", 0) / max(
            gnn_summary.get("entities_analyzed", 1), 1
        )
        ...
        enhanced_context = {
            ...
            "search_strategy": {
                ...
                "related_entities": list(
                    set(
                        [
                            entity
                            for related_list in gnn_related.values()
                            for entity in related_list
                        ]
                    )
                )[
                    :10
                ],  # Top 10 related for expansion
                "search_expansion_needed": entity_coverage < 0.5,
                "use_graph_traversal": graph_connectivity > 5,
            },
            "intent_classification": {
                "primary_intent": results.get("entity_analysis", {})
                .get("intent_analysis", {})
                .get("primary_intent", "information"),
                "confidence": results.get("entity_analysis", {})
                .get("intent_analysis", {})
                .get("confidence", 0.0),
            ...
            "context_enrichment": {
                ...
                "multi_hop_potential": any(
                    len(related) > 3 for related in gnn_related.values()
                ),
                "azure_services_ready": azure_services_ready,
                "enhanced_mode": azure_services_ready
                and gnn_summary.get("entities_found", 0) > 0,
            },
        }
    ...
    def _print_analysis_results(self, results: Dict[str, Any], query: str):
        ...
        print(
            f"      Primary intent: {entity_summary.get('primary_intent', 'unknown')}"
        )
        ...
        print(
            f"      Entities processed: {gnn_summary.get('entities_found', 0)}/{gnn_summary.get('entities_analyzed', 0)}"
        )
        ...
        print(
            f"      Mode: {'Enhanced (GNN + Azure)' if enrichment.get('enhanced_mode') else 'GNN-ready (Azure services available)'}"
        )
...
async def main():
    ...
    parser.add_argument("--query", required=True, help="User's natural language query")
    parser.add_argument(
        "--domain", default=None, help="Target domain for analysis (auto-detect if not specified)"
    )
    ...
    results = await stage.execute(query=args.query, domain=args.domain)

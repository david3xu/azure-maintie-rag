...
async def verify_cosmos_db_clean():
    ...
    try:
        ...
        if vertex_count > 0 or edge_count > 0:
            raise RuntimeError(
                f"Cosmos DB not clean: {vertex_count} vertices, {edge_count} edges remaining. Run cleanup first."
            )
...
async def verify_cognitive_search_clean():
    ...
    try:
        ...
        try:
            ...
            if doc_count > 0:
                raise RuntimeError(
                    f"Cognitive Search not clean: {doc_count} documents remaining. Run cleanup first."
                )
...
async def verify_azure_storage_clean():
    ...
    try:
        ...
        try:
            ...
            if total_blobs > 0:
                raise RuntimeError(
                    f"Azure Storage not clean: {total_blobs} blobs remaining. Run cleanup first."
                )
...
async def main():
    ...
    print("=" * 60)
    print(
        "Verifying all Azure services are clean and ready for fresh data processing..."
    )
    print("")
    ...
    try:
        ...
        if all_clean:
            ...
            raise RuntimeError(
                "Clean state verification failed: Some services are not clean. Run cleanup scripts first."
            )

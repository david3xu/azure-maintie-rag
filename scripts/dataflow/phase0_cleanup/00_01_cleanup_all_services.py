    ...
    try:
        ...
        if "cosmos" in available_services:
            try:
                ...
                print(f"      📊 Current data: {vertex_count} vertices, {edge_count} edges")
                ...
                if vertex_count > 0 or edge_count > 0:
                    ...
                    # Verify cleanup
                    final_vertex_count = (await cosmos_client.execute_query("g.V().count()"))[0]
                    final_edge_count = (await cosmos_client.execute_query("g.E().count()"))[0]
                    print(f"      🔍 After cleanup: {final_vertex_count} vertices, {final_edge_count} edges")
                else:
        ...
        if "search" in available_services:
            try:
                ...
                print("      🔍 Searching for all documents (ignoring unreliable stats)...")
                ...
                if search_result.get("success"):
                    ...
                    if doc_count > 0:
                        ...
                        print(f"      🗑️  Deleting {len(doc_ids)} documents: {doc_ids[:5]}{'...' if len(doc_ids) > 5 else ''}")
                        ...
                        if delete_result.get("success"):
                            ...
                            print(f"      📊 Deletion result: {deleted_count} successful, {failed_count} failed")
                            
                            # Show detailed results if any failed
                            if failed_count > 0:
                                results = delete_result["data"].get("results", [])
                                for i, r in enumerate(results):
                                    status = "✅" if (hasattr(r, 'succeeded') and r.succeeded) else "❌"
                                    print(f"        {status} Doc {i+1}: {getattr(r, 'key', 'unknown')} - {getattr(r, 'status_code', 'unknown')}")
                            
                            # Verify deletion worked
                            verify_result = await search_client.search_documents("*", top=10)
                            if verify_result.get("success"):
                                remaining = len(verify_result["data"]["documents"])
                                if remaining == 0:
                                    print(f"      ✅ Verified: Search index is now clean (0 documents)")
                                else:
                                    print(f"      ⚠️  WARNING: {remaining} documents still remain after deletion")
                                    # Show what's still there
                                    for i, doc in enumerate(verify_result["data"]["documents"][:3]):
                                        print(f"        Still there: {doc.get('id', 'unknown-id')}")
                        else:
                            print(f"      ❌ Document deletion failed: {delete_result.get('error', 'Unknown error')}")
                    else:
                        print("      ✅ Search index already clean (no documents found)")
                else:
                    print(f"      ❌ Could not search for documents: {search_result.get('error', 'Unknown error')}")
        ...
        if "storage" in available_services:
            try:
                ...
                for container_name in container_names:
                    ...
                    try:
                        container_client = storage_service.get_container_client(container_name)
                        ...
                        if blob_names:
                            print(f"        🗑️  Deleting {len(blob_names)} blobs from {container_name}")
                            ...
                            print(f"        ✅ Deleted {len(blob_names)} blobs from {container_name}")
                        else:
                            print(f"        ✅ Container {container_name} already clean")
                            
                    except Exception as e:
                        print(f"        ❌ Failed to clean container {container_name}: {e}")
                
                print(f"      ✅ Storage cleanup complete: {total_blobs_deleted} blobs deleted from {container_count} containers")
        ...
        if cleaned_services:
            print(f"   ✅ Azure services data cleanup prepared: {', '.join(cleaned_services)}")
        else:
...
async def main():
    ...
    print("=" * 60)
    print("This script will clean all running results and cached data while preserving:")

...
async def validate_phase0_completion():
    ...
    if not results_path.exists():
        raise RuntimeError("Phase 0 cleanup results not found. Run 'make dataflow-cleanup' first.")
    
    with open(results_path) as f:
        cleanup_results = json.load(f)
    
    if not cleanup_results.get("all_services_clean", False):
        raise RuntimeError("Phase 0 cleanup incomplete - Azure services not clean. Re-run cleanup.")
    
    print(f"   ✅ Phase 0 cleanup completed successfully")
    print(f"   📊 Services cleaned: {len(cleanup_results.get('services_verified', []))}")
    return True
    ...
    try:
        ...
        try:
            ...
            if knowledge_extraction_agent and knowledge_extraction_agent.model:
                print(f"   ✅ Knowledge Extraction Agent operational (full extraction testing deferred to Phase 3)")
                print(f"   ℹ️  Entity/relationship extraction will be validated with real data in Phase 3")
            else:
                raise RuntimeError("Knowledge Extraction Agent not properly initialized")
        except Exception as ke_init_error:
            raise RuntimeError(f"Knowledge Extraction Agent initialization failed: {ke_init_error}")
        ...
        try:
            ...
            if universal_search_agent and universal_search_agent.model:
                print(f"   ✅ Universal Search Agent operational (full search testing deferred to Phase 3)")
            else:
                raise RuntimeError("Universal Search Agent not properly initialized")
        except Exception as search_init_error:
            raise RuntimeError(f"Universal Search Agent initialization failed: {search_init_error}")
        
        print(f"   ✅ All Phase 1 agents validated")
        return True
        
    except Exception as e:
        raise RuntimeError(f"Phase 1 agent validation failed: {e}. Run 'make dataflow-validate' first.")
...
async def validate_azure_services_ready():
    ...
    try:
        ...
        required_services = {"openai", "storage", "search"}  # Use correct service names from universal_deps.py
...
async def main():
    ...
    try:
        ...
        print(f"✅ Source data: {validation_results['source_data']['file_count']} files available")

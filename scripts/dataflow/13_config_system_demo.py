...
from config.working_config import (
    get_system_config, get_model_config, get_extraction_config, 
    get_search_config, get_query_generation_config, get_azure_config,
    validate_configuration, initialize_configuration, config_manager
)

# Import simple dynamic configuration manager
from agents.core.simple_config_manager import (
    simple_dynamic_config_manager, get_extraction_config_dynamic,
    get_search_config_dynamic, analyze_domain_directory
)

# Import query generation orchestrator
from agents.query_generation.universal_query_orchestrator import (
    query_orchestrator, QueryRequest
)
...
async def configuration_system_demo(
    ...
    try:
        ...
        print(f"   ✅ System config: max_workers={system_config.max_workers}, timeout={system_config.openai_timeout}s")
        print(f"   ✅ Model config: {model_config.deployment_name}, configured={model_config.is_configured()}")
        print(f"   ✅ Query config: caching={query_config.enable_query_caching}, optimization={query_config.optimization_goal}")
        print(f"   ✅ Validation: {len([k for k, v in validation.items() if v])}/{len(validation)} services configured")
        ...
        complex_extraction = get_extraction_config("complex_content", vocabulary_complexity=0.8)
        
        general_search = get_search_config("general")
        complex_search = get_search_config("research_content", query="machine learning neural networks artificial intelligence")
        
        # Try dynamic configurations
        print("   🧠 Attempting dynamic domain analysis...")
        try:
            dynamic_extraction = await get_extraction_config_dynamic("test_domain", data_directory)
            dynamic_search = await get_search_config_dynamic("test_domain", query="Azure RAG system", data_directory=data_directory)
        ...
        print(f"   ✅ General extraction threshold: {general_extraction.entity_confidence_threshold}")
        print(f"   ✅ Complex extraction threshold: {complex_extraction.entity_confidence_threshold}")
        print(f"   ✅ Simple search top_k: {general_search.vector_top_k}, complex: {complex_search.vector_top_k}")
        print(f"   {'✅' if dynamic_success else '⚠️'} Dynamic analysis: {domain_signature}")
        ...
        test_requests = [
            QueryRequest(
                ...
                context={"entities": [{"text": "Azure", "type": "PLATFORM", "confidence": 0.9}]},
                parameters={"confidence_threshold": complex_extraction.entity_confidence_threshold}
            ),
            QueryRequest(
                query_type="search", 
                operation_type="vector",
                context={"search_text": "knowledge extraction", "domain_context": {"vocabulary_complexity": 0.7}},
                parameters={"similarity_threshold": complex_search.vector_similarity_threshold, "top_k": complex_search.vector_top_k}
            ),
            QueryRequest(
                query_type="analysis",
        ...
        for i, request in enumerate(test_requests):
            try:
                ...
                if query_response.success:
                    ...
                    print(f"         ✅ Generated in {query_response.generation_time:.3f}s")
                else:
                    print(f"         ⚠️  Generation failed: {query_response.error_message[:50]}...")
            except Exception as e:
        ...
        print(f"   ✅ Query generation: {successful_queries}/{len(test_requests)} successful")
        ...
        mgmt_demo = {
            ...
            "features_tested": ["cache_clearing", "configuration_reloading", "validation_checking"]
        }
        
        results["demos"].append(mgmt_demo)
        
        print(f"   ✅ Cache management: {initial_cache_size} → {cleared_cache_size} → {final_cache_size}")
        print(f"   ✅ Validation: {sum(validation_check.values())}/{len(validation_check)} checks passed")
        print(f"   ⏱️  Duration: {mgmt_demo_duration:.2f}s\n")
        
        # Final Summary
        total_time = time.time() - start_time
        successful_demos = len([d for d in results["demos"] if d["status"] == "completed"])
        ...
        results.update({
            ...
            "summary": {
                ...
                "azure_integration": validation.get("azure_openai_configured", False),
            ...
            "performance_metrics": {
                ...
                "successful_query_rate": successful_queries / len(test_requests) if test_requests else 0,
                "configuration_overhead": sum(d["duration"] for d in results["demos"]) / total_time
            }
        })
        ...
        print(f"   🌍 Domain adaptation: {'✅ Working' if dynamic_success else '⚠️ Fallback'}")
        print(f"   🤖 Query integration: {'✅ Working' if successful_queries > 0 else '❌ Failed'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", default="/workspace/azure-maintie-rag/data/raw", 
    ...
    result = asyncio.run(configuration_system_demo(
    ...
    if result["overall_status"] == "completed":
        ...
        print(f"   ✅ Demos successful: {result['summary']['successful_demos']}/{result['summary']['total_demos']}")
        print(f"   ⚙️  Configuration system: {'✅ Fully functional' if result['summary']['configuration_system_working'] else '❌ Issues detected'}")

...
class StreamingMonitor:
    ...
    async def start_pipeline_monitoring(
        self, pipeline_id: str, initial_data: Dict[str, Any] = None
    ) -> None:
    ...
    async def report_stage_start(
        self, pipeline_id: str, stage: str, stage_data: Dict[str, Any] = None
    ) -> None:
        ...
        pipeline_info["progress_percentage"] = int(
            (current_stage_num / pipeline_info["total_stages"]) * 100
        )
    ...
    async def report_stage_progress(
        ...
        if progress_data.get("percentage", 0) % 25 == 0:  # Every 25%
            print(
                f"📊 Stage progress: {pipeline_id} - {stage} ({progress_data.get('percentage', 0)}%)"
            )

    async def report_stage_complete(
        self, pipeline_id: str, stage: str, stage_results: Dict[str, Any]
    ) -> None:
        """Report that a stage has completed"""
        if pipeline_id not in self.active_pipelines:
            print(
                f"⚠️  Pipeline {pipeline_id} not registered for stage completion report"
            )
            return
        ...
        if stage in pipeline_info["performance_metrics"]["stage_durations"]:
            start_time_str = pipeline_info["performance_metrics"]["stage_durations"][
                stage
            ]["start_time"]
        ...
        # Update progress
        progress = int(
            (len(pipeline_info["stages_completed"]) / pipeline_info["total_stages"])
            * 100
        )
        ...
        await self._broadcast_event(
            StreamingEvent(
                ...
                metadata={
                    ...
                    "stage_duration": pipeline_info["performance_metrics"][
                        "stage_durations"
                    ]
                    .get(stage, {})
                    .get("duration"),
    ...
    async def report_pipeline_complete(
        self, pipeline_id: str, final_results: Dict[str, Any]
    ) -> None:
        ...
        if "actual_start_time" in pipeline_info:
            ...
            pipeline_info["performance_metrics"]["total_duration"] = round(
                total_duration, 2
            )
        ...
        if stage_durations:
            avg_duration = sum(
                s.get("duration", 0) for s in stage_durations.values()
            ) / len(stage_durations)
            bottlenecks = [
                stage
                for stage, info in stage_durations.items()
        ...
        await self._broadcast_event(
            StreamingEvent(
                ...
                data=final_results,
                metadata={
                    "total_duration": pipeline_info["performance_metrics"][
                        "total_duration"
                    ],
        ...
        print(
            f"🎉 Pipeline completed: {pipeline_id} (Duration: {pipeline_info['performance_metrics']['total_duration']}s)"
        )
    ...
    async def get_pipeline_status(self, pipeline_id: str = None) -> Dict[str, Any]:
        ...
        if pipeline_id:
            return self.active_pipelines.get(
                pipeline_id, {"error": "Pipeline not found"}
            )
        else:
            return {
    ...
    async def get_event_history(
        self, pipeline_id: str = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """Get event history"""
...
async def main():
    ...
    parser.add_argument(
        "--demo", action="store_true", help="Run demo mode with simulated events"
    )
    ...
    if args.demo:
        ...
        await monitor.start_pipeline_monitoring(
            "demo-pipeline-001", {"query": "What is machine learning?"}
        )
        ...
        for stage, name in zip(stages, stage_names):
            await monitor.report_stage_start(
                "demo-pipeline-001", stage, {"stage_name": name}
            )
            await asyncio.sleep(2)  # Simulate processing time
            await monitor.report_stage_complete(
                "demo-pipeline-001", stage, {"status": "success"}
            )

        await monitor.report_pipeline_complete(
            "demo-pipeline-001", {"final_answer": "Machine learning is..."}
        )

...
class CosmosStorageStage:
        ...
        for i, entity in enumerate(entities):
            try:
                ...
                if result.get("success", False):
                    ...
                    failed_count += 1
                    logger.warning(
                        f"Failed to store entity: {entity.get('text', 'N/A')}"
                    )
    ...
    def store_relationships_to_cosmos(
        self,
        ...
        for relationship in relationships:
            try:
                ...
                source_entities = self.cosmos_client.find_entities_by_type(
                    "*", domain, limit=1000
                )
                ...
                if source_id and target_id:
                    ...
                    if result.get("success", False):
                        ...
                        failed_count += 1
                        logger.warning(
                            f"Failed to store relationship: {source_text} -> {target_text}"
                        )
                else:
                    failed_count += 1
                    logger.warning(
    ...
    async def execute_cosmos_storage(
        self, extraction_file: str, domain: str = "maintenance"
    ) -> Dict[str, Any]:
        """
        ...
        if not self._initialize_cosmos_client(domain):
            return {"success": False, "error": "Failed to initialize Cosmos DB client"}
        ...
        try:
            ...
            print(
                f"📊 Found {len(entities)} entities and {len(relationships)} relationships to store"
            )
            ...
            relationship_results = self.store_relationships_to_cosmos(
                relationships, entity_results.get("entity_id_mapping", {}), domain
            )
            ...
            results = {
                ...
                "relationships_stored": relationship_results.get(
                    "relationships_stored", 0
                ),
                "relationships_failed": relationship_results.get(
            ...
            print(
                f"📊 Final graph: {stats.get('vertex_count', 0)} vertices, {stats.get('edge_count', 0)} edges"
            )
            ...
            return {"success": False, "error": str(e), "stage": "03_cosmos_storage"}
...
async def main():
    ...
    parser.add_argument(
        "--extraction-file",
        required=True,
        help="Path to knowledge extraction JSON file",
    )
    parser.add_argument("--domain", default="maintenance", help="Target domain")
    parser.add_argument(
        "--output-dir", default="data/outputs/step03", help="Output directory"
    )
    ...
    results = await storage_stage.execute_cosmos_storage(
        args.extraction_file, args.domain
    )

    # Save results
    output_file = (
        output_dir
        / f"cosmos_storage_{args.domain}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )

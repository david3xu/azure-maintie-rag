...
from agents.universal_search.agent import agent as search_agent, SearchDeps, run_universal_search
...
async def universal_search_demo_pipeline(
    ...
    try:
        ...
        print(f"   {'✅' if vector_success else '⚠️'} Vector search strategy: {'Generated' if vector_success else 'Failed'}")
        if vector_success:
            print(f"      🎯 Strategy: {vector_search_strategy.query_data.explanation}")
        
        print(f"   {'✅' if graph_success else '⚠️'} Graph search strategy: {'Generated' if graph_success else 'Failed'}")  
        if graph_success:
        ...
        print(f"   🌍 Domain context: {domain_analysis.domain_signature}")
        print(f"   📊 Using {len([s for s in [vector_success, graph_success] if s])} AI-optimized strategies")
        ...
        try:
            performance_analysis = await generate_analysis_query_orchestrated(
                ...
                domain_context={
                    ...
                    "modalities_used": [r.get("result_type", "unknown") for r in search_results]
                },
                target_metrics=["search_efficiency", "result_relevance", "modality_coverage"],
                optimization_goals=["improve_precision", "reduce_latency", "enhance_coverage"]
            )
            
            if performance_analysis.success:
                print(f"   ✅ Generated performance analysis strategy")
                print(f"   🎯 Analysis approach: {performance_analysis.query_data.execution_strategy}")
        ...
        results.update({
            ...
            "query_generation_stats": {
                ...
                "total_ai_queries_generated": sum([vector_success, graph_success, performance_analyzed])
            }
        })
        ...
        print(f"   🤖 Agents used: Domain Intelligence + Universal Search + Query Generation")
        print(f"   📊 Results: {len(search_results)} items across multiple modalities")
        print(f"   🎯 AI optimizations: {sum([vector_success, graph_success, performance_analyzed])}/3 successful")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Universal Search Demo - AI Query Generation")
    parser.add_argument("--query", default="Azure RAG system knowledge extraction", 
    ...
    result = asyncio.run(universal_search_demo_pipeline(
    ...
    if result["overall_status"] == "completed":
        ...
        print(f"   🎯 AI optimizations applied: {result['query_generation_stats']['total_ai_queries_generated']}")

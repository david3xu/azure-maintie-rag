...
async def test_domain_intelligence_connectivity():
    ...
    try:
        ...
        from agents.domain_intelligence.agent import domain_intelligence_agent, run_domain_analysis
        ...
        if not result or not result.domain_signature:
            raise RuntimeError("Domain Intelligence Agent not generating basic analysis")
...
async def test_knowledge_extraction_connectivity():
    ...
    try:
        ...
        print("   ⚠️  Note: Data processing tests will run in Phase 2 after data ingestion")
...
async def test_universal_search_connectivity():
    ...
    try:
        # Test imports
        print("   📦 Testing agent imports...")
        from agents.universal_search.agent import universal_search_agent, run_universal_search
        from agents.core.universal_deps import get_universal_deps
        print("   ✅ Agent imports successful")
        ...
        print("   🔧 Search capability testing deferred to Phase 3 (databases empty after cleanup)")
...
async def main():
    ...
    try:
        ...
        validation_results["domain_intelligence"] = await test_domain_intelligence_connectivity()
        validation_results["knowledge_extraction"] = await test_knowledge_extraction_connectivity()
        validation_results["universal_search"] = await test_universal_search_connectivity()
        ...
        print(f"📋 Next: Phase 2 will ingest data, then validate full agent processing capability")
        return True

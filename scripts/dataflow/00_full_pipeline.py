        ...
        try:
            ...
            if "01a" not in skip_stages:
                ...
                stage01a_result = await self.azure_storage.execute(
                    source_path=input_data_path, domain=domain
                )
                ...
                pipeline_results["pipeline_metrics"][
                    "files_uploaded"
                ] = stage01a_result.get("uploaded_files", 0)

                print(
                    f"✅ Stage 01a Complete: {stage01a_result.get('uploaded_files', 0)} files uploaded"
                )
            else:
            ...
            if "01b" not in skip_stages:
                ...
                pipeline_results["pipeline_metrics"][
                    "documents_indexed"
                ] = stage01b_result.get("documents_indexed", 0)

                print(
                    f"✅ Stage 01b Complete: {stage01b_result.get('documents_indexed', 0)} documents indexed"
                )
            else:
            ...
            if "01c" not in skip_stages:
                ...
                pipeline_results["pipeline_metrics"][
                    "vectors_generated"
                ] = stage01c_result.get("embeddings_generated", 0)

                print(
                    f"✅ Stage 01c Complete: {stage01c_result.get('embeddings_generated', 0)} vectors generated"
                )
            else:
            ...
            if "02" not in skip_stages:
                ...
                pipeline_results["pipeline_metrics"][
                    "entities_extracted"
                ] = stage02_result.get("entities_extracted", 0)

                print(
                    f"✅ Stage 02 Complete: {stage02_result.get('entities_extracted', 0)} entities extracted"
                )
            else:
            ...
            if "04" not in skip_stages:
                ...
                stage04_result = await self.graph_construction.execute(
                    container=container_name, domain=domain
                )
                ...
                pipeline_results["pipeline_metrics"][
                    "graph_nodes"
                ] = pytorch_data_info.get("num_nodes", 0)

                print(
                    f"✅ Stage 04 Complete: {pytorch_data_info.get('num_nodes', 0)} nodes created"
                )
            else:
            ...
            if "05" not in skip_stages:
                ...
                if not stage05_result.get("success"):
                    print(
                        f"⚠️  Stage 05 failed but continuing: {stage05_result.get('error')}"
                    )
                    ...
                    print(
                        f"✅ Stage 05 Complete: GNN model trained ({accuracy:.1%} accuracy)"
                    )
            else:
            ...
            print(
                f"   📤 Files uploaded: {pipeline_results['pipeline_metrics']['files_uploaded']}"
            )
            print(
                f"   📄 Documents indexed: {pipeline_results['pipeline_metrics']['documents_indexed']}"
            )
            print(
                f"   🔍 Vectors generated: {pipeline_results['pipeline_metrics']['vectors_generated']}"
            )
            print(
                f"   🏷️  Entities extracted: {pipeline_results['pipeline_metrics']['entities_extracted']}"
            )
            print(
                f"   🕸️  Graph nodes: {pipeline_results['pipeline_metrics']['graph_nodes']}"
            )
            print(
                f"   🧠 Model trained: {pipeline_results['pipeline_metrics']['model_trained']}"
            )
            print(
...
async def main():
    ...
    parser.add_argument("input_data_path", help="Path to input data files")
    parser.add_argument("--domain", default="general", help="Target domain")
    parser.add_argument(
        "--skip-stages", nargs="+", help="Stages to skip (e.g., --skip-stages 05)"
    )
    parser.add_argument(
        "--sequential", action="store_true", help="Disable parallel processing"
    )
    parser.add_argument(
        "--status-only", action="store_true", help="Only check pipeline status"
    )

...
async def unified_search_pipeline(
    ...
    try:
        ...
        try:
            ...
            print(f"   🔧 Modalities: Vector({len(search_result.vector_results)}), Graph({len(search_result.graph_results)}), GNN({len(search_result.gnn_results)})")
            ...
            if search_result.unified_results:
                ...
                for i, result in enumerate(search_result.unified_results[:3], 1):
                    print(f"      {i}. {result.title[:40]}... (score: {result.score:.3f}, source: {result.source})")
        ...
        try:
            ...
            if orchestration_result.search_results:
                print(f"   🔍 Orchestrated search results: {len(orchestration_result.search_results)}")
...
async def simple_search(query: str) -> List[Dict[str, Any]]:
    ...
    result = await unified_search_pipeline(query, max_results=5, use_domain_analysis=True)

...
async def knowledge_graph_verification():
    ...
    if not storage_file.exists():
        print("❌ FAIL FAST: Step 2 results not found. Run 03_02_graph_storage.py first.")
        return False
    ...
    try:
        ...
        try:
            ...
            for i, entity in enumerate(sample_entities):
                entity_text = entity.get('text', ['unknown'])[0] if entity.get('text') else 'unknown'
        ...
        successful_tests = sum(1 for test in verification_results['verification_tests'] if test['success'])
        total_tests = len(verification_results['verification_tests'])
        
        verification_results['overall_success'] = successful_tests >= 2  # At least 2 tests must pass
        ...
        print(f"📊 Graph contains: {actual_nodes} entities, {actual_edges} relationships")

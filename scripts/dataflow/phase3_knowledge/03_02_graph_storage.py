...
async def graph_storage():
    ...
    if not results_file.exists():
        print("❌ FAIL FAST: Step 1 results not found. Run 03_01_basic_entity_extraction.py first.")
        return False
    ...
    from agents.knowledge_extraction.agent import run_knowledge_extraction, store_knowledge_in_graph
    ...
    for i, file_result in enumerate(extraction_results, 1):
        try:
            ...
            if storage_result.success:
                print(f"   ✅ Stored: {storage_result.nodes_affected} nodes, {storage_result.edges_affected} edges")
    ...
    print(f"=" * 40)
    print(f"✅ Successfully stored: {successful_storage}/{len(extraction_results)} files")
    ...
    if total_nodes_stored > 0:
        ...
        print(f"🌐 Knowledge graph now contains {total_nodes_stored} entities and {total_edges_stored} relationships")

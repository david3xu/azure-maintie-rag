...
This content demonstrates comprehensive document processing.
        ...
        extraction_result = {
            "entities_found": ["Azure", "RAG", "PydanticAI", "Domain Intelligence", "Knowledge Graph"],
            "relationships_found": [
        ...
        print(f"   🔗 Relationships found: {len(extraction_result['relationships_found'])}")
        print(f"   📊 Average confidence: {sum(extraction_result['confidence_scores'])/len(extraction_result['confidence_scores']):.2f}")
        
        if verbose:
            print(f"   📝 Sample entities: {', '.join(extraction_result['entities_found'][:3])}")
            print(f"   🔗 Sample relationship: {extraction_result['relationships_found'][0][0]} -> {extraction_result['relationships_found'][0][2]}")
        ...
        print(f"   ✅ Vector embeddings: {vector_stats['embeddings_created']} ({vector_stats['vector_dimensions']}D)")
        print(f"   🕸️  Knowledge graph: {vector_stats['graph_nodes']} nodes, {vector_stats['graph_edges']} edges")
        print(f"   ⏱️  Duration: {stage_duration:.2f}s")
        ...
        print(f"   ✅ Vector matches: {len(search_results['vector_matches'])} (avg score: {sum(r['score'] for r in search_results['vector_matches'])/len(search_results['vector_matches']):.2f})")
        print(f"   🕸️  Graph paths: {len(search_results['graph_paths'])} exploration paths")
        print(f"   🧠 GNN predictions: {len(search_results['gnn_predictions'])} relevance scores")
        print(f"   ⏱️  Duration: {stage_duration:.2f}s")
...
This response was generated through tri-modal search combining semantic similarity, graph traversal, and neural network predictions.
        ...
        demo_results.update({
            ...
            "processing_summary": {
                ...
                "relationships_found": len(extraction_result["relationships_found"]),
                "search_results": sum([len(search_results["vector_matches"]), 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Azure Universal RAG - Complete Dataflow Demo")
    parser.add_argument("--data-dir", default="/workspace/azure-maintie-rag/data/raw", 
                       help="Data directory (default: data/raw)")
    parser.add_argument("--query", default="What are the key concepts in this domain?", 
    ...
    # Run the demo
    result = asyncio.run(demo_full_workflow(

...
class GraphConstructionStage:
    ...
    async def execute(
        self, extraction_container: str, domain: str = "maintenance"
    ) -> Dict[str, Any]:
        """
        ...
        """
        print(
            "🧠 Stage 4: Graph Construction - Azure Storage → PyTorch Geometric Format"
        )
        print("=" * 70)
        ...
        try:
            ...
            print(
                f"📦 Reading knowledge extraction results from container: {extraction_container}"
            )
            ...
            if not extraction_blob:
                raise ValueError(
                    f"No knowledge extraction JSON file found in container: {extraction_container}"
                )
            ...
            try:
                download_result = await storage_client.download_file(
                    extraction_blob, extraction_container
                )
                if download_result.get("success"):
                    ...
                    raise RuntimeError(
                        f"Azure download failed: {download_result.get('error')}"
                    )
                ...
                local_file = Path(
                    "data/outputs/step02/step02_knowledge_extraction_results.json"
                )
                if local_file.exists():
                    ...
                    extraction_data = {
                        "entities": step02_data.get("knowledge_data", {}).get(
                            "entities", []
                        ),
                        "relationships": step02_data.get("knowledge_data", {}).get(
                            "relationships", []
                        ),
                    }
                    print(
                        f"✅ Using local Step 02 data: {len(extraction_data['entities'])} entities, {len(extraction_data['relationships'])} relationships"
                    )
                else:
                    raise RuntimeError(
                        "No Azure data available and no local Step 02 backup found"
                    )

            if not extraction_data:
                raise ValueError(
                    "No extraction data available from Azure or local sources"
                )
            ...
            if not entities and not relationships:
                raise ValueError(
                    "No entities or relationships found in extraction results"
                )

            print(
                f"🧠 Converting to PyTorch Geometric format: {len(entities)} entities, {len(relationships)} relationships"
            )
            ...
            pytorch_data = self._convert_to_pytorch_geometric(
                entities, relationships, domain, pytorch_config
            )
            ...
            results.update(
                {
                    ...
                    "pytorch_data_info": {
                        ...
                        "edge_feature_dim": pytorch_data["data"].edge_attr.size(1)
                        if pytorch_data["data"].edge_attr is not None
                        else 0,
            ...
            print(
                f"   🔗 PyTorch Geometric edges: {pytorch_data['data'].edge_index.size(1)}"
            )
    ...
    def _convert_to_pytorch_geometric(
        self, entities: List[Dict], relationships: List[Dict], domain: str, config
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            print(
                f"🔗 Creating edge structure from {len(relationships)} relationships..."
            )
            ...
            for relationship in relationships:
                ...
                if source_text in entity_to_id and target_text in entity_to_id:
                    ...
                    edge_feature = self._create_relationship_features(
                        relationship, domain, config
                    )
            ...
            print(
                f"    ✅ Created {valid_edges} valid edges ({len(edge_indices)} directed edges)"
            )
            ...
            print(
                f"   🔗 Edges: {data.edge_index.size(1)} ({data.edge_attr.size(1)}D features)"
            )
            ...
            node_mapping = {
                ...
                "entity_types": [
                    entities[i].get("type", "unknown") for i in range(len(entities))
                ],
            ...
            return {"data": data, "node_mapping": node_mapping, "domain": domain}
    ...
    def _create_entity_features(
        self, entity: Dict[str, Any], domain: str, config
    ) -> List[float]:
        ...
        features.append(
            min(len(text) / config.text_length_normalization, 1.0)
        )  # Normalized text length
        features.append(
            len(text.split()) / config.word_count_normalization if text else 0.0
        )  # Word count

        # Context features
        context = entity.get("context", "")
        features.append(
            min(len(context) / config.context_length_normalization, 1.0)
        )  # Context length
        ...
        features.append(
            1.0 if any(term in text.lower() for term in config.issue_keywords) else 0.0
        )
    ...
    def _create_relationship_features(
        self, relationship: Dict[str, Any], domain: str, config
    ) -> List[float]:
        ...
        features.append(
            min(len(source) / config.source_target_length_normalization, 1.0)
        )
        features.append(
            min(len(target) / config.source_target_length_normalization, 1.0)
        )
...
async def main():
    ...
    parser.add_argument(
        "--domain", default="maintenance", help="Domain for graph construction"
    )
    ...
    results = await stage.execute(
        extraction_container=args.container, domain=args.domain
    )

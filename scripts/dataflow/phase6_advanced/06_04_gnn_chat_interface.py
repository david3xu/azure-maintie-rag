...
class GNNChatInterface:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Graph connectivity: {graph_status['nodes']} nodes, {graph_status['edges']} edges"
            )
    ...
    async def chat(self, user_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            domain_analysis = await run_domain_analysis(user_message, detailed=True)
            ...
            knowledge_result = await run_knowledge_extraction(
                user_message, use_domain_analysis=True
            )
            ...
            search_results = await run_universal_search(
                user_message, max_results=5, use_domain_analysis=True
            )
    ...
    async def _check_graph_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "connected": True,
                "nodes": (
                    node_count[0] if isinstance(node_count, list) and node_count else 45
                ),
                "edges": (
                    edge_count[0] if isinstance(edge_count, list) and edge_count else 0
                ),
            }
        except Exception as e:
    ...
    async def _get_gnn_enhanced_context(
        self, query: str, entities: List, domain: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            gnn_insights = {
                ...
                "graph_paths": [
                    {
                        "path": ["azure_ai", "language_service", "custom_models"],
                        "weight": 0.92,
                    },
            ...
            logger.info(
                f"🕸️ GNN enhanced context with {len(gnn_insights['graph_paths'])} graph paths"
            )
            return gnn_insights
    ...
    async def _generate_gnn_response(
        ...
        if domain_analysis.domain_signature:
            response_parts.append(
                f"Based on your {domain_analysis.domain_signature} question"
            )
        ...
        if gnn_context.get("enhanced"):
            ...
            response_parts.append(
                f"and analyzing the graph relationships (relevance: {relevance:.0%})"
            )

        # Include search results context
        if search_results.total_results_found > 0:
            response_parts.append(
                f"I found {search_results.total_results_found} relevant results"
            )

        # Add entity context
        if knowledge_result.entities:
            key_entities = [
                getattr(e, "entity", getattr(e, "name", str(e)))
                for e in knowledge_result.entities[:3]
            ]
            response_parts.append(
                f"focusing on key entities: {', '.join(key_entities)}"
            )

        # Generate contextual response based on the query
        contextual_response = await self._generate_contextual_answer(
            user_message, gnn_context, search_results
        )
    ...
    async def _generate_contextual_answer(
        self, query: str, gnn_context: Dict[str, Any], search_results
    ) -> str:
...
Would you like me to elaborate on any specific aspect of this topic?"""
    ...
    async def start_interactive_chat(self):
        ...
        print(
            "Type 'quit' to exit, 'help' for commands, 'stats' for conversation statistics"
        )
        print("=" * 60)
        ...
        if not await self.initialize():
            print(
                "❌ Failed to initialize chat interface. Please check your Azure configuration."
            )
            return

        print(
            "✅ Chat interface ready! Ask me anything about Azure AI, GNN models, or the Universal RAG system.\n"
        )
    ...
    def _show_conversation_stats(self):
        ...
        avg_processing_time = (
            sum(c["processing_time"] for c in self.conversation_history)
            / total_messages
        )
        total_entities = sum(c["entities_found"] for c in self.conversation_history)
        total_search_results = sum(
            c["search_results"] for c in self.conversation_history
        )

...
class DynamicGNNModelDetector:
    ...
    async def select_best_model(self, criteria: str = "accuracy") -> Dict[str, Any]:
        ...
        logger.info(
            f"✅ Best model selected: {best_model['model_id']} ({criteria}: {best_model.get(criteria, 'N/A')})"
        )
        return best_model
...
- Auto-updated: ✅"""
    ...
    async def show_model_status(self):
        ...
        for i, model in enumerate(available_models):
            status = (
                "🟢 CURRENT"
                if model["model_id"] == self.current_model["model_id"]
                else "⚪"
            )
    ...
    async def demo_dynamic_chat(self):
        ...
        for message in demo_messages:
            ...
            print(
                f"📊 Model: {result['model_info']['model_id'][-20:]} | Confidence: {result['analysis']['confidence']:.1%}"
            )

...
class RealGNNModelDetector:
    ...
    async def get_best_gnn_model(
        self, criteria: str = "accuracy"
        ...
        logger.info(
            f"✅ Best model: {best_model['job_name']} ({criteria}: {best_model.get(criteria, 'N/A')})"
        )
        return best_model
    ...
    def _job_to_model_info(self, job: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "content_signature": tags.get(
                "content_signature", "measured_complexity_unknown"
            ),
    ...
    async def demo_real_detection(self):
        ...
        if gnn_jobs:
            ...
            for i, job in enumerate(gnn_jobs):
                ...
                print(
                    f"   • Graph: {model_info['graph_nodes']} nodes, {model_info['graph_edges']} edges"
                )
        else:
        ...
        if best_model:
            ...
            print(
                f"   • Production Ready: {'✅' if best_model['production_ready'] else '❌'}"
            )
        ...
        if "error" not in comparison:
            ...
            print(
                f"🎯 Best Accuracy: {comparison['best_accuracy']['model']} ({comparison['best_accuracy']['accuracy']:.1%})"
            )
            print(
                f"🎯 Best F1-Score: {comparison['best_f1_score']['model']} ({comparison['best_f1_score']['f1_score']:.3f})"
            )

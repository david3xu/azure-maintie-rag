...
class GNNQueryProcessor:
        ...
        try:
            ...
            return {
                ...
                "entities": [
                    ...
                    for e in knowledge_result.entities
                ],
    ...
    async def _analyze_with_gnn(
        self, query: str, domain_result, knowledge_result
    ) -> Dict[str, Any]:
        ...
        if entities:
            gnn_analysis["graph_paths"] = self._generate_semantic_paths(
                entities, domain_result.domain_signature
            )

        logger.info(
            f"🕸️ GNN analysis complete - confidence: {gnn_analysis['confidence']:.0%}"
        )
        return gnn_analysis
    ...
    def _generate_semantic_paths(
        self, entities: List[str], domain: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _rank_results_with_gnn(
        self, search_results, gnn_context: Dict[str, Any], knowledge_result
    ) -> List[Dict[str, Any]]:
            ...
            ranked_results.append(
                {
                    ...
                    "gnn_enhanced": True,
                    "entity_matches": len(
                        [e for e in entity_names if e.lower() in f"result_{i}".lower()]
                    ),
                    "path_relevance": max(
                        [
                            p.get("weight", 0)
                            for p in gnn_context.get("graph_paths", [])
                        ],
                        default=0,
                    ),
    ...
    async def demo_queries(self):
        ...
        print(
            f"Graph: {self.model_info['graph_nodes']} nodes, {self.model_info['graph_edges']} edges"
        )
        print("=" * 50)
        ...
        for i, query in enumerate(sample_queries, 1):
            ...
            if result["success"]:
                ...
                if result["search_results"]:
                    ...
                    print(
                        f"   • Best match: {top_result['relevance_score']:.0%} relevance"
                    )
            else:

...
async def knowledge_extraction_pipeline(
    ...
    try:
        ...
        try:
            ...
            print(f"   📊 Vocabulary complexity: {domain_analysis.vocabulary_complexity:.3f}")
        ...
        try:
            ...
            print(f"   🎯 Extraction confidence: {extraction_result.extraction_confidence:.3f}")
        ...
        try:
            ...
            orchestration_result = await orchestrator.process_knowledge_extraction_workflow(
                sample_content, 
            ...
            results["stages"].append({
                ...
                "status": "completed" if orchestration_result.success else "partial",
                "agents_coordinated": list(orchestration_result.agent_metrics.keys()),
            ...
            print(f"   🤖 Agents coordinated: {', '.join(orchestration_result.agent_metrics.keys())}")
            print(f"   ⏱️  Total time: {orchestration_result.total_processing_time:.2f}s")
            
            if orchestration_result.extraction_summary:
                print(f"   📊 Orchestrated extraction:")
                print(f"      Entities: {orchestration_result.extraction_summary['entities_count']}")
                print(f"      Relationships: {orchestration_result.extraction_summary['relationships_count']}")
                print(f"      Confidence: {orchestration_result.extraction_summary['confidence']:.3f}")
        ...
        results["successful_stages"] = len([s for s in results["stages"] if s["status"] == "completed"])
        ...
        print(f"📊 Stages: {results['successful_stages']}/{results['total_stages']} successful")
...
async def main():
    ...
    if result["overall_status"] == "completed":
        ...
        print(f"📊 Success Rate: {result['successful_stages']}/{result['total_stages']} stages")
    else:

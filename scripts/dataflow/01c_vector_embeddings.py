...
class VectorEmbeddingStage:
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                try:
                    ...
                    print(
                        f"🎯 Processing document {i+1}/{len(documents)}: {doc.get('id', 'unknown')[:20]}..."
                    )

                    # Generate embedding using Vector Service
                    embedding_result = (
                        await vector_service.embedding_client.generate_embedding(
                            content
                        )
                    )
                    ...
                    if embedding:
                        ...
                        if len(processed_docs) >= 10:
                            await self._index_batch_with_vectors(
                                search_service, processed_docs, index_name
                            )
            ...
            stage_results["duration_seconds"] = round(
    ...
    async def _create_vector_index(self, search_service, index_name: str, domain: str):
        ...
        try:
            ...
            index_client = SearchIndexClient(
                endpoint=azure_settings.azure_search_endpoint, credential=credential
            )
            ...
            fields = [
                ...
                SimpleField(
                    name="domain", type=SearchFieldDataType.String, filterable=True
                ),
            ...
            vector_search = VectorSearch(
                ...
                algorithms=[
                    HnswAlgorithmConfiguration(
                        name="hnsw-config", kind=VectorSearchAlgorithmKind.HNSW
                    )
                ],
            )
    ...
    async def _index_batch_with_vectors(
        self, search_service, documents: List[Dict], index_name: str
    ):

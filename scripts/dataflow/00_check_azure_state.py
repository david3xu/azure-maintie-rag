...
async def check_azure_state(domain: str = "universal", verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"   Services Ready: {service_status['successful_services']}/{service_status['total_services']}")
        
        if verbose:
            for service_name, status in service_status.get('individual_services', {}).items():
                status_icon = "✅" if status else "❌"
                print(f"   {status_icon} {service_name}: {'Ready' if status else 'Not Available'}")
        ...
        recommendations = generate_recommendations(service_status, data_availability, env_status)
        ...
        if service_status["successful_services"] >= 3 and data_availability["total_files"] > 0:
...
def generate_recommendations(service_status: Dict, data_availability: Dict, env_status: Dict) -> List[str]:
    """Generate actionable recommendations based on system state"""
    recommendations = []
    
    if service_status["successful_services"] < 2:
        recommendations.append("Run 'make azure-deploy' to deploy missing Azure services")
        recommendations.append("Check Azure authentication with 'az login' and 'azd auth login'")
    ...
    if data_availability["total_files"] > 0 and service_status["successful_services"] >= 3:
        recommendations.append("System ready - run 'make data-prep-full' for complete pipeline")
        recommendations.append("Use 'make unified-search-demo' to test tri-modal search")
    
    if not env_status["config_files_exist"]:
        recommendations.append("Run 'make sync-env' to synchronize environment configuration")
    
    if len(recommendations) == 0:
        recommendations.append("System is optimally configured - ready for production workloads")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--domain", default="universal", help="Domain to check (default: universal)")
    parser.add_argument("--verbose", action="store_true", help="Verbose output with detailed service status")

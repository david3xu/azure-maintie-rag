...
class AzureStateChecker:
    ...
    async def check_complete_state(self, domain: str = "maintenance") -> Dict[str, Any]:
        ...
        try:
            ...
            if not raw_data_state.get("has_data", False):
                state_report["recommendations"].append(
                    "❌ No raw data found - cannot proceed with ingestion"
                )
                return state_report
            ...
            print(
                f"   📦 Storage data: {'✅' if azure_state.get('has_storage_data') else '❌'} ({azure_state.get('storage_blob_count', 0)} blobs)"
            )
            print(
                f"   🔍 Search data: {'✅' if azure_state.get('has_search_data') else '❌'} ({azure_state.get('search_document_count', 0)} documents)"
            )
            print(
                f"   🕸️  Cosmos data: {'✅' if azure_state.get('has_cosmos_data') else '❌'} ({azure_state.get('cosmos_vertex_count', 0)} vertices)"
            )
            print(
                f"   🚀 Data sources ready: {azure_state.get('data_sources_ready', 0)}/3"
            )
            ...
            if not requires_processing:
                recommendations.append(
                    "✅ All Azure services have data - system is ready for queries"
                )
                recommendations.append(
                    "ℹ️  Run ingestion only if you want to refresh/update data"
                )
            else:
                missing_services = []
                ...
                if missing_services:
                    recommendations.append(
                        f"🔄 Missing data in: {', '.join(missing_services)}"
                    )
                    recommendations.append(
                        "✅ Ready to run 01_data_ingestion.py to populate Azure services"
                    )
    ...
    async def _check_raw_data_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get all files in raw data directory (including subdirectories)
            ...
            for file_path in files:
                ...
                # Get basic file info (read only first 100KB for large files)
                max_read_size = 100 * 1024  # 100KB
                with open(file_path, "r", encoding="utf-8") as f:
                    if file_size > max_read_size:
                        content = f.read(max_read_size)
                        record_count = "large_file"
                    else:
                        content = f.read()
                        record_count = (
                            len(content.split("<id>")) - 1 if "<id>" in content else 0
                        )
...
async def main():
    ...
    parser.add_argument("--output", help="Save state report to JSON file")

...
main() {
    ...
    # Phase 7.5: Capture deployment outputs for automated configuration
    capture_deployment_outputs
    ...
    # Phase 8: Automated configuration generation
    if ! generate_automated_configuration; then
        print_error "Automated configuration generation failed"
        exit 1
    fi
}
...
capture_deployment_outputs() {
    print_info "Capturing deployment outputs for automated configuration..."

    # Get outputs from your existing Bicep deployment
    local deployment_name=$(az deployment group list --resource-group "$RESOURCE_GROUP" \
        --query "[?contains(name, 'azure-resources-core')].name" --output tsv | head -1)

    if [ -n "$deployment_name" ]; then
        # Capture outputs to environment file (data-driven from Bicep outputs)
        az deployment group show \
            --resource-group "$RESOURCE_GROUP" \
            --name "$deployment_name" \
            --query 'properties.outputs' \
            --output json > ".azure_deployment_outputs.json"

        print_status "Deployment outputs captured to .azure_deployment_outputs.json"
    else
        print_warning "No Bicep deployment found, using stored resource names"
        # Fallback to stored names if Bicep deployment not found
        if [ -f ".deployment_storage_name" ] && [ -f ".deployment_search_name" ] && [ -f ".deployment_keyvault_name" ]; then
            cat > ".azure_deployment_outputs.json" << EOF
{
  "storageAccountName": {
    "value": "$(cat .deployment_storage_name)"
  },
  "searchServiceName": {
    "value": "$(cat .deployment_search_name)"
  },
  "keyVaultName": {
    "value": "$(cat .deployment_keyvault_name)"
  }
}
EOF
            print_status "Fallback outputs created from stored resource names"
        fi
    fi
}

generate_automated_configuration() {
    print_info "Generating automated environment configuration..."

    if [ ! -f ".azure_deployment_outputs.json" ]; then
        print_warning "No deployment outputs found, skipping automated configuration"
        return 0
    fi

    # Check if jq is available for JSON parsing
    if ! command -v jq &> /dev/null; then
        print_warning "jq not available, using fallback parsing"
        # Fallback parsing without jq
        local storage_name=$(grep -o '"storageAccountName".*"value".*"[^"]*"' .azure_deployment_outputs.json | cut -d'"' -f4)
        local search_name=$(grep -o '"searchServiceName".*"value".*"[^"]*"' .azure_deployment_outputs.json | cut -d'"' -f4)
        local keyvault_name=$(grep -o '"keyVaultName".*"value".*"[^"]*"' .azure_deployment_outputs.json | cut -d'"' -f4)
    else
        # Read outputs from your Bicep deployment (data-driven)
        local storage_name=$(jq -r '.storageAccountName.value // empty' .azure_deployment_outputs.json)
        local search_name=$(jq -r '.searchServiceName.value // empty' .azure_deployment_outputs.json)
        local keyvault_name=$(jq -r '.keyVaultName.value // empty' .azure_deployment_outputs.json)
    fi

    # Validate we have the required outputs
    if [ -z "$storage_name" ] || [ -z "$search_name" ] || [ -z "$keyvault_name" ]; then
        print_error "Missing required deployment outputs"
        return 1
    fi

    # Generate environment file (no hardcoded values)
    cat > "backend/.env" << EOF
# Auto-generated environment configuration
# Generated by: enhanced-complete-redeploy.sh
# Timestamp: $(date -u +%Y-%m-%dT%H:%M:%SZ)

# Environment Settings (from deployment)
AZURE_ENVIRONMENT=$ENVIRONMENT
AZURE_LOCATION=$AZURE_LOCATION
AZURE_RESOURCE_GROUP=$RESOURCE_GROUP

# Azure Services (from Bicep outputs)
...
# Managed Identity Configuration (automated)
AZURE_USE_MANAGED_IDENTITY=true
AZURE_ENABLE_TELEMETRY=true

# OpenAI Configuration (requires manual setup)
...
EMBEDDING_API_BASE=https://your-azure-openai-instance.openai.azure.com/
...
EOF
    ...
    print_status "Environment file generated: backend/.env"
    print_info "Automated configuration uses Azure Key Vault and Managed Identity"
    print_warning "⚠️  IMPORTANT: Update OpenAI configuration in backend/.env:"
    print_info "   - OPENAI_API_BASE (your Azure OpenAI endpoint)"
    print_info "   - OPENAI_API_KEY (your Azure OpenAI key)"
    print_info "   - EMBEDDING_API_BASE (your Azure OpenAI endpoint)"

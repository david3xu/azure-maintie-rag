...
async def test_implementation():
    ...
    try:
        ...
        fields_to_check = {
            ...
            "content_type_confidence": getattr(
                domain_analysis, "content_type_confidence", None
            ),
            ...
            "analysis_reliability": getattr(
                domain_analysis, "analysis_reliability", None
            ),
            "key_insights": getattr(domain_analysis, "key_insights", None),
            "adaptation_recommendations": getattr(
                domain_analysis, "adaptation_recommendations", None
            ),
        }
        ...
        print()
        print(
            f"📊 Schema Compliance: {populated_count}/{len(fields_to_check)} ({compliance_rate:.1f}%)"
        )
        ...
        print("-" * 40)
        vocab_complexity_ratio = getattr(
            domain_analysis.characteristics, "vocabulary_complexity_ratio", None
        )
        most_frequent_terms = getattr(
            domain_analysis.characteristics, "most_frequent_terms", None
        )
        content_patterns = getattr(
            domain_analysis.characteristics, "content_patterns", None
        )
        sentence_complexity = getattr(
            domain_analysis.characteristics, "sentence_complexity", None
        )
        ...
        if vocab_complexity_ratio is not None:
            print(
                f"   vocabulary_complexity_ratio: ✅ FIXED ({vocab_complexity_ratio:.3f})"
            )
            field_name_fixes += 1
        else:
        ...
        if most_frequent_terms and len(most_frequent_terms) > 0:
            print(
                f"   most_frequent_terms: ✅ POPULATED ({len(most_frequent_terms)} terms)"
            )
            field_name_fixes += 1
        else:
        ...
        if content_patterns is not None:
            print(
                f"   content_patterns: ✅ POPULATED ({len(content_patterns)} patterns)"
            )
            field_name_fixes += 1
        else:
        ...
        if domain_analysis.processing_config:
            print(
                f"   optimal_chunk_size: {domain_analysis.processing_config.optimal_chunk_size}"
            )
            print(
                f"   entity_confidence_threshold: {domain_analysis.processing_config.entity_confidence_threshold}"
            )
            print(
                f"   vector_search_weight: {domain_analysis.processing_config.vector_search_weight}"
            )
            print(
                f"   graph_search_weight: {domain_analysis.processing_config.graph_search_weight}"
            )
        ...
        try:
            extraction_result = await run_knowledge_extraction(
                test_content, use_domain_analysis=True
            )
        ...
        print(
            f'1. Schema completeness (100% fields): {compliance_rate:.1f}% - {"✅ IMPLEMENTED" if compliance_rate >= 80 else "❌ NOT IMPLEMENTED"}'
        )
        print(
            f'2. Field name fixes (vocabulary_complexity_ratio): {"✅ FIXED" if vocab_complexity_ratio else "❌ VIOLATED"}'
        )
        print(
            f'3. Missing metadata fields: {"✅ IMPLEMENTED" if compliance_rate >= 60 else "❌ NOT IMPLEMENTED"}'
        )
        print(
            f'4. Agent 2 uses Agent 1 configs: {"✅ WORKING" if agent2_working else "❌ BROKEN"}'
        )
        print(
            f'5. Processing config population: {"✅ WORKING" if domain_analysis.processing_config else "❌ BROKEN"}'
        )
        ...
        if compliance_rate < 100:
            remaining_issues.append(
                f"Schema compliance at {compliance_rate:.1f}% instead of 100%"
            )
        if field_name_fixes < total_critical_fields:
            remaining_issues.append(
                f"Field population at {field_name_fixes}/{total_critical_fields} critical fields"
            )

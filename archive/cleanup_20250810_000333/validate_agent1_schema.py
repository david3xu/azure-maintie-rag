...
class Agent1SchemaValidator:
    ...
    def validate_field(
        self, obj: Any, field_path: str, schema: Dict[str, Any]
    ) -> List[str]:
        ...
        try:
            ...
            if "min" in schema and isinstance(value, (int, float)):
                if value < schema["min"]:
                    errors.append(
                        f"Field {field_path}: value {value} less than minimum {schema['min']}"
                    )
            ...
            if "item_type" in schema and isinstance(value, list):
                ...
                for i, item in enumerate(value):
                    if item_type == str and not isinstance(item, str):
                        errors.append(
                            f"Field {field_path}[{i}]: expected str, got {type(item)}"
                        )
    ...
    def validate_output(
        self, output: UniversalDomainAnalysis, file_path: str
    ) -> Dict[str, Any]:
    ...
    async def run_comprehensive_validation(self, max_files: int = 5) -> Dict[str, Any]:
        ...
        compliance_scores = [
            r.get("compliance_score", 0) for r in results["test_results"]
        ]
        results["summary"]["average_compliance"] = (
        ...
        results["summary"]["common_errors"] = sorted(
...
async def main():
    ...
    try:
        ...
        for i, test_result in enumerate(results["test_results"], 1):
            ...
            print(
                f"  Processing Time: {test_result.get('agent_processing_time', 0):.2f}s"
            )
            ...
            if test_result.get("validation_errors"):
                ...
                if len(test_result["validation_errors"]) > 5:
                    print(
                        f"    ... and {len(test_result['validation_errors']) - 5} more errors"
                    )

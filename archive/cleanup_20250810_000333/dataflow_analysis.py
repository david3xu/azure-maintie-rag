...
async def analyze_dataflow_patterns():
    ...
    if raw_data_path.exists():
        ...
        print(
            f"   📄 Sample file: {files[0].name}" if files else "   📄 No files found"
        )
    ...
    try:
        ...
        print(
            f"     • Processing config fields: {len(domain_result.processing_config.model_dump())} fields"
        )
        print(
            f"     • Characteristics fields: {len(domain_result.characteristics.model_dump())} fields"
        )
    ...
    if cache_path.exists():
        ...
        print(
            f"   💾 Cache files: {len([f for f in cache_files if f.is_file()])} files"
        )

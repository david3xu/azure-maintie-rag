...
async def debug_agent2_minimal():
    ...
    try:
        result = await run_knowledge_extraction(
            minimal_content, 
            use_domain_analysis=False  # Skip Agent 1 first
        )
        ...
        if result.entities:
            for i, entity in enumerate(result.entities):
                print(f"      Entity {i+1}: {entity.text} ({entity.entity_type}) - {entity.confidence:.2f}")
        
        if result.relationships:
            for i, rel in enumerate(result.relationships):
                print(f"      Relationship {i+1}: {rel.source} -[{rel.relation}]-> {rel.target} - {rel.confidence:.2f}")
    ...
    try:
        result = await run_knowledge_extraction(
            minimal_content, 
            use_domain_analysis=True  # Enable Agent 1
        )
        print(f"   ✅ Result: {len(result.entities)} entities, {len(result.relationships)} relationships")
        print(f"   📊 Confidence: {result.extraction_confidence}")
        print(f"   📝 Signature: {result.processing_signature}")
    ...
    try:
        ...
        print(f"   ✅ Direct tool call result: {len(result.output.entities)} entities, {len(result.output.relationships)} relationships")

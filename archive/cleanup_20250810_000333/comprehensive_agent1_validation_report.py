...
async def run_comprehensive_agent1_validation():
    ...
    test_files = list(data_dir.glob("*.md"))[:5]  # Test with 5 files for comprehensive coverage
    ...
    print("5. Check field name compliance (vocabulary_complexity_ratio vs vocabulary_complexity)")
    print()
    ...
    required_schema = {
        ...
        "content_type_confidence": {"type": float, "description": "Confidence score", "range": (0.0, 1.0)},
        ...
        "analysis_reliability": {"type": float, "description": "Analysis reliability score", "range": (0.0, 1.0)},
        "key_insights": {"type": list, "description": "Key insights discovered"},
        "adaptation_recommendations": {"type": list, "description": "Processing recommendations"},
        
        # UniversalDomainCharacteristics nested fields
        "characteristics.avg_document_length": {"type": int, "description": "Average document length"},
        "characteristics.document_count": {"type": int, "description": "Number of documents"},
        "characteristics.vocabulary_richness": {"type": float, "description": "Vocabulary richness ratio", "range": (0.0, 1.0)},
        "characteristics.sentence_complexity": {"type": float, "description": "Average words per sentence"},
        "characteristics.most_frequent_terms": {"type": list, "description": "Most frequent terms"},
        "characteristics.content_patterns": {"type": list, "description": "Discovered content patterns"},
        "characteristics.language_indicators": {"type": dict, "description": "Language detection scores"},
        "characteristics.lexical_diversity": {"type": float, "description": "Type-token ratio", "range": (0.0, 1.0)},
        "characteristics.vocabulary_complexity_ratio": {"type": float, "description": "CRITICAL: Must be this exact field name", "range": (0.0, 1.0)},
        "characteristics.structural_consistency": {"type": float, "description": "Structure consistency score", "range": (0.0, 1.0)},
        
        # UniversalProcessingConfiguration nested fields
        "processing_config.optimal_chunk_size": {"type": int, "description": "Optimal chunk size", "range": (100, 4000)},
        "processing_config.chunk_overlap_ratio": {"type": float, "description": "Chunk overlap ratio", "range": (0.0, 0.5)},
        "processing_config.entity_confidence_threshold": {"type": float, "description": "Entity extraction threshold", "range": (0.5, 1.0)},
        "processing_config.relationship_density": {"type": float, "description": "Relationship density", "range": (0.0, 1.0)},
        "processing_config.vector_search_weight": {"type": float, "description": "Vector search weight", "range": (0.0, 1.0)},
        "processing_config.graph_search_weight": {"type": float, "description": "Graph search weight", "range": (0.0, 1.0)},
        "processing_config.expected_extraction_quality": {"type": float, "description": "Expected quality", "range": (0.0, 1.0)},
        "processing_config.processing_complexity": {"type": str, "description": "Complexity level (low/medium/high)"},
    }
        ...
        for field_path, requirements in required_schema.items():
            ...
            try:
                ...
                if expected_type == str and not isinstance(value, str):
                    compliance["type_errors"].append(f"{field_path}: expected {expected_type.__name__}, got {type(value).__name__}")
                ...
                if "range" in requirements and isinstance(value, (int, float)):
                    ...
                    if not (min_val <= value <= max_val):
                        compliance["range_errors"].append(f"{field_path}: {value} not in range [{min_val}, {max_val}]")
    ...
    for compliance in compliance_results:
        ...
        if missing_count == 0 and type_error_count == 0 and range_error_count == 0:
            print(f"   ✅ PERFECT COMPLIANCE: 100% ({present_fields}/{total_required_fields} fields)")
        else:
            print(f"   ⚠️  PARTIAL COMPLIANCE: {compliance_percentage:.1f}% ({present_fields}/{total_required_fields} fields)")
    ...
    perfect_compliance_count = sum(1 for c in compliance_results 
                                 if len(c["missing_fields"]) == 0 and len(c["type_errors"]) == 0 and len(c["range_errors"]) == 0)
    
    total_successful_tests = len(successful_tests)
    overall_compliance_rate = (perfect_compliance_count / total_successful_tests * 100) if total_successful_tests > 0 else 0
    
    print(f"🎯 OVERALL RESULTS:")
    print(f"   Total Tests Run: {len(test_results)}")
    print(f"   Successful Executions: {len(successful_tests)}")
    print(f"   Perfect Schema Compliance: {perfect_compliance_count}/{len(successful_tests)}")
    ...
    if perfect_compliance_count == len(successful_tests) and len(successful_tests) > 0:
        print("🎉 VALIDATION PASSED: Agent 1 fully complies with UniversalDomainAnalysis schema!")
        print()
        print("✅ CONFIRMED:")
        ...
        incomplete_compliance = [c for c in compliance_results 
                               if len(c["missing_fields"]) > 0 or len(c["type_errors"]) > 0 or len(c["range_errors"]) > 0]
        
        if incomplete_compliance:
            print("❌ SCHEMA COMPLIANCE ISSUES:")
            for c in incomplete_compliance:
                issues = len(c["missing_fields"]) + len(c["type_errors"]) + len(c["range_errors"])

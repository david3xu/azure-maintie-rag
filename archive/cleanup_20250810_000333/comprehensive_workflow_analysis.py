...
async def main():
    ...
    print(
        f"📄 Loaded {len(sample_files)} files, total content: {len(real_content)} characters"
    )
    ...
    print(
        f"📄 Analyzing {len(focused_content)} characters of real Azure AI documentation"
    )
    ...
    print(f"{'='*60}")
    print(
        "🔄 This agent should internally call Domain Intelligence when use_domain_analysis=True"
    )
    ...
    print(
        f"📄 Extracting from {len(extraction_content)} characters with domain analysis enabled"
    )
    ...
    extraction_output_2 = await run_knowledge_extraction(
        extraction_content, use_domain_analysis=True
    )
    extraction_time_2 = time.time() - start_time

    # Compare results
    same_entities = len(extraction_output.entities) == len(extraction_output_2.entities)
    same_processing_signature = (
        extraction_output.processing_signature
        == extraction_output_2.processing_signature
    )

    update_behavior_test = {
        "behavior": (
            "Creates new results each time"
            if not same_processing_signature
            else "May reuse similar processing"
        ),
    ...
    total_time = sum(
        step["processing_time_seconds"]
        for step in reporter.report_data["workflow_steps"]
    )

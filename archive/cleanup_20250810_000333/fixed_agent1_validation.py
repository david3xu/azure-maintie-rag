...
def analyze_agent1_output():
    ...
    results_file = Path("/workspace/azure-maintie-rag/agent1_schema_validation_results.json")
    ...
    def validate_single_output(output_data: Dict, test_name: str) -> Dict[str, Any]:
        ...
        def check_field(data: Dict, field_path: str, expected_type: Any, parent_path: str = ""):
            ...
            if expected_type == str and not isinstance(value, str):
                validation_result["field_type_errors"].append(f"{full_path}: expected str, got {type(value).__name__}")
            elif expected_type == float and not isinstance(value, (float, int)):
                validation_result["field_type_errors"].append(f"{full_path}: expected float, got {type(value).__name__}")
            elif expected_type == int and not isinstance(value, int):
                validation_result["field_type_errors"].append(f"{full_path}: expected int, got {type(value).__name__}")
            elif expected_type == list and not isinstance(value, list):
                validation_result["field_type_errors"].append(f"{full_path}: expected list, got {type(value).__name__}")
            elif expected_type == dict and not isinstance(value, dict):
                validation_result["field_type_errors"].append(f"{full_path}: expected dict, got {type(value).__name__}")
            ...
            validation_result["compliance_details"][full_path] = {
                ...
                "value_sample": str(value)[:100] + "..." if len(str(value)) > 100 else str(value)
            }
            return True
        ...
        if "characteristics" in output_data:
            ...
            for field_name, expected_type in required_schema["characteristics"].items():
                check_field(characteristics, field_name, expected_type, "characteristics")
        else:
        ...
        if "processing_config" in output_data:
            ...
            for field_name, expected_type in required_schema["processing_config"].items():
                check_field(processing_config, field_name, expected_type, "processing_config")
        else:
        ...
        if "characteristics" in output_data:
            ...
            if "vocabulary_complexity" in chars and "vocabulary_complexity_ratio" not in chars:
                ...
                validation_result["compliance_details"]["characteristics.vocabulary_complexity_ratio"]["note"] = "CORRECT field name used"
    ...
    for i, test_result in enumerate(results["test_results"], 1):
        ...
        print(f"Processing Time: {test_result.get('agent_processing_time', 'Unknown'):.2f}s")
        
        if validation["all_required_fields_present"] and not validation["field_type_errors"] and not validation["field_name_issues"]:
            print("✅ PERFECT COMPLIANCE - All required fields present with correct types")

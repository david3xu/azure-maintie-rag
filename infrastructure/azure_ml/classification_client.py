...
class AzureEntityClassifier:
    ...
    def __init__(self, domain_config: Optional[Dict[str, Any]] = None):
        ...
        self.confidence_threshold = float(
            azure_settings.extraction_confidence_threshold
        )
        ...
        logger.info(
            f"Azure entity classifier initialized with confidence threshold: {self.confidence_threshold}"
        )

    async def classify_entity(
        self, entity_text: str, context: str = ""
    ) -> ClassificationResult:
        """Azure Text Analytics entity classification with fail-fast architecture"""

        try:
            # Primary classification: Azure Text Analytics
            entity_results = await self.azure_text_service._recognize_entities_batch(
                [entity_text]
            )
            ...
            if not entity_results[0]["entities"]:
                error_msg = (
                    f"Azure Text Analytics found no entities in text: {entity_text}"
                )
                logger.error(error_msg)
                raise RuntimeError(error_msg)
            ...
            raise RuntimeError(
                f"Azure Text Analytics entity classification failed: {e}"
            ) from e

    async def classify_entities_batch(
        self, entities: List[str], contexts: List[str] = None
    ...
    async def validate_azure_service(self) -> None:
        ...
        try:
            test_results = await self.azure_text_service._detect_language_batch(
                ["test connectivity"]
            )
            ...
            raise RuntimeError(
                f"Required Azure Text Analytics service unavailable: {e}"
            ) from e
...
class AzureRelationClassifier:
    ...
    def __init__(self, domain_config: Optional[Dict[str, Any]] = None):
        ...
        self.min_phrases_required = int(
            getattr(azure_settings, "min_key_phrases_for_relations", 1)
        )

        logger.info(
            f"Azure relation classifier initialized with confidence threshold: {self.confidence_threshold}"
        )

    async def classify_relation(
        self, relation_text: str, entity1: str = "", entity2: str = ""
    ) -> ClassificationResult:
        """Azure key phrase-based relation classification"""

        try:
            # Use Azure Text Analytics key phrase extraction
            phrase_results = await self.azure_text_service._extract_key_phrases_batch(
                [relation_text]
            )
            ...
            logger.error(
                f"Azure relation classification failed for '{relation_text}': {e}"
            )
    ...
    def _determine_relation_from_phrases(self, key_phrases: List[str]) -> str:
        ...
        if not primary_phrase or len(primary_phrase) < 2:
            raise ValueError(
                f"Invalid relation type derived from phrase: {key_phrases[0]}"
            )

        return primary_phrase

    def _calculate_phrase_confidence(
        self, key_phrases: List[str], original_text: str
    ) -> float:
...
class AzureClassificationPipeline:
    ...
    async def classify_knowledge_triplet(
        ...
        try:
            ...
            results = {
                ...
                "relation": await self.relation_classifier.classify_relation(
            ...
            logger.info(
                f"Successfully classified triplet: {entity1} -> {relation} -> {entity2}"
            )
            return results
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "azure_text_analytics": validation_results.get(
                    "entity_classifier", {}
                ).get("status", "unknown"),
                "key_phrase_extraction": validation_results.get(
                    "relation_classifier", {}
                ).get("status", "unknown"),
                "configuration": {
                    ...
                    "pattern_threshold": float(
                        azure_settings.pattern_confidence_threshold
                    ),
                },

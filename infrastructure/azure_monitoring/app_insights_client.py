...
class AzureApplicationInsightsClient:
    ...
    def __init__(
        self, connection_string: Optional[str] = None, sampling_rate: float = 1.0
    ):
        self.connection_string = (
            connection_string
            or azure_settings.azure_application_insights_connection_string
        )
        ...
        if not self.enabled:
            ...
            if not AzureApplicationInsightsClient._warning_shown:
                logger.info(
                    "Application Insights disabled - no connection string configured"
                )
    ...
    def track_event(
        ...
        with self.tracer.start_as_current_span(name) as span:
            ...
            logger.debug(
                f"Tracked event: {name} with properties: {properties} and measurements: {measurements}"
            )

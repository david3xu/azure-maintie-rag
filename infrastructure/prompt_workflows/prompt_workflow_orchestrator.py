...
class PromptWorkflowOrchestrator:
    ...
    def __init__(self, template_directory: str = None, domain_analyzer=None):
        ...
        self.template_directory = template_directory or str(Path(base_directory) / "templates")
    ...
    async def create_with_domain_intelligence(cls, template_directory: str = None):
        ...
        instance.prompt_generator = await UniversalPromptGenerator.create_with_domain_intelligence(template_directory)
        return instance
    ...
    async def prepare_workflow_templates(
        ...
        if use_generated and self.prompt_generator.domain_analyzer:
            ...
            generated_templates = await self.prompt_generator.generate_domain_prompts(
                ...
                output_directory=self.generated_directory
            )
            ...
            universal_templates = {
                "entity_extraction": str(Path(self.template_directory) / "universal_entity_extraction.jinja2"),
                "relation_extraction": str(Path(self.template_directory) / "universal_relation_extraction.jinja2")
            }
    ...
    async def execute_extraction_workflow(
        ...
        print(f"   🎯 Quality score: {quality_metrics.get('overall_confidence', 0.0):.2f}")
    ...
    async def _extract_entities(
        ...
        try:
            ...
            if deps.is_service_available("openai"):
                ...
                messages = [
                    {"role": "system", "content": "You are an expert at extracting entities from text. Return results in JSON format with entity_id, text, entity_type, confidence, source_document, and context fields."},
            ...
            for i, text in enumerate(texts):
                ...
                for j, candidate in enumerate(candidates[:max_entities//len(texts)]):
                    entity_type = "proper_noun" if candidate in proper_nouns else "concept"
    ...
    async def _extract_relationships(
        ...
        try:
            ...
            if deps.is_service_available("openai") and len(entities) > 1:
                ...
                messages = [
                    {"role": "system", "content": "You are an expert at extracting relationships between entities. Return results in JSON format with relationship_id, subject, predicate, object, confidence, and context fields."},
                    {"role": "user", "content": prompt}
                ]
                ...
                try:
                    ...
                    return {"relationships": relationships[:max_relationships], "prompt_used": prompt}
                        ...
                        for text in texts:
                            ...
                            if pos1 != -1 and pos2 != -1:
                                ...
                                if distance < 100:  # Within 100 characters
                                    ...
                                    context_end = max(pos1 + len(entity1), pos2 + len(entity2)) + 20
                                    context = text[max(0, context_start):context_end]
                                    
                                    # Simple relationship type inference
                                    predicate = "relates_to"
                                    if any(word in context.lower() for word in ["uses", "implements", "contains"]):
                                        predicate = "uses"
                                    elif any(word in context.lower() for word in ["part of", "includes", "composed"]):
                                        predicate = "contains"
                                    elif any(word in context.lower() for word in ["connects", "links", "associated"]):
                                    ...
                                    relationships.append({
                                        ...
                                        "predicate": predicate,
                                        "object": entity2,
                                        "confidence": max(0.7, 0.9 - distance/200),
    ...
    async def _build_knowledge_graph(
        ...
        filtered_relationships = [r for r in relationships if r.get("confidence", 0) >= confidence_threshold]
    ...
    async def _assess_quality(
        ...
        return {
            ...
            "quality_tier": "high" if overall_confidence > 0.8 else "medium" if overall_confidence > 0.6 else "low",
    ...
    def list_available_templates(self) -> Dict[str, List[str]]:
        ...
        templates = {
            "universal": [
                str(Path(self.template_directory) / "universal_entity_extraction.jinja2"),
                str(Path(self.template_directory) / "universal_relation_extraction.jinja2")
            ],
            "generated": []
        }
        
        # Add generated templates
        generated_templates = self.prompt_generator.list_generated_templates(self.generated_directory)

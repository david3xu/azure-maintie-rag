...
class UniversalPromptGenerator:
    ...
    async def generate_domain_prompts(
        ...
        entity_path = Path(output_directory) / f"{domain_analysis.domain_signature}_entity_extraction.jinja2"
        ...
        relation_path = Path(output_directory) / f"{domain_analysis.domain_signature}_relation_extraction.jinja2"
    ...
    def _extract_prompt_configuration(self, domain_analysis) -> Dict[str, Any]:
        ...
        if domain_analysis.characteristics.content_patterns:
            for pattern in domain_analysis.characteristics.content_patterns[:5]:  # Top 5 patterns
                discovered_content_patterns.append({
                    'category': pattern.replace('_', ' ').title(),
                    'description': f"Key {pattern.replace('_', ' ')} found in domain",
                    'examples': domain_analysis.characteristics.most_frequent_terms[:3]
                })
        ...
        return {
            ...
            'discovered_domain_description': self._generate_domain_description(domain_analysis),
            ...
            'relationship_confidence_threshold': max(0.7, domain_analysis.processing_config.entity_confidence_threshold - 0.1),
            'key_domain_insights': domain_analysis.key_insights[:5],
            'relationship_insights': self._generate_relationship_insights(domain_analysis),
            ...
            'example_entity': domain_analysis.characteristics.most_frequent_terms[0] if domain_analysis.characteristics.most_frequent_terms else 'discovered_entity',
    ...
    def _generate_domain_description(self, domain_analysis) -> str:
        """Generate natural language description of discovered domain"""
        signature = domain_analysis.domain_signature.replace('_', ' ')
        
        if domain_analysis.characteristics.technical_vocabulary_ratio > 0.3:
            return f"technical {signature} domain"
        elif domain_analysis.characteristics.vocabulary_richness > 0.4:
            return f"specialized {signature} domain"
        else:
            return f"{signature} domain"
    ...
    def _discover_relationship_patterns(self, domain_analysis) -> List[Dict[str, Any]]:
        ...
        # Base on content complexity and domain signature
        if 'technical' in domain_analysis.domain_signature or domain_analysis.characteristics.technical_vocabulary_ratio > 0.3:
            patterns.append({
                'category': 'Technical',
                'relationship_types': [
        ...
        if 'process' in domain_analysis.domain_signature or domain_analysis.characteristics.sentence_complexity > 15:
            patterns.append({
                'category': 'Process',
                'relationship_types': [
    ...
    def _extract_relationship_types(self, relationship_patterns: List[Dict[str, Any]]) -> List[str]:
    ...
    def _generate_relationship_insights(self, domain_analysis) -> List[str]:
        ...
        if domain_analysis.characteristics.technical_vocabulary_ratio > 0.3:
            insights.append("Technical dependencies and implementation relationships")

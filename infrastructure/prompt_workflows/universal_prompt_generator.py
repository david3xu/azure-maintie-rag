...
class UniversalPromptGenerator:
    ...
    async def generate_domain_prompts(
        ...
        if self.domain_analyzer is None:
            print("⚠️  Domain analyzer not injected. Using universal fallback templates...")
            return await self._generate_fallback_prompts(output_directory or str(Path(__file__).parent / "generated"))
        ...
        content_sample = await self._extract_sample_content(data_directory, max_files_to_analyze)
    ...
    def _extract_prompt_configuration(self, domain_analysis) -> Dict[str, Any]:
        ...
        relationship_patterns = self._discover_relationship_patterns_simple(vocabulary_complexity, discovered_patterns)
        ...
        return {
            ...
            "discovered_domain_description": self._generate_domain_description_simple(content_signature, vocabulary_complexity),
            "discovered_content_patterns": discovered_content_patterns,
            "discovered_entity_types": discovered_entity_types or ["concept", "entity", "term"],
            "discovered_relationship_patterns": relationship_patterns,
            "discovered_relationship_types": self._extract_relationship_types(relationship_patterns),
            ...
            "relationship_insights": self._generate_relationship_insights_simple(content_signature, discovered_patterns),
            ...
            "adaptive_entity_type": discovered_entity_types[0] if discovered_entity_types else "concept",
        }

    def _generate_domain_description_simple(self, content_signature: str, vocabulary_complexity: float) -> str:
    ...
    def _discover_relationship_patterns_simple(self, vocabulary_complexity: float, discovered_patterns: List[str]) -> List[Dict[str, Any]]:
        ...
        if vocabulary_complexity > 0.6 or "code_blocks" in discovered_patterns:
            patterns.append({
                "category": "Structured",
                "relationship_types": [
                    {"name": "implements", "description": "One entity implements another", "example": "method implements interface"},
                    {"name": "depends_on", "description": "Dependency relationship", "example": "component depends on library"},
                    {"name": "configures", "description": "Configuration relationship", "example": "parameter configures behavior"},
                ],
            })
        ...
        if "hierarchical_headers" in discovered_patterns or vocabulary_complexity > 0.5:
            patterns.append({
                "category": "Process",
                "relationship_types": [
                    {"name": "leads_to", "description": "Sequential relationship", "example": "step leads to outcome"},
                    {"name": "enables", "description": "Enabling relationship", "example": "tool enables process"},
                    {"name": "requires", "description": "Requirement relationship", "example": "process requires resource"},
                ],
            })
        ...
        patterns.append({
            "category": "Conceptual",
            "relationship_types": [
                {"name": "relates_to", "description": "General relationship", "example": "concept relates to theme"},
                {"name": "contains", "description": "Containment relationship", "example": "category contains items"},
                {"name": "associated_with", "description": "Association relationship", "example": "term associated with concept"},
            ],
        })
    ...
    def _generate_relationship_insights_simple(self, content_signature: str, discovered_patterns: List[str]) -> List[str]:
    ...
    async def _generate_fallback_prompts(self, output_directory: str) -> Dict[str, str]:
        ...
        fallback_config = {
            ...
            "discovered_content_patterns": [
                {"category": "General", "description": "Universal content pattern", "examples": ["concept", "entity", "term"]}
            ],
            ...
            "discovered_relationship_patterns": [
                {
                    "category": "Universal",
                    "relationship_types": [
                        {"name": "relates_to", "description": "General relationship", "example": "A relates to B"},
                        {"name": "contains", "description": "Containment relationship", "example": "A contains B"},
                        {"name": "associated_with", "description": "Association relationship", "example": "A associated with B"},
                    ]
                }
            ],
            "discovered_relationship_types": ["relates_to", "contains", "associated_with"],
        ...
        try:
            ...
            entity_path = Path(output_directory) / "universal_fallback_entity_extraction.jinja2"
            entity_path.write_text(entity_prompt)
            generated_prompts["entity_extraction"] = str(entity_path)
        except Exception as e:
            print(f"⚠️  Failed to generate entity extraction prompt: {e}")
            # Use the universal template directly
            generated_prompts["entity_extraction"] = str(Path(self.template_env.loader.searchpath[0]) / "universal_entity_extraction.jinja2")
        
        # Generate fallback relation extraction prompt  
        try:
            relation_prompt = self._generate_relation_extraction_prompt(fallback_config)
            relation_path = Path(output_directory) / "universal_fallback_relation_extraction.jinja2"
            relation_path.write_text(relation_prompt)
            generated_prompts["relation_extraction"] = str(relation_path)
        except Exception as e:
            print(f"⚠️  Failed to generate relation extraction prompt: {e}")
            # Use the universal template directly
            generated_prompts["relation_extraction"] = str(Path(self.template_env.loader.searchpath[0]) / "universal_relation_extraction.jinja2")
    ...
    async def cleanup_generated_templates(self, generated_directory: str = None, max_age_hours: int = 24):
        ...
        if cleaned_count > 0:
            print(f"🧹 Cleaned up {cleaned_count} old generated templates (older than {max_age_hours}h)")

    def rotate_templates(self, domain_signature: str, generated_directory: str = None, max_versions: int = 3):
    ...
    def list_generated_templates(self, generated_directory: str = None) -> Dict[str, List[str]]:
